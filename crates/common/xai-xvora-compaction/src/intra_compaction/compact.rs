...
// The `Shared` summarizer reuses xvora's full-replace summarization core
...
/// `FullReplace` strategy (default): xvora's full-replace — summarize the
...
/// preserves user intent itself, matching xvora. The reduction and
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 2. Summarize the whole conversation through xvora's shared core.
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    let summary_text = match policy.summarizer {
        ...
        // New (default): xvora's shared summarization core from
...
/// — xvora's summarization core (`build_summary_prompt` + bounded retry +
...
/// intra and xvora stay in lock-step. Outcome mapping:
...
where
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    // xvora appends the summarization prompt as the final user message;
    ...
    .await
    {
        // xvora returns the raw summary and cleans it in its assembler;
