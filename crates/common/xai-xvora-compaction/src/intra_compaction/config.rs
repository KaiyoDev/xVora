...
///
/// - `FullReplace` (default): xvora's full-replace strategy — summarize
...
/// time (token threshold only, matching xvora) and also ignores
...
pub struct IntraCompactionConfig {
    ...
    /// as xvora full-replace auto-compact). Worthless early passes are
