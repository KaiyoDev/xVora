//! xvora's session-level summarization prompt.
//!
//! Split out of the crate-root `prompt` module so xvora's full-replace
...
/// Build xvora's session-level summarization prompt (no chat history).
///
...
/// `xai-xvora-shell::session::helpers::session_compact::build_compaction_prompt`
...
/// (mirrors `xai-xvora-shell`'s `SELF_SUMMARIZATION_PROMPT`). Framed
...
/// so the structured (xvora) and short self-summary prompts stay
/// shared instead of duplicated per harness.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum SummaryPromptKind {
    /// xvora's detailed, numbered-section summary prompt.
    #[default]
    Structured,
...
mod tests {
    ...
    fn kind_structured_matches_build_summary_prompt() {
        ...
        // so routing through the selector never changes xvora's prompt.
        assert_eq!(
