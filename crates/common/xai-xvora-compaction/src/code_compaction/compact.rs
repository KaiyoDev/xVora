//! xvora's full-replace compaction pass.
//!
...
//! ```
//!
//! Per-harness concerns stay in the product host (for example `xai-xvora-shell`): the triggers, the
...
pub struct FullReplaceContext<T> {
    ...
    /// turn). xvora keeps this; pass empty to drop it.
...
///
/// Returned by [`sample_full_replace_summary`] for harnesses (xvora's
...
/// Run xvora's full-replace compaction pass and return the rebuilt
...
///
/// This is the seam xvora's shell uses: it drives the verbatim → fitted →
    ...
    O: FullReplaceObserver + ?Sized,
{
    ...
    let prompt = CompactionPrompt {
        // xvora appends the summarization prompt as the final user
...
mod tests {
    ...
    /// returns a structured summary must produce xvora's exact compacted
    /// history shape, with the LLM output cleaned and the agent-state reminder
    /// carried through as the final item.
    #[tokio::test]
    async fn full_replace_produces_xvora_history_shape() {
        let llm_turns = vec![
