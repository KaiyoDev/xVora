...
//!
//! - xvora chat has a real tokenizer (`TextTokenizer` / `ImageTokenizer`) and
//!   counts whole turns via `GrokTurn::get_num_tokens`.
//! - xvora-build estimates with `bytes / 4`.
//!
...
//! (xvora chat: `ItemTokenCounter<Arc<GrokTurn>>`), so exactly one mechanism
//! is in play.
