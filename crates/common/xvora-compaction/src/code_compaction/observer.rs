//! Observability seam for the full-replace (xvora-build) pass.
//!
//! The shared orchestrator reports per-attempt and terminal outcomes through
//! this trait so each harness can emit its own telemetry (xvora-build:
...
//!
//! Emission points are part of the behavior contract: the xvora-build observer
