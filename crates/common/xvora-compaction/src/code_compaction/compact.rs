//! xvora-build's full-replace compaction pass.
//!
//! xvora-build does not select a tail to keep; it summarizes the whole
...
//! ```
//!
//! Per-harness concerns stay in the product host (for example `xvora-shell`): the triggers, the
...
pub struct FullReplaceContext<T> {
    ...
    /// turn). xvora-build keeps this; pass empty to drop it.
...
///
/// Returned by [`sample_full_replace_summary`] for harnesses (xvora-build's
...
/// Run xvora-build's full-replace compaction pass and return the rebuilt
...
///
/// This is the seam xvora-build's shell uses: it drives the verbatim → fitted →
    ...
    O: FullReplaceObserver + ?Sized,
{
    ...
    let prompt = CompactionPrompt {
        // xvora-build appends the summarization prompt as the final user
...
mod tests {
    ...
    /// returns a structured summary must produce xvora-build's exact compacted
