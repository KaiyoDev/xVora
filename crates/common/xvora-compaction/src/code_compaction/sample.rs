...
//!
//! The canonical `sample → classify → retry` loop, used by **both** xvora-build's
//! full-replace pass ([`sample_full_replace_summary`](super::sample_full_replace_summary))
//! and xvora chat's intra `Shared` summarizer
...
//! the winning summary (xvora-build cleans in its assembler; intra cleans via
...
pub enum SampleRetryError {
    ...
    Failure {
        ...
        /// signal the xvora-build host uses to step down its input size).
