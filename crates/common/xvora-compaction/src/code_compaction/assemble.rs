//! Compacted-history assembly (xvora-build's rebuild structure, generic).
//!
//! Moved from `xvora-chat-state::compaction_utils::build_compacted_history` and
...
//! ```
//!
//! xvora-build is the canonical harness. The summary carrier text is built by
...
pub fn assemble_compacted_history<T: CompactionItemFactory>(
    ...
    // xvora-build keeps the legacy `<user_query>`-wrapped continuation text and
