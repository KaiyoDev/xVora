...
//! candidate items, the *current* user-queries preamble (xvora chat
...
/// round's user messages (xvora chat: verbatim from the raw request, with
/// attachment refs). `conversation_id` / `response_id` are threaded
/// through for log correlation only.
///
/// Observer events (the xvora chat observer maps them to the
