...
//! stay in each product host (for example `xvora-shell`).
//!
//! The crate depends on **neither** a conversation-type crate nor
//! `xvora-sampling-types`. It is decoupled from both xvora chat and
//! xvora-build hosts through a small set of trait seams:
//!
...
//!
//! - [`code_compaction`] — xvora-build's whole-session **full-replace**
//!   subsystem (prompt/summary/failure/config, assemble, orchestration).
//! - [`intra_compaction`] — xvora chat's tail-keep, per-step pass.
//! - [`inter_compaction`] — xvora chat's chunked, between-turn pass.
//!
...
//! (active-agent-state `<system-reminder>` formatting shared by xvora chat and
//! xvora-build; hosts still own snapshotting and host-only sections).
...
// xvora-build's full-replace subsystem now lives under `code_compaction`;
