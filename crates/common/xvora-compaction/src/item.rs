...
//! xvora-build implements it for `xvora_sampling_types::ConversationItem`.
//!
...
///
/// This is the common denominator of `GrokRole` (xvora chat) and the
/// `ConversationItem` variants (xvora-build).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CompactionRole {
    /// System prompt.
    System,
    /// Developer prompt (xvora chat) — maps to System on harnesses without a
...
pub trait CompactionItem {
    ...
    ///
    /// Returns an owned `String` because some harnesses (xvora chat's
    ...
    /// Whether this item carries a *prior compaction summary* (xvora chat: a
...
pub trait CompactionItemBuilder: CompactionItem + Clone {
    ...
    /// conversation (xvora chat: a `Developer` turn with category
    ...
    /// requests/results (xvora chat: keep only `Channel` contents of an
    ...
    /// `max_tokens` (xvora-build style: `max_bytes = max_tokens * 4`, prefix
...
/// constructs the typed harness items that make up xvora-build's rebuilt
/// history.
///
/// This is a sibling of [`CompactionItemBuilder`], not a part of it, on
/// purpose. `CompactionItemBuilder` is already implemented by xvora chat's
/// `GrokTurn`; adding these constructors to it as required methods would break
/// that impl. They are also xvora-build-specific (xvora chat's tail-keep path
...
///
/// The xvora-build implementor (`ConversationItem`) maps each constructor to the
...
/// operate over `&[Arc<T>]` (xvora chat stores turns as `Arc<GrokTurn>`).
