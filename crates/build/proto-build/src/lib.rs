...
impl XaiProtoBuilder {
    ...
    fn emit_rerun_if_changed<'a>(
        ...
        //
        // Use real temp files (not /dev/stdout|/dev/null): those Unix paths fail
        // on Windows with "No such file or directory" even when protoc itself is
        // installed correctly (GitHub Actions windows-latest).
        for proto in protos {
            ...
            command
                ...
                .arg(format!(
                    "--descriptor_set_out={}",
                    desc_file.path().to_str().context("desc path not UTF-8")?
                ));
            ...
            let output = fs::read_to_string(dep_file.path()).with_context(|| {
                format!(
