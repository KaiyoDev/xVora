...
Sample hooks for Xvora. Copy to `~/.xvora/hooks/` to enable globally, or to `<project>/.xvora/hooks/` for project-scoped hooks (requires `/hooks-trust`).
...
**Install:**
```sh
mkdir -p ~/.xvora/hooks/bin
cp examples/hooks/safe-shell.json ~/.xvora/hooks/
cp examples/hooks/bin/safe-shell-guard.sh ~/.xvora/hooks/bin/
chmod +x ~/.xvora/hooks/bin/safe-shell-guard.sh
```
...
```sh
mkdir -p ~/.xvora/hooks/bin
cp examples/hooks/no-recursive-grep.json ~/.xvora/hooks/
cp examples/hooks/bin/no-recursive-grep-guard.py ~/.xvora/hooks/bin/
chmod +x ~/.xvora/hooks/bin/no-recursive-grep-guard.py
```
...
Appends session metadata to `~/.xvora/session-audit.log` — event, session ID, cwd, timestamp.

**Install:**
```sh
mkdir -p ~/.xvora/hooks/bin
cp examples/hooks/session-log.json ~/.xvora/hooks/
cp examples/hooks/bin/session-log.sh ~/.xvora/hooks/bin/
chmod +x ~/.xvora/hooks/bin/session-log.sh
```
...
Logs all tool calls to `~/.xvora/tool-activity.log` — tool name, event type, effective tool name, backgrounded status.

**Install:**
```sh
mkdir -p ~/.xvora/hooks/bin
cp examples/hooks/tool-logger.json ~/.xvora/hooks/
cp examples/hooks/bin/tool-logger.sh ~/.xvora/hooks/bin/
chmod +x ~/.xvora/hooks/bin/tool-logger.sh
```
...
Remove the JSON file from `~/.xvora/hooks/`. The hook stops running on the next session.