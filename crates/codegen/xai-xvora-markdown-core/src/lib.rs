//! Headless markdown analysis sharing Xvora's exact `pulldown-cmark` config.
//!
...
//! feature set, shared with `xai-xvora-markdown` so analysis matches what Grok
...
/// The exact `pulldown-cmark` option set Xvora uses to render markdown.
///
...
/// Parse `text` with Xvora's options; count elements and flag structural issues.
