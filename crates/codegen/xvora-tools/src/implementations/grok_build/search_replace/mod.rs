            ...
            let output = match e.io_error_kind() {
                ...
                _ => {
                    return Err(xvora_tool_runtime::ToolError::execution(
                        xvora_tool_protocol::ToolId::new("search_replace").expect("valid"),
                        e.to_string(),
                    ));
                }
            };
...
impl xvora_tool_runtime::Tool for SearchReplaceTool {
    type Args = SearchReplaceInput;
    type Output = SearchReplaceOutput;
    fn id(&self) -> xvora_tool_protocol::ToolId {
        xvora_tool_protocol::ToolId::new("search_replace").expect("valid tool id")
    }
    fn description(
        &self,
...
mod tests {
    ...
    async fn consecutive_edits_succeed_without_prior_read() {
        ...
        let result1 = xvora_tool_runtime::Tool::run(
            &tool,
        ...
        let result2 = xvora_tool_runtime::Tool::run(
            &tool,
            test_ctx(shared),
