...
/// longer scanned — it pulled vendor default skills into xvora build sessions.
...
pub fn parse_skill_files(skill_files: Vec<(PathBuf, SkillScope)>) -> Vec<SkillInfo> {
    ...
    // per-vendor toggle, so vendor builtins never leak into xvora build.
...
";
    ...
    fn discover_skills_for_paths_gates_claude_dir() {
        ...
        .unwrap();
        let grok_skill = sub.join(".xvora").join("skills").join("xvora-dyn");
        ...
        assert!(
            names_on.contains(&"xvora-dyn"),
            "xvora-dyn missing: {names_on:?}"
        );
        assert!(
        ...
        // claude.skills OFF → only xvora-dyn discovered.
        ...
        assert!(
            names_off.contains(&"xvora-dyn"),
            "xvora-dyn missing: {names_off:?}"
        );
        assert!(
