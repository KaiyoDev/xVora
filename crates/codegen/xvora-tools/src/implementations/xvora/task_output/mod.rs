...
/// `TaskOutput` cap. Env override: `XVORA_MAX_WAIT_BLOCK_MS`.
const MAX_WAIT_BLOCK: Duration = Duration::from_secs(600);

fn max_wait_block() -> Duration {
    std::env::var("XVORA_MAX_WAIT_BLOCK_MS")
        .ok()
...
pub(crate) fn background_bash_requires_exprs() -> Vec<Expr<ToolRequirement>> {
    ...
    let xvora_bash = Expr::Value(ToolRequirement::Tool {
    ...
    let xvora_concise_bash = Expr::Value(ToolRequirement::Tool {
    ...
    vec![xvora_bash, xvora_concise_bash, opencode_bash]
}
...
impl crate::types::tool_metadata::ToolMetadata for TaskOutputTool {
    ...
    fn description_template(&self) -> &str {
        ...
        // fallback mirrors the default xvora toolset.
...
mod tests {
    ...
    fn resources_with_backend_query() -> (
        ...
        tokio::sync::mpsc::UnboundedReceiver<
            crate::implementations::xvora::task::types::SubagentEvent,
        >,
    ) {
        use crate::implementations::xvora::task::backend::{
            ChannelBackend, SubagentBackendResource,
        };
        use crate::implementations::xvora::task::types::SubagentEvent;
    ...
    fn unwrap_query(
        event: crate::implementations::xvora::task::types::SubagentEvent,
    ) -> crate::implementations::xvora::task::types::SubagentQueryRequest {
        match event {
            crate::implementations::xvora::task::types::SubagentEvent::Query(q) => q,
