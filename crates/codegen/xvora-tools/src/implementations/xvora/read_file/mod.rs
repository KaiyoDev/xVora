...
crate::register_resource!("xvora", "ReadFile", ReadFileParams);
    ...
    async fn execute(
        ...
        let Some(spec) = admitted_spec else {
            ...
            return Box::pin(async_stream::stream! {
                yield tool_runtime::ToolStreamItem::Terminal(this.run(ctx, input)
                . await);
            });
        };
        Box::pin(async_stream::stream! {
            ...
            tool_runtime::stream_chunk(spec, & content[..window_end], window_end
            as u64, & mut last_total, false,) { yield
            tool_runtime::ToolStreamItem::Progress(p); } window_start =
            window_end; } } yield
            tool_runtime::ToolStreamItem::Terminal(Ok(output)); } Err(e) => yield
