...
//! (in `xvora-tools`) and the subagent coordinator (in `xvora-shell`).
//!
//! ## Resource types
//!
...
/// defense-in-depth check in `xvora-shell`'s subagent coordinator.
...
/// the cross-crate drain loop in `xvora-shell` relies on
...
/// xvora-shell produces it, the channel protocol carries it, and the
/// compaction pipeline in chat-state (via `RunningSubagentSummary`)
...
///
/// Set by xvora-shell immediately before a prompt turn begins executing so
