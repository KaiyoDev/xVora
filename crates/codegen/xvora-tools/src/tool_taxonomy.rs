...
//! `ToolInput`, proto, or runtime). A future `xvora-tool-taxonomy` leaf crate
...
mod tests {
    ...
    fn namespace_round_trips_snake_case_with_pascal_aliases() {
        ...
        fn wire_and_pascal(ns: ToolNamespace) -> (&'static str, &'static str) {
            match ns {
                ToolNamespace::Xvora => ("xvora", "Xvora"),
                ToolNamespace::XvoraConcise => ("xvora_concise", "XvoraConcise"),
                ToolNamespace::XvoraHashline => ("xvora_hashline", "XvoraHashline"),
