...
//! after every tool call to check for SKILL.md files in `.xvora/skills/`,
...
/// Directories that contain skill definitions (`.xvora/skills/`, `.agents/skills/`,
/// `.claude/skills/`, `.cursor/skills/`). Shared between startup skill discovery
/// and runtime `SkillDiscoveryReminder`.
pub const SKILL_CONFIG_DIRS: &[&str] = &[".xvora", ".agents", ".claude", ".cursor"];
...
impl SkillDiscoveryReminder {
    ...
    /// (`.xvora/skills/`, `.agents/skills/`, or `.claude/skills/`).
...
impl Reminder for SkillDiscoveryReminder {
    ...
    async fn collect_reminders(
        ...
        // discovery cannot find these because it looks for `.xvora/skills/`
        // sub-directories in *ancestor* dirs, and user-scope skills
        // (~/.xvora/) are outside the git root so the walk breaks early.
