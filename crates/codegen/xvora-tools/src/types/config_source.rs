...
pub enum ConfigSource {
    ...
    Builtin,
    /// Bundled skill shipped with the binary (extracted to ~/.xvora/skills/
    /// or injected via bundled skill dirs).
    Bundled { path: PathBuf },
    /// Server-synced (e.g. ~/.xvora/server-skills from the skill store).
    Server { path: PathBuf },
    /// Project-scoped: cwd/.xvora/ or cwd/.claude/.
    Project { path: PathBuf },
    /// User-scoped: ~/.xvora/ or ~/.claude/.
    User { path: PathBuf },
