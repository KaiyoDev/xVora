...
//!
//! `ToolSearchIndex` is defined in `xvora-tools` to keep the tool crate
//! backend-agnostic. The concrete implementation lives in `xvora-shell`
...
/// in `xvora-shell` maps `mcp_initialized` to `is_ready`.
...
/// serialized), injected by `xvora-shell` after MCP initialization.
#[derive(Clone)]
