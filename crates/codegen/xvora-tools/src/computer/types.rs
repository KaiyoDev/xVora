...
/// (`xvora-shell::extensions::task::KillTaskResponse`) and deserialized
/// by clients (xvora-pager), so it derives both serde directions.
...
///
/// Implemented by:
/// - `LocalTerminalBackend` (in xvora-tools, spawns processes)
/// - `AcpTerminalBackend` (in xvora-shell, calls ACP protocol)
#[async_trait::async_trait]
