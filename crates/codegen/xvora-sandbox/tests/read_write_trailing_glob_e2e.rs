...
//!
//! ```text
//! cargo test -p xvora-sandbox --test read_write_trailing_glob_e2e -- --nocapture
//! ```
...
fn trailing_glob_read_write_grants_parent_directory() {
    ...
    let xvora_home = root.join("xvora-home");
    ...
    for dir in [&workspace, &xvora_home, &cache.join("registry-a1b2")] {
        fs::create_dir_all(dir).expect("create fixture dir");
    }

    let cache_abs = dunce::canonicalize(&cache).expect("canonicalize cache");
    fs::write(
        xvora_home.join(config::SANDBOX_CONFIG_FILENAME),
        format!(
            "[profiles.cargo]\nextends = \"workspace\"\nread_write = [{:?}]\n",
...
fn subprocess_entry() {
    ...
    let profile: xvora_sandbox::ProfileName = "cargo".parse().expect("profile name");
    let mut mgr = xvora_sandbox::SandboxManager::new(profile, &workspace);
