//! Integration tests for xvora-sandbox.
//!
...
fn test_sandbox_logger() {
    use xvora_sandbox::{SandboxEvent, SandboxLogger};
...
fn test_should_restrict_child_network_default() {
    ...
    assert!(!xvora_sandbox::should_restrict_child_network());
}
