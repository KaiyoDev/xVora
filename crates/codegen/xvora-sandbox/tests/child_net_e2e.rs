...
fn spawn_probe(socket: &Path, filtered: bool) -> std::process::Output {
    ...
    if filtered {
        ...
        let filter = xvora_sandbox::child_net::prebuilt_child_network_filter();
        // SAFETY: the closure only runs prctl against the parent-built program.
        unsafe {
            cmd.pre_exec(move || xvora_sandbox::child_net::install_child_network_filter(filter));
        }
    }
...
fn restrict_child_network_std_arms_from_config_without_apply() {
    ...
    manager.install();
    assert!(
        xvora_sandbox::should_restrict_child_network(),
