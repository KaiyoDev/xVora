...
fn skip_if_enforcement_unavailable() -> bool {
    ...
    let support = xvora_sandbox::SandboxManager::support_info();
...
fn profile_from_env() -> xvora_sandbox::ProfileName {
    xvora_sandbox::ProfileName::Custom(std::env::var(PROFILE_ENV).expect(PROFILE_ENV))
}
...
fn subprocess_profile_and_bwrap_reexec(profile: &xvora_sandbox::ProfileName, workspace: &Path) {
    #[cfg(target_os = "linux")]
    {
        if !xvora_sandbox::is_inside_bwrap() {
            match xvora_sandbox::bwrap_reexec_for_profile(profile, workspace) {
                Some(mut cmd) => {
...
fn subprocess_block_deny(workspace: &Path) {
    ...
    let mut sandbox = xvora_sandbox::SandboxManager::new(profile.clone(), workspace);
    ...
    #[cfg(target_os = "linux")]
    match xvora_sandbox::verify_read_deny_enforced(&profile, workspace) {
...
fn subprocess_hook_write_deny_marker_spoof(_grok_home: &Path) {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        match xvora_sandbox::verify_hook_write_deny_enforced() {
            Ok(()) => {
...
fn subprocess_devbox_marker_spoof(workspace: &Path) {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let verified = xvora_sandbox::verify_data_write_deny_enforced(
            &xvora_sandbox::ProfileName::Devbox,
            workspace,
        );
        ...
        let mut sandbox =
            xvora_sandbox::SandboxManager::new(xvora_sandbox::ProfileName::Devbox, workspace);
        if let Err(e) = sandbox.apply(workspace) {
            eprintln!("sandbox apply failed: {e}");
            std::process::exit(3);
        }
...
fn subprocess_devbox_genuine(workspace: &Path) {
    let profile = xvora_sandbox::ProfileName::Devbox;
    subprocess_profile_and_bwrap_reexec(&profile, workspace);
    let mut sandbox = xvora_sandbox::SandboxManager::new(profile, workspace);
    if let Err(e) = sandbox.apply(workspace) {
        eprintln!("sandbox apply failed: {e}");
        std::process::exit(3);
    }
    ...
    #[cfg(target_os = "linux")]
    if let Err(e) = xvora_sandbox::verify_data_write_deny_enforced(
        &xvora_sandbox::ProfileName::Devbox,
        workspace,
    ) {
