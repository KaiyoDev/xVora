...
//! cargo run -p xvora-sandbox --example sandbox_smoke_test
//!
//! # Test strict profile
//! cargo run -p xvora-sandbox --example sandbox_smoke_test -- strict
//!
//! # Test read-only profile
//! cargo run -p xvora-sandbox --example sandbox_smoke_test -- read-only
//! ```
...
fn main() {
    ...
    let tmp_test = Path::new("/tmp/.xvora-sandbox-test");
