//! XvoraEnvironment configuration for the shell crate family.
//!
//! The environment presets (per-environment endpoint URLs, the staging
//! trust check, `EnvVarGuard`) live in the [`xai_xvora_env`] leaf crate so
...
//! - `XVORA_GATEWAY_URL` — when set to a valid URL, `MvpAgent` spawns a
...
#[cfg(test)]
pub use xai_xvora_env::EnvVarGuard;
pub use xai_xvora_env::{
...
pub const XVORA_GATEWAY_URL_ENV: &str = "XVORA_GATEWAY_URL";
...
pub const XVORA_DISABLE_CUSTOM_BRIDGE_ENV: &str = "XVORA_DISABLE_CUSTOM_BRIDGE";
/// `true` when the custom-method bridge passthrough is force-disabled via
/// [`XVORA_DISABLE_CUSTOM_BRIDGE_ENV`]. Accepts `1`/`true` (case-insensitive).
pub fn custom_bridge_disabled() -> bool {
    std::env::var(XVORA_DISABLE_CUSTOM_BRIDGE_ENV)
        .map(|v| {
            let v = v.trim();
...
/// Parse `XVORA_GATEWAY_URL` into a [`url::Url`]. Unset, empty, or
...
pub fn parse_gateway_url() -> Option<url::Url> {
    let raw = std::env::var(XVORA_GATEWAY_URL_ENV).ok()?;
    if raw.is_empty() {
        return None;
    }
    if true {
        tracing::warn!(
            env = XVORA_GATEWAY_URL_ENV,
            "XVORA_GATEWAY_URL is set but this build lacks the `chat` feature; staying in local mode"
        );
        return None;
    }
    match url::Url::parse(&raw) {
        ...
        Err(err) => {
            tracing::warn!(
                env = XVORA_GATEWAY_URL_ENV, error = % err,
                "XVORA_GATEWAY_URL is not a valid URL; falling back to local mode (raw value omitted to avoid leaking userinfo)"
            );
            None
        }
    }
...
mod tests {
    ...
    fn parse_gateway_url_returns_none_when_unset() {
        let _env = EnvVarGuard::remove(XVORA_GATEWAY_URL_ENV);
        assert!(parse_gateway_url().is_none());
    }
    #[test]
    fn parse_gateway_url_returns_none_when_empty() {
        let _env = EnvVarGuard::set(XVORA_GATEWAY_URL_ENV, "");
        assert!(parse_gateway_url().is_none());
    }
    #[test]
    fn parse_gateway_url_returns_none_for_malformed_url() {
        let _env = EnvVarGuard::set(XVORA_GATEWAY_URL_ENV, "not a url");
        assert!(
    ...
    fn custom_bridge_disabled_defaults_false_when_unset() {
        let _env = EnvVarGuard::remove(XVORA_DISABLE_CUSTOM_BRIDGE_ENV);
        assert!(!custom_bridge_disabled());
    }
    #[test]
    fn custom_bridge_disabled_true_for_one_and_true() {
        for v in ["1", "true", "TRUE", " true "] {
            let _env = EnvVarGuard::set(XVORA_DISABLE_CUSTOM_BRIDGE_ENV, v);
            assert!(
