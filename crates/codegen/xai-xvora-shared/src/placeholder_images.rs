...
//! image bytes.
//!
//! Both the TUI ([`xai_xvora_pager::prompt_images`]) and the server-side
...
//! [`xai_xvora_pager::prompt_images::display_text`]. The shape of this
...
/// position (the two diverge — see `AttachedImages` in `xai-xvora-tools`).
...
/// [`AttachedImages`](xai_xvora_tools::types::resources::AttachedImages))
...
///   separator — see [`xai_xvora_pager::prompt_images::display_text`].
...
/// [`xai_xvora_pager::prompt_images::read_image_at_path`], which has no
...
fn decode_image_mime(data: &[u8]) -> Option<&'static str> {
    xai_xvora_tools::util::image_validate::validate_image_bytes_with(data, false)
        .ok()
        .map(|(_, _, mime)| mime)
}
...
mod tests {
    ...
    /// `xai_xvora_pager::prompt_images::display_text`. A
