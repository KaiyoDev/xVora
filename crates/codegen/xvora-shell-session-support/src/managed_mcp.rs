...
///
/// Single source of truth lives in `xvora-workspace` (which matches policy
/// `serverName`s against it); re-exported here so the two never drift.
pub use xvora_workspace::permission::resolution::MANAGED_MCP_PREFIX;
...
pub fn to_managed_name(display_name: &str) -> String {
    ...
    xvora_shell_base::util::truncate(&raw, MANAGED_MCP_NAME_MAX_CHARS).to_string()
}
