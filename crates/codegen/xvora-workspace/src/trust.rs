...
//!
//! The store is rooted at [`xvora_config::user_xvora_home`] — the **Option**
...
impl TrustStore {
    ...
    ///
    /// Resolves via [`xvora_config::user_xvora_home`], never
    /// [`xvora_config::xvora_home`], so it never falls back to a cwd-relative
    ...
    pub fn default_path() -> Option<PathBuf> {
        Self::default_path_in(xvora_config::user_xvora_home())
    }
...
pub fn migrate_legacy_hook_trust() {
    ...
    MIGRATED.call_once(|| {
        let Some(legacy_file) = xvora_hooks::trust::legacy_trust_file_path() else {
            return;
        };
...
fn migrate_legacy_hook_trust_in(legacy_file: &Path, store: &mut TrustStore) -> usize {
    ...
    let projects = match xvora_hooks::trust::list_trusted_projects_with_file(legacy_file) {
        Ok(p) => p,
        Err(e) => {
            tracing::warn!(
...
mod tests {
    ...
    fn default_path_sources_from_user_xvora_home() {
        ...
        assert_eq!(
            ...
            xvora_config::user_xvora_home().map(|h| h.join(TRUST_FILE_NAME))
        );
    }
