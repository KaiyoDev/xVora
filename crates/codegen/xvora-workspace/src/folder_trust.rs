...
//! provisional and re-checked rather than cached — is a `xvora-shell`
...
use xvora_config_types::{BoolFlag, RemoteSettings};
...
///
/// Kept local (not in `xvora-version`) on purpose: adding a symbol to that
...
mod tests {
    ...
    // tests (the workspace crate has no `serial_test`/`xvora-test-support`
