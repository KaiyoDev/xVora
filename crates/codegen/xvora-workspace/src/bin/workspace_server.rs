...
use url::Url;
use xvora_workspace::config::WorkspaceServerMetadata;
use xvora_workspace::daemonize;
use xvora_workspace::diag_server;
use xvora_workspace::preview_supervisor::{self, PreviewArgs, PreviewVisibility};
...
/// `/usr/local/bin/xvora-preview-proxy` child (see `cli.rs` for the proxy's
...
async fn run(args: Args, cwd: PathBuf) -> anyhow::Result<()> {
    ...
    let direct_otlp = match std::env::var("XVORA_WORKSPACE_OTLP_ENDPOINT") {
        Ok(endpoint) if !endpoint.is_empty() => {
            match tracing_util::init_fastrace(endpoint.clone(), SERVICE_NAME.to_owned(), None) {
                Ok(()) => {
    ...
    {
        use xvora_sandbox::{ProfileName, SandboxManager};
        let profile = match std::env::var("XVORA_SANDBOX_PROFILE").ok() {
            ...
            None if xvora_sandbox::trust_bwrap_marker_for_devbox() => ProfileName::Devbox,
        ...
        if profile == ProfileName::Off {
            ...
            sandbox.install();
            let active = xvora_sandbox::is_active();
            ...
            tracing::info!(
                ...
                xvora_sandbox::should_restrict_child_network(), "{status_msg}"
            );
        }
    }
    let auth_provider = xvora_workspace::hub_auth::provider(&url, args.auth_config.as_deref())?;
    ...
    let ws_handle = xvora_workspace::handle::connect_local_workspace(
        cwd,
        url,
        auth_provider,
        metadata,
