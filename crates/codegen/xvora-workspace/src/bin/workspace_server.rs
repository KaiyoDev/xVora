...
/// Forwarded 1:1 to the `/usr/local/bin/xvora-preview-proxy` child (see `cli.rs` for the proxy's flag names).
    ...
    let url = Url::parse(&args.hub_url).map_err(|e| anyhow::anyhow!("invalid --hub-url: {e}"))?;
    {
        ...
        if profile == ProfileName::Off {
            ...
            tracing::info!(
                ...
                active,
                restrict_network_at_known_linux_launches = xvora_sandbox::should_restrict_child_network(),
                "{status_msg}"
            );
        }
    }
...
mod tests {
    ...
    fn oom_protect_flag_parses() {
        let args = Args::try_parse_from(["xvora-workspace-server", "--oom-protect"]).unwrap();
        assert!(args.oom_protect);
    }
    #[test]
