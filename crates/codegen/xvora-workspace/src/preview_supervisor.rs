...
//! spawns the unchanged `/usr/local/bin/xvora-preview-proxy` binary as a
...
impl PreviewArgs {
    ...
    /// `xvora-preview-proxy/src/cli.rs`). Absent options and a false
...
/// `xvora-preview-proxy`'s `/__control/activity` route).
...
/// the default in `xvora-preview-proxy/src/cli.rs`).
