...
//! FsNotify adapters that forward [`fsnotify`] events to workspace subsystems (hunk tracker, codebase graph, workspace event broadcast).
//!
...
/// Convert [`fsnotify::FsEventKind`] to the wire-type [`workspace_types::FsEventKind`].
...
/// Each affected path is re-broadcast as [`WorkspaceEvent::FsChanged`](workspace_types::WorkspaceEvent::FsChanged) on the event bus.
///
/// The task exits when:
