...
/// Convert [`xvora_fsnotify::FsEventKind`] to the wire-type [`xvora_workspace_types::FsEventKind`].
...
pub(crate) fn to_workspace_event_kind(kind: FsEventKind) -> xvora_workspace_types::FsEventKind {
    match kind {
        FsEventKind::Created => xvora_workspace_types::FsEventKind::Created,
        FsEventKind::Modified => xvora_workspace_types::FsEventKind::Modified,
        FsEventKind::Removed => xvora_workspace_types::FsEventKind::Removed,
        FsEventKind::Renamed => xvora_workspace_types::FsEventKind::Renamed,
        // `#[non_exhaustive]` fallback.
        _ => xvora_workspace_types::FsEventKind::Modified,
    }
}

/// Spawn a background task that reads [`FsEvent`]s from a broadcast receiver and forwards `FilesChanged` to the hunk tracker.
/// Each affected path is re-broadcast as [`WorkspaceEvent::FsChanged`](xvora_workspace_types::WorkspaceEvent::FsChanged) on the event bus.
///
/// The task exits when:
                        ...
                        Ok(FsEvent::FilesChanged { ref paths, kind }) => {
                            ...
                            for path in paths {
                                let _ = events_tx.send(
                                    xvora_workspace_types::WorkspaceEvent::FsChanged {
                                        path: path.clone(),
                                        kind: ws_kind,
                                    },
                                );
...
pub(crate) async fn refresh_codebase_graph_after_head_change(
    ...
    xvora_tools::util::detach_command(&mut diff_cmd);
    diff_cmd.envs(xvora_tools::util::pager_env());
    ...
    if let Some(count) = files_updated {
        let _ = events_tx.send(
            xvora_workspace_types::WorkspaceEvent::CodebaseIndexUpdated {
                files_indexed: count,
            },
        );
    }
...
pub(crate) fn ws_event_to_codebase_graph_event(
    ...
    let graph_kind = match kind {
        ...
        xvora_workspace_types::FsEventKind::Removed => FileEventKind::Removed,
        xvora_workspace_types::FsEventKind::Renamed => FileEventKind::Renamed,
    };
...
mod tests {
    ...
    fn workspace_event_kind_round_trip() {
        use xvora_workspace_types::FsEventKind as WsKind;
        assert_eq!(
