...
//!
//! Delegates to the existing discovery implementations in `xvora-agent` and `xvora-tools` rather than duplicating them.
//! The workspace stores configuration on [`WorkspaceShared`] and the [`MpscChannel`] methods call these helpers.
//!
//! Re-exports [`AgentsMdTracker`] from `xvora-tools` for external consumers that need per-session project-instruction tracking.
