...
//! `xvora-agent` and `xvora-tools` rather than duplicating them.
...
//!
//! Re-exports [`AgentsMdTracker`] from `xvora-tools` for external
...
// workspace crate without a direct xvora-tools dependency.
...
///
/// Delegates to [`xvora_agent::prompt::skills::list_skills`] with
...
///
/// Delegates to [`xvora_agent::plugins::discover_plugins`] with
