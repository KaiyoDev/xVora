...
/// (highest priority), matching `xvora-shell::config::find_project_configs`.
...
fn load_config_toml_permissions(cwd: &Path) -> Vec<Sourced<PermissionRule>> {
    ...
    if let Some(global_path) = xvora_config::user_xvora_home().map(|g| g.join("config.toml"))
        && global_path.is_file()
    {
...
/// [`xvora_config::claude_managed_settings_path`] (platform path such as
...
mod tests {
    ...
    async fn managed_config_toml_rules_resolve_as_non_admin_defaults() {
        ...
        let layers =
            xvora_config::managed_config_layers_at(Some(system.path()), Some(user.path()));
