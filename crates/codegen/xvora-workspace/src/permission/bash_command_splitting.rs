...
///
/// ```bash
/// XAI_API_KEY='xvora-some-key' cargo run --bin xvora-pager
/// ```
///
...
///   prefix: ["XAI_API_KEY=xvora-some-key"]
///   highlighted_words: ["cargo", "run", "--bin", "xvora-pager"]
///   suffix: []
...
mod tests {
    ...
    fn test_parse_plain_commands_from_script() {
        ...
        let environment_key_command = "XAI_API_KEY='xvora-some-key' cargo run --bin xvora-pager";
        assert_eq!(
            primary_command_from_script(environment_key_command),
            Some(BashCommandHighlights {
                prefix: vec!["XAI_API_KEY=xvora-some-key".to_owned()],
                highlighted_words: vec![
                    "cargo".to_owned(),
                    "run".to_owned(),
                    "--bin".to_owned(),
                    "xvora-pager".to_owned()
                ],
                suffix: vec![],
            })
        );
        ...
        let redirection_command = "cargo build --bin xvora-pager 2>&1";
        assert_eq!(
        ...
        let another_long_command = "cargo test --package xvora-shell --lib -- permission::bash_command_splitting::tests::test_parse_plain_commands_from_script --exact --nocapture 2>&1";
        assert_eq!(
