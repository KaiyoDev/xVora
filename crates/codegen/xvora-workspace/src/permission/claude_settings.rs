...
// permissions), consider extracting to a shared helper (e.g., in xvora-hooks
...
// =============================================================================
//
// `xvora-shell::claude_import` writes the marker. We re-implement a small
...
pub fn is_claude_import_marked() -> bool {
    ...
    // lives in xvora-shell (inaccessible from here at runtime). They also
    ...
    let Some(config_path) = xvora_config::user_xvora_home().map(|g| g.join("config.toml")) else {
        return false;
    };
