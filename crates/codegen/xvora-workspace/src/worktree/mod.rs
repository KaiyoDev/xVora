...
//!
//! Moved from `xvora-shell/src/session/worktree.rs` into the workspace
...
/// `worktrees.db` (`fast_worktree::resolve_xvora_home`), so checkout dirs and
/// the metadata DB always live under the same `.xvora` tree. That resolver
/// canonicalizes its `$HOME` fallback to match `xvora_config::xvora_home()`,
/// so worktree paths also agree with trust/hooks and other xvora-home paths.
fn xvora_home() -> std::path::PathBuf {
    fast_worktree::resolve_xvora_home().unwrap_or_else(|_| {
        dirs::home_dir()
