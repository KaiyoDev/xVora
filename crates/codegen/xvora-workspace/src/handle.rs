...
impl WorkspaceHandle {
    ...
    fn build(
        ...
        let (hook_registry, hook_load_errors) = {
            use xvora_hooks::discovery::{HookSource, load_hooks_from_sources};
    ...
    pub fn create_session_with_tracker_and_viewer_ctx(
        ...
        let system_notify_channel = system_notifications
            .then(xvora_tools::notification::types::ToolNotificationHandle::channel);
    ...
    pub fn hook_registry(&self) -> xvora_hooks::discovery::HookRegistry {
    ...
    pub fn hook_load_errors(&self) -> &[xvora_hooks::error::HookError] {
        ...
        let env = {
            ...
            .in_span(
                fastrace::Span::root(
                    ...
                    trace_parent.unwrap_or_else(tracing_util::local_or_random_span_ctx),
                )
                .with_properties(|| {
                    [
                        ("session_id", session_id.to_owned()),
        ...
        Arc::new(
            move |sid: tool_protocol::SessionId, params: Option<serde_json::Value>| {
                ...
                let bind_parent = params
                    ...
                    .unwrap_or_else(tracing_util::local_or_random_span_ctx);
...
pub async fn connect_local_workspace(
    ...
    let tool_config = xvora_agent::workspace_xvora_toolset();
...
fn rewind_all_outcomes_from_env() -> bool {
    xvora_config::env_bool("XVORA_WORKSPACE_REWIND_ALL_OUTCOMES").unwrap_or(false)
}
...
pub(crate) mod tests {
    ...
    async fn fork_session_cwd_override_used_when_set() {
        let handle = make_handle();
        let alt = std::env::temp_dir().join("xvora-workspace-test-cwd-override");
    ...
    async fn hook_registry_snapshot_is_disconnected() {
        ...
        {
            let spec = xvora_hooks::config::HookSpec {
                name: "injected".into(),
                event: xvora_hooks::event::HookEventName::SessionStart,
