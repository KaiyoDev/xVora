...
use dashmap::DashMap;
use file_utils::events::{Event, EventWriter, ToolOutcome};
use file_utils::queue::UploadQueueStats;
use tool_protocol::{ToolServerLifecycleStatus, ToolServerStatusPayload};
...
/// `XVORA_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS`.
...
pub struct ActivityTracker {
    ...
    /// `XVORA_WORKSPACE_EVENTS_ENABLED` is off). Consumed by
...
impl ActivityTracker {
    ...
    /// idle-hold cap comes from `XVORA_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS`
...
/// The durability idle-hold cap from `XVORA_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS`.
fn durability_idle_hold_max_from_env() -> u64 {
    durability_idle_hold_from_raw(std::env::var("XVORA_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS").ok())
}
