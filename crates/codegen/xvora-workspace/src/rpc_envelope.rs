...
//! (the envelope types are canonical in `xvora_workspace_types::rpc`).
//!
...
pub fn rpc_error_to_workspace(err: RpcError) -> WorkspaceError {
    if let Some(kind) = xvora_workspace_types::rpc::deploy::DeployError::from_wire_code(&err.code) {
...
mod tests {
    ...
    fn error_code_round_trip_all_variants() {
        ...
        variants.extend(
            xvora_workspace_types::rpc::deploy::DeployError::ALL
                .into_iter()
