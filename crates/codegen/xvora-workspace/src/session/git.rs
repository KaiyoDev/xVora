...
///
/// 1. `supplied_cwd` is a grok-managed worktree (`~/.xvora/worktrees/...`).
...
pub fn restore_code_checkout_allowed(supplied_cwd: &Path, persisted_cwd: Option<&str>) -> bool {
    let worktrees_dir = xvora_tools::util::xvora_home::xvora_home().join("worktrees");
...
const REWIND_GIT_ENV: &str = "XVORA_WORKSPACE_REWIND_GIT";
...
mod tests {
    ...
    fn strip_url_credentials_removes_token() {
        let url_with_token = "https://x-access-token:secret-token@github.com/KaiyoDev/example.git";
        assert_eq!(
            strip_url_credentials(url_with_token),
            "https://github.com/KaiyoDev/example.git"
        );
    }
    #[test]
    fn strip_url_credentials_preserves_clean_https_url() {
        let clean_url = "https://github.com/KaiyoDev/example.git";
        assert_eq!(strip_url_credentials(clean_url), clean_url);
    }
    #[test]
    fn strip_url_credentials_preserves_ssh_url() {
        let ssh_url = "git@github.com:KaiyoDev/example.git";
        assert_eq!(strip_url_credentials(ssh_url), ssh_url);
    }
    #[test]
    fn strip_url_credentials_removes_username_password() {
        let url_with_creds = "https://alice:secret@github.com/KaiyoDev/example.git";
        assert_eq!(
    ...
    fn test_resolve_persisted_session_git_metadata_collects_sorted_unique_remotes() {
        ...
        .unwrap();
        repo.remote("backup", "https://gitlab.com/KaiyoDev/example.git")
            .unwrap();
        repo.remote("duplicate", "https://github.com/KaiyoDev/example.git")
            .unwrap();
        ...
        assert_eq!(
            ...
            vec![
                "https://github.com/KaiyoDev/example.git".to_string(),
                "https://gitlab.com/KaiyoDev/example.git".to_string(),
            ]
        );
    }
    #[test]
    fn test_resolve_persisted_session_git_metadata_captures_head() {
        let tmp = tempfile::tempdir().unwrap();
        let repo = git2::Repository::init(tmp.path()).unwrap();
        repo.remote("origin", "https://github.com/KaiyoDev/example.git")
            .unwrap();
        let metadata = resolve_persisted_session_git_metadata_sync(tmp.path());
    ...
    fn test_resolve_persisted_session_git_metadata_worktree_resolves_remotes() {
        ...
        assert_eq!(
            metadata.git_remotes,
            vec!["https://github.com/KaiyoDev/example.git".to_string()],
        );
    }
    #[test]
    ...
    fn test_effective_worktree_cwd_empty_offset() {
        let result =
            effective_worktree_cwd("/home/user/.xvora/worktrees/repo/ab-123-a", Path::new(""));
    ...
    fn normalize_ssh_scp_url() {
        assert_eq!(
            normalize_repo_url("git@github.com:KaiyoDev/example.git"),
            Some("github.com/KaiyoDev/example".into()),
        );
    }
    #[test]
    fn normalize_https_url() {
        assert_eq!(
            normalize_repo_url("https://github.com/KaiyoDev/example.git"),
            Some("github.com/KaiyoDev/example".into()),
        );
    }
    #[test]
    fn normalize_ssh_and_https_produce_same_result() {
        let ssh = normalize_repo_url("git@github.com:KaiyoDev/example.git");
        let https = normalize_repo_url("https://github.com/KaiyoDev/example.git");
        assert_eq!(ssh, https);
    }
    #[test]
    fn normalize_https_without_git_suffix() {
        assert_eq!(
            normalize_repo_url("https://github.com/KaiyoDev/example"),
            Some("github.com/KaiyoDev/example".into()),
        );
    }
    #[test]
    fn normalize_https_with_credentials() {
        assert_eq!(
            normalize_repo_url("https://x-access-token:secret@github.com/KaiyoDev/example.git"),
            Some("github.com/KaiyoDev/example".into()),
        );
    }
    #[test]
    fn normalize_ssh_scheme_url() {
        assert_eq!(
            normalize_repo_url("ssh://git@github.com/KaiyoDev/example.git"),
            Some("github.com/KaiyoDev/example".into()),
        );
    }
    #[test]
    fn normalize_ssh_scheme_with_port() {
        assert_eq!(
            normalize_repo_url("ssh://git@github.com:22/KaiyoDev/example.git"),
            Some("github.com/KaiyoDev/example".into()),
        );
    }
    #[test]
    ...
    fn normalize_http_url() {
        assert_eq!(
            normalize_repo_url("http://github.com/KaiyoDev/example.git"),
            Some("github.com/KaiyoDev/example".into()),
        );
    }
    #[test]
    ...
    fn normalize_git_plus_ssh_scheme() {
        assert_eq!(
            normalize_repo_url("git+ssh://git@github.com/KaiyoDev/example.git"),
            Some("github.com/KaiyoDev/example".into()),
        );
    }
    #[test]
    fn normalize_git_plus_https_scheme() {
        assert_eq!(
            normalize_repo_url("git+https://github.com/KaiyoDev/example.git"),
            Some("github.com/KaiyoDev/example".into()),
        );
    }
    #[test]
    fn normalize_scp_no_user() {
        assert_eq!(
            normalize_repo_url("github.com:KaiyoDev/example.git"),
            Some("github.com/KaiyoDev/example".into()),
        );
    }
    #[test]
    ...
    fn resolve_normalized_remote_urls_deduplicates_across_transports() {
        let tmp = tempfile::tempdir().unwrap();
        let repo = git2::Repository::init(tmp.path()).unwrap();
        repo.remote("origin", "git@github.com:KaiyoDev/example.git")
            .unwrap();
        repo.remote("https-mirror", "https://github.com/KaiyoDev/example.git")
            .unwrap();
        let urls = resolve_normalized_remote_urls(tmp.path());
        assert_eq!(urls, vec!["github.com/KaiyoDev/example"]);
    }
}
#[cfg(test)]
...
mod restore_code_tests {
    ...
    fn restore_code_checkout_allowed_worktree_cwd_is_allowed() {
        let worktrees = Path::new("/home/u/.xvora/worktrees");
        assert!(restore_code_checkout_allowed_in(
            Path::new("/home/u/.xvora/worktrees/home-u-repo/2026-05-22-9f2e51ce"),
            Some("/home/u/repo"),
            worktrees,
        ));
    }
    ...
    fn restore_code_checkout_allowed_source_repo_with_worktree_session_is_refused() {
        ...
        assert!(!restore_code_checkout_allowed_in(
            Path::new("/home/u/repo"),
            Some("/home/u/.xvora/worktrees/home-u-repo/2026-05-22-9f2e51ce"),
            worktrees,
        ));
    }
    #[test]
