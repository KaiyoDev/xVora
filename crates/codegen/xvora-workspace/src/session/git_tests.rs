...
fn strip_url_credentials_removes_token() {
    let url_with_token = "https://x-access-token:secret-token@github.com/xvora-org/example.git";
    assert_eq!(
        strip_url_credentials(url_with_token),
        "https://github.com/xvora-org/example.git"
    );
}

#[test]
fn strip_url_credentials_preserves_clean_https_url() {
    let clean_url = "https://github.com/xvora-org/example.git";
    assert_eq!(strip_url_credentials(clean_url), clean_url);
}

#[test]
fn strip_url_credentials_preserves_ssh_url() {
    let ssh_url = "git@github.com:xvora-org/example.git";
    assert_eq!(strip_url_credentials(ssh_url), ssh_url);
}

#[test]
fn strip_url_credentials_removes_username_password() {
    let url_with_creds = "https://alice:secret@github.com/xvora-org/example.git";
    assert_eq!(
...
fn test_resolve_persisted_session_git_metadata_collects_sorted_unique_remotes() {
    ...
    repo.remote("backup", "https://gitlab.com/xvora-org/example.git")
        .unwrap();
    // Same effective URL as origin after credential stripping, so it tests dedup
    repo.remote("duplicate", "https://github.com/xvora-org/example.git")
        .unwrap();
    ...
    assert_eq!(
        ...
        vec![
            "https://github.com/xvora-org/example.git".to_string(),
            "https://gitlab.com/xvora-org/example.git".to_string(),
        ]
    );
}
...
fn test_resolve_persisted_session_git_metadata_captures_head() {
    let tmp = tempfile::tempdir().unwrap();
    let repo = git2::Repository::init(tmp.path()).unwrap();
    repo.remote("origin", "https://github.com/xvora-org/example.git")
        .unwrap();
...
fn test_resolve_persisted_session_git_metadata_worktree_resolves_remotes() {
    ...
    assert_eq!(
        metadata.git_remotes,
        vec!["https://github.com/xvora-org/example.git".to_string()],
    );
}
...
fn collapse_home_path_requires_whole_component() {
    ...
    assert_eq!(
        collapse_home_path(Path::new("/Users/user/xvora"), Some(home)),
        "/Users/user/xvora"
    );
    assert_eq!(
...
async fn get_worktree_info_standalone_grok_marker() {
    ...
    std::fs::write(
        clone.join(".git").join("xvora-worktree-source"),
        main.display().to_string(),
    )
    .unwrap();
    assert!(clone.join(".git").is_dir());
    assert!(clone.join(".git").join("xvora-worktree-source").is_file());
...
async fn get_worktree_info_tilde_collapses_home_prefix() {
    ...
    let fake_main = home.join("xvora-fake-main-repo-for-wt-display");
    std::fs::write(
        clone.join(".git").join("xvora-worktree-source"),
    ...
    assert_eq!(
        main_repo.as_deref(),
        Some("~/xvora-fake-main-repo-for-wt-display")
    );
}
...
fn get_worktree_info_db_record_without_marker() {
    let tmp = tempfile::tempdir().unwrap();
    let home = dunce::canonicalize(tmp.path()).unwrap().join("xvora-home");
    std::fs::create_dir_all(&home).unwrap();
    let _env = crate::LockedTestEnv::lock().set("xvora_home", &home);
...
fn test_effective_worktree_cwd_empty_offset() {
    let result = effective_worktree_cwd("/home/user/.xvora/worktrees/repo/ab-123-a", Path::new(""));
    assert_eq!(result, "/home/user/.xvora/worktrees/repo/ab-123-a");
}
...
fn test_effective_worktree_cwd_nested_offset() {
    let result = effective_worktree_cwd(
        "/home/user/.xvora/worktrees/repo/ab-123-b",
        Path::new("packages/frontend/src"),
    );
    assert_eq!(
        result,
        "/home/user/.xvora/worktrees/repo/ab-123-b/packages/frontend/src"
    );
}
...
fn test_effective_cwd_roundtrip_with_compute_offset() {
    ...
    let worktree_root = "/home/user/.xvora/worktrees/myrepo/ab-test-a";
...
fn normalize_ssh_scp_url() {
    assert_eq!(
        normalize_repo_url("git@github.com:xvora-org/example.git"),
        Some("github.com/xvora-org/example".into()),
    );
}
...
fn normalize_https_url() {
    assert_eq!(
        normalize_repo_url("https://github.com/xvora-org/example.git"),
        Some("github.com/xvora-org/example".into()),
    );
}
...
fn normalize_ssh_and_https_produce_same_result() {
    let ssh = normalize_repo_url("git@github.com:xvora-org/example.git");
    let https = normalize_repo_url("https://github.com/xvora-org/example.git");
    assert_eq!(ssh, https);
}
...
fn normalize_https_without_git_suffix() {
    assert_eq!(
        normalize_repo_url("https://github.com/xvora-org/example"),
        Some("github.com/xvora-org/example".into()),
    );
}
...
fn normalize_https_with_credentials() {
    assert_eq!(
        normalize_repo_url("https://x-access-token:secret@github.com/xvora-org/example.git"),
        Some("github.com/xvora-org/example".into()),
    );
}
...
fn normalize_ssh_scheme_url() {
    assert_eq!(
        normalize_repo_url("ssh://git@github.com/xvora-org/example.git"),
        Some("github.com/xvora-org/example".into()),
    );
}
...
fn normalize_ssh_scheme_with_port() {
    assert_eq!(
        normalize_repo_url("ssh://git@github.com:22/xvora-org/example.git"),
        Some("github.com/xvora-org/example".into()),
    );
}
...
fn normalize_http_url() {
    assert_eq!(
        normalize_repo_url("http://github.com/xvora-org/example.git"),
        Some("github.com/xvora-org/example".into()),
    );
}
...
fn normalize_git_plus_ssh_scheme() {
    assert_eq!(
        normalize_repo_url("git+ssh://git@github.com/xvora-org/example.git"),
        Some("github.com/xvora-org/example".into()),
    );
}
...
fn normalize_git_plus_https_scheme() {
    assert_eq!(
        normalize_repo_url("git+https://github.com/xvora-org/example.git"),
        Some("github.com/xvora-org/example".into()),
    );
}
...
fn normalize_scp_no_user() {
    assert_eq!(
        normalize_repo_url("github.com:xvora-org/example.git"),
        Some("github.com/xvora-org/example".into()),
    );
}
...
fn resolve_normalized_remote_urls_deduplicates_across_transports() {
    let tmp = tempfile::tempdir().unwrap();
    let repo = git2::Repository::init(tmp.path()).unwrap();
    repo.remote("origin", "git@github.com:xvora-org/example.git")
        .unwrap();
    repo.remote("https-mirror", "https://github.com/xvora-org/example.git")
        .unwrap();

    let urls = resolve_normalized_remote_urls(tmp.path());
    assert_eq!(urls, vec!["github.com/xvora-org/example"]);
}
