//! Wire types for the `xvora-workspace` API.
//!
...
//! `xvora-workspace-grpc` crate. The Rust types defined here are
//! the source of truth.
...
///
/// Lives in `xvora-workspace-types` (instead of `xvora-workspace` or
/// `xvora-mcp`) so both the permission-validation layer and the MCP
/// transport layer can depend on it without dragging the full workspace or
/// rmcp into each other. Re-exported by `xvora_workspace::permission`
