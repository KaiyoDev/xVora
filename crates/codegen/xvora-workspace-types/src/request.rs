...
//!   Adding it here would force every consumer of `xvora-workspace-types` (including the eventual WASM browser SDK) to pull in tokio.
...
//! The runtime crate (`xvora-workspace`) wraps this in its own `Request<T>` that adds the cancellation token and extensions map.
//!
//! Splitting the envelope this way keeps `xvora-workspace-types` tokio-free.
