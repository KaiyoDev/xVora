...
//!   here would force every consumer of `xvora-workspace-types`
...
//! (`xvora-workspace`) wraps this in its own `Request<T>` that
//! adds the cancellation token and extensions map.
//!
//! Splitting the envelope this way keeps `xvora-workspace-types`
