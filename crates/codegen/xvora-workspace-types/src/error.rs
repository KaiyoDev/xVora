...
//!
//! - **`Tool`**: a natural definition would be `Tool(#[from] xvora_tools::ToolError)`.
//!   That coupling would force this crate to depend on
//!   `xvora-tools`, which would defeat the lightweight
...
/// notably `std::io::Error` and `xvora_tools::ToolError`) happens
