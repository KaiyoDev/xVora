...
//!
//! `xvora_hooks` pulls in `git2`/`reqwest`/`xvora-tools`, too heavy for
...
/// Wire mirror of `xvora_hooks::discovery::HookRegistry`.
///
...
/// Wire mirror of `xvora_hooks::config::HookSpec`.
///
...
/// `xvora-hooks`, so a server-side test (`xvora-workspace`'s
...
/// Wire mirror of `xvora_hooks::event::HookEventName`.
///
