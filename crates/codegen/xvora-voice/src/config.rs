...
/// owned by the pager (`XVORA_VOICE_MODE` / `[features] voice_mode` / remote);
...
pub struct VoiceConfig {
    ...
    ///
    /// `x-grok-client-identifier` value (e.g. `"xvora-shell"`).
    #[serde(skip)]
    pub client_identifier: String,
    /// `User-Agent` value (e.g. `"xvora-shell/1.2.3 (macos; aarch64)"`).
    #[serde(skip)]
...
mod tests {
    ...
    /// voice gate — default on, remote kill switch / `XVORA_VOICE_MODE`).
    #[test]
