...
//! `xvora-shell` and `xvora-pager` can depend on it without pulling
//! in domain logic.
//!
...
/// Plugin scope.
///
/// Maps from `PluginScope` in `xvora-agent`. Variant renames:
...
///
/// Maps from `PluginOrigin` in `xvora-agent`. Optional on [`PluginInfo`]
...
/// Hook event type.
///
/// Maps from `HookEventName` in `xvora-hooks`. The source type's
