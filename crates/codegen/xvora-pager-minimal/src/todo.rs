...
use xvora_shell::tools::TodoStatus;
    ...
    let mut lines: Vec<Line<'static>> = todos
        ...
        .map(|t| {
            let (glyph, style) = match t.status {
                ...
                TodoStatus::Completed => (xvora_pager::glyphs::check_mark(), theme.muted()),
                TodoStatus::Cancelled => (
                    xvora_pager::glyphs::ballot_x(),
...
mod tests {
    ...
    use xvora_shell::tools::{TodoItem, TodoPriority};
