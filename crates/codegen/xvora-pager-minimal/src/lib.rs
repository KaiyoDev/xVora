...
//! # Wiring
//!
//! `xvora-pager` (the lib) does **not** depend on this crate — that would be
...
//! seam ([`xvora_pager::minimal_hook`]) of function pointers, and the
//! composition-root binary (`xvora-pager-bin`) calls [`install`] once at
...
/// subsequent calls are ignored (see [`xvora_pager::minimal_hook`]).
pub fn install() {
