//! Minimal (scrollback-native) render mode: `xvora --minimal`.
//!
//! In this mode finalized conversation blocks are printed once into the terminal's *native* scrollback.
//! The printing goes through `ratatui_inline::Terminal::insert_before` and reuses `EntryRenderer`.
...
//! # Wiring
//!
//! `xvora-pager` (the lib) does **not** depend on this crate: this crate reads deeply into the pager's [`AppView`] / view model.
//! A reverse dependency would be a cargo cycle.
//! Instead the pager exposes function-pointer hooks ([`xvora_pager::minimal_hook`]).
//! The composition-root binary (`xvora-pager-bin`) calls [`install`] once at startup to register this crate's [`draw`] entry point.
...
/// Register the minimal-mode render hooks with `xvora-pager`.
///
...
/// Idempotent: subsequent calls are ignored (see [`xvora_pager::minimal_hook`]).
pub fn install() {
    xvora_pager::minimal_hook::install(xvora_pager::minimal_hook::MinimalHooks { draw });
}
