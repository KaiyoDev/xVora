...
use xvora_pager::app::PagerTerminal;
use xvora_pager::app::app_view::{ActiveView, AppView};
use xvora_pager::minimal_api;
use xvora_pager::render::Renderable;
use xvora_pager::scrollback::state::ScrollbackState;
use xvora_pager::scrollback::wrappers::EntryRenderer;
use xvora_pager::theme::Theme;
use xvora_pager::views::prompt_widget::{PromptBg, PromptStyle};
use xvora_pager::views::turn_status;
...
pub(super) fn live_left_inset(_appearance: &xvora_pager::appearance::AppearanceConfig) -> u16 {
    0
}
...
fn clear_btw_geometry(agent: &mut xvora_pager::app::agent_view::AgentView) {
    agent.last_btw_selection_model =
        xvora_pager::scrollback::text_selection::ResolvedSelectionModel::default();
...
pub(super) fn prompt_style(
    ...
    input_mode: xvora_pager::app::agent_view::PromptInputMode,
    theme: &Theme,
    multiline: bool,
) -> PromptStyle {
    PromptStyle {
...
pub fn draw_live(app: &mut AppView, terminal: &mut PagerTerminal) {
    ...
    xvora_pager::render::draw::draw_frame(terminal, cursor, |frame, _link_spans| {
        let area = frame.area();
        ...
        agent.active_pane = xvora_pager::app::agent_view::AgentPane::Prompt;
        ...
        if minimal_api::extensions_modal(agent).is_some() {
            ...
            if let Some(state) = minimal_api::extensions_modal_mut(agent) {
                xvora_pager::views::extensions_modal::render_extensions_modal(
                    frame.buffer_mut(),
                    area,
                    state,
                    None,
        ...
        if let (Some(btw), Some(btw_area)) = (agent.btw_state.as_ref(), btw_area) {
            ...
            xvora_pager::views::btw_overlay::render_btw_panel(
                frame.buffer_mut(),
                btw,
                btw_area,
                tick,
        ...
        (
            ...
            result
                .post_flush_escapes
                .map(xvora_pager::terminal::overlay::PostFlush::from),
        )
    });
}
fn live_tail_renderer<'a>(
    entry: &'a xvora_pager::scrollback::entry::ScrollbackEntry,
    theme: &'a Theme,
    appearance: &xvora_pager::appearance::AppearanceConfig,
    cwd: &'a std::path::Path,
    tick: u64,
...
fn minimal_advance_phase_timer(
    agent: &mut xvora_pager::app::agent_view::AgentView,
) -> Option<xvora_pager::acp::tracker::TurnActivity> {
...
fn render_minimal_status(
    buf: &mut Buffer,
    area: Rect,
    agent: &xvora_pager::app::agent_view::AgentView,
    activity: &Option<xvora_pager::acp::tracker::TurnActivity>,
...
fn render_config_status_line(
    ...
    area: Rect,
    agent: &mut xvora_pager::app::agent_view::AgentView,
    frame: &xvora_pager::views::status_line::StatusLineFrame,
    theme: &Theme,
) {
    if area.height == 0 || area.width == 0 {
        return;
    ...
    if let Some(width) = minimal_api::status_line_inner_width(area.width, padding) {
        agent.last_status_line_size = Some(xvora_pager::views::status_line::RowSize {
            cols: width,
            lines: area.height,
        });
    }
...
fn render_idle_hint(buf: &mut Buffer, area: Rect, theme: &Theme) {
    ...
    buf.set_style(area, style);
    let auto = xvora_pager::app::minimal_auto_set_for_mouse_leak();
    let switch_back = xvora_pager::app::minimal_show_switch_back_to_fullscreen();
...
fn render_prompt_info(
    ...
    use xvora_pager::views::context_bar::fmt_tokens;
...
fn minimal_pending_hint(
    pending: &Option<xvora_pager::app::app_view::PendingAction>,
) -> Option<String> {
...
mod tests {
    ...
    fn config_status_line_paints_and_records_the_script_size() {
        use std::sync::Arc;
        use xvora_pager::views::status_line::{
    ...
    fn tail_height_uses_owning_session_cwd_for_tool_paths() {
        use xvora_pager::app::agent::AgentState;
        use xvora_pager::scrollback::RenderBlock;
        use xvora_pager::scrollback::entry::ScrollbackEntry;
        use xvora_pager::scrollback::types::DisplayMode;
        ...
        let appearance = super::super::commit::committed_appearance(
            &xvora_pager::appearance::AppearanceConfig::default(),
        );
        let theme = Theme::current();
    ...
    fn minimal_status_shows_rich_activity_and_idle_hint() {
        use xvora_pager::acp::tracker::TurnActivity;
        use xvora_pager::app::agent::AgentState;
        let theme = Theme::current();
        ...
        xvora_pager::app::set_minimal_show_switch_back_to_fullscreen_for_test(false);
        let a = agent();
        let mut buf = Buffer::empty(area);
        ...
        xvora_pager::app::set_minimal_show_switch_back_to_fullscreen_for_test(true);
        let mut buf = Buffer::empty(area);
        render_minimal_status(&mut buf, area, &a, &None, None, &theme);
    ...
    fn minimal_status_shows_idle_watching_cue() {
        ...
        a.session.scheduled_tasks.insert(
            "loop-1".to_string(),
            xvora_pager::app::agent::ScheduledTaskInfo {
    ...
    fn prompt_style_bash_mode_shows_bang_prefix() {
        ...
        use xvora_pager::appearance::AppearanceConfig;
    ...
    fn prompt_info_shows_session_mode_flag() {
        ...
        let render = |a: &xvora_pager::app::agent_view::AgentView| -> String {
            let mut buf = Buffer::empty(area);
    ...
    fn pending_hint_formats_press_again() {
        ...
        use xvora_pager::app::app_view::PendingAction;
        use xvora_pager::input::key::KeyShortcut;
