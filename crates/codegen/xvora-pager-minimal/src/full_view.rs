...
use xvora_pager::app::app_view::AppView;
...
pub fn pump_transcript(app: &mut AppView) {
    ...
    let appearance = super::commit::committed_appearance(&app.appearance);
    {
        ...
        let prev_thinking = xvora_pager::appearance::cache::load_show_thinking_blocks();
        ...
        xvora_pager::appearance::cache::set_show_thinking_blocks(prev_thinking);
    }
...
fn finish_transcript(app: &mut AppView, id: xvora_pager::app::agent::AgentId, out: String) {
    if out.is_empty() {
    ...
    let path = std::env::temp_dir().join(format!("xvora-transcript-{}.ansi", uuid::Uuid::new_v4()));
...
mod tests {
    use super::*;
    use xvora_pager::scrollback::block::RenderBlock;
    ...
    fn transcript_expands_thinking_committed_collapsed() {
        let theme = Theme::current();
        let appearance = super::super::commit::committed_appearance(
            &xvora_pager::appearance::AppearanceConfig {
