...
pub async fn run_command_hook(
    ...
    let debug_payloads = std::env::var("XVORA_HOOK_DEBUG").is_ok_and(|v| v == "1");
    if debug_payloads {
        tracing::trace!(
            hook_name = %spec.name,
    ...
    let mut cmd = if is_shell_command {
        ...
        #[cfg(not(unix))]
        {
            let inv = xai_xvora_config::shell::shell_command_argv(&command_str);
    ...
    // `xai_xvora_tools::util::detach_command`: Unix uses the same setsid /
    ...
    // plugin) can spoof `XVORA_HOOK_EVENT`, `XVORA_HOOK_NAME`, `XVORA_SESSION_ID`,
    // `XVORA_WORKSPACE_ROOT`, or `CLAUDE_PROJECT_DIR` -- which are the
    ...
    let mut child = match cmd
        ...
        .env("XVORA_HOOK_EVENT", envelope.hook_event_name.to_string())
        .env("XVORA_HOOK_NAME", &spec.name)
        .env("XVORA_SESSION_ID", ctx.session_id)
        .env("XVORA_WORKSPACE_ROOT", ctx.workspace_root)
        // Compatibility alias for external hooks that read this env name.
        // Same value as `XVORA_WORKSPACE_ROOT`; native `.xvora` hooks should use
...
mod tests {
    ...
    fn resolve_relative_path() {
        let spec = HookSpec {
            ...
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::path::PathBuf::from("/project/.xvora/hooks"),
            extra_env: std::collections::HashMap::new(),
        };
        assert_eq!(
            resolve_command_path(&spec),
    ...
    fn find_unresolved_skips_runner_vars() {
        ...
        let v = find_unresolved_env_vars(
            "${XVORA_HOOK_EVENT}/${CLAUDE_PROJECT_DIR}/${XVORA_SESSION_ID}/foo",
            &env,
        );
        assert!(
            v.is_empty(),
    ...
    async fn test_tilde_expansion_runs_via_shell() {
        ...
        // Inject HOME via extra_env so `sh -c "~/.xvora-test-hooks-gb856/..."`
        ...
        let spec = HookSpec {
            ...
            enabled: true,
            command: Some(std::path::PathBuf::from(
                "~/.xvora-test-hooks-gb856/tilde-test.sh",
            )),
            command_raw: Some("~/.xvora-test-hooks-gb856/tilde-test.sh".to_string()),
            url: None,
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::env::temp_dir(),
