...
pub struct RawHandler {
    ...
    /// keys (`XVORA_HOOK_EVENT`, `XVORA_HOOK_NAME`, `XVORA_SESSION_ID`,
    /// `XVORA_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`) are stripped at load
...
pub struct HookSpec {
    ...
    ///    (`XVORA_HOOK_EVENT`, `XVORA_HOOK_NAME`, `XVORA_SESSION_ID`,
    ///    `XVORA_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`) are stripped at
    ///    load time and a tracing warning is emitted.
    /// 2. Plugin-injected vars merged in by the plugin adapter
    ///    (`xai-xvora-agent::plugins::hooks_adapter`). The adapter sets
    ///    `XVORA_PLUGIN_ROOT`, `CLAUDE_PLUGIN_ROOT`, `XVORA_PLUGIN_DATA`,
    ...
    ///    (`XVORA_HOOK_EVENT`, `XVORA_HOOK_NAME`, `XVORA_SESSION_ID`,
    ///    `XVORA_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`). These are
...
mod tests {
    ...
    fn parse_hook_file_expands_env_var_in_command_from_process_env() {
        let key = "XVORA_HOOKS_PARSE_TEST_CMD_PROC_ENV";
    ...
    fn parse_hook_file_expands_env_var_in_url_from_process_env() {
        let key = "XVORA_HOOKS_PARSE_TEST_URL_PROC_ENV";
    ...
    fn parse_hook_file_preserves_unresolved_env_refs_in_command() {
        let key = "XVORA_HOOKS_PARSE_TEST_NEVER_SET_AT_LOAD_TIME";
    ...
    fn parse_hook_file_preserves_unresolved_env_refs_in_url() {
        let key = "XVORA_HOOKS_PARSE_TEST_URL_NEVER_SET_AT_LOAD_TIME";
        with_env_var(key, None, || {
            let json = format!(
                r#"{{
                    "hooks": {{
    ...
    fn parse_hook_file_matcher_is_not_env_expanded() {
        let key = "XVORA_HOOKS_PARSE_TEST_MATCHER_VAR";
    ...
    fn parse_hook_file_matcher_braced_var_is_not_env_expanded() {
        ...
        let key = "XVORA_HOOKS_PARSE_TEST_MATCHER_BRACED";
    ...
    /// User attempts to set runner-reserved keys (XVORA_HOOK_*,
    /// XVORA_SESSION_ID, XVORA_WORKSPACE_ROOT, CLAUDE_PROJECT_DIR) via
                        ...
                        "hooks": [
                            {
                                ...
                                "command": "echo hi",
                                "env": {
                                    "XVORA_HOOK_EVENT": "spoofed",
                                    "XVORA_HOOK_NAME": "spoofed",
                                    "XVORA_SESSION_ID": "spoofed",
                                    "XVORA_WORKSPACE_ROOT": "/etc",
