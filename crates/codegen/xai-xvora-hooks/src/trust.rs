...
/// (`<user_xvora_home>/trusted-hook-projects`), or `None` when no user grok home
/// resolves. Retained only for the one-time migration into folder-trust.
pub fn legacy_trust_file_path() -> Option<PathBuf> {
    Some(xai_xvora_config::user_xvora_home()?.join("trusted-hook-projects"))
}
...
pub fn disable_hook(hook_name: &str) -> Result<(), String> {
    let file = disabled_hooks_file_path()
        .ok_or_else(|| "no user grok home (set $XVORA_HOME or $HOME)".to_string())?;
...
/// Returns the path to `$XVORA_HOME/disabled-hooks`, or `None` when no user grok
/// home resolves.
fn disabled_hooks_file_path() -> Option<PathBuf> {
    Some(xai_xvora_config::user_xvora_home()?.join("disabled-hooks"))
}
