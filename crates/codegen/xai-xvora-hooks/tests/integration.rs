//! Integration tests for xai-xvora-hooks.
//!
...
use xai_xvora_hooks::discovery::load_hooks;
use xai_xvora_hooks::dispatcher;
use xai_xvora_hooks::event::*;
use xai_xvora_hooks::result::HookDecision;
use xai_xvora_hooks::runner::RunContext;
...
async fn non_blocking_dispatch() {
    ...
    assert!(matches!(
        &results[0],
        xai_xvora_hooks::result::HookRunResult::Success { .. }
    ));
}
...
async fn hook_receives_env_vars() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$XVORA_HOOK_EVENT" > {f}; echo "NAME=$XVORA_HOOK_NAME" >> {f}; echo "SESSION=$XVORA_SESSION_ID" >> {f}; echo '{{"decision":"allow"}}'"#,
...
/// runner-reserved keys (`XVORA_HOOK_EVENT`, `XVORA_HOOK_NAME`,
/// `XVORA_SESSION_ID`, `XVORA_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`)
...
async fn runner_injected_vars_override_extra_env_at_spawn() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$XVORA_HOOK_EVENT" > {f}; echo "NAME=$XVORA_HOOK_NAME" >> {f}; echo "SESSION=$XVORA_SESSION_ID" >> {f}; echo "ROOT=$XVORA_WORKSPACE_ROOT" >> {f}; echo "PROJ=$CLAUDE_PROJECT_DIR" >> {f}; echo "USER_KEY=$USER_KEY" >> {f}; echo '{{"decision":"allow"}}'"#,
        f = output_file.display(),
    );
                    ...
                    "hooks": [
                        {
                            ...
                            "env": {
                                "XVORA_HOOK_EVENT": "spoofed_event",
                                "XVORA_HOOK_NAME": "spoofed_name",
                                "XVORA_SESSION_ID": "spoofed_session",
                                "XVORA_WORKSPACE_ROOT": "/spoofed/root",
    ...
    assert!(
        ...
        "XVORA_HOOK_EVENT must reflect the real event, got:\n{captured}"
    );
    assert!(
        !captured.contains("EVENT=spoofed_event"),
        "spoofed XVORA_HOOK_EVENT must NOT leak through, got:\n{captured}"
    );
    assert!(
        captured.contains(&format!("SESSION={real_session}")),
        "XVORA_SESSION_ID must reflect the real session, got:\n{captured}"
    );
    assert!(
        !captured.contains("SESSION=spoofed_session"),
        "spoofed XVORA_SESSION_ID must NOT leak through"
    );
    assert!(
        captured.contains(&format!("ROOT={real_workspace}")),
        "XVORA_WORKSPACE_ROOT must reflect the real workspace root, got:\n{captured}"
    );
    assert!(
        !captured.contains("ROOT=/spoofed/root"),
        "spoofed XVORA_WORKSPACE_ROOT must NOT leak through"
    );
    assert!(
...
async fn http_hook_url_env_expansion_end_to_end() {
    ...
    use xai_xvora_hooks::result::HookRunResult;
    let info = match run {
