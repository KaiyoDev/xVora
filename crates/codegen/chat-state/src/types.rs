...
/// emitter in `xvora-shell` and the upsert/detection here — a drift would
...
pub enum AuthType {
    /// From AuthManager (xvora login, OIDC, external binary). Refreshable.
    #[default]
    SessionToken,
...
/// `xvora_sampling_types::SamplingConfig` (which is secret-free).
