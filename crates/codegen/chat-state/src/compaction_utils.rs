...
//! no I/O, no actor state. They live in `xvora-chat-state` so that both
//! this crate and `xvora-shell` can share them without duplication.
use std::collections::BTreeSet;
use xvora_sampling_types::{ContentPart, ConversationItem, SyntheticReason, ToolResultItem};
...
/// `xvora-chat-state` and `xvora-shell` share the same format.
...
pub struct CompactedHistoryInput<'a> {
    ...
    /// When `false` (the default), recent messages come first (xvora-build
    /// ordering).
    ...
    /// carrier and the xvora-build summary.
    ...
    /// "Total summaries generated so far …" footer. Ignored by the xvora-build
