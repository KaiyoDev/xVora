...
//! `xvora_shell::util::config::set_theme()` (and friends), invoked
...
static CURRENT: AtomicU8 = AtomicU8::new(ThemeKind::XvoraNight as u8);
...
/// back to `XvoraNight` if the byte is somehow out of range (which
...
fn theme_kind_from_u8(byte: u8) -> ThemeKind {
    match byte {
        x if x == ThemeKind::XvoraNight as u8 => ThemeKind::XvoraNight,
        x if x == ThemeKind::XvoraDay as u8 => ThemeKind::XvoraDay,
...
/// When `None`, `to_theme_kind()` defaults to `XvoraNight` / `XvoraDay`.
...
///
/// Precedence:
/// 1. Environment variable (`XVORA_THEME`)
/// 2. Config file (`[ui].theme`)
/// 3. Default: `XvoraNight`
#[must_use]
...
fn resolve_from_appearance(appearance: Option<system_appearance::SystemAppearance>) -> ThemeKind {
    ...
    appearance
        ...
        .unwrap_or(ThemeKind::XvoraNight)
}
...
/// and the user's dark/light theme mapping. Falls back to `XvoraNight`
...
// -- Disk reads --------------------------------------------------------------
//
// All writes go through `xvora_shell::util::config::set_theme()` (and
...
pub fn reset_for_test() {
    ...
    CURRENT.store(ThemeKind::XvoraNight as u8, Ordering::Relaxed);
...
mod tests {
    ...
    fn terminal_native_lock_pins_kind_and_blocks_apply_kind() {
        with_test_env(|| {
            set(ThemeKind::XvoraDay);
            set_terminal_native_lock(true);
            assert!(terminal_native_locked());
            assert_eq!(current_kind(), ThemeKind::XvoraNight, "nominal kind");

            let applied = super::super::Theme::apply_kind(ThemeKind::XvoraDay);
            assert_eq!(applied, ThemeKind::XvoraNight, "apply_kind must no-op");
            assert_eq!(current_kind(), ThemeKind::XvoraNight);
    ...
    fn terminal_native_lock_serves_terminal_default_palette() {
        with_test_env(|| {
            ...
            assert_ne!(
                theme.text_primary,
                super::super::Theme::xvoraday().text_primary,
                "must not serve the cached (XvoraDay) theme"
            );
        });
    }
    ...
    fn terminal_native_lock_caps_quantize_at_ansi16() {
        ...
        with_test_env(|| {
            ...
            for input in [
                Color::Rgb(0x26, 0x26, 0x26), // xvoraday text_primary
    ...
    fn resolve_no_osc11_explicit_auto_and_default() {
        with_test_env(|| {
            assert_eq!(
                resolve_from_config(Some(ThemeKind::XvoraDay), false),
                ThemeKind::XvoraDay
            );
            assert!(!is_auto_mode());
            ...
            assert_eq!(resolve_from_config(None, false), ThemeKind::XvoraNight);
        });
    }
    ...
    #[test]
    fn resolve_auto_dark_system_returns_xvoranight() {
        with_test_env(|| {
            system_appearance::set_mock(Some(system_appearance::SystemAppearance::Dark));
            let result = resolve_auto();
            assert_eq!(result, ThemeKind::XvoraNight);
        });
    }

    #[test]
    fn resolve_auto_light_system_returns_xvoraday() {
        with_test_env(|| {
            system_appearance::set_mock(Some(system_appearance::SystemAppearance::Light));
            let result = resolve_auto();
            assert_eq!(result, ThemeKind::XvoraDay);
        });
    }

    #[test]
    fn resolve_auto_detection_failure_returns_xvoranight() {
        with_test_env(|| {
    ...
    #[test]
    fn resolve_from_config_no_config_returns_xvoranight() {
        with_test_env(|| {
    ...
    fn resolve_from_config_explicit_theme_returns_it() {
        with_test_env(|| {
            let result = resolve_from_config(Some(ThemeKind::XvoraDay), true);
            assert_eq!(result, ThemeKind::XvoraDay);
            assert!(
                !is_auto_mode(),
    ...
    fn set_then_current_kind_round_trips() {
        with_test_env(|| {
            ...
            set(ThemeKind::XvoraDay);
            assert_eq!(current_kind(), ThemeKind::XvoraDay);
        });
    }
