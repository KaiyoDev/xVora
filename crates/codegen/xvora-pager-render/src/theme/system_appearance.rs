...
/// in `config.toml`. When `None`, defaults to `XvoraNight` / `XvoraDay`.
///
...
pub fn to_theme_kind(
    ...
    match appearance {
        SystemAppearance::Light => light_theme.unwrap_or(ThemeKind::XvoraDay),
        SystemAppearance::Dark => dark_theme.unwrap_or(ThemeKind::XvoraNight),
    }
}
