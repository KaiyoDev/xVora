...
//! ## Color support
//!
//! XvoNight is defined in `Color::Rgb` (truecolor).
...
impl ThemeKind {
    ...
    pub fn available() -> &'static [ThemeKind] {
        ...
        const NO_TRUECOLOR: &[ThemeKind] = &[ThemeKind::XvoNight, ThemeKind::XvoDay];
    ...
    /// XvoNight uses neutral grays that survive quantization cleanly.
    ...
    pub fn from_name(name: &str) -> Option<Self> {
        ...
        match lower.as_str() {
            ...
            "xvonight" | "xvora-night" | "dark" => Some(Self::XvoNight),
            "tokyonight" | "tokyo-night" | "tokyo" => Some(Self::TokyoNight),
            "xvoday" | "xvora-day" | "light" | "day" => Some(Self::XvoDay),
...
/// Human-friendly display name for a canonical theme value (e.g. `"xvonight"` becomes `"xvora Night"`).
/// Falls back to `value` verbatim.
pub fn display_name_for_canonical(value: &str) -> &str {
    match value {
        "auto" => "Auto",
        "xvonight" => "xvora Night",
        "xvoday" => "xvora Day",
...
impl Theme {
    ...
    pub fn current() -> Self {
        ...
        let base = match cache::current_kind() {
            ThemeKind::XvoNight => Self::xvonight(),
            ThemeKind::TokyoNight => Self::tokyonight(),
            ThemeKind::XvoDay => Self::xvoday(),
            ThemeKind::RosePineMoon => Self::rosepine_moon(),
            ThemeKind::OscuraMidnight => Self::oscura_midnight(),
            // Auto is resolved to a concrete theme before being stored; if reached, fall back to XvoNight
            ThemeKind::Auto => Self::xvonight(),
        };
    ...
    ///    An audit on xvonight found 18 of 27 fields became DarkGray or silver, erasing every state signal.
    ...
    /// XvoNight on a white terminal, for example, would show white "holes" through every sunken surface.
    fn ansi16_chrome_overrides(self, dark: bool) -> Self {
        ...
        Self {
            ...
            // Without this every XvoNight bg field quantizes to Color::Black and these become invisible
            ...
            // Markdown content: naive Basic quantize lands XvoNight md_code / md_muted / h4-h6 on DarkGray (ANSI 8)
...
mod tests {
    ...
    fn is_auto_returns_false_for_concrete_variants() {
        assert!(!ThemeKind::XvoNight.is_auto());
        assert!(!ThemeKind::XvoDay.is_auto());
    ...
    fn ansi16_overrides_light_inverts_high_contrast_and_elevated_bg() {
        ...
        use ratatui::style::Color;
        let t = Theme::xvoday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_state_accents_pin_to_polarity_aware_hue() {
        ...
        use ratatui::style::Color;
        let t_dark = Theme::XvoNight().ansi16_chrome_overrides(true);
        ...
        let t_light = Theme::XvoDay().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_canvas_matching_surfaces_use_theme_polarity() {
        ...
        // Color::Reset would defer to the user's terminal canvas, which can disagree with the theme (e.g. XvoNight on a white-canvas terminal).
        use ratatui::style::Color;
        let t_dark = Theme::XvoNight().ansi16_chrome_overrides(true);
    ...
    fn scrollbar_thumb_contrasts_with_track_in_all_themes() {
        ...
        for &kind in ThemeKind::ALL {
            let theme = match kind {
                ThemeKind::XvoNight => Theme::xvonight(),
                ThemeKind::XvoDay => Theme::xvoday(),
    ...
    #[test]
    fn ansi16_quantize_without_override_collapses_xvonight_backgrounds() {
        // Regression-ratchet for the override gate in `Theme::current`
        // Naive `Basic` quantization maps every dark XvoNight bg field to `Color::Black`, erasing the hierarchy
        ...
        use ratatui::style::Color;
        let q = Theme::xvonight().quantized(color_support::ColorLevel::Basic);
        for (name, color) in [
            ("bg_base", q.bg_base),
            ("bg_light", q.bg_light),
            ("bg_dark", q.bg_dark),
    ...
    fn from_str_matches_from_name_for_all_canonicals() {
        ...
        let cases = [
            ...
            ("xvonight", ThemeKind::XvoNight),
            ("xvora-night", ThemeKind::XvoNight),
            ("dark", ThemeKind::XvoNight),
            ("tokyonight", ThemeKind::TokyoNight),
            ("tokyo-night", ThemeKind::TokyoNight),
            ("tokyo", ThemeKind::TokyoNight),
            ("xvoday", ThemeKind::XvoDay),
            ("xvora-day", ThemeKind::XvoDay),
            ("light", ThemeKind::XvoDay),
            ("day", ThemeKind::XvoDay),
