...
//! The default theme is XvoraNight (neutral gray base with TokyoNight accents).
//!
//! ## Color support
//!
//! XvoraNight is defined in `Color::Rgb` (truecolor). At startup,
...
impl ThemeKind {
    ...
    pub fn available() -> &'static [ThemeKind] {
        ...
        const NO_TRUECOLOR: &[ThemeKind] = &[ThemeKind::XvoraNight, ThemeKind::XvoraDay];
    ...
    pub fn display_name(self) -> &'static str {
        match self {
            Self::XvoraNight => "xvoranight",
            Self::TokyoNight => "tokyonight",
            Self::XvoraDay => "xvoraday",
    ...
    /// when quantized to 256 or 16 colors. XvoraNight uses neutral grays
    /// that survive quantization cleanly.
    pub fn requires_truecolor(self) -> bool {
        match self {
            Self::XvoraNight => false,
...
/// `"xvoranight"` → `"xVora Night"`). Falls back to `value` verbatim.
pub fn display_name_for_canonical(value: &str) -> &str {
    match value {
        "auto" => "Auto",
        "xvoranight" | "groknight" => "xVora Night",
        "xvoraday" | "grokday" => "xVora Day",
...
impl Theme {
    ...
    pub fn current() -> Self {
        ...
        let base = match cache::current_kind() {
            ThemeKind::XvoraNight => Self::xvoranight(),
            ThemeKind::TokyoNight => Self::tokyonight(),
            ThemeKind::XvoraDay => Self::xvoraday(),
            ...
            // if reached, fall back to XvoraNight.
            ThemeKind::Auto => Self::xvoranight(),
        };
        ...
        //      (e.g., `TERM=xterm`, `TERM=ansi`, or `XVORA_FORCE_COLOR_LEVEL=basic`),
    ...
    ///    on xvoranight, erasing every state signal). We pin each
    ...
    /// theme (e.g., XvoraNight on a white terminal would show white
    ...
    fn ansi16_chrome_overrides(self, dark: bool) -> Self {
        ...
        Self {
            ...
            // XvoraNight bg field quantizes to Color::Black and these
            // become invisible.
...
mod tests {
    ...
    fn is_auto_returns_false_for_concrete_variants() {
        assert!(!ThemeKind::XvoraNight.is_auto());
        assert!(!ThemeKind::XvoraDay.is_auto());
    ...
    fn ansi16_overrides_light_inverts_high_contrast_and_elevated_bg() {
        ...
        use ratatui::style::Color;
        let t = Theme::xvoraday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_state_accents_pin_to_polarity_aware_hue() {
        ...
        use ratatui::style::Color;
        let t_dark = Theme::xvoranight().ansi16_chrome_overrides(true);
        ...
        let t_light = Theme::xvoraday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_canvas_matching_surfaces_use_theme_polarity() {
        ...
        // the theme disagrees with the terminal profile (e.g. XvoraNight
    ...
    fn scrollbar_thumb_contrasts_with_track_in_all_themes() {
        ...
        for &kind in ThemeKind::ALL {
            let theme = match kind {
                ThemeKind::XvoraNight => Theme::xvoranight(),
                ThemeKind::XvoraDay => Theme::xvoraday(),
    ...
    #[test]
    fn ansi16_quantize_without_override_collapses_xvoranight_backgrounds() {
        // Regression-ratchet for the override gate in `Theme::current`:
        // naive `Basic` quantization maps every dark XvoraNight bg field
        ...
        use ratatui::style::Color;
        let q = Theme::xvoranight().quantized(color_support::ColorLevel::Basic);
        for (name, color) in [
            ("bg_base", q.bg_base),
            ("bg_light", q.bg_light),
            ("bg_dark", q.bg_dark),
    ...
    fn from_name_concrete_variants_still_work() {
        assert_eq!(
            ThemeKind::from_name("xvoranight"),
            Some(ThemeKind::XvoraNight)
        );
        assert_eq!(ThemeKind::from_name("dark"), Some(ThemeKind::XvoraNight));
        assert_eq!(ThemeKind::from_name("xvoraday"), Some(ThemeKind::XvoraDay));
        assert_eq!(ThemeKind::from_name("light"), Some(ThemeKind::XvoraDay));
        assert_eq!(
    ...
    fn from_str_matches_from_name_for_all_canonicals() {
        ...
        let cases = [
            ...
            ("xvoranight", ThemeKind::XvoraNight),
            ("xvora-night", ThemeKind::XvoraNight),
            ("groknight", ThemeKind::XvoraNight),
            ("grok-night", ThemeKind::XvoraNight),
            ("dark", ThemeKind::XvoraNight),
            ("tokyonight", ThemeKind::TokyoNight),
            ("tokyo-night", ThemeKind::TokyoNight),
            ("tokyo", ThemeKind::TokyoNight),
            ("xvoraday", ThemeKind::XvoraDay),
            ("xvora-day", ThemeKind::XvoraDay),
            ("grokday", ThemeKind::XvoraDay),
            ("grok-day", ThemeKind::XvoraDay),
            ("light", ThemeKind::XvoraDay),
            ("day", ThemeKind::XvoraDay),
