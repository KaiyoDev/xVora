//! Detects whether xvora is running inside an editor's embedded `:terminal` (Neovim/Vim `:terminal`, Emacs `vterm`).
//!
...
///
/// Adding a new env marker here requires extending `HOST_TERMINAL_ENV_VARS` in `xvora-pager-pty-harness/src/pty.rs`.
