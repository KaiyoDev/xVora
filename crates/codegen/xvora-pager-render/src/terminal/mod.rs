...
/// True when `VSCODE_GIT_ASKPASS_MAIN` points into an official VS Code Remote
/// server install (`.vscode-server` / `.vscode-server-insiders`).
...
/// `xvora-pager-pty-harness/src/pty.rs` (test-env hygiene — the PTY
