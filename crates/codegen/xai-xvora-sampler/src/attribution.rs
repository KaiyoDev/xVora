...
//! it" buckets.
//!
//! `xai-xvora-sampler` is intentionally decoupled from `xai-xvora-shell`
...
/// tools live in `xai-xvora-tools`
/// (`crates/codegen/xai-xvora-tools/src/implementations/`), have their
/// own HTTP clients that do not flow through `SamplingClient`, and
/// hook into the `xai_xvora_tools::ApiKeyProvider` trait rather than
/// this enum.
...
/// `xai_xvora_shell::auth::token_suffix` (which truncates to 12 chars
...
/// `xai-xvora-shell/src/auth/manager.rs` to match, otherwise the
