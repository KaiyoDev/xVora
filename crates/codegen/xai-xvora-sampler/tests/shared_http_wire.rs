...
use xai_xvora_sampler::SamplingClient;
use xai_xvora_test_support::spawn_counting_server;

/// Pin the env these assertions depend on before any client is built, so
/// ambient shell exports (`XVORA_SAMPLER_SHARED_CLIENT=0`,
/// `XVORA_POOL_MAX_IDLE=0`) cannot flip the expected pooling behavior.
fn pin_env() {
    ...
    PIN.call_once(|| {
        ...
        unsafe {
            std::env::remove_var("XVORA_SAMPLER_SHARED_CLIENT");
            std::env::set_var("XVORA_POOL_MAX_IDLE", "2");
            std::env::set_var("XVORA_POOL_IDLE_TIMEOUT_SECS", "90");
        }
    });
}
