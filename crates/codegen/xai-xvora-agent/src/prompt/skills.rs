...
//! live in `xai_xvora_tools::implementations::skills::discovery`.

use std::collections::{HashMap, HashSet};
use std::path::{Path, PathBuf};

use crate::plugins::discovery::PluginScope;
use xai_xvora_tools::implementations::skills::types::skill_name_from_path;
pub use xai_xvora_tools::implementations::skills::types::{SkillInfo, SkillScope};
/// Re-export so agent-side discovery (and the shell) can name the resolved
/// vendor-compat config without reaching into `xai_xvora_tools` directly.
pub use xai_xvora_tools::types::compat::CompatConfig;

use xai_xvora_tools::implementations::skills::discovery::{
...
/// (e.g. `Repo` for `.xvora/plugins/`). Native skills always win bare-name
...
pub async fn list_skills_with_plugins(
    ...
    let mut skills = list_skills_with_options(
        ...
        &xai_xvora_tools::util::xvora_home::xvora_home(),
        compat,
    )
    .await;
...
pub fn collect_skill_config_dirs(
    ...
    // config; `.xvora` and `.agents` are always present. When all cells are on
    // this list equals the historical `[".xvora", ".agents", ".claude", ".cursor"]`.
    ...
    // Priority 3: Global user dirs. `.xvora` comes from `xvora_home` (which may
...
/// only visits known config roots (`.xvora`, `.agents`, `.claude`, `.cursor`),
...
fn collect_config_skills(config_paths: &[String], git_root: Option<&Path>) -> Vec<SkillInfo> {
    ...
    for skill in &mut skills {
        skill.config_source = Some(
            xai_xvora_tools::types::config_source::ConfigSource::ConfigToml {
...
fn stamp_plugin_fields(skills: &mut [SkillInfo], plugin: &crate::plugins::LoadedPlugin) {
    ...
    for skill in skills.iter_mut() {
        ...
        skill.config_source = Some(xai_xvora_tools::types::config_source::ConfigSource::Plugin {
...
pub(crate) fn format_skill_for_injection(skill: &SkillInfo) -> Option<String> {
    skill.body.as_ref().filter(|b| !b.is_empty()).map(|body| {
        xai_xvora_tools::implementations::skills::skill::build_skill_message(skill, body)
    })
}
    ...
    for name in names {
        ...
        let skill = discovered.iter().find(|s| {
            s.name.eq_ignore_ascii_case(name)
                || xai_xvora_tools::implementations::skills::skill::format_skill_name(s)
        ...
        match xai_xvora_tools::implementations::skills::skill::load_skill_with_body(skill).await {
...
mod tests {
    ...
    fn collect_config_skills_stamps_config_toml_source() {
        ...
        match &skills[0].config_source {
            Some(xai_xvora_tools::types::config_source::ConfigSource::ConfigToml { path }) => {
    ...
    async fn list_skills_auto_and_config_overlap_keeps_config_toml_source() {
        ...
        assert!(
            matches!(
                ...
                Some(xai_xvora_tools::types::config_source::ConfigSource::ConfigToml { .. })
            ),
    ...
    // these functions now live in xai-xvora-tools::implementations::skills::discovery
    // and xai-xvora-tools::types::skill_discovery_tracker, tested there.
    ...
    fn collect_skill_config_dirs_gates_vendor_dirs() {
        ...
        assert!(ends_with(&dirs, ".xvora"), "grok must remain: {dirs:?}");
    }
    ...
    fn dedupe_same_scope_cross_harness_loser_resurfaces() {
        // A `.claude` skill claiming a `.xvora`-owned name (both User scope)
