...
//! - `Default` -- the legacy Xvora prefix (built by the shell layer).
...
use xai_xvora_tools::types::skill_discovery_tracker::{XmlRenderMode, format_announcement_xml};
...
pub enum UserMessageTemplate {
    /// Legacy Xvora prefix: `<user_info>` + optional `<git_status>`.
...
pub struct UserMessageContext {
    ...
    /// string should use `xai_xvora_shell::util::uname::os_kernel_and_release`
    /// (or equivalent).
