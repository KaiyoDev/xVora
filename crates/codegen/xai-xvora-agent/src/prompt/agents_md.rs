...
//! `*.md` files in `.xvora/rules/` and `.claude/rules/` directories.
...
/// Find `*.md` files in `.xvora/rules/`, `.claude/rules/`, and `.cursor/rules/`,
...
async fn read_agents_config_with_options(
    ...
    let global_dir = xai_xvora_tools::util::xvora_home::xvora_home();
    ...
    // Always include xvora_home (~/.xvora/) first, then ~/.claude/ and ~/.cursor/
...
/// Used by `xai-xvora-shell` to structurally detect legacy untagged AGENTS.md
...
fn render_agents_md(configs: &[AgentConfigFile]) -> Option<String> {
    ...
    for config in configs {
        ...
        // .xvora/rules/*.md) so globs/paths metadata doesn't leak into the
        // system prompt as raw YAML.
        let is_rules_file = config.file_path.contains("/.xvora/rules/")
            || config.file_path.contains("/.claude/rules/");
        let content = if is_rules_file {
            xai_xvora_tools::implementations::skills::skill::extract_skill_body(&config.content)
        } else {
