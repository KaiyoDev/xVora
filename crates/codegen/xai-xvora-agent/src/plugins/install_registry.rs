...
/// Lives here (not in xai-xvora-plugin-marketplace) to keep dependency direction sane.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl InstallRegistry {
    ...
    pub fn save_atomic(&self) -> Result<(), InstallError> {
        ...
        if std::env::var_os("XAI_XVORA_TEST_FAIL_REGISTRY_SAVE_AFTER_SERIALIZE").is_some() {
    ...
    pub fn resolve_install_dir() -> PathBuf {
        ...
        xai_xvora_config::xvora_home().join(DEFAULT_INSTALL_DIR_NAME)
    }
