...
//!
//! Plugins from project directories (`.xvora/plugins/`, `.claude/plugins/`)
...
impl TrustStore {
    ...
    pub fn load() -> Self {
        // Gate on user_xvora_home() so a project's `.xvora/trusted-plugins` is never
        // read as the user trust store when neither XVORA_HOME nor a home dir resolves.
        let Some(grok) = xai_xvora_config::user_xvora_home() else {
            return Self {
                trusted: HashSet::new(),
