...
///
/// Replaces `${XVORA_PLUGIN_ROOT}`, `${CLAUDE_PLUGIN_ROOT}`,
/// `${XVORA_PLUGIN_DATA}`, and `${CLAUDE_PLUGIN_DATA}` with the provided values.
///
/// Delegates to [`xai_xvora_tools::util::substitute_plugin_tokens`], the single
...
pub fn substitute_env_vars(s: &str, plugin_root: &str, plugin_data: &str) -> String {
    xai_xvora_tools::util::substitute_plugin_tokens(s, Some(plugin_root), Some(plugin_data))
}
...
mod tests {
    ...
    fn substitute_env_vars_replaces_all() {
        let input = "${XVORA_PLUGIN_ROOT}/bin:${CLAUDE_PLUGIN_ROOT}/lib:${XVORA_PLUGIN_DATA}/cache";
