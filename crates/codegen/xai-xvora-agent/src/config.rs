...
use xai_xvora_tools::implementations::codex;
use xai_xvora_tools::implementations::xvora;
use xai_xvora_tools::implementations::xvora_concise;
...
use xai_xvora_tools::implementations::use_tool;
...
pub fn xvora_hashline_toolset(
    hashline_tools: Vec<xai_xvora_tools::registry::types::ToolConfig>,
) -> ToolServerConfig {
    let mut tools: Vec<xai_xvora_tools::registry::types::ToolConfig> = vec![bash_tool_config()];
    ...
    pub fn resolve_dir(self, agent_name: &str, project_cwd: &std::path::Path) -> ResolvedMemoryDir {
        match self {
            Self::User => ResolvedMemoryDir {
                path: xai_xvora_config::xvora_home()
                    .join("agent-memory")
                    .join(agent_name),
...
/// these into `xai_xvora_tools::registry::types::ToolsetConfig.bash`
...
impl AgentDefinition {
    ...
    fn scope_from_path(path: &Path) -> AgentScope {
        ...
        let grok = xai_xvora_config::user_xvora_home();
...
impl AgentDefinition {
    ...
    pub fn override_file_tools(
        &mut self,
        file_tools: Vec<xai_xvora_tools::registry::types::ToolConfig>,
    ) {
