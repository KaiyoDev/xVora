...
use std::sync::Arc;
use xai_xvora_tools::bridge::ToolBridge;
use xai_xvora_tools::computer::types::{AsyncFileSystem, TerminalBackend};
use xai_xvora_tools::notification::ToolNotificationHandle;
use xai_xvora_tools::registry::types::SessionContext;
...
/// tool registry in `xai-xvora-tools` (also used by the hook matcher).
fn claude_tool_kind(name: &str) -> Option<ToolKind> {
    xai_xvora_tools::types::kind_for(name)
}
...
pub struct AgentBuilder {
    ...
    parent_scheduler_handle:
        Option<xai_xvora_tools::implementations::xvora::scheduler::types::SchedulerHandle>,
    ...
    memory_backend: Option<Arc<dyn xai_xvora_tools::types::memory_backend::MemoryBackend>>,
    web_search_config: xai_xvora_tools::implementations::web_search::WebSearchConfig,
    ...
    lsp: Option<std::sync::Arc<dyn xai_xvora_tools::implementations::lsp::LspBackend>>,
    ...
    app_builder_deployer_config:
        xai_xvora_tools::implementations::xvora::deploy_app::AppBuilderDeployerConfig,
    ...
    compat: xai_xvora_tools::types::compat::CompatConfig,
    ...
    attribution_callback: Option<xai_xvora_tools::SharedAttributionCallback>,
    ...
    /// `resolve_max_mcp_output_bytes_for_cwd` in xai-xvora-shell).
...
fn ensure_plan_mode_tools(tool_config: &mut xai_xvora_tools::registry::types::ToolServerConfig) {
    use xai_xvora_tools::implementations::xvora;
    let existing: std::collections::HashSet<&str> =
        tool_config.tools.iter().map(|tc| tc.id.as_str()).collect();
    let missing_enter = !existing.contains("Xvora:enter_plan_mode");
    let missing_exit = !existing.contains("Xvora:exit_plan_mode");
...
fn merge_tool_params(
    tool_config: &mut xai_xvora_tools::registry::types::ToolServerConfig,
    ids: &[&str],
...
impl AgentBuilder {
    pub fn new(
        ...
        Self {
            ...
            terminal_backend,
            fs_backend: Arc::new(xai_xvora_tools::computer::local::LocalFs),
            notification_handle,
            ...
            system_reminder_tag: xai_xvora_tools::reminders::DEFAULT_REMINDER_TAG,
    ...
    pub fn with_preloaded_skills(
        mut self,
        skills: Vec<xai_xvora_tools::implementations::skills::types::SkillInfo>,
    ) -> Self {
    ...
    pub fn with_memory_backend(
        mut self,
        backend: Arc<dyn xai_xvora_tools::types::memory_backend::MemoryBackend>,
    ) -> Self {
    ...
    pub fn with_parent_scheduler_handle(
        mut self,
        handle: xai_xvora_tools::implementations::xvora::scheduler::types::SchedulerHandle,
    ) -> Self {
    ...
    pub fn with_web_search_config(
        mut self,
        config: xai_xvora_tools::implementations::web_search::WebSearchConfig,
    ) -> Self {
    ...
    /// `XVORA_WEB_FETCH` env var.
    pub fn with_web_fetch_config(
        mut self,
        config: xai_xvora_tools::implementations::xvora::web_fetch::WebFetchConfig,
    ) -> Self {
    ...
    pub fn with_lsp(
        mut self,
        handle: std::sync::Arc<dyn xai_xvora_tools::implementations::lsp::LspBackend>,
    ) -> Self {
    ...
    pub fn with_image_gen_config(
        mut self,
        config: xai_xvora_tools::implementations::xvora::image_gen::ImageGenConfig,
    ) -> Self {
    ...
    pub fn with_video_gen_config(
        mut self,
        config: xai_xvora_tools::implementations::xvora::video_gen::VideoGenConfig,
    ) -> Self {
    ...
    pub fn with_app_builder_deployer_config(
        mut self,
        config: xai_xvora_tools::implementations::xvora::deploy_app::AppBuilderDeployerConfig,
    ) -> Self {
    ...
    pub fn with_api_key_provider(
        mut self,
        provider: xai_xvora_tools::types::SharedApiKeyProvider,
    ) -> Self {
    ...
    /// Set public model slugs advertised in the Xvora Task description.
    ...
    ///
    /// When disabled, `Xvora:ask_user_question` is stripped from the
    ...
    pub async fn build(mut self) -> Result<Agent, AgentBuildError> {
        ...
        if definition.inject_default_tools {
            if self.memory_backend.is_some() {
                use xai_xvora_tools::implementations::memory;
                tool_config
                    .tools
            ...
            if self.web_search_config.is_enabled() {
                use xai_xvora_tools::implementations::xvora;
                tool_config.tools.push((&xvora::WebSearchTool).into());
            }
            if self.web_fetch_config.is_enabled() {
                use xai_xvora_tools::implementations::xvora;
                tool_config.tools.push((&xvora::WebFetchTool).into());
            }
            if self.lsp.is_some() {
                tool_config
                    .tools
                    .push((&xai_xvora_tools::implementations::xvora::LspTool).into());
            }
            if self.image_gen_config.image_gen_enabled() {
                tool_config
                    .tools
                    .push((&xai_xvora_tools::implementations::xvora::ImageGenTool).into());
            }
            if self.image_gen_config.image_edit_enabled() {
                tool_config
                    .tools
                    .push((&xai_xvora_tools::implementations::xvora::ImageEditTool).into());
            }
            if self.video_gen_config.is_enabled() {
                tool_config
                    .tools
                    .push((&xai_xvora_tools::implementations::xvora::ImageToVideoTool).into());
                tool_config.tools.push(
                    (&xai_xvora_tools::implementations::xvora::ReferenceToVideoTool).into(),
                );
            }
            ...
            if self.write_file_enabled && !has_write_tool {
                tool_config
                    .tools
                    .push((&xai_xvora_tools::implementations::opencode::OpenCodeWriteTool).into());
            }
            ensure_plan_mode_tools(&mut tool_config);
        }
        if self.memory_backend.is_none() {
            let xvora_ns = xai_xvora_tools::types::tool::ToolNamespace::Xvora.to_string();
            let mem_search_id = format!(
                "{xvora_ns}:{}",
                xai_xvora_tools::implementations::memory::MEMORY_SEARCH_TOOL_NAME
            );
            let mem_get_id = format!(
                "{xvora_ns}:{}",
                xai_xvora_tools::implementations::memory::MEMORY_GET_TOOL_NAME
            );
            tool_config
                .tools
        ...
        if !self.ask_user_question_enabled {
            let ask_user_id = format!(
                "{}:ask_user_question",
                xai_xvora_tools::types::tool::ToolNamespace::Xvora,
            );
        ...
        if task_stripped {
            use xai_xvora_tools::types::tool::ToolNamespace;
            ...
            if !has_satisfier(ToolNamespace::Xvora, "run_terminal_cmd", true)
                && !has_satisfier(ToolNamespace::XvoraConcise, "run_terminal_cmd", true)
        ...
        if let xai_xvora_tools::implementations::xvora::web_fetch::WebFetchConfig::Enabled {
            ref params,
        } = self.web_fetch_config
            && let Ok(params_value) = serde_json::to_value(params)
            && let Some(obj) = params_value.as_object()
        {
            merge_tool_params(&mut tool_config, &["Xvora:web_fetch"], obj);
        }
        ...
        if let Some(bytes) = self.mcp_max_output_bytes {
            tool_bridge.toolset().resources.lock().await.insert(
                xai_xvora_tools::types::resources::TruncationCfg(
                    xai_xvora_tools::types::context::TruncationConfig {
        ...
        if use_backend_search {
            if web_search_enabled && definition.hosted_tool_allowed("web_search") {
                hosted_tools.push(xai_xvora_sampling_types::HostedTool::WebSearch {
                    allowed_domains: None,
                });
            }
            if definition.hosted_tool_allowed("x_search") {
                hosted_tools.push(xai_xvora_sampling_types::HostedTool::XSearch);
            }
        }
...
fn resolve_shell_for_prompt() -> String {
    ...
    #[cfg(not(unix))]
    {
        xai_xvora_config::shell::detect_windows_shell()
            .name()
            .to_string()
    }
}
...
mod tests {
    ...
    fn entry(name: &str, desc: &str, source: SubagentSource) -> SubagentEntry {
        SubagentEntry {
            ...
            config_source: xai_xvora_tools::types::config_source::ConfigSource::Builtin,
        }
    }
    #[test]
    ...
    fn build_task_description_shadowed_builtin_uses_user_desc() {
        let subagents = vec![SubagentEntry {
            ...
            config_source: xai_xvora_tools::types::config_source::ConfigSource::Project {
    ...
    fn task_model_guidance_resolves_model_param_override() {
        use xai_xvora_tools::types::template_renderer::TemplateRenderer;
        use xai_xvora_tools::types::tool::ToolKind;
    ...
    async fn build_pager_agent(
        ...
        use xai_xvora_tools::computer::local::LocalTerminalBackend;
        use xai_xvora_tools::notification::ToolNotificationHandle;
        AgentBuilder::new(
            std::env::temp_dir(),
    ...
    async fn pager_flag_combinations_satisfy_tool_invariants() {
        ...
        let cases: &[PagerFlagCase] = &[
            PagerFlagCase {
                label: "xvora / subagents+ask_user",
                profile: AgentDefinition::default_xvora,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "xvora / subagents / no-ask-user",
                profile: AgentDefinition::default_xvora,
                subagents: true,
                ask_user: false,
            },
            PagerFlagCase {
                label: "xvora / no-subagents / ask_user",
                profile: AgentDefinition::default_xvora,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "xvora / no-subagents / no-ask-user",
                profile: AgentDefinition::default_xvora,
                subagents: false,
                ask_user: false,
            },
            PagerFlagCase {
                label: "xvora-ask-user / subagents",
                profile: AgentDefinition::xvora_ask_user,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "xvora-ask-user / no-subagents",
                profile: AgentDefinition::xvora_ask_user,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "xvora-plan",
                profile: AgentDefinition::xvora_plan,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "xvora-plan / no-ask-user",
                profile: AgentDefinition::xvora_plan,
                subagents: true,
                ask_user: false,
            },
            PagerFlagCase {
                label: "xvora-plan-no-subagents",
                profile: AgentDefinition::xvora_plan_no_subagents,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "xvora-plan-no-subagents / no-ask-user",
                profile: AgentDefinition::xvora_plan_no_subagents,
                subagents: false,
                ask_user: false,
            },
    ...
    async fn curated_empty_toolset_fails_agent_build() {
        use xai_xvora_tools::computer::local::LocalTerminalBackend;
        use xai_xvora_tools::notification::ToolNotificationHandle;
        let mut profile = crate::config::AgentDefinition::default_xvora();
    ...
    async fn plan_mode_injected_ask_user_question_receives_params() {
        use xai_xvora_tools::computer::local::LocalTerminalBackend;
        use xai_xvora_tools::implementations::xvora::ask_user_question::AskUserQuestionParams;
        use xai_xvora_tools::notification::ToolNotificationHandle;
        use xai_xvora_tools::types::resources::Params;
        let profile = crate::config::AgentDefinition::default_xvora();
        assert!(
            !profile
                .tool_config
                .tools
                .iter()
                .any(|tc| tc.id == "Xvora:ask_user_question"),
    ...
    async fn build_with_tools(tools: Vec<String>, disallowed: Vec<String>) -> crate::agent::Agent {
        use xai_xvora_tools::computer::local::LocalTerminalBackend;
        use xai_xvora_tools::notification::ToolNotificationHandle;
        let mut def = crate::config::AgentDefinition::default_xvora();
        def.tools = tools;
    ...
    fn session_tools_allowed_clamp() {
        ...
        assert!(def.session_tools_allowed("Xvora:read_file"));
    ...
    async fn spawning_blocked_disables_all_background_bash_modes() {
        use xai_xvora_tools::computer::local::LocalTerminalBackend;
        use xai_xvora_tools::implementations::xvora::bash::BashParams;
        use xai_xvora_tools::notification::ToolNotificationHandle;
        use xai_xvora_tools::types::resources::Params;
    ...
    async fn requested_enabled_web_tools_survive_allowlist() {
        use xai_xvora_tools::computer::local::LocalTerminalBackend;
        use xai_xvora_tools::implementations::xvora::web_fetch::WebFetchConfig;
        use xai_xvora_tools::implementations::web_search::WebSearchConfig;
        use xai_xvora_tools::notification::ToolNotificationHandle;
        let mut definition = crate::config::AgentDefinition::default_xvora();
    ...
    /// xvora toolsets have no Skill tool — skills are read from
    ...
    async fn disallowed_web_search_strips_function_and_hosted_tools() {
        ...
        assert!(
            !hosted
                .iter()
                .any(|t| matches!(t, xai_xvora_sampling_types::HostedTool::WebSearch { .. })),
            "hosted WebSearch must be removed when web_search is disallowed, got: {hosted:?}"
        );
        assert!(
            hosted
                .iter()
                .any(|t| matches!(t, xai_xvora_sampling_types::HostedTool::XSearch)),
