...
This crate extracts a first-class `Agent` type from `xai-xvora-shell`.
...
`xai-xvora-shell`, another in-process host, or a headless batch runner.
...
in `.xvora/agents/` (project-level) or `~/.xvora/agents/` (user-level).

```rust
use xai_xvora_agent::{AgentDefinition, AgentBuilder};
use xai_xvora_tools::notification::ToolNotificationHandle;

// 1. Parse the definition file
let def = AgentDefinition::from_file(".xvora/agents/code-reviewer.md")?;
...
```rust
use xai_xvora_agent::discovery;

// Find all .md files in .xvora/agents/ directories
...
└────────▲─────────┘
         │ depends on
┌────────┴─────────┐
│  xai-xvora-shell  │  uses AgentBuilder to create
│  (session host)  │  Agent during session setup
└──────────────────┘
```

- **`xai-xvora-tools`**: Provides `ToolBridge`, `ToolRegistry`,
  `ToolState`, `SystemReminderLayer`, and tool implementations.
  `xai-xvora-agent` depends on it for tool setup.
- **`xai-xvora-shell`**: The application shell. Uses `AgentBuilder`
  to construct an `Agent` during session creation. The shell
  re-exports some modules from `xai-xvora-agent` (AGENTS.md
...
|---|---|---|
| `xvora` | extend | Default agent for software engineering tasks |
...
```bash
# Check
cargo check -p xai-xvora-agent

# Test
cargo test -p xai-xvora-agent

# Clippy
cargo clippy -p xai-xvora-agent --fix --allow-dirty
