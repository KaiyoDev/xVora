...
// matching per-platform optional dependency (@kaiyodev/xvora-<platform>)
...
if (!SUPPORTED.has(key)) {
    console.error(`@kaiyodev/xvora: unsupported platform ${key}`);
    process.exit(0);
}
...
function resolvePlatformPackageDir() {
    const platformPkg = `@kaiyodev/xvora-${key}`;
    try {
...
if (!version) {
    console.error('@kaiyodev/xvora: unable to determine version');
    process.exit(0);
}
...
function installBinary(binName, sourceDir, vendorSubpath) {
    ...
    if (fs.existsSync(brPath)) {
        ...
        console.error(`@kaiyodev/xvora: missing binary at ${brPath}`);
        return false;
    }
    ...
    if (IS_WINDOWS) {
        ...
        try {
            ...
            try {
                ...
                console.error(`@kaiyodev/xvora: failed to update ${canonicalPath}: ${e2.message}`);
...
if (!platformDir) {
    console.error(`@kaiyodev/xvora: platform package @kaiyodev/xvora-${key} not installed.`);
    console.error('  This usually means npm was invoked with --no-optional, or the install failed.');
    console.error('  Try: npm install -g @kaiyodev/xvora');
    process.exit(0);
}
