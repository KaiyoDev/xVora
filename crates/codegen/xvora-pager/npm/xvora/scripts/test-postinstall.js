...
// postinstall.js and the bin/xvora trampoline.
//
...
// ─── Extracted logic (mirrors postinstall.js and bin/xvora exactly) ─────
...
function installVersionedBinary(vendoredBinPath, version, canonicalDir) {
    const canonicalPath = path.join(canonicalDir, 'xvora');
    fs.mkdirSync(canonicalDir, { recursive: true });

    const versionedName = `xvora-${version}`;
...
/** Bootstrap canonical from vendored (same as bin/xvora trampoline). */
...
test('symlink swap is atomic (no intermediate missing state)', () => {
    ...
    try {
        const binDir = path.join(dir, 'bin');
        const canonicalPath = path.join(binDir, 'xvora');
...
test('three sequential upgrades: v1 -> v2 -> v3 all coexist', () => {
    ...
    try {
        ...
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'xvora')), 'grok-0.1.3');
...
test('bootstrapCanonical creates versioned binary from vendored', () => {
    ...
    try {
        ...
        assert.strictEqual(result, path.join(binDir, 'xvora'));
...
test('full lifecycle: install, upgrade, cleanup', () => {
    ...
    try {
        ...
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'xvora')), 'grok-0.1.142');
        assert.strictEqual(fs.readFileSync(path.join(binDir, 'xvora'), 'utf8'), 'v3');
    } finally {
        cleanup(dir);
    }
});
...
test('downgrade: installing older version than current', () => {
    ...
    try {
        ...
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'xvora')), 'grok-0.1.140');
        assert.strictEqual(fs.readFileSync(path.join(binDir, 'xvora'), 'utf8'), 'v1');
...
// ═══════════════════════════════════════════════════════════════════════
// grok vs xvora-pager Isolation Tests
// ═══════════════════════════════════════════════════════════════════════
 ...
 * Uses prefix + leading digit to avoid grok-* matching xvora-pager-*.
 */
...
test('installing both grok and xvora-pager creates independent symlinks', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        const vendored = path.join(dir, 'vendored');
        ...
        installNamedBinary(vendored, 'xvora', '0.1.141', binDir);
        installNamedBinary(vendoredPager, 'xvora-pager', '0.1.141', binDir);

        // Both symlinks exist and point to correct targets
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'xvora')), 'grok-0.1.141');
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'xvora-pager')), 'xvora-pager-0.1.141');
        ...
        assert.strictEqual(fs.readFileSync(path.join(binDir, 'xvora-pager-0.1.141'), 'utf8'), 'pager-binary');
    } finally {
        cleanup(dir);
    }
});

test('cleanup of grok-* does not remove xvora-pager-*', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        fs.mkdirSync(binDir, { recursive: true });
        ...
        // xvora-pager versions (should not be touched)
        fs.writeFileSync(path.join(binDir, 'xvora-pager-0.1.138'), 'old-pager-1');
        fs.writeFileSync(path.join(binDir, 'xvora-pager-0.1.139'), 'old-pager-2');
        fs.writeFileSync(path.join(binDir, 'xvora-pager-0.1.141'), 'current-pager');

        cleanupOldVersionsNamed(binDir, 'xvora', '0.1.141');
        ...
        // ALL xvora-pager versions must be untouched
        assert.ok(fs.existsSync(path.join(binDir, 'xvora-pager-0.1.138')), 'xvora-pager-0.1.138 must survive grok cleanup');
        assert.ok(fs.existsSync(path.join(binDir, 'xvora-pager-0.1.139')), 'xvora-pager-0.1.139 must survive grok cleanup');
        assert.ok(fs.existsSync(path.join(binDir, 'xvora-pager-0.1.141')), 'xvora-pager-0.1.141 must survive grok cleanup');
    } finally {
        cleanup(dir);
    }
});

test('cleanup of xvora-pager-* does not remove grok-*', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        fs.mkdirSync(binDir, { recursive: true });
        ...
        // Old xvora-pager versions
        fs.writeFileSync(path.join(binDir, 'xvora-pager-0.1.138'), 'old-pager-1');
        fs.writeFileSync(path.join(binDir, 'xvora-pager-0.1.139'), 'old-pager-2');
        fs.writeFileSync(path.join(binDir, 'xvora-pager-0.1.140'), 'old-pager-3');
        // Current pager
        fs.writeFileSync(path.join(binDir, 'xvora-pager-0.1.141'), 'current-pager');

        cleanupOldVersionsNamed(binDir, 'xvora-pager', '0.1.141');

        // xvora-pager cleanup: current + N-1 kept, older removed
        assert.ok(fs.existsSync(path.join(binDir, 'xvora-pager-0.1.141')), 'current pager should exist');
        assert.ok(fs.existsSync(path.join(binDir, 'xvora-pager-0.1.140')), 'N-1 pager should be kept');
        assert.ok(!fs.existsSync(path.join(binDir, 'xvora-pager-0.1.139')), 'N-2 pager should be removed');
        assert.ok(!fs.existsSync(path.join(binDir, 'xvora-pager-0.1.138')), 'N-3 pager should be removed');
...
test('full dual-binary lifecycle: install, upgrade, cleanup both', () => {
    ...
    try {
        ...
        installNamedBinary(vendored, 'xvora', '0.1.140', binDir);
        installNamedBinary(vendoredPager, 'xvora-pager', '0.1.140', binDir);
        ...
        installNamedBinary(vendored, 'xvora', '0.1.142', binDir);
        installNamedBinary(vendoredPager, 'xvora-pager', '0.1.142', binDir);

        // Cleanup both independently
        cleanupOldVersionsNamed(binDir, 'xvora', '0.1.142');
        cleanupOldVersionsNamed(binDir, 'xvora-pager', '0.1.142');
        ...
        assert.ok(fs.existsSync(path.join(binDir, 'xvora-pager-0.1.142')));
        assert.ok(fs.existsSync(path.join(binDir, 'xvora-pager-0.1.141')));
        assert.ok(!fs.existsSync(path.join(binDir, 'xvora-pager-0.1.140')));

        // Symlinks correct
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'xvora')), 'grok-0.1.142');
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'xvora-pager')), 'xvora-pager-0.1.142');
    } finally {
        cleanup(dir);
    }
});
...
test('grok installs normally regardless of platform key', () => {
    ...
    try {
        ...
        for (const platform of ['darwin-arm64', 'linux-x64', 'linux-arm64']) {
            const result = installNamedBinary(vendored, 'xvora', '0.1.150', binDir);
...
test('xvora-pager installs when vendored binary exists (darwin-arm64 path)', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        const vendoredPager = path.join(dir, 'vendored-pager');
        ...
        const result = installNamedBinary(vendoredPager, 'xvora-pager', '0.1.150', binDir);
        assert.ok(fs.existsSync(result.versionedPath), 'pager versioned binary should exist');
        assert.strictEqual(fs.readlinkSync(result.canonicalPath), 'xvora-pager-0.1.150');
...
test('Linux pager vendor files are not required for grok install', () => {
    ...
    try {
        ...
        fs.writeFileSync(path.join(vendorBase, 'darwin-arm64', 'xvora-pager'), 'mac-pager');
        ...
        assert.ok(!fs.existsSync(path.join(vendorBase, 'linux-x64', 'xvora-pager')));
        assert.ok(!fs.existsSync(path.join(vendorBase, 'linux-arm64', 'xvora-pager')));
        ...
        const result = installNamedBinary(grokVendored, 'xvora', '0.1.150', binDir);
...
test('skipping pager install on Linux does not affect grok cleanup', () => {
    ...
    try {
        ...
        fs.writeFileSync(vendored, 'grok-v1');
        installNamedBinary(vendored, 'xvora', '0.1.149', binDir);
        fs.writeFileSync(vendored, 'grok-v2');
        installNamedBinary(vendored, 'xvora', '0.1.150', binDir);

        // Simulate Linux: only run grok cleanup, skip pager entirely
        cleanupOldVersionsNamed(binDir, 'xvora', '0.1.150');
        ...
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'xvora')), 'grok-0.1.150');
...
test('canonical pager from non-npm install is preserved on Linux', () => {
    ...
    try {
        ...
        const pagerVersioned = path.join(binDir, 'xvora-pager-0.1.150');
        fs.writeFileSync(pagerVersioned, 'installer-pager-binary');
        fs.chmodSync(pagerVersioned, 0o755);
        const pagerCanonical = path.join(binDir, 'xvora-pager');
        fs.symlinkSync('xvora-pager-0.1.150', pagerCanonical);
        ...
        assert.strictEqual(fs.readlinkSync(pagerCanonical), 'xvora-pager-0.1.150');
    } finally {
        cleanup(dir);
    }
});
