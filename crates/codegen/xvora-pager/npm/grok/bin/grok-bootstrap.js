...
//   2. bootstrap it from the per-platform @xvora-official/grok-<platform>
...
const pkgName = '@xvora-official/grok';
...
function resolvePlatformPackageDir() {
    const platformPkg = `@xvora-official/grok-${process.platform}-${process.arch}`;
    try {
...
function resolveBinary() {
    ...
    if (!platformDir) {
        ...
        console.error(`  Expected sibling package @xvora-official/grok-${process.platform}-${process.arch}.`);
