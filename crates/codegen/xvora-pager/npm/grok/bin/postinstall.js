...
// matching per-platform optional dependency (@xvora-official/grok-<platform>)
...
if (!SUPPORTED.has(key)) {
    console.error(`@xvora-official/grok: unsupported platform ${key}`);
    process.exit(0);
}
...
function resolvePlatformPackageDir() {
    const platformPkg = `@xvora-official/grok-${key}`;
    try {
...
if (!version) {
    console.error('@xvora-official/grok: unable to determine version');
    process.exit(0);
}
...
function installBinary(binName, sourceDir, vendorSubpath) {
    ...
    if (!fs.existsSync(versionedPath) && !writeVendorBinary(brotliPath, binaryPath, versionedPath)) {
        console.error(`@xvora-official/grok: missing binary at ${brotliPath}`);
        return false;
    }
    ...
    if (IS_WINDOWS) {
        ...
        try {
            ...
            try {
                ...
                console.error(`@xvora-official/grok: failed to update ${canonicalPath}: ${e2.message}`);
    ...
    if (!fs.existsSync(canonicalPath)) {
        console.error(`@xvora-official/grok: ${canonicalName} did not resolve after install`);
        return false;
    }
...
if (!platformDir) {
    console.error(`@xvora-official/grok: platform package @xvora-official/grok-${key} not installed.`);
    console.error('  This usually means npm was invoked with --no-optional, or the install failed.');
    console.error('  Try: npm install -g @xvora-official/grok');
    process.exit(0);
}
...
function installBinLink(platformDir) {
    ...
    try {
        ...
        console.error(`@xvora-official/grok: bin link not installed: ${e.message}`);
        try { fs.unlinkSync(tmp); } catch {}
    }
}
    ...
    || (() => {
        try {
            const resolved = execSync(
                'npm config get @xvora-official:registry',
