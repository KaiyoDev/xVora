...
This renders, for example, `xvora-shell-status-line │ xVora 4.5 │ 12% ctx`. Items appear in the order you list them, and long ones are elided with `…`: the directory and session name at 40 columns, the model at 30.
...
Point `command` at a script. xVora pipes [JSON](#available-data) to it on stdin and shows its stdout. A `~/` prefix expands to your home directory.

```toml
[ui.status_line]
type = "command"
command = "~/.xvora/statusline.sh"
```
...
| `transcript_path` | Path to the session's `updates.jsonl`. The file is xVora's own update stream, so a script that parses another tool's transcript format will not read it |
...
| `version` | xVora release, for display |
...
Fields xVora cannot source are omitted rather than sent as placeholders, so the row never shows a fabricated value. Always guard them: `jq -r` prints the literal text `null` for a missing key, so write `// 0` or `// "?"` in jq, and `?.` in JavaScript.

## Example

Save a script (for example `~/.xvora/statusline.sh`), make it executable with `chmod +x`, and set it as `command`. This one uses [`jq`](https://jqlang.org/); Python and Node.js parse JSON natively. The payload carries no dirty-file count, so the script calls `git` for that.
...
- Test with mock input: `echo '{"session_id":"t","workspace":{"current_dir":"/tmp/demo"},"model":{"display_name":"xVora 4.5"},"context_window":{"used_percentage":25}}' | ./statusline.sh`
...
- **Nothing shows.** xVora reads `[ui.status_line]` at startup, so restart it after editing `config.toml`. Restarting is enough: when the new client attaches, the agent switches the row on for a session that is still running. The row only renders once the agent view is active, so not on the welcome screen or while a subagent view is open full screen. Check that `type` is not `disabled`, and that a command script is executable and writes to stdout.
- **A message in the row.** A row beginning `[ui.status_line]` means xVora could not use that section as written: it either names the key it could not read, or names what the mode you chose still needs. `xvora inspect` lists the same problems, including keys this version does not know, which is where to look when the row is switched off. Everything it could read still applies, and xVora leaves the section as you wrote it rather than rewriting one it cannot read. Setting `type = "disabled"` removes the row and the message.
- **A blank row that never fills.** The agent is not sending status updates, which usually means a `xvora` or leader process older than this client. Restart the leader or update xVora.
- **Only your own config can set this.** A `command` row runs a program, so it is read from your `~/.xvora/config.toml` and from configuration your administrator manages. A repository cannot set one: a repo-local `.grok/config.toml` is read for MCP servers only, and `[ui.status_line]` is not among the keys any project-scoped layer can supply, so cloning a repo cannot make xVora run its script.
