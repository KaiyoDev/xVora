...
xVora reads settings from config files, environment variables, and CLI flags. This page covers the common options. The field list for `config.toml`, `managed_config.toml`, and `requirements.toml` is [26-config-reference.md](26-config-reference.md) (extracted to `~/.xvora/docs/user-guide/` on launch).
...
2. **Environment variables** (e.g. `XAI_API_KEY`, `XVORA_MEMORY`)
3. **`requirements.toml` / MDM** (org-enforced; clamps every config layer below, including the overlay)
4. **`XVORA_CONFIG` / `XVORA_CONFIG_PATH` overlay** (above `config.toml` and managed, below `requirements.toml` / MDM)
5. **config.toml** (`~/.xvora/config.toml`)
6. **`managed_config.toml`** (org-deployed defaults; below `config.toml`)
7. **Built-in defaults**

Within the config-file tier, the layers merge lowest-to-highest: `managed_config.toml` → `config.toml` → `XVORA_CONFIG` overlay → `requirements.toml` / MDM. So `requirements.toml` and MDM clamp **both** your `config.toml` and the overlay.

`XVORA_CONFIG` / `XVORA_CONFIG_PATH` (tier 4) are config **overlays**: a merged config layer, not direct-setting environment variables like `XAI_API_KEY` (tier 2). They set config keys (subject to the allowlist below), so read them as part of the config-file tier rather than the env-var tier.

### Injecting config with `XVORA_CONFIG`

A harness or ACP client that launches `xvora agent stdio` can inject settings without writing a `config.toml` or relocating `$XVORA_HOME`:

- **`XVORA_CONFIG`**: an inline JSON object overlay.
- **`XVORA_CONFIG_PATH`**: an *additional* file overlay (not a replacement for `config.toml`), a JSON or TOML file read by its extension (`.json` → JSON, else TOML). `XVORA_CONFIG` wins if both are set. An empty `XVORA_CONFIG` is treated as unset, and a malformed one logs a warning and falls through to `XVORA_CONFIG_PATH`.

The overlay is **deep-merged** on top of your `config.toml` (it overrides only the keys it sets), placed above the user/managed layers but **below** `requirements.toml` / MDM so an enterprise pin still wins. A malformed blob is ignored with a warning. This mirrors `CODEX_CONFIG` from the `codex-acp` adapter (a JSON object merged into the session config); xVora is ACP-native, so the overlay lives in the agent itself. It only affects settings read from the merged config, and it is **not** a permission-escalation path. The overlay is confined, fail-closed, to an **allowlist** of soft settings (`models`, `features`, a narrowed `toolset`, and a `shell_environment_policy` limited to its filter fields, which select among env names the launcher already controls and cannot inject an env value into tool subprocesses); every other table is dropped at the choke point, so the overlay cannot spawn commands, set auth policy, redirect network traffic, elevate trust, or add a discovery source. Even on the allowlisted settings, a specific set of security gates read the raw disk layers rather than the overlay. The `ConfigLayers::env_overlay` rustdoc is the canonical list of what the overlay can and cannot reach and which gates read it overlay-free; see also the [internal environment-variables reference](../internal/22-environment-variables.md). Use `XVORA_DEFAULT_SELECTED_PERMISSION` for headless permission control. For example, to set the default reasoning effort:

```bash
XVORA_CONFIG='{"models": {"default_reasoning_effort": "high"}}' xvora agent stdio
```
...
Location: `~/.xvora/config.toml`. If the file is missing, xVora uses its built-in defaults, so you only need to set the values you want to override.
...
# Env vars XVORA_MAX_PARALLEL_IMAGE_GEN_CALLS / XVORA_MAX_PARALLEL_VIDEO_GEN_CALLS
...
You can also flip it from the settings pane (`/settings` → **Disable vim input mode**); xVora writes your choice to `[ui] simple_mode`. `simple_mode` and `vim_mode` are independent — one governs the prompt editor, the other governs scrollback navigation. See [Keyboard Shortcuts](03-keyboard-shortcuts.md) for the full binding reference.
...
You can also override this with `XVORA_DEFAULT_SELECTED_PERMISSION`, which is handy for headless or agent test runs that shouldn't mutate `config.toml`. Precedence: env var → `config.toml` → `always_allow_all_sessions`.
...
Toggle it at runtime with `/vim-mode`, or from `/settings` → **Vim scrollback navigation**. xVora writes the change to `[ui] vim_mode` immediately and applies it to every future pager session, including new agents and subagents in the same process. There's no per-session override — `config.toml` is the source of truth on next launch. `vim_mode` is independent of `simple_mode`.

#### Screen mode

`[ui] screen_mode` is the **default render mode** for plain `xvora` launches. Set it from `/settings` → **Default screen mode** (restart required) or edit `config.toml` by hand — both write the file. CLI flags (`--minimal` / `--fullscreen`) and slash commands (`/minimal` / `/fullscreen`) are session-scoped and do **not** write this key; after a slash switch, the reverse command returns you for that session only.
...
Each setting also has an environment-variable override, applied on first load only (again, handy for headless / test runs): `XVORA_SCROLL_SPEED`, `XVORA_SCROLL_MODE`, `XVORA_INVERT_SCROLL` (`1`/`true`/`0`/`false`), and `XVORA_SCROLL_LINES`. Precedence: env var → `config.toml` → default. Unrecognized values fall back to the default, and out-of-range numbers clamp.
...
`allow_local` is off by default (SSRF fail-closed). Turn it on (or set `XVORA_WEB_FETCH_ALLOW_LOCAL=1`) and `web_fetch` may reach **explicit** loopback hosts only — private, link-local, and cloud-metadata ranges stay blocked. Resolution: TOML > env > default off.
...
`[toolset.ask_user_question]` is honored across **requirements.toml**, **managed config**, and your user **`config.toml`**. Precedence: requirements → env (`XVORA_ASK_USER_QUESTION_TIMEOUT_ENABLED` / `XVORA_ASK_USER_QUESTION_TIMEOUT_SECS`) → user config → managed → defaults. Set `timeout_enabled = false` in your user config to disable the automatic questionnaire timeout for yourself; `timeout_secs` must be a positive integer. You can also toggle `timeout_enabled` from `/settings` → **Ask-Question timeout** (under Agent & Approval); changes apply to newly started sessions.
...
Remote (HTTP/SSE) servers receive a default `User-Agent: xvora-cli/<version>` header; a
valid `User-Agent` entry in `headers` overrides it (Figma servers receive bare
`xvora-cli`). See [MCP servers](07-mcp-servers.md) for details.

MCP servers can also be set per-project in `.grok/config.toml`. Project-scoped config contributes `[mcp_servers]`, `[plugins]`, and `[permission]` rules; every other section loads only from `~/.xvora/config.toml`.

Priority for `[mcp_servers]` and `[plugins]`: `.grok/config.toml` (current dir) > `<repo-root>/.grok/config.toml` > `~/.xvora/config.toml`. `[permission]` rules aren't overridden by priority — they merge across all files with `deny` > `ask` > `allow` (see [22-permissions-and-safety.md](22-permissions-and-safety.md)).

### Memory

Persist knowledge across sessions. Enable memory with `XVORA_MEMORY=1`, `[memory] enabled = true`, or managed remote settings.
...
[subagents]
enabled = true
sampling_limit = 12                   # concurrent in-flight subagent sampling calls per process; defaults to max_concurrent (32) when unset (XVORA_SUBAGENT_SAMPLING_LIMIT)
...
```toml
[workflows]
enabled = false                       # disable background workflows (or XVORA_WORKFLOWS=0)
```

Project workflows are discovered from `<repo-root>/.grok/workflows/`; user workflows from `~/.xvora/workflows/`. Discovery and invocation key off the script's `meta.name`, so keep each filename aligned with its `meta.name`. Built-ins win over project names, and project names win over user names, so keep names unique across scopes.
...
`xvora inspect` reports cells that still need session-start resolution as `?` until a value is available; cells with an explicit env or TOML value use that value. Affected discovery entries report `compatibilityStatus: "unresolved"` in JSON and `[compat unresolved]` in human output.
...
`[hints]` holds small persisted UI preferences: remembered answers and modal layout. xVora writes these for you as you use the TUI, but you can edit or delete them by hand; removing a key restores the default.

`[hints]` is read from the **effective config merge**, with the usual precedence: system managed → user `managed_config.toml` → user `config.toml` → user `requirements.toml` → system `requirements.toml`, higher layers winning. The TUI only ever **writes** these to your user `~/.xvora/config.toml`.
...
enabled = true
items = ["action-required", "spinner", "activity", "session-name", "xvora"]
```
...
| `title.items` | array | (see above) | Items shown in the title bar. Options: `action-required`, `spinner`, `activity`, `session-name`, `cwd`, `model`, `turn-timer`, `xvora`. |
...
| xVora Desktop | None (native) | N/A | N/A |
| Unknown | BEL | No | No |

With `method = "auto"`, xVora detects the terminal brand and picks the best protocol. Set `method` explicitly to override that.

#### Notification hooks

Run your own commands when events fire. Hooks receive `$XVORA_EVENT`, `$XVORA_MESSAGE`, and `$XVORA_SESSION_ID` in the environment.

```toml
# macOS native notification
[[ui.notifications.hooks]]
command = "terminal-notifier -title 'xVora' -message '$XVORA_MESSAGE'"
events = ["turn_complete", "approval_required"]
only_unfocused = true
timeout_secs = 10
...
# Push to ntfy server
[[ui.notifications.hooks]]
command = "curl -s -d '$XVORA_MESSAGE' ntfy.sh/my-grok-alerts"
...
- **`[features] telemetry`** / `XVORA_TELEMETRY_ENABLED` — the product-analytics master switch. `/privacy` doesn't change it.
- **Coding data, retention, and training** — the Settings row `/privacy` opens; coding-data sharing, separate from telemetry.
- **`[telemetry] trace_upload`** / `XVORA_TELEMETRY_TRACE_UPLOAD` — session traces; follows telemetry when unset.
- **`[telemetry] otel_*`** / `XVORA_EXTERNAL_OTEL` — external OTEL to your own collector (below).
...
Set these only to point telemetry at your own infrastructure or to switch parts off. The built-in endpoint and credentials are managed by xVora — leave them unset to use the defaults.
...
```toml
[telemetry]
otel_enabled = true                                       # external OTEL master switch (= XVORA_EXTERNAL_OTEL)
...
- `minimum_version` (`XVORA_MINIMUM_VERSION`) is a soft anti-downgrade floor. The
  updater skips a target below it and keeps the current version. It never blocks
  startup.
- `maximum_version` (`XVORA_MAXIMUM_VERSION`) is a soft ceiling. The updater caps
  its target at it and never installs above it.
- `required_minimum_version` (`XVORA_REQUIRED_MINIMUM_VERSION`) and
  `required_maximum_version` (`XVORA_REQUIRED_MAXIMUM_VERSION`) are hard bounds. If
  the running version is outside the range, the CLI exits at startup and instructs
  the user to install an approved version. `xvora update` and `xvora --version` keep
...
- An explicit `xvora update --version X` is allowed above the ceiling, to recover
...
[model.company-grok]
model = "grok-4.6"
base_url = "https://xvora-proxy.acme.com/"
name = "xVora 4.6 (Proxy)"
...
Location: `~/.xvora/pager.toml`. This controls the TUI's look and feel. Changes apply on restart.
...
| `XVORA_AUTH_PROVIDER_COMMAND` | External auth binary path |
| `XVORA_AUTH_PROVIDER_LABEL` | Display name on TUI login screen |
| `XVORA_AUTH_TOKEN_TTL` | Token lifetime in seconds |
| `XVORA_AUTH_EARLY_INVALIDATION_SECS` | Seconds before expiry to refresh (default: 300) |
| `XVORA_OIDC_ISSUER` | OIDC issuer URL |
| `XVORA_OIDC_CLIENT_ID` | OIDC client ID |

### Endpoints

| Variable | Description |
|----------|-------------|
| `XVORA_CLI_CHAT_PROXY_BASE_URL` | Override API proxy base URL |
...
| `XVORA_SUBAGENTS` | Enable (`1`) or disable (`0`) subagents |
| `XVORA_WORKFLOWS` | Enable (`1`) or disable (`0`) background workflows and select the `/goal` driver (default on: host-owned workflow driver; off: legacy `update_goal`) |
| `XVORA_WEB_FETCH` | Enable (`1`) or disable (`0`) the web_fetch tool |
| `XVORA_WEB_FETCH_ALLOW_LOCAL` | Allow `web_fetch` to explicit loopback hosts only (`localhost` / `127.0.0.0/8` / `::1`). Same as `[toolset.web_fetch] allow_local`. Default off; private/metadata stay blocked. |
| `XVORA_AGENT` | Custom agent definition path or name |
| `XVORA_SANDBOX` | Sandbox profile (off, workspace, devbox, read-only, strict; or a custom profile name) |
| `XVORA_EXIT_TIMEOUT_SECS` | Seconds after a quit is requested before the process is force-exited if teardown hangs (default: 20, `0` disables; a hard exit follows 5s later) |

### Logging

| Variable | Description |
|----------|-------------|
| `XVORA_LOG_FILE` | Write logs to this file path (used verbatim as the path) |
| `RUST_LOG` | Log level filter (e.g. `debug`); controls the `XVORA_LOG_FILE` log and headless stderr output |

### Paths

| Variable | Description |
|----------|-------------|
| `XVORA_HOME` | Override config directory (default: `~/.grok`) |
| `XVORA_RESPECT_GITIGNORE` | Force gitignore filtering on (`1`) or off (`0`); overrides `[tools] respect_gitignore` |

### Telemetry

| Variable | Description |
|----------|-------------|
| `XVORA_TELEMETRY_ENABLED` | Enable/disable telemetry |
| `XVORA_TELEMETRY_TRACE_UPLOAD` | Enable/disable session trace upload |
| `XVORA_TELEMETRY_MIXPANEL_ENABLED` | Enable/disable Mixpanel specifically |
| `XVORA_EXTERNAL_OTEL` | External OTEL to your collector (see [24-monitoring-usage.md](24-monitoring-usage.md)) |
| `XVORA_FEEDBACK_ENABLED` | Enable/disable feedback system |
| `XVORA_DEPLOYMENT_KEY` | Management API key for enterprise |
...
| Path | Description |
|------|-------------|
| `~/.xvora/config.toml` | Main configuration file |
| `~/.xvora/pager.toml` | TUI appearance configuration |
...
| `~/.xvora/lsp.json` | LSP server configuration (user-scoped) |
| `~/.xvora/logs/` | Internal log files (e.g. `unified.jsonl`, MCP server logs) |
...
|------|--------------------|
| `.grok/config.toml` | MCP servers, plugins, permission rules, and the `[mcp] max_output_bytes` tool-result cap (other sections load only from `~/.xvora/config.toml`) |
...
|--------|----------|-------|
| User | `~/.xvora/lsp.json` | All projects |
