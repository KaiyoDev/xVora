...
Project rules let you configure xVora per project or directory. By placing an AGENTS.md file in your repository, you can set coding conventions, build instructions, style guides, and any other instructions that xVora should follow when working in that codebase.

---

## What Are Project Rules?

Project rules are Markdown files that xVora reads and adds to its context. xVora follows their content for every interaction in that tree.

This is the primary mechanism for teaching xVora about your project's conventions, so you need not restate them each session.

---

## Supported File Names

xVora checks for these filenames (in this order) within each directory:
...
xVora loads every matching file in a directory, so a folder that contains both `AGENTS.md` and `CLAUDE.md` contributes both. On case-insensitive filesystems, names that resolve to the same file (such as `Agents.md` and `AGENTS.md`) are deduplicated and counted once. `Claude.md`, `CLAUDE.md`, and `CLAUDE.local.md` are supported for compatibility with Claude Code workflows. When Claude compatibility is enabled (the default), xVora also scans your home-level `~/.claude/` directory for these filenames and, at each directory level, checks `.claude/CLAUDE.md` and `.claude/CLAUDE.local.md` -- the locations Claude Code uses for project memory. With Cursor compatibility enabled, the home-level `~/.cursor/` directory is scanned the same way.

### Rules Directories

In addition to AGENTS.md files, xVora scans for `*.md` files in rules directories at each level (`<dir>`) from the repo root to the current working directory:
...
xVora also scans home-level rules, regardless of where it starts. These roots are already vendor-specific, so rules live directly under `rules/`:

| Location | Notes |
|----------|-------|
| `$XVORA_HOME/rules/` (default `~/.xvora/rules/`) | Always scanned; applies to all projects |
...
xVora scans for project rules in this order:

1. **Home rules**: `$XVORA_HOME`, then enabled `~/.claude/` and `~/.cursor/` sources
...
When xVora runs in `~/projects/my-app/src/components/`, it loads all three files. The instructions accumulate, so xVora sees all of them.

### Deeper Files Take Precedence

xVora orders the files from the repo root to the current working directory, so files in deeper directories appear later in its context and take precedence when instructions conflict. In the example above, if the root says "Use styled-components" but `components/AGENTS.md` says "Use CSS modules", the CSS modules instruction wins because it appears later.

### Auto-Loading Behavior

- xVora loads the files from the repo root to the current working directory automatically at session start.
- When xVora reads, lists, or edits files in directories outside that initial set, it detects any project instruction files there, notes their paths, and reads them when they apply to the task.
...
xVora appends this text to the session's system prompt. Use it for session-specific customization.

To replace the system prompt entirely, pass `--system-prompt-override` (alias `--system-prompt`). xVora uses the text verbatim and skips both the default system prompt and `--rules`. (Text passed with `--rules`, by contrast, is wrapped in a `<human_rules>` block and appended to the default prompt.)
...
As top-level instruction files, xVora discovers only the recognized filenames listed under [Supported File Names](#supported-file-names) — not custom names such as `AGENTS.local.md` or `notes.md`. (Inside a rules directory such as `.grok/rules/`, every `*.md` file is loaded regardless of name.)
...
| Path | Purpose |
|------|---------|
| `.grok/config.toml` | Project-scoped MCP servers, plugins, and permission rules (other settings load only from `~/.xvora/config.toml`) |
...
This shows each project instruction file it finds, with its path and approximate token count. Use it to confirm xVora picks up your rules.
