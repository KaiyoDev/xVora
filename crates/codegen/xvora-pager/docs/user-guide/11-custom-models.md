...
By default, xVora uses models hosted by SpaceXAI, and new sessions start with `grok-4.5`. Default models require no configuration. Authenticate with `xvora login` or an API key, then start a session.
...
Set a persistent default in `~/.xvora/config.toml`:
...
xVora supports three API backends. Set `api_backend` in your `[model.*]` config to choose which protocol the model uses:
...
When you omit `api_backend`, xVora uses `chat_completions`.

To send provider-specific authentication or version headers -- for example, Anthropic's `x-api-key` -- use the `extra_headers` field described below. xVora sends those headers verbatim with every request to the endpoint.

---

## Configuring Custom Models

Add custom model endpoints in `~/.xvora/config.toml` under `[model.<name>]` sections:
...
xVora resolves the API key in this order:
...
3. Your signed-in session token (from `xvora login`), for a model with no `api_key`/`env_key` of its own
4. The `XAI_API_KEY` environment variable (global fallback; xVora also accepts `XVORA_CODE_XAI_API_KEY` for backward compatibility)

### Context Window

The `context_window` value tells xVora when to trigger auto-compaction. When you override a known model, xVora inherits that model's context window. When you define a new model and omit `context_window`, xVora defaults to 200,000 tokens, so set it explicitly to match your provider.
...
Some gateways route or version on the query string. `query_params` appends percent-encoded query parameters to every request xVora makes for a model. For example, a gateway that selects an API version this way:
...
When you override a built-in model, xVora starts with the default configuration (including the correct `base_url`), then applies only the fields you specify. Unspecified fields inherit from the default.
...
The `messages` backend uses the Anthropic Messages protocol. Anthropic authenticates with an `x-api-key` header rather than `Authorization: Bearer`, so pass your key through `extra_headers`, which xVora sends verbatim.
...
Point xVora at a custom OpenAI-compatible `/v1/models` endpoint instead of the default. Use this when your models sit behind a corporate gateway or a self-hosted inference service.

### Environment Variables

| Variable | Required | Description |
|----------|----------|-------------|
| `XVORA_MODELS_BASE_URL` | Yes | Base URL for inference. xVora fetches the model list from `{base_url}/models`. |
| `XAI_API_KEY` | Yes | API key sent as `Authorization: Bearer`. xVora also accepts `XVORA_CODE_XAI_API_KEY`. |
| `XVORA_MODELS_LIST_URL` | No | Override the model-list URL when it differs from `{base_url}/models`. |
...
When you use `[endpoints]` with partial model overrides, xVora inherits the `base_url` from the endpoints config, so you do not need to specify it in each `[model.*]` section.

### Auth Behavior

When you set `models_base_url`, xVora uses API key auth (`Authorization: Bearer`) instead of session auth. You do not need `xvora login` -- the API key is enough.
...
```bash
export XVORA_WEB_SEARCH_MODEL="grok-4.5"
```

If you point web search at a custom model, you also need a `[model.*]` entry so xVora can reach it. Server-side ("backend") web search runs only when the model sets `supports_backend_search = true` (and the build enables backend search); it does not depend on `api_backend`:
