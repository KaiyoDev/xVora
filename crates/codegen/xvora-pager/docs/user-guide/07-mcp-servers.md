...
MCP (Model Context Protocol) servers extend xVora with external tool integrations. They let xVora interact with any service that implements the MCP standard.

---

## What Are MCP Servers?

An MCP server is a process that exposes tools to xVora over a standardized protocol. When you configure an MCP server, its tools become available to the model alongside xVora's built-in tools. The model can discover and call these tools during a session.
...
xVora spawns a local process and communicates over stdin/stdout:
...
>
> - env `XVORA_MAX_MCP_OUTPUT_BYTES` or `MAX_MCP_OUTPUT_BYTES` (bytes; xVora-native
...
default `User-Agent: xvora-cli/<version>` header, where `<version>` is the xVora binary
...
send the bare token `xvora-cli` with no version unless the config supplies its own
`User-Agent`.
...
xvora mcp list
xvora mcp list --json          # Machine-readable output

# Add a stdio server. Everything after -- is the server command, so flags
# like -y reach the server instead of being parsed by xvora.
xvora mcp add filesystem -- npx -y @modelcontextprotocol/server-filesystem /path/to/dir
...
xvora mcp add --transport sse linear https://mcp.linear.app/sse

# Remove a server
xvora mcp remove github

# Enable or disable a local/TOML (or compat-sourced) server
xvora mcp enable github
xvora mcp disable github

# Diagnose a server's configuration and connectivity
xvora mcp doctor               # Check every configured server
xvora mcp doctor github        # Check one server
xvora mcp doctor --json        # Machine-readable output
```

The transport defaults to `stdio`; pass `--transport http` or `--transport sse` for remote servers.

By default `xvora mcp add` writes to `~/.xvora/config.toml` (`--scope user`). Use `--scope project` to write to `.grok/config.toml` in the current directory instead, which can be committed and shared with your team (see [Project-Scoped MCP Servers](#project-scoped-mcp-servers)). Header and environment variable values are stored verbatim, so reference secrets as `${VAR}` instead of pasting them into a committed project config (see [Example Configurations](#example-configurations)). `xvora mcp list` shows servers from both scopes, marking project-scoped ones with `(project)` and disabled ones with `(disabled)`.

`xvora mcp remove` searches both scopes and exits 0 after removing the server. It exits 1 when the name is not found, or when the name is defined in both user and project scope — pass `--scope` to say which one to remove.

`xvora mcp enable` / `disable` persist the personal on/off state to user `~/.xvora/config.toml` (`disabled_mcp_servers`, and `[mcp_servers.<name>].enabled` when that entry exists). Scope:

- **Known names:** user/project xVora TOML, names already on the disabled list, compat sources (`.mcp.json`, Claude, Cursor), and **plugin** MCP servers (same discovery as doctor/`/mcps`).
...
When a server exposes a native HTTP/SSE endpoint, prefer the `url` form over wrapping it in a stdio proxy such as `npx mcp-remote <url>`. xVora handles HTTP/SSE and OAuth directly, so the native form avoids an extra subprocess per session. It also registers xVora's own OAuth client with the provider.

xVora walks from the current directory up to the git repo root, loading `.grok/config.toml` at each level:
...
Project-scoped files contribute `[mcp_servers]`, `[plugins]`, and `[permission]` entries. xVora reads most other config sections only from `~/.xvora/config.toml`.
...
You can enable or disable MCP servers without restarting xVora (TUI `/mcps` or CLI — see [CLI Management](#cli-management)).
...
xVora loads MCP server configurations from multiple sources for compatibility:

| Source | Format | Location | Configurable |
|--------|--------|----------|-------------|
| `config.toml` | Native xVora config | `~/.xvora/config.toml`, `.grok/config.toml` | Always on |
...
For MCP servers that require OAuth authentication, xVora handles the credential flow automatically. When an MCP server requests OAuth credentials, xVora opens a browser-based authorization flow and stores the resulting tokens for future use.
...
To avoid putting secrets in the config file, reference an environment variable with `${VAR}` (or `${VAR:-default}`). xVora expands string fields in `[mcp_servers.*]` — `url`, `command`, `args`, and the values in `env` and `headers` — at load time:
...
1. The parent session actually connected the server (see Extensions / `xvora inspect`)
...
For stdio servers, xVora captures the process's standard error to `~/.xvora/logs/mcp/<server>.stderr.log`, truncated on each launch. Check this file when a server starts but fails to handshake:
...
```bash
RUST_LOG=debug XVORA_LOG_FILE=/tmp/xvora.log xvora
