...
# Most restrictive profile (read CWD + system paths, write CWD + temp dirs + ~/.xvora/, no child network)
grok --sandbox strict
```
...
| `strict`              | CWD + system paths | CWD + `~/.xvora/` + `/tmp` + `/var/tmp`         | Blocked¹      | Untrusted code                    |
...
**strict** -- The most restrictive profile, for reviewing untrusted code. The agent can only read files within the current working directory and essential system paths. Writes are limited to CWD, `~/.xvora/`, and temp directories. Child-process network access is blocked on Linux (no-op on macOS).

---

## Custom Profiles

Create custom sandbox profiles in `~/.xvora/sandbox.toml` (global) or `.xvora/sandbox.toml` (per-project):
...
Sandbox events are logged to `~/.xvora/sandbox-events.jsonl` for debugging. Events include:
