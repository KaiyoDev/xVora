...
xvora -p "Format all files" --yolo

# Run tests and fix failures
xvora -p "Run the tests and fix any failures" --cwd ~/projects/my-app --yolo
```
...
Grok stores data in `~/.xvora` (override with `XVORA_HOME`; see [Environment Variables for Headless](#environment-variables-for-headless)):
...
For containers or CI, mount `~/.xvora` read-only:
