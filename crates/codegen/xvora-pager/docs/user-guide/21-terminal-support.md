...
xVora runs as a full-screen TUI. It relies on terminal support for color,
...
Run `/doctor` in xVora to check the current session and see available fixes. If
xVora cannot start, run `xvora doctor` in your shell. Use `xvora doctor --json`
...
`xvora doctor --json` reports the same color capability when piped. Microphone
...
`/doctor fix tmux-clipboard` or `xvora doctor fix dcs-passthrough --yes`.
...
to guess if that directory is unavailable or unsafe. xVora preserves the file's
...
xVora deliberately does **not** run `tmux source-file` or change the live tmux
...
xVora detects these terminal emulators from environment variables:
...
- Inside tmux, variables that identify the outer terminal may not reach xVora.
...
Inside tmux there are two separate questions: what color xVora emits, and what
...
xVora writes through up to three routes, shown in `/doctor` under **Clipboard**:

- **native** — the local operating-system clipboard.
- **tmux** — the tmux paste buffer when xVora runs inside tmux.
...
terminal focused. Older compositors may require xVora to remain focused until
the copy message appears. xVora shows a startup warning when this applies; run
`/doctor` for the detected status and steps.

`XVORA_CLIPBOARD_NO_DATA_CONTROL=1` is an advanced fallback that disables the
data-control route. Copies then use command-line clipboard tools.

#### OSC 52 kill switch

xVora emits OSC 52 on Linux and across tmux, SSH, or displayless containers when
that route is enabled. A terminal that does not implement OSC 52 may display the
encoded payload as text. Set `XVORA_CLIPBOARD_NO_OSC52=1` before starting xVora to
...
A remote xVora process normally cannot read the local terminal's selection. Use
...
When xVora cannot identify the outer terminal over SSH, it predicts that OSC 52
...
clipboard. Each copy is still saved to a backup file (`~/.xvora/last-copy.txt` by
default; override with `XVORA_COPY_FILE`); the toast names that path when delivery
...
through `xvora wrap`, for example `xvora wrap ssh user@host`. The same command can
wrap container and pod shells. It also restores terminal modes after a dropped
connection.

When an SSH session is not using `xvora wrap`, xVora shows the one-time tip
...
`[ui.contextual_hints]` in `$XVORA_HOME/config.toml`. This setting does not hide
the Doctor recommendation.

For repeated SSH use, Doctor offers `xvora doctor fix ssh-wrap`. It also shows
...
> **Warning**: `xvora wrap` is experimental and may not work in every setup.
...
Zellij and tmux control mode can limit the alternate screen. xVora normally uses
inline mode in those environments. Run `/doctor` to see the detected condition.
You can configure `[terminal] alt_screen` in `~/.xvora/pager.toml`, or run
`xvora --no-alt-screen` to confirm inline mode works.

### Zellij keybindings interfere with xVora

Zellij can intercept Ctrl/Alt keys before they reach xVora. On Zellij 0.41 or
...
mode, if `Ctrl+G` still does not reach xVora, open the command palette and select
...
WezTerm ships with the Kitty keyboard protocol disabled. Run `/doctor` in xVora.
...
keys. xVora therefore does not negotiate the protocol there, and Shift+Enter can
...
If xVora stops receiving mouse input, re-enable mouse reporting in the terminal:
...
After about 10 seconds without a transcript, xVora stops capture and shows
...
to the terminal hosting xVora. Open **System Settings → Privacy & Security →
...
Run `xvora doctor`, or run `/doctor` while voice mode is on. The **Voice** section
shows the microphone xVora would use. If no input device is available, Doctor
...
problem, set `XVORA_VOICE_CAPTURE=inprocess` to use the in-process fallback for
comparison.
...
terminals, Terminal.app, Konsole, mlterm, and others). xVora therefore
**does not** reorder RTL by default.

If Arabic or Persian in **scrollback** (or list content) reads backwards,
enable app-side reordering in `~/.xvora/pager.toml` (or project config):
