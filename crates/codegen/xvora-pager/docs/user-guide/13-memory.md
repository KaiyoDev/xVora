...
Memory lets xVora recall facts, decisions, and patterns from earlier sessions. xVora indexes the information you save and searches it automatically, so a new session can reuse relevant context.

---

## What Is Memory?

Without memory, each xVora session starts fresh: the model knows nothing about previous sessions. When you enable memory, xVora can:
...
2. `XVORA_MEMORY` env var: `1`/`true` enables, `0`/`false` disables
...
xVora suffixes each workspace directory with a short hash of the repository's identity. The identity is the `origin` remote in `org/repo` form when the directory is a Git repository with an `origin` remote, or the directory path otherwise. Because clones and worktrees of the same repository share an `origin` remote, they also share one memory directory.
...
Ask xVora to remember something, and it appends the note to a `MEMORY.md` file -- the workspace file for project-specific items, or the global `~/.xvora/memory/MEMORY.md` for cross-project preferences:

```
> remember to always open PR links after pushing
```

xVora records entries as durable statements under organized headings, such as `## Preferences`, `## Project Context`, or `## Debugging`. The file watcher reindexes the change on the next memory search, so the new entry is searchable within the current session.
...
Run `/remember` with no text to enter remember mode, where the next line you type becomes the note. Either way, xVora opens a review panel showing the note (with an optional rewritten version you can toggle with `Tab`); the note is written only after you confirm. On save, xVora shows `Memory saved to ~/.xvora/memory/MEMORY.md`.

### Forget

Ask xVora to forget something, and it finds and removes the matching entry:
...
Ask what xVora remembers:

```
> what do you remember?
```

xVora searches across all memory files and summarizes what it knows, grouped by source: global preferences, project-specific knowledge, and session history. Use `/memory` to browse the raw files.
...
When you save a note with `/remember`, xVora confirms in the scrollback:

```
Memory saved to ~/.xvora/memory/MEMORY.md
```

Background saves — flush, dream, and session-end — run silently and do not post a scrollback message. Use `/memory` at any time to browse what xVora has stored.
...
Dream also runs automatically. By default, xVora checks the consolidation gates at launch and periodically during a session, and runs Dream once enough time has passed and enough sessions have accumulated:
...
On the first turn of each session, xVora automatically searches memory for content relevant to the current project and injects it as context. This means xVora starts with knowledge from previous sessions without a reminder.
...
xVora searches memory automatically, but you can also trigger searches manually in the chat:
...
The `xvora memory` command manages memory from the shell. It has one subcommand, `clear`:
...
| `flush_model` | unset | Model for the flush turn. When unset or `""`, xVora uses the session's primary model. |
...
When a session memory is old, xVora attaches a staleness note to it in search results. Older results get a stronger reminder to verify the current state before you rely on them. These notes help you spot stored facts that might no longer be accurate. Global and workspace memories never receive staleness notes, because they hold curated long-term knowledge.

---

## File Watcher

By default, xVora watches `~/.xvora/memory/` for external file changes. If you edit memory files directly (e.g., in your editor), the changes are picked up automatically on the next memory search:
...
2. Check `XVORA_MEMORY` or `[memory] enabled` in effective TOML.
3. Check for `XVORA_MEMORY=0` or a deprecated compatibility flag overriding config.
