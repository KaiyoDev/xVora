...
Agent mode runs xVora as a long-lived server that clients talk to over [ACP](https://agentclientprotocol.com) (JSON-RPC). Use it from IDEs, SDKs, eval harnesses, and custom apps. For a one-shot prompt that prints and exits, use `xvora -p` instead ([headless mode](14-headless-mode.md)).
...
The [Agent Client Protocol (ACP)](https://agentclientprotocol.com) defines how clients talk to coding agents over JSON-RPC. With xVora it covers:
...
```bash
xvora agent --always-approve --model grok-4.6 stdio
xvora agent --always-approve serve --bind 127.0.0.1:2419 --secret <token>
```
...
Clients connect over WebSocket and authenticate with the secret token. If you omit `--secret`, the agent prints a generated token at startup, or set `XVORA_AGENT_SECRET`. The process keeps state across client reconnects. Permissions match other entry points; see [Permissions and safety](22-permissions-and-safety.md).

This is a server you run yourself — xVora's hosted cloud sandboxes do not run `xvora agent serve`.
...
```bash
xvora agent --always-approve headless --grok-ws-url wss://your-relay.example.com/ws
```
...
Beyond the base ACP protocol, xVora defines extension methods under the `x.ai/` prefix for SpaceXAI-specific functionality. These cover:
...
class GrokACPChat {
  ...
  async init() {
    this.proc = spawn("xvora", ["agent", "--always-approve", "stdio"]);
