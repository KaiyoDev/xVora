//! SSH wrap tip: over SSH without `xvora wrap`, advertise that wrapping the
...
/// Ephemeral-tip dedup key for the SSH `xvora wrap` hint.
...
/// Build "Over SSH? Run `xvora wrap ssh <host>` locally for clipboard +
    ...
    EphemeralTip {
        ...
        ..EphemeralTip::new(
            ...
            Line::from(vec![
                ...
                Span::styled("xvora wrap ssh <host>", command),
...
mod tests {
    ...
    fn ssh_wrap_tip_advertises_local_wrap() {
        ...
        assert_eq!(
            text,
            "Over SSH? Run xvora wrap ssh <host> locally for clipboard + terminal restore"
        );
    }
