...
fn canonical_and_short_ids_resolve_to_canonical_id() {
    ...
    assert_eq!(command, "xvora doctor fix ssh-wrap");
    assert_eq!(
        resolve_fix_id(command.strip_prefix("xvora doctor fix ").unwrap()).unwrap(),
        SSH_WRAP_ID
    );
...
fn tmux_fix_registry_resolves_every_short_and_canonical_id() {
    for (id, handle, _) in automatic_fix_choices() {
        ...
        assert_eq!(
            ...
            format!("xvora doctor fix {handle}")
        );
    }
}
...
fn tmux_managed_items_coexist_and_each_apply_is_one_transaction() {
    ...
    assert_eq!(content.matches("# >>> xvora doctor >>>").count(), 1);
    for id in [
        TMUX_CLIPBOARD_ID,
        DCS_PASSTHROUGH_ID,
        TMUX_EXTENDED_KEYS_ID,
...
fn bash_zsh_and_fish_plans_use_exact_paths_and_aliases() {
    ...
    for (shell, relative, alias) in [
        ("/bin/bash", ".bashrc", "alias ssh='xvora wrap ssh'"),
        ("/bin/zsh", ".zshrc", "alias ssh='xvora wrap ssh'"),
        (
            "/usr/local/bin/fish",
...
fn shell_aliases_expand_to_exact_argv_and_bypass_is_explicit() {
    ...
    let xvora = temp.path().join("xvora");
    std::fs::write(
        &xvora,
        format!(
            "#!/bin/sh\nprintf '%s\\n' \"$@\" > '{}'\n",
    ...
    .unwrap();
    use std::os::unix::fs::PermissionsExt as _;
    std::fs::set_permissions(&xvora, std::fs::Permissions::from_mode(0o755)).unwrap();

    if let Some(bash) = find_on_path("bash") {
        let rc = temp.path().join("bashrc");
        std::fs::write(&rc, "alias ssh='xvora wrap ssh'\n").unwrap();
        let command = format!(
    ...
    let mut shell = std::process::Command::new(bash);
    shell
        .args(["-ic", "alias ssh='xvora wrap ssh'; command ssh host"])
        .env("CAPTURE", &capture)
        .env(
            "PATH",
            format!(
    ...
    if let Some(fish) = find_on_path("fish") {
        ...
        let fish_grok = temp.path().join("fish-xvora");
        std::fs::write(
            &fish_grok,
            format!(
                "#!/bin/sh\nprintf '%s\\n' \"$@\" > '{}'\n",
        ...
        std::fs::write(&rc, "alias ssh 'fish-xvora wrap ssh'\n").unwrap();
        let command = format!(
...
/// tmux applies xvora's managed block last and the features merge.
