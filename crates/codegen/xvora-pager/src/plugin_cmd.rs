...
fn available_plugins(registry: &InstallRegistry) -> Vec<PluginEntry> {
    ...
    for source in &sources {
        ...
        for plugin in xvora_plugin_marketplace::scan_marketplace(&root).entries {
            let already_installed =
                xvora_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    registry,
                    &identity,
...
fn source_identity(source: &xvora_plugin_marketplace::MarketplaceSource) -> String {
    match &source.kind {
...
fn resolve_marketplace_root(
    ...
    match &source.kind {
        ...
        SourceKind::Git { url, branch } => {
            let cache = xvora_plugin_marketplace::git::default_cache_root();
            xvora_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache,
...
fn cmd_install_marketplace(
    source: &str,
    mref: &xvora_plugin_marketplace::install_resolve::MarketplaceRef,
    trust: bool,
) -> Result<()> {
    if !trust {
...
fn marketplace_update(
    ...
    marketplace_update_with_cache_root(
        sources,
        name,
        &xvora_plugin_marketplace::git::default_cache_root(),
    )
}
...
fn marketplace_update_with_cache_root(
    ...
    for source in sources {
        ...
        if let SourceKind::Git { url, branch } = &source.kind {
            match xvora_plugin_marketplace::git::force_sync_source_cache(
                url,
                branch.as_deref(),
                cache_root,
            ) {
...
mod tests {
    ...
    fn trust_prompt_marketplace_has_no_error_framing() {
        ...
        assert!(msg.contains(
            "To proceed, re-run with --trust:\n  grok plugin install sentry@KaiyoDev/plugin-marketplace --trust"
        ));
    ...
    fn current_head(repo: &Path) -> String {
        let output = xvora_plugin_marketplace::git::git_command()
            .current_dir(repo)
            .args(["rev-parse", "HEAD"])
            .output()
            .unwrap();
