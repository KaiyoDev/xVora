...
//! Separate from `xvora-pager-render`'s `vte::Perform`, which discards OSC and quantizes colour.
...
impl SanitizedText {
    ...
    pub fn new(text: &str) -> Self {
        let expanded = xvora_pager_render::appearance::expand_tabs(text);
...
fn safe_link_target(uri: &str) -> Option<Arc<str>> {
    ...
    // The shared gate stops at the scheme, enough for xvora's own text but not a script's
