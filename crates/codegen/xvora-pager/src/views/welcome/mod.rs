...
mod tests {
    ...
    fn grouped_entries_insert_headers() {
        let entries = vec![
            make_entry("s1", "Fix auth", "xvora"),
            make_entry("s2", "Add streaming", "xvora"),
        ...
        // Groups are sorted alphabetically: fw-1 before xvora.
        // Header positions: 0 (fw-1), 2 (xvora)
        ...
        assert!(
            matches!(&result[2], crate::views::picker::PickerEntry::Header { label } if label == &"xvora")
        );
    }
    ...
    fn grouped_entries_rows_are_indented() {
        let entries = vec![make_entry("s1", "Fix auth", "xvora")];
