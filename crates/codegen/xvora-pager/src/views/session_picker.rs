...
/// For paths with only one normal component (e.g., `/xvora`), returns that component alone.
/// Does not perform tilde expansion; callers provide absolute paths.
/// Returns `"unknown"` for empty input.
///
/// Examples: `/home/user/fw/1` becomes `"fw-1"`, `/xvora` becomes `"xvora"`, `/` becomes `"/"`.
///
...
mod tests {
    ...
    fn repo_name_from_cwd_standard_path() {
        assert_eq!(repo_name_from_cwd("/home/user/xvora"), "user-xvora");
    }
    ...
    fn repo_name_from_cwd_single_dir() {
        assert_eq!(repo_name_from_cwd("/xvora"), "xvora");
    }
