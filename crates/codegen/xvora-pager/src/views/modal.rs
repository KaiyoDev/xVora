...
pub enum ActiveModal {
    ...
    SessionPicker {
        ...
        content_results: Option<Vec<xvora_shell::extensions::session_search::SearchSessionHit>>,
...
fn fit_docs_ask_grok_tip(docs_path: &str, width: usize) -> String {
    ...
    let long =
        format!("Tip · Ask xVora about the docs ({docs_path}) — e.g. \"how do I set up MCP?\"");
    if long.width() <= width {
        return long;
    }
    let short = format!("Tip · Ask xVora about the docs · {docs_path}");
...
mod doc_picker_tip_tests {
    ...
    fn doc_picker_renders_tip_with_path() {
        ...
        assert!(
            all.contains("Tip") && all.contains("Ask xVora"),
            "missing tip footer:\n{all}"
        );
        assert!(
