...
/// Display label for a section header, e.g. `"Managed by xvora.com (3)"`.
pub fn section_label(section: &McpSectionId, count: usize) -> String {
    match section {
        McpSectionId::Managed => format!("Managed by xvora.com ({count})"),
        McpSectionId::Plugin(name) => format!("Plugin: {name} ({count})"),
        McpSectionId::Local => format!("Local ({count})"),
    }
}

/// Base xvora.com connectors URL (no team). Prefer [`managed_connectors_url`] when opening.
pub const MANAGED_SECTION_CONNECTORS_URL: &str = "https://xvora.com/connectors";
...
mod tests {
    ...
    fn section_description_lines_managed_includes_connectors_url() {
        ...
        assert_eq!(lines[1], "[xvora.com/connectors]");
        assert!(
        ...
        assert_eq!(with_team[1], "[xvora.com/connectors?teamId=team-1]");
    }

    #[test]
    fn managed_connectors_url_display_strips_scheme() {
        assert_eq!(managed_connectors_url_display(None), "xvora.com/connectors");
        assert_eq!(
            managed_connectors_url_display(Some("team-uuid-1")),
            "xvora.com/connectors?teamId=team-uuid-1"
        );
    }
