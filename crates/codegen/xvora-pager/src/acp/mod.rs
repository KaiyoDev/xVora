...
/// When `xvora.api_key` was advertised, non-interactive credentials were available: do not promote to interactive auto-Login.
...
mod tests {
    ...
    /// If a change re-orders `build_auth_methods()` to put `xvora.api_key` anywhere other than first (the shape of a past regression), this test fails.
    ...
    fn shell_built_auth_methods_for_byok_user_skip_login_screen() {
        ...
        assert!(
            ...
            "shell built auth_methods for a BYOK user, but the pager still \
             ...
             xvora.api_key first or the pager stopped treating xvora.api_key as \
             a no-login method.",
        );
        assert!(label.is_none());
        assert!(method_id.is_none());
    ...
    /// Inverse direction: when `xvora.api_key` is NOT in the list, the pager MUST show the login screen.
    /// We assert this with `xvora.api_key` present LATER in the list (the shape of a past regression) and confirm the pager still requires login.
