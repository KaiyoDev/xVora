...
pub struct NotificationMeta {
    ...
    /// Monotonic per-process sequence parsed from `eventId` (`"{sessionId}-{counter}"`, see `xvora-shell util::event_id`).
...
/// The stamp is `_meta.isReplay`, set by xvora-shell's `forward_raw_replay_line` during `session/load`.
///
...
pub mod user_message_chunk_meta {
    ...
    /// See [`xvora_shell::session::PromptOrigin::hide_user_echo_from_scrollback`].
