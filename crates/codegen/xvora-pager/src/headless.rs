...
use xvora_shell::sampling::error::{RATE_LIMITED_ERROR_CODE, rate_limited_user_message};
...
fn apply_agent_flag(agent: &Option<String>, config: &mut xvora_shell::agent::config::Config) {
...
pub async fn run_single_turn(
    ...
    if options.trust {
        xvora_shell::agent::folder_trust::grant_folder_trust(&cwd);
    }
    ...
    if options.self_verify {
        prompt_blocks.push(acp::ContentBlock::Text(acp::TextContent::new(
            skill_body(xvora_shell::builtin::CHECK_SKILL_MD).to_string(),
        )));
    }
        ...
        let n = n.clamp(2, 10);
        {
            prompt_blocks.insert(
                ...
                acp::ContentBlock::Text(acp::TextContent::new(format!(
                    ...
                    skill_body(xvora_shell::builtin::BEST_OF_N_SKILL_MD)
                ))),
            );
        }
    }
    ...
    if track_active {
        ...
        let _ = xvora_shell::active_sessions::try_unregister(&session_id);
    }
    cancel.cancel();
    match prompt_result {
        Some(Ok(resp)) => {
