...
pub fn setting_description(key: &str, fallback: &'static str) -> &'static str {
    match (locale(), key) {
        ...
        (Locale::Vi, "language") => {
            "Ngôn ngữ giao diện cho menu, cài đặt và phím tắt. Có hiệu lực ngay. Biến môi trường XVORA_LANG vẫn ghi đè."
        }
        (Locale::En, "language") => {
            "UI language for menus, settings, and shortcuts. Takes effect immediately. Environment variable XVORA_LANG overrides this setting."
        }
...
pub fn chrome(key: &str) -> &'static str {
    ...
    match (locale(), key) {
        ...
        (Locale::Vi, "settings.tip.long") => {
            "Mẹo · Hỏi xVora: \"đổi theme sang xvoraday\" hoặc \"compact mode làm gì?\""
        }
        (Locale::En, "settings.tip.long") => {
            "Tip · Ask xVora: \"change theme to xvoraday\" or \"what does compact mode do?\""
        }
