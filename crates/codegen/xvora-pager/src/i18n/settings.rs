...
pub fn setting_description(key: &str, fallback: &'static str) -> &'static str {
    match (locale(), key) {
        (Locale::Vi, "compact_mode") => "Giảm khoảng trống quanh tin nhắn để hiển thị nhiều nội dung hơn. Tự bật khi terminal ≤ 20 hàng.",
        (Locale::En, "compact_mode") => "Reduce padding around messages for more content density. Auto-enabled while the terminal is 20 rows or shorter.",
        (Locale::Vi, "screen_mode") => "Cách xVora mở lần sau: Toàn màn hình (mặc định) hoặc Tối giản. Ghi [ui] screen_mode vào config.toml. Cần khởi động lại. Trong phiên dùng /minimal hoặc /fullscreen.",
        (Locale::En, "screen_mode") => "How xVora opens next time: Fullscreen (default when unset) or Minimal. Writes [ui] screen_mode in config.toml. Restart required. Switch this session only with /minimal or /fullscreen.",
        (Locale::Vi, "show_timestamps") => "Hiện giờ cạnh tin nhắn người dùng và phản hồi agent.",
        (Locale::En, "show_timestamps") => "Show clock time next to user messages and agent responses.",
        (Locale::Vi, "show_timeline") => "Thanh tick theo lượt thay thanh cuộn: hover xem trước, click nhảy tới.",
        (Locale::En, "show_timeline") => "Per-turn tick rail in place of the scrollbar: hover previews a turn, click jumps to it.",
        (Locale::Vi, "simple_mode") => "Ô nhập kiểu readline thay vì phím vim. Thử nghiệm.",
        (Locale::En, "simple_mode") => "Use plain readline-style input instead of vim keys in the prompt. Experimental.",
        (Locale::Vi, "vim_mode") => "Bật phím vim (h/j/k/l, gg/G, /) để điều hướng scrollback. Không ảnh hưởng ô nhập.",
        (Locale::En, "vim_mode") => "Enable vim keys (h/j/k/l, gg/G, /) for navigating the scrollback. Does not affect the input prompt.",
                (Locale::Vi, "language") => "Ngôn ngữ giao diện cho menu, cài đặt và phím tắt. Có hiệu lực ngay. Biến môi trường XVORA_LANG vẫn ghi đè.",
        (Locale::En, "language") => "UI language for menus, settings, and shortcuts. Takes effect immediately. Environment variable XVORA_LANG overrides this setting.",
        ...
        (Locale::En, "auto_dark_theme") => "Theme to use when the system is in dark mode (only with theme=auto).",
        (Locale::Vi, "auto_light_theme") => "Theme khi hệ thống ở chế độ sáng (chỉ khi theme=auto).",
        (Locale::En, "auto_light_theme") => "Theme to use when the system is in light mode (only with theme=auto).",
        (Locale::Vi, "render_mermaid") => "Cách hiện khối mermaid: auto/on thêm hàng bấm để mở sơ đồ; off hiện mã nguồn.",
        (Locale::En, "render_mermaid") => "How mermaid code blocks are shown: auto/on add a clickable row to open the rendered diagram; off shows the raw source.",
        (Locale::Vi, "permission_mode") => "Default = hành vi mặc định agent; Ask = hỏi mỗi tool; Auto = LLM duyệt tool an toàn; Always approve = tự duyệt tất cả.",
        (Locale::En, "permission_mode") => "Default uses the agent's built-in behavior; Ask prompts for each tool action; Auto uses an LLM classifier for risky tools; Always approve grants all permissions automatically.",
        (Locale::Vi, "remember_tool_approvals") => "Hiện tùy chọn Always allow trong hộp quyền. Áp dụng ask/auto; Always-approve vẫn bỏ qua. Cần khởi động lại.",
        (Locale::En, "remember_tool_approvals") => "Show Always allow options in permission prompts so you can stop being re-asked about a specific command or tool. Applies in ask and auto; Always-approve still skips all prompts. Restart required.",
        (Locale::Vi, "multiline_mode") => "Khi bật, Enter xuống dòng và Shift+Enter gửi. Reset mỗi phiên.",
        (Locale::En, "multiline_mode") => "When on, Enter inserts a newline and Shift+Enter sends. Resets each session.",
        (Locale::Vi, "default_model") => "Mô hình cho phiên mới. Đổi cũng chuyển phiên hiện tại. Chọn (no override) để xóa.",
        (Locale::En, "default_model") => "Model used for new sessions. Changing this also switches the active session. Pick (no override) to clear.",
        (Locale::Vi, "max_thoughts_width") => "Độ rộng panel suy nghĩ của agent (40-500, mặc định 120).",
        (Locale::En, "max_thoughts_width") => "Column width budget for the agent's thoughts panel (40-500, default 120).",
        (Locale::Vi, "show_thinking_blocks") => "Hiện khối suy nghĩ/reasoning khi streaming.",
        (Locale::En, "show_thinking_blocks") => "Show agent thinking/reasoning blocks in the scrollback while streaming.",
        (Locale::Vi, "prompt_suggestions") => "Sau mỗi lượt, gợi ý prompt tiếp theo dạng ghost text (Tab để nhận). Gọi model nhỏ mỗi lượt.",
        (Locale::En, "prompt_suggestions") => "After each turn, predict your likely next prompt and show it as ghost text in the input (Tab to accept). Uses a small model call per turn.",
        (Locale::Vi, "group_tool_verbs") => "Gộp các tool read/search/list liên tiếp và subagent thành một hàng tóm tắt.",
        (Locale::En, "group_tool_verbs") => "Fold consecutive read/search/list tool calls and subagent rows into one summary row; finished thoughts fold into the group too.",
        (Locale::Vi, "respect_manual_folds") => "Giữ khối đã thu gọn thủ công khi streaming; dừng auto-scroll khi mở rộng. Thử nghiệm.",
        (Locale::En, "respect_manual_folds") => "Keep manually folded blocks as-is while streaming and stop auto-scroll when expanding a block. Experimental.",
        (Locale::Vi, "collapsed_edit_blocks") => "Hiện edit dạng +N/-M một dòng và gộp edit cùng file; mở rộng để xem diff.",
        (Locale::En, "collapsed_edit_blocks") => "Show edits as one-line +N/-M diffstat summaries and merge back-to-back edits to the same file into one block; expand a row to see the diffs.",
        (Locale::Vi, "display_refresh_auto_cadence") => "Màn hình tần số cao: TUI stream/cuộn nhanh hơn. Tắt giữ ~60 Hz. Cần khởi động lại.",
        (Locale::En, "display_refresh_auto_cadence") => "On high-refresh displays, the TUI will stream/scroll faster to match the display. Off keeps the classic ~60 Hz cadence. Restart required.",
        (Locale::Vi, "scroll_speed") => "Hệ số tốc độ cuộn chuột/trackpad (1-100). Cao hơn = nhanh hơn.",
        (Locale::En, "scroll_speed") => "Mouse-wheel and trackpad scroll speed multiplier (1-100). Higher = faster.",
        (Locale::Vi, "scroll_mode") => "Ép kiểu cuộn wheel hoặc trackpad khi tự nhận sai thiết bị.",
        (Locale::En, "scroll_mode") => "Force wheel or trackpad scroll behavior when auto-detection misreads your device.",
        (Locale::Vi, "scroll_lines") => "Số dòng mỗi lần cuộn (1-10). Chưa đặt thì dùng profile terminal.",
        (Locale::En, "scroll_lines") => "Lines per scroll tick for both wheel and trackpad (1-10). Until set, each terminal's own profile applies.",
        ...
        (Locale::Vi, "keep_text_selection") => "Thời gian giữ vùng chọn trong app và double-click làm gì (thu gọn vs chọn & copy từ).",
        (Locale::En, "keep_text_selection") => "How long in-app selection stays on screen and what double-click does (fold vs. select & copy a word).",
        (Locale::Vi, "coding_data_sharing") => "Cho phép SpaceXAI lưu và huấn luyện trên dữ liệu phiên coding hay không.",
        (Locale::En, "coding_data_sharing") => "Controls whether SpaceXAI may retain and train on coding session data.",
        (Locale::Vi, "default_selected_permission") => "Hàng con trỏ chọn sẵn trên hộp hỏi quyền.",
        (Locale::En, "default_selected_permission") => "Which row the cursor preselects on permission prompts.",
        (Locale::Vi, "ask_user_question_timeout") => "Khi bật, tool ask_user_question hết hạn sau một khoảng thay vì chờ mãi.",
        (Locale::En, "ask_user_question_timeout") => "When on, the ask_user_question tool will time out after a set period instead of blocking forever.",
        (Locale::Vi, "plan_mode") => "Khi bật, agent tóm tắt kế hoạch trước khi chạy tool hoặc sửa file.",
        (Locale::En, "plan_mode") => "When on, the agent summarises a plan before running tools or making edits.",
        ...
        (Locale::En, "contextual_hints") => "Show brief, in-context keyboard hints as you work; toggle each one individually.",
        (Locale::Vi, "auto_update") => "Tự tải và cài cập nhật pager khi khởi động. Cần khởi động lại.",
        (Locale::En, "auto_update") => "Automatically download and install pager updates on startup. Restart required.",
        (Locale::Vi, "hunk_tracker_mode") => "File change nào agent theo dõi dạng hunk. Off tắt hẳn (kể cả LOC). Cần khởi động lại.",
        (Locale::En, "hunk_tracker_mode") => "Which file changes the agent tracks as hunks. Off disables tracking (and LOC stats) entirely. Restart required.",
        (Locale::Vi, "voice_capture_mode") => "Cách phím giọng (Ctrl+Space / F8) hoạt động: Bật/tắt hoặc Giữ để nói.",
        (Locale::En, "voice_capture_mode") => "How the voice chord (Ctrl+Space / F8) behaves: Toggle or Hold to talk.",
        (Locale::Vi, "voice_stt_language") => "Ngôn ngữ STT cho dictation. Mặc định tiếng Anh; System dùng locale máy khi hỗ trợ.",
        (Locale::En, "voice_stt_language") => "Speech-to-text language for voice dictation. English by default; System uses your locale when supported.",
        (Locale::Vi, "fork_secondary_model") => "Mô hình agent phụ khi fork. Chọn (no override) để xóa.",
        (Locale::En, "fork_secondary_model") => "Model used for the secondary agent when forking. Pick (no override) to clear.",
        _ => fallback,
    }
}
