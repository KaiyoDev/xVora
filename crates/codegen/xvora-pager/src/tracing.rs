...
pub fn init_tracing() -> TracingHandle {
    ...
    let directives = format!(
        "xvora_shell=info,xvora_pager=trace,xvora_tools=info,session_search=info,acp_lib=info,{RMCP_SSE_NOISE_TARGET}=error,sampling_log=off,{ACP_UPDATE_TARGET}=debug,{ACP_UPDATE_PAYLOAD_TARGET}={payload_level}"
    );
