...
///
/// Defined in `xvora-telemetry` so the firehose directives and the pager
...
pub fn init_tracing() -> TracingHandle {
    ...
    let directives = format!(
        "xvora_shell=info,xvora_pager=trace,xvora_tools=info,acp_lib=info,{RMCP_SSE_NOISE_TARGET}=error,sampling_log=off,{ACP_UPDATE_TARGET}=debug,{ACP_UPDATE_PAYLOAD_TARGET}={payload_level}"
    );
