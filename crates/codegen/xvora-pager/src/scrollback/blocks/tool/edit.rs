...
use xvora_pager_diff::{DiffHunk, diff_hunks_to_patch};
...
fn render_diff_hunks_core(
    ...
    for (i, hunk) in hunks.iter().enumerate() {
        ...
        for line in hunk {
            ...
            let text = xvora_pager_render::appearance::expand_tabs(trimmed);
    ...
    for hunk in hunks {
        for line in hunk {
            if matches!(line.tag, ChangeTag::Equal | ChangeTag::Insert) && line.ln > 0 {
                ...
                out.insert(
                    line.ln,
                    xvora_pager_render::appearance::expand_tabs(trimmed).into_owned(),
                );
            }
        }
    }
...
pub fn compute_file_scoped_styles(
    ...
    for (idx, line) in file_text.lines().enumerate() {
        ...
        let expanded = xvora_pager_render::appearance::expand_tabs(line);
...
fn map_spans_for_line(
    line: &xvora_pager_diff::DiffLine,
    expanded: &str,
    by_new_line: &HashMap<usize, EditLineStyles>,
    theme: &Theme,
...
mod tests {
    ...
    fn test_diff_reflow_preserves_banded_syntect_styles_across_wrap() {
        let _guard = pin_XvoNight_syntect();
    ...
    /// Pins XvoNight via the shared test-lock guard.
    /// Hold it for the whole test so a concurrent theme flip can't skew the compared highlighter walks.
    fn pin_XvoNight_syntect() -> std::sync::MutexGuard<'static, ()> {
        let guard = crate::theme::cache::pin_theme();
        assert!(
            !Theme::XvoNight().diff_uses_line_fg(),
            "XvoNight must be banded for Equal-line syntect path in render"
        );
        guard
    }
    ...
    /// Caller pins the theme via `pin_XvoNight_syntect` (lock is not reentrant).
