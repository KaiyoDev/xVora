...
//! the generic [`CodeBlockSpan`](xvora_markdown::CodeBlockSpan) API) and
...
pub struct MermaidBlock {
    ...
    /// from [`CodeBlockSpan::body`](xvora_markdown::CodeBlockSpan::body). For
    ...
    /// [`CodeBlockSpan::output_line_range`](xvora_markdown::CodeBlockSpan::output_line_range).
...
///
/// `XvoraDay` is the only light theme; every other concrete theme (and the
/// `XvoraNight` default that `Auto` resolves to before it reaches the cache) is
/// dark. The render worker maps this to `xvora_mermaid::MermaidTheme`; it
...
pub fn theme_is_dark(theme: ThemeKind) -> bool {
    !matches!(theme, ThemeKind::XvoraDay)
}
...
mod tests {
    ...
    #[test]
    fn theme_is_dark_maps_xvoraday_to_light_only() {
        assert!(
            !theme_is_dark(ThemeKind::XvoraDay),
            "XvoraDay is the light theme"
        );
        for dark in [
            ThemeKind::XvoraNight,
