...
mod tests {
    ...
    fn sample_models() -> ModelState {
        ...
        models.available.insert(
            id_fast.clone(),
            acp::ModelInfo::new(id_fast.clone(), "xvora 4.5".to_string()),
        );
        let id_pro = acp::ModelId::new(Arc::from("grok-4.3"));
        models.available.insert(
            id_pro.clone(),
            acp::ModelInfo::new(id_pro.clone(), "xvora 4.3".to_string()),
        );
    ...
    fn model_resolves_by_display_name() {
        ...
        let result = cmd.run(&mut ctx, "xvora 4.5");
        match result {
    ...
    fn model_suggest_args_returns_available_models() {
        ...
        assert!(
            items
                .iter()
                .any(|i| i.display.starts_with("xvora 4.5") && i.insert_text == "xvora 4.5")
        );
        assert!(
            items
                .iter()
                .any(|i| i.display == "xvora 4.3" && i.insert_text == "xvora 4.3")
        );
    }
    #[test]
    ...
    fn pager_builtin_triggers_are_reserved_in_shell() {
        ...
        assert!(
            ...
            "pager builtin trigger keys missing from the shell's \
             PAGER_COMMAND_KEYS (xvora-shell/src/session/slash_commands.rs); \
