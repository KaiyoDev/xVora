...
impl SlashCommand for ModelCommand {
    ...
    fn run(&self, ctx: &mut CommandExecCtx, args: &str) -> CommandResult {
        ...
        // Prefer an exact full-string catalog match first. Model display names often contain spaces ("xvora 4.5").
        // If we split on the last token first, a shorter catalog entry ("xvora") would steal the prefix and treat "4.5" as an effort level
...
mod tests {
    ...
    fn empty_query_returns_one_row_per_logical_model() {
        ...
        let (pid, pinfo) = plain_model("grok-4.5", "xvora 4.5");
        ...
        let plain = items.iter().find(|i| i.match_text == "xvora 4.5").unwrap();
        assert_eq!(plain.insert_text, "xvora 4.5");
    }
    ...
    fn run_prefers_full_multi_word_model_name_over_prefix_plus_effort() {
        // The catalog has both "xvora" (reasoning) and "xvora 4.5"
        // `/model xvora 4.5` must select the full name, not treat "4.5" as an effort on "xvora"
        let mut state = ModelState::default();
        let (short_id, short_info) = model_with_reasoning("xvora", "xvora");
        let (long_id, long_info) = model_with_reasoning("grok-4.5", "xvora 4.5");
        state.available.insert(short_id, short_info);
        state.available.insert(long_id.clone(), long_info);
        let mut ctx = dummy_exec_ctx(&state);
        let result = ModelCommand.run(&mut ctx, "xvora 4.5");
        match result {
            CommandResult::Action(Action::SetDefaultModel(resolved_id)) => {
                assert_eq!(resolved_id, long_id);
            }
            other => panic!("expected SetDefaultModel(xvora 4.5), got {other:?}"),
        }
    }
    ...
    fn run_rejects_effort_for_non_reasoning_model() {
        let mut state = ModelState::default();
        let (id, info) = plain_model("grok-4.5", "xvora 4.5");
        state.available.insert(id, info);
        let mut ctx = dummy_exec_ctx(&state);
        let result = ModelCommand.run(&mut ctx, "xvora 4.5 high");
    ...
    /// Case-insensitive matching against the catalog: `/model xvora 4.5` resolves to the same `ModelId` as `/model xvora 4.5`.
    #[test]
