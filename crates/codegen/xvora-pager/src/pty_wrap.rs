//! Local PTY wrapper: the engine behind `xvora wrap` (see [`crate::wrap_cmd`]).
//!
...
///
/// This is the engine behind `xvora wrap`: it spawns
...
pub(crate) fn run_wrapped_command(program: &str, args: &[String]) -> Result<i32> {
    ...
    // `xvora` reached over SSH — that its OSC 52 clipboard writes are being
    // intercepted here and copied to the real local clipboard. The inner xvora
    // reads this (see `xvora_pager_render::clipboard::osc52_sink_active`) to
    // *trust* OSC 52 even when it can't detect an OSC-52-capable terminal,
    // which is the usual SSH case (only `TERM` propagates, so the inner xvora
    ...
    // these two without clearing it. The canonical `XVORA_OSC52_SINK` is
    ...
    cmd.env("XVORA_OSC52_SINK", "1");
    cmd.env("LC_XVORA_OSC52_SINK", "1");
    ...
    // detached — `xvora wrap` is short-lived and exits with the child.
