...
pub fn worktree_label_index() -> std::collections::HashMap<PathBuf, String> {
    ...
    for rec in records {
        let Some(label) = rec
            .metadata
            .as_ref()
            .and_then(|m| m.get(xvora_shell::session::worktree::META_KEY_LABEL))
            .and_then(|v| v.as_str())
            .filter(|s| !s.is_empty())
        else {
            continue;
    ...
    for ancestor in cwd.ancestors() {
        if let Ok(Some(record)) = db.get(&ancestor.to_string_lossy()) {
            let label = record
                .metadata
                .as_ref()?
                .get(xvora_shell::session::worktree::META_KEY_LABEL)?
                .as_str()
                .filter(|s| !s.is_empty())
                .map(String::from);
            return label;
...
///
/// `XVORA_NERD_FONTS=1` forces Powerline; `XVORA_NERD_FONTS=0` forces the fallback.
pub(crate) fn branch_icon() -> &'static str {
    static ICON: OnceLock<&str> = OnceLock::new();
    ICON.get_or_init(|| {
        decide_branch_icon(
            std::env::var("XVORA_NERD_FONTS").ok().as_deref(),
            HostOs::current(),
...
///
/// An explicit `XVORA_NERD_FONTS` override (`0`/`false` → off, anything else →
...
mod tests {
    ...
    fn cwd_git_info_lazy_non_repo_is_none() {
        let p = Path::new("/nonexistent-xvora-git-info-lazy-test-zzz");
