...
fn plant_local_build_session(cwd: &std::path::Path, session_id: &str) -> std::path::PathBuf {
    ...
    let encoded = xvora_shell::util::xvora_home::encode_cwd_dirname(&cwd.to_string_lossy());
...
fn make_ask_user_question_args(
    ...
    let req = AskUserQuestionExtRequest {
        ...
        mode: xvora_tools::implementations::xvora::ask_user_question::AskUserQuestionMode::Default,
        questions: vec![
...
const POLICY_WARNING: &str =
    xvora_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
