...
")]
    ...
    /// `XVORA_WORKSPACE_COMMAND=1` to enable it locally for testing.
    #[command(hide = true)]
    ...
    /// `~/.xvora/config.toml` or when the `XVORA_AGENT_DASHBOARD=0` env
    /// var is set.
    Dashboard,
}
...
pub struct ServeArgs {
    ...
    #[arg(long, env = "XVORA_AGENT_SECRET")]
...
fn version_with_channel() -> &'static str {
    ...
    V.get_or_init(|| {
        let label = xvora_update::channel_label();
        xvora_version::display_version_with_commit(env!("VERSION_WITH_COMMIT"), label)
    })
}
...
pub struct PagerArgs {
    ...
    /// persists per-segment markdown to grep. Sets `XVORA_COMPACTION_MODE`.
    ...
    /// Default for all `xvora -p` runs is to wait (up to `--background-wait-timeout`)
    ...
    #[arg(long, env = "XVORA_SANDBOX", value_name = "PROFILE")]
    ...
    /// minimal, set `[ui] screen_mode = "minimal"` in ~/.xvora/config.toml.
    #[arg(long = "minimal")]
    pub minimal: bool,
    ...
    #[arg(long = "log-sampling", env = "XVORA_LOG_SAMPLING", hide = true)]
...
impl PagerArgs {
    ...
    /// explicit `--sandbox`/`XVORA_SANDBOX` that differs from the saved profile is
    ...
    pub fn saved_resume_profile(&self) -> Option<String> {
        ...
        match self.resume_target() {
            ...
            ResumeTarget::MostRecentForCwd => {
                xvora_shell::session::persistence::resumed_session_sandbox_profile(None, cwd)
            }
    ...
    fn resolve_startup_sandbox(explicit: Option<&str>, saved: Option<String>) -> SandboxStartup {
        match (explicit, saved) {
            (Some(x), Some(s))
                if x.parse::<xvora_sandbox::ProfileName>().ok()
                    != s.parse::<xvora_sandbox::ProfileName>().ok() =>
            {
    ...
    /// whitespace. This is the `xvora "<prompt>"` launch form; the headless
...
mod tests {
    ...
    fn version_flag_exits_zero() {
        let err = PagerArgs::try_parse_from(["xvora", "--version"]).unwrap_err();
    ...
    fn version_short_flag_exits_zero() {
        let err = PagerArgs::try_parse_from(["xvora", "-v"]).unwrap_err();
        assert_eq!(err.kind(), clap::error::ErrorKind::DisplayVersion);
        assert!(
            err.exit_code() == 0,
            "-v must exit 0; got {}",
    ...
    fn resume_target_classifies_flags() {
        assert_eq!(
            PagerArgs::try_parse_from(["xvora"]).unwrap().resume_target(),
            ResumeTarget::None
        );
        assert_eq!(
    ...
    fn leader_mgmt_list_info_kill_parse() {
        ...
        let kill = PagerArgs::try_parse_from(["xvora", "leader", "kill"]).expect("xvora leader kill");
        assert!(matches!(
            kill.command,
            Some(Command::Leader(LeaderMgmtArgs {
    ...
    fn initial_prompt_trims_and_ignores_whitespace_only() {
        let args = PagerArgs::try_parse_from(["xvora", "  spaced  "]).expect("padded prompt parses");
    ...
    fn reasoning_effort_and_effort_alias_parse_same_field() {
        ...
        let alias =
            PagerArgs::try_parse_from(["xvora", "--effort", "high"]).expect("--effort alias parses");
    ...
    fn reasoning_effort_accepts_max_and_remapped_ids() {
        ...
        let deep =
            PagerArgs::try_parse_from(["xvora", "--reasoning-effort", "deep"]).expect("deep parses");
