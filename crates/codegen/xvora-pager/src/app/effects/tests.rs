...
fn register_session_in(root: &std::path::Path, id: &str) -> acp::SessionId {
    use xvora_shell::active_sessions::{ActiveSession, register_in};
...
fn unregister_best_effort_removes_entry_when_lock_free() {
    ...
    assert!(
        xvora_shell::active_sessions::list_in(dir.path()).expect("list").is_empty(),
...
fn unregister_best_effort_is_nonblocking_under_lock_contention() {
    ...
    assert_eq!(
        xvora_shell::active_sessions::list_in(dir.path()).expect("list").len(), 1,
...
async fn foreign_scan_task_echoes_sequence_without_enabled_sources() {
    ...
    execute(
        Effect::ScanForeignSessions {
            ...
            compat: xvora_workspace::foreign_sessions::EnabledForeignSessionSources::default(),
