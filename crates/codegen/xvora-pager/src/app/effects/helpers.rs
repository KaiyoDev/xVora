...
pub(super) fn format_acp_error(err: &acp::Error, is_api_key_auth: bool) -> String {
    ...
    if err.code == acp::ErrorCode::InvalidParams && let Some(data) = &err.data
        && let Some(msg) = xvora_shell::sampling::error::error_detail_from_data(data)
        && !msg.is_empty()
    {
        return msg;
    }
...
pub(crate) fn sanitize_user_error(raw: &str) -> String {
    if raw.contains(fast_worktree::OUT_OF_DISK_CONTEXT)
...
pub(crate) async fn persist_setting(
    ...
    match key {
        ...
        "language" => {
            ...
            xvora_shell::util::config::set_language(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "max_thoughts_width" => {
...
/// serializes (`xvora_shell::extensions::task::KillTaskResponse` +
/// `xvora_shell::session::result::ExtMethodResult`) so the contract stays
...
pub(super) fn unregister_active_session_best_effort(session_id: &acp::SessionId) {
    unregister_active_session_best_effort_in(
        &xvora_shell::util::xvora_home::xvora_home(),
        session_id,
    );
}
pub(super) fn unregister_active_session_best_effort_in(
    ...
    match xvora_shell::active_sessions::try_unregister_in(root, session_id) {
        Ok(true) => {}
        Ok(false) => {
            tracing::debug!(
