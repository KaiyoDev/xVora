...
fn title_miss_hint_escapes_arg_and_suggests_search() {
    ...
    assert!(
        hint.contains("xvora sessions search"),
        "missing hint: {hint}"
    );
}
...
fn worktree_failure_message_hint_follows_threaded_provenance() {
    ...
    assert!(msg.contains("xvora sessions search"), "{msg}");
...
fn headless_title_pin_is_caller_aware() {
    ...
    let mut interactive =
        crate::app::cli::PagerArgs::try_parse_from(["xvora", "-r", "batch run"]).unwrap();
    interactive
    ...
    let mut headless =
        crate::app::cli::PagerArgs::try_parse_from(["xvora", "-p", "next", "-r", "batch run"])
            .unwrap();
    headless
        .pin_local_resume_target_for_cwd(Some(&cwd_str))
        .unwrap();
...
fn pin_prefers_restored_child_over_same_id_in_other_cwd() {
    ...
    let mut args =
        crate::app::cli::PagerArgs::try_parse_from(["xvora", "-r", "legacy-remote-7"]).unwrap();
    args.pin_local_resume_target_for_cwd(Some(&cwd_str))
        .unwrap();
...
async fn materialization_consumes_pinned_id_after_concurrent_rename() {
    ...
    let mut args = crate::app::cli::PagerArgs::try_parse_from(["xvora", "-r", "alpha"]).unwrap();
    args.pin_local_resume_target_for_cwd(Some(&cwd_str))
        .unwrap();
...
fn pin_ambiguous_title_errors_before_sandbox() {
    ...
    let mut args = crate::app::cli::PagerArgs::try_parse_from(["xvora", "-r", "Dup"]).unwrap();
    let msg = args
        .pin_local_resume_target_for_cwd(Some(&cwd_str))
        .unwrap_err()
        .to_string();
...
async fn pinned_no_match_does_not_retry_title_after_sandbox() {
    ...
    let mut args = crate::app::cli::PagerArgs::try_parse_from(["xvora", "-r", "ghost"]).unwrap();
    args.pin_local_resume_target_for_cwd(Some(&cwd_str))
        .unwrap();
...
async fn duplicate_legacy_id_is_not_title_addressable() {
    ...
    let mut args =
        crate::app::cli::PagerArgs::try_parse_from(["xvora", "-r", "locked down"]).unwrap();
    args.pin_local_resume_target_for_cwd(Some(&cwd_str))
        .unwrap();
    assert!(args.resume_target_pinned);
