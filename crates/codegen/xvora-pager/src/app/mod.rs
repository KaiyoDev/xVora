...
pub(crate) fn voice_mode_requirement_pin() -> Option<bool> {
    xvora_config::load_merged_requirements().and_then(|req| {
        req.get("features")
...
pub(crate) fn resolve_voice_mode_enabled(
    ...
    use xvora_shell::agent::config::{BoolFlag, ConfigSource};
...
pub(crate) enum ScreenMode {
    ...
    ///
    /// All minimal-mode rendering lives in the sibling `xvora-pager-minimal`
...
impl ScreenMode {
    ...
    /// allowlist (`xvora-telemetry`'s `KNOWN_SCREEN_MODES`); renaming one
...
pub fn join_early_prefetch(
    handle: Option<xvora_shell::agent::models::EarlyPrefetchHandle>,
) -> Option<xvora_shell::util::config::RemoteSettings> {
    let handle = handle?;
