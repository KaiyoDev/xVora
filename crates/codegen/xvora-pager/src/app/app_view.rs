...
/// [`xvora_workspace::folder_trust::decide`] verdict; when the feature flag
...
/// (toolset) gate via [`xvora_shell::tier::is_restricted_tier_name`] so
...
pub struct AppView {
    ...
    /// "Don't ask me again" opt-out from [`xvora_shell::util::config::resolve_hints`];
    ...
    /// empty and inert outside `--minimal`; the `xvora-pager-minimal` crate
    ...
    /// `xvora_shell::util::config::resolve_auto_permission_mode_enabled`.
...
impl AppView {
    ...
    pub fn deferred_model_switch_from_cli(
        ...
        acp::ModelId,
        Option<xvora_shell::sampling::types::ReasoningEffort>,
    )> {
...
impl AppView {
    ...
    /// Log [`xvora_telemetry::events::AnnouncementCtaShown`] for each
...
pub(crate) mod tests {
    ...
    fn needs_animation_gates_todo_badge_flash() {
        ...
        app.agents
            ...
            .unwrap()
            .todo
            .update_todos(vec![xvora_shell::tools::TodoItem {
                content: "do the thing".into(),
                priority: Default::default(),
                status: xvora_shell::tools::TodoStatus::InProgress,
                meta: None,
            }]);
        assert!(
    ...
    fn welcome_ctrl_u_update_keeps_priority_over_foreign_resume() {
        ...
        app.apply_foreign_resume_detection(
            launch_token,
            &canonical_cwd,
            Some(xvora_workspace::foreign_sessions::RecentForeignSession {
                tool: xvora_workspace::foreign_sessions::ForeignSessionTool::Cursor,
