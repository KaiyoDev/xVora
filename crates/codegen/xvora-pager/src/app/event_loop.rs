...
/// [`decide`](xvora_workspace::folder_trust::decide) precedence.
///
...
pub(crate) async fn run(
    ...
    let launch_effective_ui = xvora_shell::config::load_effective_config()
        .ok()
    ...
    app.sharing_enabled = remote_settings
        ...
        .unwrap_or(false);
    app.plugin_cta_enabled = xvora_config::env_bool("XVORA_PLUGIN_CTA")
    ...
    if let Some(ref raw) = effective_config {
        ...
        if let Some(table) = raw.as_table() {
            app.voice_config = xvora_voice::VoiceConfig::from_config_table(table);
        }
    }
    ...
    {
        use xvora_shell::util::config::{resolve_announcements, resolve_tips};
        ...
        if !app.tips.is_empty() {
            ...
            app.tip = xvora_shell::util::tips::pick_and_advance(&app.tips, &xvora_home);
        }
    }
    ...
    loop {
        ...
        if let VoiceState::ColdStart { hold, target } = app.voice_state {
            if app.voice_cmd_tx.is_none() && app.voice_can_start_pipeline() {
                ...
                app.voice_state = VoiceState::Idle;
                app.voice_ui_active = false;
                app.show_toast("Voice pipeline could not start — restart xVora");
            } else {
