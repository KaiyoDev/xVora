...
//! inside [`xvora_mermaid::render_checked`] is a no-op there: a panic in the
...
//!
//! 1. The pager re-execs itself as `xvora-pager __mermaid-render` (see
...
//!    via [`xvora_mermaid::run_with_timeout`], which **kills and reaps** the
...
//!    ([`xvora_mermaid::RenderLimits`]) and the raster megapixel/height caps
//!    (see `xvora-mermaid`). The parent also rejects obviously-oversized
