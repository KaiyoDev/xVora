...
//!
//! ```bash
//! cargo test -p xvora-pager --lib -- app::leader_cluster --ignored --test-threads=1
//! ```
//!
...
impl PagerLeaderCluster {
    ...
    async fn spawn_leader_generation(&mut self) {
        ...
        let control_state = LeaderServerControlState::new(LeaderServerMetadata {
            ...
            // MUST be the client-side comparison source (xvora_version), not
        ...
        // xvora-shell/src/agent/app.rs (inside its LocalSet body) — a
        // deliberate copy so production stays untouched. Second copy of the
        // same wiring: xvora-shell/tests/test_leader_soak.rs ("Real agent
