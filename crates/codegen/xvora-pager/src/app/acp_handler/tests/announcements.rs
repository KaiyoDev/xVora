#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// layer-injected seam so the developer's real `~/.xvora` cannot leak in.
    #[test]
    ...
    /// through the layer-injected seam (no real `~/.xvora` reads).
    #[test]
    ...
    /// layer-injected seam (no real `~/.xvora` reads).
    #[test]
