#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn live_turn_completed_finalizes_viewer_turn_and_duplicate_is_noop() {
        ...
        let affected = handle_ext_notification(
            &xvora_turn_completed_notif("sess-view", "pid-driver", "end_turn", false),
            &mut app,
        );
    ...
    fn unknown_error_kind_from_wire_is_never_sniff_reclassified() {
        ...
        let _ = handle_ext_notification(
            &xvora_turn_completed_failed_with_error_kind(
                "sess-view",
                "pid-driver",
    ...
    fn live_turn_completed_driver_arms_reconcile() {
        ...
        let _ = handle_ext_notification(
            &xvora_turn_completed_notif("sess-drive", "pid-local", "cancelled", false),
            &mut app,
        );
    ...
    fn silent_wake_turn_completed_is_markerless() {
        ...
        let affected = handle_ext_notification(
            &xvora_wake_turn_completed_notif(
                "sess-wake",
                "task-completed-bg1",
    ...
    fn chatty_wake_turn_completed_pushes_one_marker() {
        ...
        let affected = handle_ext_notification(
            &xvora_wake_turn_completed_notif("sess-wake", "task-completed-bg1", None),
            &mut app,
        );
        assert!(affected);
    ...
    fn wake_turn_stop_affordance_offered_then_cleared_at_terminal() {
        ...
        let _ = handle_ext_notification(
            &xvora_wake_turn_completed_notif("sess-wake", "task-completed-bg2", None),
            &mut app,
        );
        let _ = handle(
    ...
    fn scheduler_fired_turn_completed_keeps_adopted_path() {
        ...
        let affected = handle_ext_notification(
            &xvora_wake_turn_completed_notif("sess-cron", "scheduler-fired-abc", Some(1_000)),
            &mut app,
        );
    ...
    fn silent_errored_wake_pushes_failure_marker() {
        ...
        let _ = handle_ext_notification(
            &xvora_turn_completed_notif("sess-wake", "task-completed-bg1", "error", false),
            &mut app,
        );
    ...
    fn goal_terminal_snapshots_epoch_so_next_silent_wake_stays_markerless() {
        ...
        let _ = handle_ext_notification(
            &xvora_turn_completed_notif("sess-wake", "goal-summary-g1", "end_turn", false),
            &mut app,
        );
        let len_before = app.agents[&AgentId(0)].scrollback.len();

        let _ = handle_ext_notification(
            &xvora_turn_completed_notif("sess-wake", "task-completed-bg1", "end_turn", false),
            &mut app,
        );
    ...
    fn wake_terminal_during_command_snapshots_epoch_for_next_silent_wake() {
        ...
        let _ = handle_ext_notification(
            &xvora_turn_completed_notif("sess-wake", "task-completed-bg2", "end_turn", false),
            &mut app,
        );
    ...
    fn silent_cancelled_or_rate_limited_wake_stays_markerless() {
        ...
        for stop_reason in ["cancelled", "rate_limit"] {
            let _ = handle_ext_notification(
                &xvora_turn_completed_notif("sess-wake", "task-completed-bg1", stop_reason, false),
                &mut app,
            );
        }
    ...
    fn chatty_send_now_cancelled_wake_is_markerless() {
        ...
        let _ = handle_ext_notification(
            &xvora_turn_completed_notif_with_cancel_trigger(
                "sess-wake",
                "task-completed-bg1",
                "cancelled",
                "send_now",
    ...
    fn chatty_user_cancelled_wake_pushes_cancelled_marker() {
        ...
        let _ = handle_ext_notification(
            &xvora_turn_completed_notif("sess-wake", "task-completed-bg1", "cancelled", false),
            &mut app,
        );
    ...
    fn chatty_rate_limited_wake_closes_with_failure_marker() {
        ...
        let _ = handle_ext_notification(
            &xvora_turn_completed_notif("sess-wake", "task-completed-bg1", "rate_limit", false),
            &mut app,
        );
    ...
    fn wake_terminal_during_local_turn_pushes_nothing() {
        ...
        let affected = handle_ext_notification(
            &xvora_wake_turn_completed_notif("sess-wake", "task-completed-bg1", Some(6_000)),
            &mut app,
        );
    ...
    fn live_stop_hooks_during_turn_stash_instead_of_standalone_block() {
        ...
        let _ = handle_ext_notification(
            &xvora_hook_execution_notif("sess-stop", "stop", false),
            &mut app,
        );
    ...
    fn replayed_stop_hooks_render_as_standalone_block() {
        ...
        let _ = handle_ext_notification(
            &xvora_hook_execution_notif("sess-replay", "stop", true),
            &mut app,
        );
    ...
    fn foreign_turn_stop_hooks_never_stash_under_running_turn() {
        ...
        let _ = handle_ext_notification(
            &xvora_hook_execution_notif_for_prompt("sess-foreign", "stop", Some("pid-old"), false),
            &mut app,
        );
        ...
        let _ = handle_ext_notification(
            &xvora_hook_execution_notif_for_prompt("sess-foreign", "stop", Some("pid-new"), false),
            &mut app,
        );
        let agent = app.agents.get(&AgentId(0)).unwrap();
        let pending = agent
    ...
    fn foreign_stop_hooks_refused_at_idle_tail_marker() {
        ...
        let _ = handle_ext_notification(
            &xvora_turn_completed_notif("sess-idle-foreign", "pid-new", "end_turn", false),
            &mut app,
        );

        // The marker's own batch (matching pid) merges…
        let _ = handle_ext_notification(
            &xvora_hook_execution_notif_for_prompt(
                "sess-idle-foreign",
                "stop",
                Some("pid-new"),
                false,
    ...
    fn stop_cancelled_hooks_fold_into_the_cancelled_marker() {
        ...
        for terminal_first in [true, false] {
            ...
            let terminal =
                xvora_turn_completed_notif("sess-cancelled-hooks", "pid-c", "cancelled", false);
            let batch = xvora_hook_execution_notif_for_prompt(
                "sess-cancelled-hooks",
                "stop_cancelled",
                Some("pid-c"),
                false,
    ...
    fn stamped_stop_hooks_merge_past_interleaved_tail_block() {
        ...
        let _ = handle_ext_notification(
            &xvora_turn_completed_notif("sess-interleaved", "pid-new", "end_turn", false),
            &mut app,
        );
        app.agents
            .get_mut(&AgentId(0))
    ...
    fn same_name_stash_repeat_goes_standalone() {
        ...
        let _ = handle_ext_notification(
            &xvora_hook_execution_notif("sess-stash-dup", "stop", false),
            &mut app,
        );
        let _ = handle_ext_notification(
            &xvora_hook_execution_notif("sess-stash-dup", "stop", false),
    ...
    fn stash_key_prefers_wire_prompt_id() {
        ...
        let _ = handle_ext_notification(
            &xvora_hook_execution_notif_for_prompt("sess-wire-key", "stop", Some("pid-a"), false),
            &mut app,
        );
    ...
    fn session_end_stop_hooks_without_live_turn_stay_standalone() {
        ...
        let _ = handle_ext_notification(
            &xvora_hook_execution_notif("sess-end", "stop", false),
            &mut app,
        );
    ...
    fn non_stop_lifecycle_hooks_keep_standalone_block() {
        ...
        let _ = handle_ext_notification(
            &xvora_hook_execution_notif("sess-ls", "session_start", false),
            &mut app,
        );
    ...
    fn replayed_turn_completed_blocks_session_loaded_adoption() {
        ...
        let affected = handle_ext_notification(
            &xvora_turn_completed_notif("sess-1", "p-run", "end_turn", true),
            &mut app,
        );
        assert!(
            !affected,
    ...
    fn background_driver_live_turn_completed_arms_reconcile_and_reports_change() {
        ...
        let affected = handle_ext_notification(
            &xvora_turn_completed_notif("sess-bg", "pid-bg", "cancelled", false),
            &mut app,
        );
        assert!(
            affected,
    ...
    fn second_load_does_not_inherit_first_loads_replay_window_state() {
        ...
        let _ = handle_ext_notification(
            &xvora_turn_completed_notif("sess-1", "p-first", "end_turn", true),
            &mut app,
        );
        assert!(
            app.agents[&id]
    ...
    fn wake_stop_hooks_render_standalone_at_arrival() {
        ...
        let _ = handle_ext_notification(
            &xvora_wake_turn_completed_notif("sess-wake-idle", "task-completed-bg1", None),
            &mut app,
        );
        let _ = handle_ext_notification(
            &xvora_hook_execution_notif_for_prompt(
    ...
    fn replay_completed_without_elapsed_is_turn_completed_not_zero() {
        ...
        begin_replay(&mut app);
        let _ = handle_ext_notification(
            &xvora_turn_completed_notif("sess-1", "p1", "end_turn", true),
            &mut app,
        );
        let agent = app.agents.get(&AgentId(0)).unwrap();
        match last_session_event(&agent.scrollback) {
    ...
    fn replay_silent_rate_limited_wake_stays_markerless() {
        ...
        let len_before = app.agents[&AgentId(0)].scrollback.len();
        let _ = handle_ext_notification(
            &xvora_turn_completed_notif("sess-wake", "task-completed-bg1", "rate_limit", true),
            &mut app,
        );
        let agent = app.agents.get(&AgentId(0)).unwrap();
        assert!(agent.replayed_terminal_prompts.contains("task-completed-bg1"));
    ...
    fn replay_duplicate_pid_one_marker() {
        let mut app = make_app_with_agent("sess-1");
        begin_replay(&mut app);
        let n = xvora_turn_completed_replay(
            "sess-1",
            "p1",
            "end_turn",
            Some(10),
    ...
    fn replay_silent_wake_records_pid_no_marker() {
        ...
        let len_before = app.agents[&AgentId(0)].scrollback.len();
        let _ = handle_ext_notification(
            &xvora_turn_completed_notif("sess-wake", "task-completed-bg1", "end_turn", true),
            &mut app,
        );
        let agent = app.agents.get(&AgentId(0)).unwrap();
        assert!(agent.replayed_terminal_prompts.contains("task-completed-bg1"));
    ...
    fn replay_stop_hooks_stay_standalone() {
        ...
        begin_replay(&mut app);
        let _ = handle_ext_notification(
            &xvora_hook_execution_notif_for_prompt("sess-1", "stop", Some("p1"), true),
            &mut app,
        );
        let _ = handle_ext_notification(
            &xvora_turn_completed_replay(
    ...
    fn xvora_last_turn_summary_notif(
        session_id: &str,
        summary: &str,
    ...
    fn last_turn_summary_shows_until_replaced() {
        ...
        let _ = handle_ext_notification(
            &xvora_turn_completed_notif("sess-lts", "pid-a", "end_turn", false),
            &mut app,
        );
        let affected = handle_ext_notification(
            &xvora_last_turn_summary_notif("sess-lts", "Did the thing", Some("pid-a")),
            &mut app,
        );
        assert!(affected);
        assert_eq!(
        ...
        let _ = handle_ext_notification(
            &xvora_turn_completed_notif("sess-lts", "pid-b", "cancelled", false),
            &mut app,
        );
        ...
        let _ = handle_ext_notification(
            &xvora_turn_completed_notif("sess-lts", "pid-c", "end_turn", false),
            &mut app,
        );
        let affected = handle_ext_notification(
            &xvora_last_turn_summary_notif("sess-lts", "Did the next thing", Some("pid-c")),
            &mut app,
        );
        assert!(affected);
        assert_eq!(
