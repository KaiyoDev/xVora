...
pub(super) fn handle_settings_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
        ...
        && app.permission_mode_from_soft_default
    {
        ...
        let root = xvora_shell::config::load_effective_config().ok();
...
pub(super) fn apply_soft_default_permission_mode(
    ...
    let mode = xvora_shell::util::config::resolve_permission_mode(effective_ui, remote);
...
/// `xvora-shell/src/agent/mvp_agent.rs`. The shell side derives `Serialize`
...
/// `xvora-shell/src/agent/mvp_agent.rs` when adding fields that both sides
/// need.
