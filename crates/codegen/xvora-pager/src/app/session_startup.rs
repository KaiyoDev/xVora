...
pub fn parent_session_is_worktree(session_id: &str, cwd: &Path) -> bool {
    ...
    let sessions_root = xvora_shell::util::xvora_home::xvora_home().join("sessions");
    let encoded = xvora_shell::util::xvora_home::encode_cwd_dirname(&cwd_str);
...
async fn resolve_existing_session(
    ...
    let progress: xvora_shell::session::restore::ProgressCallback =
...
mod tests {
    ...
    async fn chat_resume_passthrough_keeps_cwd_collision_refusal() {
        ...
        let encoded = xvora_shell::util::xvora_home::encode_cwd_dirname(&cwd_str);
        let sessions_cwd_dir = xvora_shell::util::xvora_home::xvora_home()
            .join("sessions")
            .join(&encoded);
