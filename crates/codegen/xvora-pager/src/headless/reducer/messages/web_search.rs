...
//! Folds a completed search inline (or the generic client split on failure), and parses xvora's `WebSearchCall` output into the wire hit array.
...
/// xvora nests query/sources under `action`, with a flat `{"query",...,"sources"}` fallback.
