...
//! render mode.
//!
//! `xvora-pager` cannot depend on `xvora-pager-minimal`: that crate reads
...
//!
//! The composition-root binary (`xvora-pager-bin`) wires it up once at
//! startup by calling `xvora_pager_minimal::install()`. When nothing has
