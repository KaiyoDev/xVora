...
//!
//! A dependency on `xvora-pager-minimal` would be a cargo cycle: that crate reads this crate's `AppView`, `views::*` widgets, and `scrollback`.
...
//!
//! The `xvora-pager-bin` binary calls `xvora_pager_minimal::install()` once at startup.
...
/// Renders one minimal-mode frame; the installed implementation is `xvora_pager_minimal::draw`.
