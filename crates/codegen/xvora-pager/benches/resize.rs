...
use xvora_pager::scrollback::{RenderBlock, ScrollbackState};
...
fn bash_output(i: usize) -> String {
    (0..40)
        .map(|l| format!("crates/codegen/xvora-pager/src/file_{i}_{l}.rs:{l}: match found"))
        .collect::<Vec<_>>()
        .join("\n")
}
    ...
    for i in 0..TURNS {
        ...
        push(
            ...
            RenderBlock::edit_with_hunks(
                format!("crates/codegen/xvora-pager/src/mod_{i}.rs"),
                xvora_pager_diff::diff_hunks_from_strings(&old, &new, 1),
            ),
            old.len() + new.len(),
        );
        push(
