...
use xvora_pager::views::settings_modal::{
...
fn make_state() -> SettingsModalState {
    ...
    xvora_pager::app::set_voice_mode_enabled_for_test(true);
...
fn follow_up_behavior_picker_enter_dispatches_set_commit() {
    ...
    match outcome {
        SettingsKeyOutcome::Action(Action::SetFollowUpBehavior(mode)) => {
            assert_eq!(mode, xvora_pager::appearance::FollowUpBehavior::Steer);
        }
...
fn defaults_round_trip_through_registry() {
    use xvora_pager::settings::{SettingValue, current_value_for};
    let reg = SettingsRegistry::defaults();
    ...
    xvora_pager::appearance::cache::set_keep_text_selection(
        xvora_pager::appearance::TextSelection::Flash,
    );
    xvora_pager::appearance::cache::set_show_thinking_blocks(true);
    xvora_pager::appearance::cache::set_prompt_suggestions(true);
    xvora_pager::appearance::cache::set_group_tool_verbs(true);
    xvora_pager::appearance::cache::set_page_flip_on_send(true);
    xvora_pager::appearance::cache::set_combine_queued_prompts(false);
    ...
    for meta in reg.all() {
        ...
        let default_value = xvora_pager::settings::default_value_for(meta);
...
// Strangler-fig dispatch-layer tests for the typed Actions are in `crates/codegen/xvora-pager/src/app/dispatch.rs::tests`
...
fn pr6_current_value_for_reads_pager_snapshot() {
    use xvora_pager::settings::current_value_for;
...
fn pr6_permission_mode_picker_enter_dispatches_set_permission_mode_commit() {
    use xvora_pager::app::actions::PermissionModeKind;
    let reg = SettingsRegistry::defaults();
...
fn reset_overlay_dims_all_rows_except_target() {
    ...
    use xvora_pager::views::settings_modal::ResetConfirmOverlay;
...
/// The settings modal renders a 1-line "Ask xvora" tip footer at the bottom of the content area.
...
fn docs_footer_renders_for_browse_and_picker() {
    ...
    for fixture_label in ["browse", "picker"] {
        ...
        assert!(
            all_text.contains("Ask xvora"),
            "[{fixture_label}] docs footer (`Ask xvora`) must appear in the rendered modal:\n\
             {all_text}"
        );
        assert!(
...
fn render_modal_to_string(s: &mut SettingsModalState, width: u16, height: u16) -> String {
    ...
    let mut buf = Buffer::empty(area);
    xvora_pager::views::settings_modal::render_settings_modal(&mut buf, area, s, false, None);
...
fn pr14_default_model_picker_commits_resolved_model_id() {
    ...
    // Walk down past row 0 ("(no override)") to row 1 ("xvora 4.5").
    let outcome = handle_settings_key(&mut s, &press(KeyCode::Down));
    assert!(
        matches!(outcome, SettingsKeyOutcome::Changed),
...
fn pr8_default_model_and_max_thoughts_width_defaults_roundtrip() {
    ...
    assert_eq!(
        xvora_pager::settings::default_value_for(dm_meta),
    ...
    assert_eq!(
        xvora_pager::settings::default_value_for(mt_meta),
...
fn pr9_privacy_slash_command_takes_no_arguments() {
    use xvora_pager::slash::commands::builtin_commands;
    use xvora_pager::slash::registry::CommandRegistry;
...
fn pr10_plan_mode_picker_enter_dispatches_set_commit() {
    use xvora_pager::app::actions::PlanModeKind;
...
fn render_mermaid_picker_enter_dispatches_set_commit() {
    use xvora_pager::appearance::RenderMermaid;
...
fn pr14_string_settings_use_known_model_validator() {
    use xvora_pager::settings::DynamicEnumSource;
    let reg = SettingsRegistry::defaults();
...
fn keep_text_selection_picker_enter_dispatches_set_commit() {
    use xvora_pager::appearance::TextSelection;
    ...
    xvora_pager::appearance::cache::set_keep_text_selection(TextSelection::Flash);
    let mut s = make_state();
    navigate_to(&mut s, "keep_text_selection");
    let _ = handle_settings_key(&mut s, &press(KeyCode::Enter));
...
fn keep_text_selection_hold_snapshot_seeds_picker_at_hold() {
    ...
    xvora_pager::appearance::cache::set_keep_text_selection(TextSelection::Hold);
    let ui = UiConfig {
...
fn invert_scroll_enter_dispatches_typed_setter() {
    xvora_pager::appearance::cache::set_invert_scroll(true);
    let mut s = make_state();
    navigate_to(&mut s, "invert_scroll");
    let outcome = handle_settings_key(&mut s, &press(KeyCode::Enter));
...
fn show_thinking_blocks_space_dispatches_typed_setter() {
    ...
    xvora_pager::appearance::cache::set_show_thinking_blocks(false);
    let mut s = make_state();
...
fn prompt_suggestions_space_dispatches_typed_setter() {
    // Pin off so space toggles to true.
    xvora_pager::appearance::cache::set_prompt_suggestions(false);
    let mut s = make_state();
...
fn group_tool_verbs_cache_off_dispatches_on() {
    ...
    xvora_pager::appearance::cache::set_group_tool_verbs(false);
    let mut s = SettingsModalState::new(
        Arc::new(SettingsRegistry::defaults()),
        UiConfig::default(),
        PagerLocalSnapshot::default(),
...
fn collapsed_edit_blocks_space_dispatches_typed_setter() {
    ...
    xvora_pager::appearance::cache::set_collapsed_edit_blocks(false);
    let mut s = make_state();
...
fn collapsed_edit_blocks_cache_on_dispatches_off() {
    ...
    xvora_pager::appearance::cache::set_collapsed_edit_blocks(true);
    let mut s = SettingsModalState::new(
        Arc::new(SettingsRegistry::defaults()),
        UiConfig::default(),
        PagerLocalSnapshot::default(),
