...
use xvora_pager_pty_harness::PtyHarness;
pub(crate) use xvora_pager_pty_harness::{SGR_SCROLL_DOWN, SGR_SCROLL_UP};
...
///
/// Harness mirror: the scroll-matrix runner (`xvora-pager-pty-harness/src/scroll_matrix/runner.rs`) ports this send loop onto `WheelStep` tables.
...
///
/// Harness mirror: `spawn_settled_marker_session` in `xvora-pager-pty-harness/src/scroll_matrix/session.rs` is a port of this preamble.
...
///
/// Harness mirror: `spawn_streaming_marker_session` in `xvora-pager-pty-harness/src/scroll_matrix/session.rs` is a port of this preamble.
/// Fixes must flow both ways.
