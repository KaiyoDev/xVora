...
// `seed_fake_oauth` / `oauth_credential_ops` live in `xvora_pager_pty_harness::flows` (re-exported above)
...
// `wait_for_labels_absent` lives in `xvora_pager_pty_harness::flows` (re-exported above)
...
pub(crate) fn spawn_esc_double_press_pager(content: &ContentController) -> PtyHarness {
    let binary = pager_binary().expect("resolve pager binary");
    let value = xvora_pager::app::app_view::ESC_DOUBLE_PRESS_TEST_MS.to_string();
    PtyHarness::spawn_with_content_env_ops(
        &binary,
        DEFAULT_ROWS,
        DEFAULT_COLS,
