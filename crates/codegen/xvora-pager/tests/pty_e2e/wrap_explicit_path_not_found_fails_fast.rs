...
/// PTY spawn fails, the exec fallback engages ("wrapped mode failed"), exec ENOENTs ("failed to exec"), and xvora exits 1.
...
fn wrap_explicit_path_not_found_fails_fast() {
    let (code, raw) = run_wrap(&["/nonexistent-xvora-wrap-e2e/prog", "arg"], &[]);
