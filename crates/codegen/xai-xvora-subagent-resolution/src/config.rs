...
//! `xai_xvora_shell::config::{SubagentRole, SubagentPersona, PersonaIOField}`.
//!
//! Methods that remain in `xai-xvora-shell` (on `SubagentsConfig`):
...
"#;
        ...
        assert_eq!(
            ...
            Some(".xvora/prompts/researcher.md")
        );
