...
//!
//! These produce the exact wire format that the xvora sampling client expects, validated against the real sampling client.
...
/// SSE `event:` name and payload `type` of the non-standard doom-loop check event (`xvora_sampling_types::DOOM_LOOP_CHECK_EVENT_TYPE`).
