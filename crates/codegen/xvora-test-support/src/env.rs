...
fn local_grok_binary_path() -> PathBuf {
    target_dir()
        .join("debug")
        .join(format!("xvora-pager{}", std::env::consts::EXE_SUFFIX))
}
...
fn ensure_local_grok_binary(binary: &Path) {
    ...
    let output = Command::new(&cargo)
        ...
        .args(["build", "-p", "xvora-pager", "--bin", "xvora-pager"])
        .output()
        .unwrap_or_else(|e| panic!("failed to spawn {cargo} to build xvora-pager: {e}"));

    assert!(
        output.status.success(),
        "failed to build xvora-pager for lifecycle tests (exit {:?})\nstdout:\n{}\nstderr:\n{}",
        output.status.code(),
    ...
    assert!(
        binary.exists(),
        "xvora-pager build completed but binary missing at {}",
        binary.display()
    );
}

/// Resolve grok binary: `XVORA_BINARY` env (CI) or a locally built `xvora-pager` binary.
