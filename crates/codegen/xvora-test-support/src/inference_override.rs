...
impl InferenceRequestKind {
    fn classify(headers: &HeaderMap, body: &Value) -> Self {
        if nonempty_header(headers, "x-xvora-turn-idx").is_some() {
            return Self::Foreground;
        }
        if nonempty_header(headers, "x-xvora-req-id").is_some() {
...
impl InferenceOverrides {
    ...
    pub(crate) fn classify(
        ...
        let fingerprint =
            nonempty_header(headers, "x-xvora-req-id").map(|request_id| ModelCallFingerprint {
                endpoint,
                kind,
