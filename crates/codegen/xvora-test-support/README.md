...
runner, and sandboxed process env. Consumed by `xvora-shell` integration
tests, `xvora-pager-pty-harness` (`ContentController`), and `xvora-sampler`
tests.
...
(`xvora-pager-pty-harness`). This file is the API reference for the shared
...
| `env` | `grok_binary()` (`XVORA_BINARY` env → `CARGO_BIN_EXE` → local debug build of `xvora-pager`), `git_workdir()` (temp git repo, forces full libgit2 init), `test_env_cmd_tokio(cmd, mock_url, home)` (sandboxed HOME **and XVORA_HOME** — Windows resolves `~` via USERPROFILE, so HOME alone doesn't sandbox — + mock endpoints + telemetry kill-switches). |
...
| Consumer | Uses | Notes |
|----------|------|-------|
| `xvora-shell` `tests/*.rs` | Everything | Direct imports (`use xvora_test_support::*` or module paths); no local shim. |
| `xvora-pager-pty-harness` `src/content.rs` | `MockInferenceServer`, `MockModelEntry` (re-exported as `MockModel`) | `ContentController` wraps the server and **keeps the HOME-sandbox `TempDir` + `env_for_pager()` harness-side**; presets `allow_access` + a fixed default response at construction. |
| `xvora-sampler` `tests/test_actor.rs` | `sse` generators | Happy-path payloads only; the actor keeps its own router for stall/conditional fixtures. |
