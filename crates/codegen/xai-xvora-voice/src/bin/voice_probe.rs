...
//! ```bash
//! export XAI_API_KEY=...
//! cargo run -p xai-xvora-voice --bin voice-probe -- --seconds 5
//! ```
...
async fn main() -> anyhow::Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "info,xai_xvora_voice=debug".into()),
        )
        .init();
    ...
    if args.mic_only {
        ...
        {
            let (bytes, chunks) =
                xai_xvora_voice::run_mic_only_probe(config.sample_rate, args.seconds)?;
...
fn load_config(path: Option<&std::path::Path>) -> VoiceConfig {
    ...
    if let Ok(home) = std::env::var("XVORA_HOME")
...
Environment:
  ...
  RUST_LOG        optional (default info,xai_xvora_voice=debug)
