...
//!
//! Credentials are stored in `$XVORA_HOME/mcp_credentials.json`, keyed by a
...
impl McpCredentialStore {
    ...
    pub fn save_default(&self) -> Result<()> {
        let path = Self::default_path().ok_or_else(|| {
            McpCredentialError::Other("no user grok home (set $XVORA_HOME or $HOME)".into())
        })?;
        self.save_to(&path)
    }
    ...
    fn default_path() -> Option<PathBuf> {
        Some(xai_xvora_config::user_xvora_home()?.join(CREDENTIALS_FILENAME))
    }
}
...
mod tests {
    ...
    /// `$XVORA_HOME/mcp_credentials.json`. Existing credential files must keep
