...
fn jemalloc_allocator_stats() -> Option<xvora_pager::memory_trace::AllocatorStats> {
    ...
    unsafe {
        ...
        Some(xvora_pager::memory_trace::AllocatorStats {
...
fn main() {
    ...
    #[cfg(all(feature = "jemalloc", unix))]
    {
        xvora_pager::memory_trace::install_allocator_stats_provider(jemalloc_allocator_stats);
        xvora_pager::memory_trace::install_allocator_dump_provider(jemalloc_stats_dump);
    }
    #[cfg(all(feature = "jemalloc", unix))]
...
async fn async_main(args: PagerArgs) -> Result<()> {
    ...
    let sandbox_profile_arg = match args.startup_sandbox_profile(saved_profile.as_deref()) {
        ...
        xvora_pager::app::cli::SandboxStartup::Conflict { requested, saved } => {
            eprintln!(
    ...
    if let Some(command) = args.command.take() {
        match command {
            ...
            Command::Mcp(mcp_args) => {
                init_tracing_simple("cli");
                return xvora_pager::mcp_cmd::run(mcp_args).await;
            }
            ...
            Command::Export(export_args) => {
                init_tracing_simple("cli");
                return xvora_pager::export_cmd::run(export_args);
            }
            ...
            Command::Memory(memory_args) => {
                return xvora_pager::memory_cmd::run(memory_args);
            }
            Command::Update {
                check,
                json,
    ...
    let headless_prompt = xvora_pager::headless::HeadlessPrompt::from_args(
    ...
    if headless_prompt.is_some() || args.memory_flush {
        ...
        let json_schema = args
            ...
            .transpose()?;
        if json_schema.is_some() && args.output_format == xvora_pager::headless::OutputFormat::Plain
        {
    ...
    let result = xvora_pager::app::run(args, bg_update_rx).await;
    xvora_sandbox::flush();
    match result {
        Ok(true) => {
