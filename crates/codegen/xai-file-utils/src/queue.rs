...
use xai_xvora_auth::AuthCredentialProvider;
...
/// Resolve a `XVORA_UPLOAD_QUEUE_AUTH_PROBE_SECS` override (seconds) into a probe
...
/// `session_id`, no GCS path). Read by `xai_xvora_workspace::recovery`.
...
impl UploadQueue {
    ...
    pub fn spawn(
        ...
        Self::spawn_with_concurrency(xvora_home, resolver, retry_policy, DEFAULT_MAX_CONCURRENT)
    }
    ...
    pub fn spawn_with_concurrency(
        ...
        let queue_dir = xvora_home.join("upload_queue");
        if let Err(e) = std::fs::create_dir_all(&queue_dir) {
            tracing::warn!(error = % e, "Failed to create upload queue dir");
        }
        if let Some(raw_secs) = std::env::var("XVORA_UPLOAD_QUEUE_AUTH_PROBE_SECS")
            .ok()
            .and_then(|s| s.parse::<u64>().ok())
        {
            match auth_park_probe_override(raw_secs) {
                ...
                None => {
                    tracing::warn!(
                        "Ignoring XVORA_UPLOAD_QUEUE_AUTH_PROBE_SECS={raw_secs}: a zero probe \
        ...
        let max_queue_bytes = std::env::var("XVORA_UPLOAD_QUEUE_MAX_BYTES")
            .ok()
...
/// of entries removed.
pub fn cleanup_orphaned_uploads(xvora_home: &Path, max_age: Duration) -> u64 {
    let cleaned = cleanup_queue_dir(&xvora_home.join("upload_queue"), max_age, None);
