...
pub enum Event {
    ...
    /// `TODO_GATE_*` discriminator constant in `xai-xvora-shell::session::events`.
    TodoGateFired {
        fires: u32,
        pending: usize,
        in_progress: usize,
    ...
    /// constants in `xai-xvora-shell::session::events`.
    ...
    /// in `xai-xvora-shell::session::events`.
    ...
    /// discriminator constants in `xai-xvora-shell::session::events`.
    ...
    /// `xai-xvora-shell::session::events`.
    ...
    /// `xai-xvora-shell::session::events`. Fail-open never pauses the goal.
    ...
    /// enumerated by
    /// `xai-xvora-shell::session::goal_stop_detector::PATTERN_LABELS`;
...
/// Telemetry mirror of `xai-xvora-shell`'s `GoalClassifierVerdict`. Two
/// crates due to the orphan rule; the conversion lives in
/// `xai-xvora-shell/src/session/events.rs`.
#[derive(Debug, Clone, Copy, Serialize)]
#[serde(rename_all = "snake_case")]
...
/// Telemetry mirror of `xai-xvora-shell`'s `GoalPauseReason`. The two types
