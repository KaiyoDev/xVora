...
impl WorktreeDb {
    /// Open (or create) the DB at `xvora_home/worktrees.db`.
    pub fn open(xvora_home: &Path) -> Result<Self> {
        Self::open_at(&xvora_home.join(WORKTREES_DB_FILE))
    }
    ...
    /// Open the default DB at `~/.xvora/worktrees.db`.
    ///
    /// Discovers xvora home via `xvora_dirs::resolve_grok_home` (`$xvora_home`,
    /// else the canonicalized `<home>/.xvora`).
    ...
    fn journal_mode_and_base_path(xvora_home: &Path) -> (JournalMode, PathBuf) {
        let base_path = xvora_home.join(WORKTREES_DB_FILE);
    ...
    pub fn resolve_db_path(xvora_home: &Path) -> PathBuf {
        let (mode, base_path) = Self::journal_mode_and_base_path(xvora_home);
    ...
    pub fn open_read_only(xvora_home: &Path) -> RegistryOpen {
        let (mode, base_path) = Self::journal_mode_and_base_path(xvora_home);
...
/// Resolve the xvora home: `$xvora_home`, else `<home>/.xvora`.
pub fn resolve_grok_home() -> Result<PathBuf> {
    xvora_dirs::resolve_grok_home()
        .context("neither $xvora_home nor a home directory could be resolved")
}

/// Serializes tests that mutate the process-global `xvora_home` env var so they
...
/// `xvora_home` at a fresh private tmp dir, and restores the prior value on drop.
/// Use instead of hand-rolling the lock + restore guard + tmp dir per test.
///
/// `Drop` restores `xvora_home` before `_lock` releases, so the env is correct
...
pub(crate) struct GrokHomeFixture {
    ...
    /// The isolated xvora home; pass to `WorktreeDb::open` to read the same DB
...
impl GrokHomeFixture {
    pub(crate) fn new() -> Self {
        ...
        // via xvora_home, sparing the test hot loop set_journal_mode's retry
        // sleeps. This open has exclusive access (nothing reaches the path
        // until xvora_home points here); set_journal_mode's retry is the actual
        // race fix.
        let _ = WorktreeDb::open(&home);
        let prev = std::env::var_os("xvora_home");
        // SAFETY: the fixture holds the xvora_home env lock for its whole
        // lifetime, so no other test thread reads or writes the environment.
        unsafe { std::env::set_var("xvora_home", &home) };
        Self {
            _lock: lock,
            prev,
...
impl Drop for GrokHomeFixture {
    fn drop(&mut self) {
        // SAFETY: the fixture still holds the xvora_home env lock here, so no
        // other test thread reads or writes the environment during restore.
        unsafe {
            match self.prev.take() {
                Some(p) => std::env::set_var("xvora_home", p),
                None => std::env::remove_var("xvora_home"),
            }
            if self.touched_grove_env {
