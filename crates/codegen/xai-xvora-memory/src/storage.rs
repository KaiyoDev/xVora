...
//! `~/.xvora/memory/{project-slug}-{hash8}/` to avoid polluting the user's repo.

use std::path::{Path, PathBuf};

use xai_xvora_tools::util::xvora_home::xvora_home;

/// Scope for a memory write operation.
/// Write-operation scope. Distinct from `xai_xvora_agent::config::MemoryScope` (agent memory dir).
