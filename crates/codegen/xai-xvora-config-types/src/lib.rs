...
use xai_xvora_announcements::RemoteAnnouncement;
...
pub struct RemoteSettings {
    ...
    /// `app::subscription` module). Forwarded from the `xvora_settings`
    ...
    /// default (on). Sits below env `XVORA_FOLDER_TRUST`, user
    ...
    /// remote settings `xvora_settings.max_mcp_output_bytes` — global default
    ...
    /// `xvora_settings` remote settings key) to enable it. See
    ...
    /// `xai_xvora_telemetry::external::apply_remote_policy`.
    #[serde(default)]
    ...
    /// `XVORA_TWO_PASS_COMPACTION` / default (off).
    #[serde(default)]
    ...
    /// Controlled via remote settings (`non_git_warning` in `xvora_settings`).
    ...
    /// ghost text), from the `xvora_settings` remote settings flag. Sits below
    /// env (`XVORA_PROMPT_SUGGESTIONS_MODEL`) and `[models] prompt_suggestion`
    /// in config.toml, above the client hint and the built-in
    /// `xvora-0.1` default. The effective model is catalog-guarded: when
    ...
    /// Overridable locally via `XVORA_VOICE_MODE`. Free-tier SuperGrok upsell
    ...
    /// User's subscription tier from remote settings `xvora_access_gate`.
    ...
    /// Whether the user is allowed to use Xvora. Set by remote settings
    /// `xvora_access_gate` targeting rules. `None` = no server response
    ...
    /// Server-controlled via the remote settings `xvora_usage_redirect_url`
    ...
    /// `xvora_settings`. Per-model override on `ModelInfo`
    /// (`xvora_models`) takes precedence; user config and env var
    ...
    /// `0` disables. Env (`XVORA_COMPACTION_WALL_CLOCK_SECS`) overrides it.
    ...
    /// Env (`XVORA_COMPACTION_MODE`) and user config override it.
    #[serde(default)]
    pub compaction_mode: Option<String>,
    /// Segments verbatim detail (`none` | `minimal` | `balanced` | `verbose`)
    /// from remote settings. Env (`XVORA_COMPACTION_DETAIL`) and config override it.
    #[serde(default)]
    pub compaction_detail: Option<String>,
    /// remote settings verbatim-input flag; env (`XVORA_COMPACTION_VERBATIM_INPUT`) and config override it. `None` = default (true).
    #[serde(default)]
    ...
    /// workspace exposure), from `xvora_settings.workspace_command_enabled`.
...
/// only guaranteed to work with a compatible harness (cursor vs xvora).
...
pub struct GoalRoleModel {
    ...
    pub model: String,
    /// Harness `agent_type` (e.g. "cursor", "xvora-plan") whose
    /// `AgentDefinition` decides the role subagent's harness flavor (system
    /// prompt + cursor-vs-xvora toolset), applied REGARDLESS of the
...
mod tests {
    ...
    fn remote_settings_image_description_model_round_trip() {
        let json = r#"{"image_description_model": "xvora"}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(s.image_description_model.as_deref(), Some("xvora"));
    ...
    fn remote_settings_prompt_suggestion_model_round_trip() {
        let json = r#"{"prompt_suggestion_model": "xvora-0.1"}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(s.prompt_suggestion_model.as_deref(), Some("xvora-0.1"));
        let out = serde_json::to_string(&s).unwrap();
        let s2: RemoteSettings = serde_json::from_str(&out).unwrap();
