//! Memory-system configuration value types, extracted from xai-xvora-shell
...
//! `xai-xvora-shell` — `resolve()` depends on `toml` and on shell-internal
...
///
/// When enabled, watches `~/.xvora/memory/` for `.md` file changes (create,
...
///
/// On session init, directories under `~/.xvora/memory/` are scanned:
