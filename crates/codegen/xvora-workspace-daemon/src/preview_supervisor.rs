...
//!
//! After the workspace-server self-daemonizes (see [`crate::daemonize`]) it spawns the unchanged `/usr/local/bin/xvora-preview-proxy` binary.
...
pub const PREVIEW_PROXY_BIN_PATH: &str = "/usr/local/bin/xvora-preview-proxy";
...
impl PreviewArgs {
    /// Map the forwarded fields to the proxy's exact CLI flag names (see `xvora-preview-proxy/src/cli.rs`).
...
/// The proxy's control path for the last-activity stamp; mirrors `xvora-preview-proxy`'s `/__control/activity` route.
const PREVIEW_ACTIVITY_PATH: &str = "/__control/activity";

/// Effective proxy `--control-port` when the supervisor didn't set one (mirrors the default in `xvora-preview-proxy/src/cli.rs`).
...
/// It is a trait rather than the concrete tracker so this crate stays free of `xvora-workspace`.
...
mod tests {
    ...
    fn sample_cfg() -> PreviewArgs {
        PreviewArgs {
            ...
            auth_redirect: Some("https://xvora.com/preview-auth".to_owned()),
            allow_public: true,
    ...
    fn to_argv_maps_every_flag_to_the_proxy_cli_names() {
        // Flag names must match xvora-preview-proxy/src/cli.rs exactly.
        assert_eq!(
            sample_cfg().to_argv(),
            vec![
                "--preview-port",
