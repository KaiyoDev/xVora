...
//!
//! Two halves of one concern, both used only by the `xvora-workspace-server` binary and never by the `xvora-workspace` library:
//!
...
//!
//! This crate deliberately does not depend on `xvora-workspace`.
