...
/// This is a private copy of `xvora_workspace::util::is_lock_contended`, so this crate does not depend on `xvora-workspace`.
...
mod tests {
    ...
    fn process_name_matches_own_argv0() {
        ...
        // Different test runners name the binary differently (e.g. Cargo uses `xvora_workspace_daemon-<hash>`).
