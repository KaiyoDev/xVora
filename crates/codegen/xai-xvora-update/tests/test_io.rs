...
//!
//! These tests touch global process state — `XVORA_HOME` (a `OnceLock` in
//! `xai-xvora-config`), `XVORA_TEST_VERSION`, and `NPM_TOKEN` — so they
//! must run serially. Once `XVORA_HOME` is initialized for a process, it can't
//! be changed; we set it from a single shared `OnceLock` and reset the
//! contents of the directory between tests.
//!
//! The patterns here mirror the XVORA_HOME isolation used in other
//! integration tests.
...
use xai_xvora_update::write_version_cache;
...
#[tokio::test]
#[serial]
async fn write_version_cache_creates_file_at_xvora_home() {
    let _ = test_home();
    reset();
...
async fn cache_is_fresh() -> bool {
    // Mirror the implementation: look at version.json under XVORA_HOME,
...
//
// The function honors `XVORA_TEST_VERSION` for testing. We exercise it
...
async fn get_installed_version_uses_env_var_override() {
    ...
    unsafe {
        std::env::set_var("XVORA_TEST_VERSION", "9.9.9");
    }
    let v = xai_xvora_update::version::get_installed_grok_version();
    assert_eq!(v, "9.9.9");
    unsafe {
...
async fn get_installed_version_with_env_var_takes_precedence() {
    ...
    let real = {
        ...
        xai_xvora_update::version::get_installed_grok_version()
    };

    unsafe {
        std::env::set_var("XVORA_TEST_VERSION", "0.0.0-test");
    }
    let overridden = xai_xvora_update::version::get_installed_grok_version();
...
async fn get_installed_version_handles_alpha_prerelease_in_env() {
    ...
    unsafe {
        std::env::set_var("XVORA_TEST_VERSION", "0.1.200-alpha.5");
    }
    let v = xai_xvora_update::version::get_installed_grok_version();
...
async fn get_installed_version_does_not_validate_env_var_format() {
    ...
    unsafe {
        std::env::set_var("XVORA_TEST_VERSION", "not-a-version");
    }
    let v = xai_xvora_update::version::get_installed_grok_version();
