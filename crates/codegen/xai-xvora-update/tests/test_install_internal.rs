...
//!
//! Wires together a wiremock-mocked GCS bucket + an isolated `XVORA_HOME`
...
//!
//! The function reads `xvora_home()` (a process-wide `OnceLock`), so all
//! tests in this binary share a single `XVORA_HOME` and run serially via
//! `#[serial]`.
...
use xai_xvora_update::UpdateConfig;
use xai_xvora_update::auto_update::{install_internal_from_base, install_internal_from_bases};
...
#[tokio::test]
#[serial]
async fn install_internal_creates_xvora_home_subdirs_if_missing() {
    let _ = test_home();
    reset_home();
