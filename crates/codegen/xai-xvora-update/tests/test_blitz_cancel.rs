...
use xai_xvora_update::auto_update::install_internal_from_base;
...
/// `cargo nextest run -p xai-xvora-update --run-ignored all`.
#[tokio::test(flavor = "multi_thread")]
#[serial]
...
async fn blitz_fuzz_stress() {
    ...
    let iterations: usize = std::env::var("XVORA_BLITZ_ITERS")
        .ok()
