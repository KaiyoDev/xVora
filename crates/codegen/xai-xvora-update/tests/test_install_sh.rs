...
//!
//! The installer lives in the sibling `xai-xvora-pager` crate; it is resolved by
...
fn script_path(name: &str) -> Option<PathBuf> {
    dunce::canonicalize(
        Path::new(env!("CARGO_MANIFEST_DIR")).join(format!("../xai-xvora-pager/scripts/{name}")),
    )
    .ok()
    .filter(|p| p.exists())
}
...
fn run_installer(install_sh: &Path, home: &Path, fakebin: &Path, mode: &str, shell: &str) -> bool {
    ...
    let status = Command::new("/bin/bash")
        ...
        .env("PATH", path_env)
        .env("SHELL", shell)
        .env("XVORA_BIN_DIR", home.join(".xvora").join("bin"))
        .env("XVORA_CHANNEL", "stable")
        .env("FAKE_MODE", mode)
        .status()
