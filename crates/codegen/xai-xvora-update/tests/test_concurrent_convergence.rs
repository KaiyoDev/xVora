...
use xai_xvora_update::auto_update::{ensure_latest_on_disk, install_internal_from_base, run_update};
use xai_xvora_update::version::installed_on_disk_version;
...
/// Lay down a managed-install layout in the test XVORA_HOME:
/// `bin/xvora -> ../downloads/xvora-<version>-<platform>` (what
...
async fn concurrent_different_version_installs_do_not_corrupt_each_other() {
    ...
    // Pre-fix, BOTH of these wrote to downloads/xvora-0.1.tmp concurrently
