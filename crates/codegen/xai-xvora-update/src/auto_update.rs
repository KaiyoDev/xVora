...
use xai_xvora_shell::util::xvora_home::{grok_application, xvora_home};
...
pub fn print_update_status(status: &UpdateStatus, json: bool) -> anyhow::Result<()> {
    ...
    if status.update_available {
        if let Some(latest_version) = status.latest_version.as_deref() {
            println!(
                "A new version of xvora is available: {} -> {}{}",
                status.current_version, latest_version, channel_label
            );
        } else {
            println!("A new version of xvora is available.");
        }
        return Ok(());
    }
    ...
    println!("xvora - v{}{}", status.current_version, channel_label);
    Ok(())
}
...
fn env_installer() -> Option<&'static str> {
    if let Ok(v) = std::env::var("XVORA_INSTALLER") {
    ...
    if std::env::var_os("XVORA_MANAGED_BY_NPM").is_some() {
        return Some("npm");
    }
    if std::env::var_os("XVORA_MANAGED_BY_INTERNAL").is_some() {
...
async fn run_update_subcommand(run_mode: UpdateRunMode) -> Result<Option<tokio::process::Child>> {
    ...
    match run_mode {
        ...
        UpdateRunMode::NonBlocking => {
            ...
            xai_xvora_tools::util::detach_command(&mut cmd);
            let child = cmd.spawn()?;
            Ok(Some(child))
        }
    }
...
fn resolve_restart_exe() -> Result<std::path::PathBuf> {
    let canonical = xvora_application();
    if canonical.exists() {
        return Ok(canonical);
    }
...
pub fn restart_grok() -> Result<()> {
    ...
    cmd.env_clear();
    cmd.envs(std::env::vars_os().filter(|(k, _)| k != "XVORA_AUTO_UPDATE"));
...
async fn activate_verified_download(download: &VerifiedDownload) -> Result<()> {
    ...
    // Atomic swap of ~/.xvora/bin/{grok,agent} -> downloaded binary.
...
async fn regenerate_completions(binary: &std::path::Path, xvora_home: &std::path::Path) {
    // Derive $HOME independently — xvora_home may be overridden via XVORA_HOME
    // env var, so xvora_home.parent() isn't necessarily the user's home dir.
    #[allow(deprecated)]
    let user_home = std::env::home_dir().unwrap_or_default();

    let completions: &[(&str, std::path::PathBuf)] = &[
        ("bash", xvora_home.join("completions/bash/grok.bash")),
...
/// like `../downloads/xvora-0.1.203-linux-x86_64`.  When they share the same
...
/// Swap `~/.xvora/bin/{grok,agent}` to point at `binary_path`. Returns the
...
///
/// `bin_prefix` is the binary name prefix, e.g. `"grok"` or `"xvora-pager"`.
/// Files must match `{bin_prefix}-{digit}*` to be considered versioned binaries
/// (this avoids `grok-*` matching `xvora-pager-*` or `grok-latest`).
///
...
async fn cleanup_old_downloads(dir: &std::path::Path, bin_prefix: &str, current_version: &str) {
    ...
    while let Ok(Some(entry)) = entries.next_entry().await {
        ...
        // binary (avoids `grok-latest`, `xvora-pager-*` when prefix is `grok`).
...
async fn gh_release_download(tag: &str, pattern: &str, dest: &std::path::Path) -> Result<()> {
    ...
    .stderr(Stdio::piped());
    xai_xvora_tools::util::detach_command(&mut cmd);
    cmd.envs(xai_xvora_tools::util::pager_env());
...
async fn install_gh_release(target: Option<&str>) -> Result<()> {
    ...
    // ~/.xvora/downloads/ (legacy layout — skips the grok-latest indirection).
    ...
    for name in ["grok", "agent"] {
        ...
        if let Ok(existing_target) = tokio::fs::read_link(&system_link).await {
            ...
            if target_str.contains(".xvora/downloads/") && !target_str.ends_with("grok-latest") {
...
fn warn_if_other_grok_processes_running() {
    ...
    cmd.args(["-f", "grok"])
        ...
        .stdout(Stdio::piped())
        .stderr(Stdio::null());
    xai_xvora_tools::util::detach_std_command(&mut cmd);
...
pub async fn run_update(
    ...
    if !force && std::env::var_os("XVORA_AUTO_UPDATE").is_none() {
        eprintln!("  Please restart Grok.");
    }
...
async fn refresh_deployment_config() {
    if !xai_xvora_shell::managed_config::has_principal() {
        return;
    }
    if !xai_xvora_shell::managed_config::is_fetch_enabled() {
        return;
    }
    // Clear a logged-out team's files before deciding to fetch (mirrors the loop).
    xai_xvora_shell::managed_config::clear_orphan();
    if !xai_xvora_shell::config::is_managed_config_stale_for(
        &xai_xvora_shell::managed_config::current_serving_identity(),
    ) {
        return;
    }
    match xai_xvora_shell::managed_config::sync().await {
...
mod tests {
    ...
    fn test_relative_symlink_target_same_dir() {
        // downloads/xvora-latest -> grok-0.1.203 (same directory)
        let target = std::path::Path::new("/home/alice/.xvora/downloads/xvora-0.1.203");
    ...
    async fn test_cleanup_old_downloads_does_not_touch_other_binaries() {
        ...
        // grok and xvora-pager should not interfere with each other.
    ...
    fn test_reinstall_hint_npm_mentions_npm_command() {
        ...
        assert!(
            hint.contains("@xai-official/xvora"),
    ...
    async fn test_cleanup_old_downloads_files_with_non_digit_suffix_skipped() {
        ...
        // ignored (e.g. grok-latest, xvora-pager-* when prefix is grok).
        ...
        // grok-latest and xvora-pager-* must be untouched.
    ...
    //   1. XVORA_INSTALLER (npm | internal | gh-release | gh)
    //   2. XVORA_MANAGED_BY_NPM       → npm
    //   3. XVORA_MANAGED_BY_INTERNAL  → internal
    ...
    impl InstallerEnvGuard {
        fn isolate() -> Self {
            const VARS: &[&str] = &[
                "XVORA_INSTALLER",
                "XVORA_MANAGED_BY_NPM",
                "XVORA_MANAGED_BY_INTERNAL",
    ...
    fn test_env_installer_explicit_npm() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("XVORA_INSTALLER", "npm") };
    ...
    fn test_env_installer_explicit_internal() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("XVORA_INSTALLER", "internal") };
    ...
    fn test_env_installer_explicit_gh_release() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("XVORA_INSTALLER", "gh-release") };
    ...
    fn test_env_installer_explicit_gh_alias() {
        ...
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("XVORA_INSTALLER", "gh") };
        assert_eq!(env_installer(), Some("gh-release"));
    }
    ...
    fn test_env_installer_explicit_unknown_value_returns_none() {
        ...
        // to the other env vars or to config. So `XVORA_INSTALLER=brew`
        // disables the env-installer detection entirely.
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("XVORA_INSTALLER", "brew") };
        // Even if MANAGED_BY_NPM is also set, the explicit var wins (and rejects).
        unsafe { std::env::set_var("XVORA_MANAGED_BY_NPM", "1") };
        assert_eq!(
            env_installer(),
            None,
            "explicit XVORA_INSTALLER=brew must early-return None, not fall through"
        );
    }
    ...
    fn test_env_installer_explicit_empty_returns_none() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("XVORA_INSTALLER", "") };
        assert_eq!(env_installer(), None);
    }
    ...
    fn test_env_installer_managed_by_npm_any_value() {
        ...
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("XVORA_MANAGED_BY_NPM", "") };
        assert_eq!(env_installer(), Some("npm"));
    }
    ...
    fn test_env_installer_managed_by_internal() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("XVORA_MANAGED_BY_INTERNAL", "1") };
        assert_eq!(env_installer(), Some("internal"));
    }
    ...
    fn test_env_installer_managed_by_npm_wins_over_npm_config_user_agent() {
        ...
        let _g = InstallerEnvGuard::isolate();
        unsafe {
            std::env::set_var("XVORA_MANAGED_BY_NPM", "1");
    ...
    fn test_env_installer_explicit_internal_wins_over_npm_managed() {
        // XVORA_INSTALLER=internal must override an inherited MANAGED_BY_NPM.
        let _g = InstallerEnvGuard::isolate();
        unsafe {
            std::env::set_var("XVORA_INSTALLER", "internal");
            std::env::set_var("XVORA_MANAGED_BY_NPM", "1");
        }
        assert_eq!(env_installer(), Some("internal"));
    }
