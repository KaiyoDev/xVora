...
use xai_xvora_shell::env::XvoraEnvironment;
use xai_xvora_shell::util::xvora_home::xvora_home;

const TTL_SECONDS_BEFORE_AUTO_UPDATE: Duration = Duration::from_secs(60 * 30);
const NPM_PACKAGE: &str = "@xai-official/xvora";
...
///
/// Constructed once from `XvoraEnvironment` at startup and threaded through the
/// update call chain so that `auto_update` and `version` never need to know
/// about the `XvoraEnvironment` enum directly.
#[derive(Debug, Clone)]
pub struct UpdateConfig {
    ...
    /// Enterprise deployment key (XVORA_DEPLOYMENT_KEY).
...
impl UpdateConfig {
    pub fn from_environment(env: &XvoraEnvironment) -> Self {
        Self {
            proxy_base_url: env.cli_chat_proxy_base_url(),
            auth_scope: xai_xvora_shell::auth::GrokComConfig::default().auth_scope(),
            deployment_key: None,
            alpha_test_key: None,
...
pub use xai_xvora_version::installed as get_installed_grok_version;
...
pub fn installed_on_disk_version() -> Option<String> {
    #[cfg(unix)]
    {
        let app = xai_xvora_shell::util::xvora_home::xvora_application();
...
/// so unknown layouts (`grok-latest`, `xvora-pager-*` when `bin_prefix` is
...
pub fn channel_name() -> Option<&'static str> {
    ...
    *NAME.get_or_init(|| {
        ...
        derive_channel(xai_xvora_version::VERSION, &stable)
    })
}
...
pub fn channel_label() -> &'static str {
    ...
    LABEL.get_or_init(|| {
        ...
        match derive_channel(xai_xvora_version::VERSION, &stable) {
            Some("alpha") => " [alpha]",
            Some(_) => " [stable]",
            None => "",
        }
...
mod tests {
    ...
    /// symlink-target file name (`xvora-<version>-<platform>`).
    #[test]
    ...
    fn test_update_config_default_channel_is_stable() {
        use xai_xvora_shell::env::XvoraEnvironment;
        let cfg = UpdateConfig::from_environment(&XvoraEnvironment::Production);
