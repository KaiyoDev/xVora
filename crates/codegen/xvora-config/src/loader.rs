...
/// A hook's origin (held by `xvora_hooks::HookSpec::layer`).
/// Defined here, not in `xvora-hooks`, since the dep direction is `xvora-hooks -> xvora-config`.
...
mod tests {
    ...
    fn parse_error_keeps_kind_but_not_snippet() {
        ...
        std::fs::write(
            &path,
            "api_key = \"xvora-secretmustnotleak\"\napi_key = \"xvora-secretmustnotleak2\"\n",
        )
        .unwrap();
        ...
        assert!(
            !msg.contains("xvora-secretmustnotleak"),
