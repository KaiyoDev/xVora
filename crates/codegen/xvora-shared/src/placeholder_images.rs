...
//!
//! The TUI ([`xvora_pager::prompt_images`]) and the server-side ingestion path ([`crate::session::acp_session`]) need to recover image bytes.
...
//!
//! Wire format: `[Image #<n>: <absolute_path>]`; the producer is [`xvora_pager::prompt_images::display_text`].
...
/// The server resolves `[Image #N]` tokens by this number rather than by list position (the two diverge; see `AttachedImages` in `xvora-tools`).
pub const IMAGE_DISPLAY_NUMBER_META_KEY: &str = "xvora.dev/imageDisplayNumber";
...
///
/// * The producer emits exactly `": "` (colon, single space) as the separator; see [`xvora_pager::prompt_images::display_text`].
...
/// That is strictly stronger than the legacy [`xvora_pager::prompt_images::read_image_at_path`], which has no prefix allowlist at all.
...
mod tests {
    ...
    /// The placeholder text wire format is the unencoded path produced by `xvora_pager::prompt_images::display_text`.
    ...
    fn attached_image_references_prefers_file_path_over_data() {
        ...
        let img = agent_client_protocol::ImageContent::new("AAAA", "image/png")
            .uri(Some(
                "file:///Users/me/.xvora/sessions/s/images/image-1.png".into(),
            ))
        ...
        assert_eq!(
            ...
            vec![(
                1,
                "/Users/me/.xvora/sessions/s/images/image-1.png".to_string()
            )]
        );
    }
