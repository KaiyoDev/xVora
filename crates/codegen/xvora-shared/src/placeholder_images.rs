...
//! image bytes.
//!
//! Both the TUI ([`xvora_pager::prompt_images`]) and the server-side
...
//! [`xvora_pager::prompt_images::display_text`]. The shape of this
...
/// position (the two diverge — see `AttachedImages` in `xvora-tools`).
...
/// [`AttachedImages`](xvora_tools::types::resources::AttachedImages))
...
///   separator — see [`xvora_pager::prompt_images::display_text`].
...
/// [`xvora_pager::prompt_images::read_image_at_path`], which has no
...
mod tests {
    ...
    /// `xvora_pager::prompt_images::display_text`. A
