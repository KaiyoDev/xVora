...
///
/// When [`xvora_tty_utils::redirect_native_stderr`] has been called, this writes to the dup'd fd that points at the real terminal.
...
pub fn with_locked_stderr<T>(f: impl FnOnce(&mut std::fs::File) -> T) -> T {
    ...
    let mut file = xvora_tty_utils::dup_tui_stderr().unwrap_or_else(|_| {
