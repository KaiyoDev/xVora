...
//! `xvora-shell-manual_auth` with the `reason`/`trigger`/`token_kind`/`principal`
...
use xai_xvora_telemetry::client;
use xai_xvora_telemetry::config::{TelemetryConfig, TelemetryMode};
use xai_xvora_telemetry::events::{AuthTokenKind, ManualAuth, ManualAuthReason, ManualAuthSurface};
...
async fn manual_auth_posts_to_events_endpoint_as_grok_shell_manual_auth() {
    ...
    xai_xvora_telemetry::log_event(ManualAuth {
    ...
    let event = loop {
        let found = bodies.lock().unwrap().iter().find_map(|b| {
            ...
            (e.get("event_name")?.as_str()? == "xvora-shell-manual_auth").then(|| e.clone())
        });
        if let Some(e) = found {
            break e;
        }
        assert!(
            ...
            "no xvora-shell-manual_auth POST received"
        );
