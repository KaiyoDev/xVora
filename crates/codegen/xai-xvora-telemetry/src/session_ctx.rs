...
//!
//! Extracted from `xai-xvora-shell::agent::telemetry`.
...
pub enum EmitterOrigin {
    /// `xai-xvora-shell` (and the pager/TUI that emit through it).
    Shell,
    /// `xai-xvora-workspace` (remote sampler / workspace server).
    Workspace,
}
...
/// event-name prefix (`xvora-shell-*` vs `grok-workspace-*`).
///
...
mod tests {
    ...
    /// `format!("xvora-shell-{suffix}")` event name byte-for-byte, since every
