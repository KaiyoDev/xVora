...
//!
//! Enterprise customers point the Xvora CLI at *their own* OpenTelemetry
//! collector (standard `OTEL_*` env vars + the `XVORA_EXTERNAL_OTEL` master
...
//!   headers.
//! - Default **off**: with `XVORA_EXTERNAL_OTEL` unset (or no exporter
...
impl IdentityAttrs {
    pub fn from_snapshot(snapshot: &xai_xvora_auth::CredentialSnapshot) -> Self {
        Self {
    ...
    if cfg.internal_pipeline_consumed_otel_vars {
        tracing::warn!(
            "external otel: refusing to activate — the internal trace pipeline consumed \
             ...
             XVORA_INTERNAL_OTLP_* to use the external stream."
        );
        return None;
    }
