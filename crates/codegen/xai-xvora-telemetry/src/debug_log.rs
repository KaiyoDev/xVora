...
//! - PerSession (`XVORA_DEBUG_LOG=1`): a routing layer fans each session's
...
//! - SingleFile (explicit path via `XVORA_LOG_FILE` or `XVORA_DEBUG_LOG=<path>`):
...
use xai_xvora_config::xvora_home;
...
impl DebugSource {
    fn label(self) -> &'static str {
        match self {
            Self::GrokLogFile => "XVORA_LOG_FILE",
            Self::GrokDebugLog => "XVORA_DEBUG_LOG",
        }
    }
}
...
///
/// Lives here (not in `xai-xvora-pager`) so the firehose directives below and
...
/// safe. See `xai-xvora-pager/src/tracing.rs` for the consumer side.
...
// Broad firehose filter for the routing/XVORA_DEBUG_LOG sources: capture our
...
// until added here.
const FIREHOSE_BASE_DIRECTIVES: &str = "info,xai_xvora_pager=debug,xai_xvora_shell=debug,xai_xvora_tools=debug,xai_xvora_telemetry=debug,xai_xvora_agent=debug,xai_xvora_mcp=debug,xai_acp_lib=debug,sampling_log=off";
...
// XVORA_DEBUG_LOG single-file source (mirrors `default_file_filter`).
fn firehose_filter() -> EnvFilter {
    EnvFilter::new(firehose_directives())
}

// RUST_LOG-respecting filter for the XVORA_LOG_FILE source: DEBUG default, honor
// RUST_LOG, silence sampling_log (preserves XVORA_LOG_FILE back-compat).
...
/// filter by source (XVORA_LOG_FILE respects RUST_LOG). Open failures warn AFTER
...
pub(crate) enum DebugTarget {
    /// `XVORA_DEBUG_LOG=1` → route per session into `<dir>` (`~/.xvora/debug`).
    PerSession { dir: PathBuf },
    /// An explicit path → one flat `fmt` file, routing bypassed.
    SingleFile { path: PathBuf, src: DebugSource },
}

/// Resolve the debug target, honoring precedence: explicit XVORA_LOG_FILE wins
/// (single file, RUST_LOG filter); else XVORA_DEBUG_LOG — a truthy bool routes
...
pub(crate) fn resolve_debug_target() -> Option<DebugTarget> {
    let grok_log_file = std::env::var_os("XVORA_LOG_FILE");
    let grok_debug_log = std::env::var_os("XVORA_DEBUG_LOG");
...
mod tests {
        ...
        for v in ["0", "false", "off", "no", "", "  "] {
            assert!(
                resolve_debug_target_inner(None, Some(OsStr::new(v)), Path::new("/debug"))
                    .is_none(),
                "expected None for XVORA_DEBUG_LOG={v:?}"
            );
        }
    }
    ...
    fn resolve_target_debug_log_enabled_is_per_session_dir() {
        for v in ["1", "true", "on", "yes"] {
            ...
            assert_eq!(
                ...
                "expected PerSession for XVORA_DEBUG_LOG={v:?}"
            );
        }
    }
    ...
    fn resolve_target_empty_log_file_falls_through_to_debug_log() {
        // Empty / whitespace XVORA_LOG_FILE is treated as unset (mirrors XVORA_DEBUG_LOG).
        for blank in ["", "   "] {
            let target = resolve_debug_target_inner(
    ...
    fn resolve_target_non_utf8_debug_log_path_is_single_file() {
        ...
        // A non-UTF-8 XVORA_DEBUG_LOG value is a path, not a bool keyword, and its
    ...
    fn layer_routes_event_by_session_span() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            ...
            tracing::info_span!("session", session_id = %"sess-xyz").in_scope(|| {
                tracing::info!(target: "xai_xvora_shell", "inside session");
            });
            tracing::info!(target: "xai_xvora_shell", "outside session");
        });
        crate::appender::flush_file_log_guards();
    ...
    fn layer_routes_under_real_firehose_filter() {
        ...
        // `xai_xvora_telemetry::session_ctx`) survives the real filter so
        ...
        tracing::subscriber::with_default(subscriber, || {
            tracing::info_span!(
                target: "xai_xvora_telemetry::session_ctx",
                "session",
                session_id = %"sid-real"
            )
            .in_scope(|| {
                tracing::debug!(target: "xai_xvora_shell", "filtered routing works");
            });
        });
        crate::appender::flush_file_log_guards();
    ...
    fn layer_routes_two_sessions_to_distinct_files() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            tracing::info_span!("session", session_id = %"sid-one").in_scope(|| {
                ...
                tracing::info!(target: "xai_xvora_shell", "one first");
                tracing::info!(target: "xai_xvora_shell", "one second");
            });
            tracing::info_span!("session", session_id = %"sid-two").in_scope(|| {
                tracing::info!(target: "xai_xvora_shell", "two only");
            });
        });
        crate::appender::flush_file_log_guards();
