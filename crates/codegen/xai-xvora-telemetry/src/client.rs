...
//!
//! Extracted from `xai-xvora-shell::agent::telemetry::track`. The HTTP client is
...
/// whichever [`EmitterOrigin`] prefix it carries (`xvora-shell-` /
...
pub async fn track(event_name: &str, request_id: &str, ctx: &UserContext, mut metadata: Metadata) {
    ...
    if let Some(ref mixpanel) = client.mixpanel {
        ...
        props.insert("app_name".into(), json!("Xvora"));
...
mod tests {
    ...
    /// identical to the previous `strip_prefix("xvora-shell-")` behavior.
    #[test]
    ...
    /// (e.g. a future `"xvora-shell-ext-"` origin), an earlier `ALL` entry could
