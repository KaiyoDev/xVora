...
//!
//! Extracted from `xai-xvora-shell::agent::unique_identifier` so the
//! telemetry engine can stamp events without depending on shell internals.
//! `$XVORA_HOME` is resolved through `xai-xvora-config::xvora_home`.
...
/// This function caches the result in `$XVORA_HOME/agent_id` so subsequent calls
...
fn load_or_compute_agent_id() -> String {
    let cache_path = xai_xvora_config::xvora_home().join("agent_id");
...
///
/// Enabled only when `XVORA_TELEMETRY_SPECIAL_USER=1` (or `true`). There is no
/// hardcoded username allowlist.
pub fn is_special_user() -> bool {
    matches!(
        std::env::var("XVORA_TELEMETRY_SPECIAL_USER").as_deref(),
