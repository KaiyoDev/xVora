...
//! targets `xai_xvora_hooks` or `xai_xvora_agent::plugins` at key lifecycle
...
/// Filters to hooks (`xai_xvora_hooks`) and plugins (`xai_xvora_agent::plugins`) targets.
    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let filter = tracing_subscriber::filter::EnvFilter::new(
        "xai_xvora_hooks=debug,xai_xvora_agent::plugins=debug",
    );
