...
//!
//! `xai-xvora-pager` uses this module to set up OTLP
...
const ENV_OTEL_FILTER: &str = "XVORA_OTEL_FILTER";
...
///
/// Built by the binaries (`xai-xvora-pager`) from their own
/// configuration. The credentials provider is constructed by shell's
/// `xai_xvora_shell::auth::credential_provider::build_otel_credential_provider`.
pub struct OtelLayerConfig {
    ...
    /// Value for the `X-XAI-Token-Auth` header (typically `"xai-xvora-cli"`).
...
pub struct OtelClientInfo {
    /// Binary name (`xvora-pager`) -> `client.name`.
...
///
/// - `client_name`: binary name (e.g. `"grok-tui"`, `"xvora-pager"`) -- stored as
    ...
    S: tracing::Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let otel_filter = tracing_subscriber::filter::EnvFilter::try_new(&otel_filter)
        .unwrap_or_else(|e| {
            eprintln!(
                "[otel] Invalid XVORA_OTEL_FILTER '{}': {}. Using default '{}'.",
...
fn build_export_headers(
    ...
    snapshot: &xai_xvora_auth::CredentialSnapshot,
...
mod tests {
    ...
    use std::sync::Mutex;
    use xai_xvora_auth::{AuthCredentialProvider, CredentialSnapshot, HttpAuth};
    ...
    fn make_exporter(
        ...
        RefreshableSpanExporter {
            ...
            token_header_value: Arc::from("xai-xvora-cli"),
