...
//!
//! These structs were extracted from `xai-xvora-shell` so they can be
...
// ---------------------------------------------------------------------------
// Pager events (called from xai-xvora-pager via log_event)
// ---------------------------------------------------------------------------
...
/// first prompt when Xvora is launched from a non-project directory).
...
/// attributed to Xvora: the user saw the gate in Xvora, went and
...
pub struct SubscriptionActivated {
    ...
    /// is strongly attributable to Xvora's upsell surface.
...
/// `xvora-shell-manual_auth` (the `manual_auth` binding gets the `xvora-shell-`
