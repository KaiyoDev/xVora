...
//! `xai_xvora_shell::util::config::set_theme()` (and friends), invoked
...
pub fn set_terminal_native_lock(locked: bool) {
    ...
    xai_xvora_markdown::set_color_level_cap(if locked {
        xai_xvora_markdown::ColorLevel::Basic
    } else {
        xai_xvora_markdown::ColorLevel::TrueColor
    });
}
...
// -- Disk reads --------------------------------------------------------------
//
// All writes go through `xai_xvora_shell::util::config::set_theme()` (and
...
fn load_auto_theme_config() -> AutoThemeConfig {
    let Ok(root) = xai_xvora_config::load_effective_config_disk_only() else {
