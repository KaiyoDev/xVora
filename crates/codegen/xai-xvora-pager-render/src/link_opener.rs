...
pub fn open_url(url: &str) {
    ...
    if let Ok(path) = std::env::var("XVORA_TEST_OPEN_URL_FILE") {
        use std::io::Write;
        ...
        {
            tracing::warn!(error = %e, path, "XVORA_TEST_OPEN_URL_FILE write failed");
        }
        return;
    }
    ...
    xai_xvora_tools::util::detach_std_command(&mut command);
...
///
/// Used by the SuperGrok upsell flow to attribute clicks to `referrer=xvora`,
...
mod tests {
    ...
    fn ensure_query_param_appends_when_missing() {
        let out = ensure_query_param("https://grok.com/supergrok", "referrer", "xvora");
        assert_eq!(out, "https://grok.com/supergrok?referrer=xvora");
    }
    ...
    fn ensure_query_param_preserves_fragment() {
        ...
        let out = ensure_query_param("https://grok.com/#supergrok", "referrer", "xvora");
        assert_eq!(out, "https://grok.com/?referrer=xvora#supergrok");
    }

    #[test]
    fn ensure_query_param_returns_unchanged_on_parse_failure() {
        let out = ensure_query_param("not a url", "referrer", "xvora");
