...
//!
//! Disk writes live in `xai_xvora_shell::util::config::set_<field>()`.
...
use xai_xvora_shared::ui_config::UiConfig;
...
/// Default ON when unset. The `XVORA_PROMPT_SUGGESTIONS` env var overrides
...
/// `XVORA_SCROLL_MODE` overrides `[ui].scroll_mode` (the `XVORA_SCROLL_SPEED`
...
pub fn load_scroll_mode() -> ScrollMode {
    SCROLL_MODE_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("XVORA_SCROLL_MODE")
                .ok()
...
/// `XVORA_INVERT_SCROLL` (`1`/`true`/`0`/`false`) overrides `[ui].invert_scroll`.
pub fn load_invert_scroll() -> bool {
    INVERT_SCROLL_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("XVORA_INVERT_SCROLL")
                .ok()
...
/// its own lines-per-tick. `XVORA_SCROLL_LINES` overrides `[ui].scroll_lines`.
pub fn load_scroll_lines() -> Option<u8> {
    SCROLL_LINES_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("XVORA_SCROLL_LINES")
                .ok()
...
fn load_bool_from_effective_config(key: &str, default: bool) -> bool {
    let root = match xai_xvora_config::load_effective_config_disk_only() {
        Ok(r) => r,
...
fn load_str_from_effective_config(key: &str) -> Option<String> {
    let root = xai_xvora_config::load_effective_config_disk_only().ok()?;
    root.get("ui")
        .and_then(|ui| ui.get(key))
        .and_then(|v| v.as_str())
        .map(str::to_string)
