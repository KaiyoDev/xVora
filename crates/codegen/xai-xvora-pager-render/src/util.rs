...
pub use xai_xvora_config::xvora_home;
...
/// User-facing label for the user grok directory (``~/.xvora`` or ``$XVORA_HOME``).
///
/// Derived from resolved [`xvora_home()`] vs `xai_xvora_config::default_xvora_home()`,
/// not from whether `XVORA_HOME` is set in the environment.
pub fn display_xvora_home_prefix() -> String {
    if xvora_home() == xai_xvora_config::default_xvora_home() {
        "~/.xvora".to_string()
    } else {
...
pub fn is_under_user_xvora_home(path: &Path) -> bool {
...
mod tests {
    ...
    #[test]
    fn display_xvora_home_prefix_default_install() {
        if std::env::var("XVORA_HOME").is_ok() {
            return;
        }
        assert_eq!(display_xvora_home_prefix(), "~/.xvora");
    }
