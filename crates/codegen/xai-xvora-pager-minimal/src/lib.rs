...
//! # Wiring
//!
//! `xai-xvora-pager` (the lib) does **not** depend on this crate — that would be
...
//! seam ([`xai_xvora_pager::minimal_hook`]) of function pointers, and the
//! composition-root binary (`xai-xvora-pager-bin`) calls [`install`] once at
...
/// Register the minimal-mode render hooks with `xai-xvora-pager`.
///
...
/// subsequent calls are ignored (see [`xai_xvora_pager::minimal_hook`]).
pub fn install() {
    xai_xvora_pager::minimal_hook::install(xai_xvora_pager::minimal_hook::MinimalHooks { draw });
}
