...
use crate::paths::{system_config_dir, user_xvora_home};
...
pub fn load_from_disk() -> std::io::Result<toml::Value> {
    load_user_config_layer(user_xvora_home().as_deref(), "config.toml")
}
...
pub fn load_managed_config() -> std::io::Result<toml::Value> {
    load_user_config_layer(user_xvora_home().as_deref(), MANAGED_CONFIG_FILENAME)
}
...
/// `.xvora/<filename>` (the cwd-fallback would silently promote an untrusted
/// project `.xvora` to the user tier).
...
pub fn managed_config_layers() -> Vec<ManagedConfigLayer> {
    managed_config_layers_at(system_config_dir().as_deref(), user_xvora_home().as_deref())
}
...
impl ConfigLayers {
    ...
    /// wraps this with the `XVORA_CAMPAIGNS_OVERRIDE` env layer.
...
/// `XVORA_CAMPAIGNS=0` or `[features] campaigns = false` on pre-campaign base.
pub fn campaigns_application_disabled(base_effective: &toml::Value) -> bool {
    if crate::env_bool("XVORA_CAMPAIGNS") == Some(false) {
        return true;
    }
    base_effective
        .get("features")
...
/// (`xai_xvora_shell::util::config::load_effective_config`) when remote campaigns
/// or `XVORA_CAMPAIGNS_OVERRIDE` must be honored. The name mirrors the
...
pub fn load_dismissed_ids_from_home() -> std::collections::HashSet<String> {
    let Some(home) = crate::user_xvora_home() else {
...
/// can't be parsed (broken `XVORA_TEST_VERSION` in dev), silently strips
...
pub fn apply_version_overrides_with_registered(value: &mut toml::Value) -> std::io::Result<()> {
    match xai_xvora_version::installed_semver() {
...
mod tests {
    ...
    /// `XVORA_CAMPAIGNS=0` disables campaign application regardless of config.
    /// `XVORA_CAMPAIGNS` is process-global, so this test serializes itself with a
    ...
    fn kill_switch_env_var_disables() {
        ...
        let prior = std::env::var_os("XVORA_CAMPAIGNS");
        let empty = toml::Value::Table(Default::default());

        // SAFETY: ENV_GUARD serializes this against itself; no other test in the
        // crate mutates or reads XVORA_CAMPAIGNS concurrently.
        unsafe { std::env::set_var("XVORA_CAMPAIGNS", "0") };
        assert!(campaigns_application_disabled(&empty));

        unsafe { std::env::remove_var("XVORA_CAMPAIGNS") };
        assert!(!campaigns_application_disabled(&empty));

        match prior {
            Some(v) => unsafe { std::env::set_var("XVORA_CAMPAIGNS", v) },
            None => unsafe { std::env::remove_var("XVORA_CAMPAIGNS") },
        }
    }
}
