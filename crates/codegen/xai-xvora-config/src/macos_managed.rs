...
//! preference domain `ai.x.xvora` (`requirements_toml_base64`). Only admin-*forced*
...
fn read_forced_requirements() -> Option<String> {
    ...
    // per-user domain, which a local user can set (`defaults write ai.x.xvora`)
...
mod tests {
    ...
    fn forced_payload_is_not_env_expanded() {
        ...
        let prior = std::env::var("XVORA_MDM_NO_EXPAND_TEST").ok();
        unsafe { std::env::set_var("XVORA_MDM_NO_EXPAND_TEST", "attacker") };
        let decoded = decode_managed_toml(&b64("base_url = \"${XVORA_MDM_NO_EXPAND_TEST}/v1\"\n"));
        unsafe {
            match prior {
                Some(p) => std::env::set_var("XVORA_MDM_NO_EXPAND_TEST", p),
                None => std::env::remove_var("XVORA_MDM_NO_EXPAND_TEST"),
            }
        }
        assert_eq!(
            decoded
                ...
                .and_then(|v| v.as_str()),
            Some("${XVORA_MDM_NO_EXPAND_TEST}/v1"),
