//! Filesystem locations for Xvora config files and binaries.
...
/// The default user Xvora directory (`~/.xvora`, canonicalized) used when
/// `XVORA_HOME` is unset. Exposed so callers (e.g. display helpers) can detect
/// whether [`xvora_home()`] is the default without duplicating the computation.
///
...
/// `~/.xvora/marketplace-cache`. `dunce` strips the prefix whenever the path
...
/// `xai_fast_worktree::db::resolve_xvora_home` (deliberately standalone crate).
pub fn default_xvora_home() -> PathBuf {
    #[allow(deprecated)]
    let home = std::env::home_dir().unwrap_or_else(|| PathBuf::from("."));
    dunce::canonicalize(&home).unwrap_or(home).join(".xvora")
}

/// Per-user config directory: `$XVORA_HOME` or `~/.xvora`. Created if needed.
pub fn xvora_home() -> PathBuf {
    XVORA_HOME
        .get_or_init(|| {
            let xvora_home = if let Ok(v) = std::env::var("XVORA_HOME") {
                PathBuf::from(v)
            } else {
                default_xvora_home()
            };
...
/// `$XVORA_HOME` is set or a home directory is found, `None` otherwise. Unlike
/// [`xvora_home()`], this never falls back to a cwd-relative `.xvora`, so callers
/// that *scan* user-global grok resources (hooks, marketplace sources, ...) don't
/// mistake a project's `.xvora` tree for the user-global one when no home resolves.
pub fn user_xvora_home() -> Option<PathBuf> {
    #[allow(deprecated)]
    let resolvable = std::env::var_os("XVORA_HOME").is_some() || std::env::home_dir().is_some();
    resolvable.then(xvora_home)
}

/// Canonical Xvora application path: `$XVORA_HOME/bin/xvora` (Unix) or `xvora.exe` (Windows).
pub fn xvora_application() -> PathBuf {
    let name = if cfg!(windows) { "xvora.exe" } else { "xvora" };
    xvora_home().join("bin").join(name)
}
...
/// `xvora_home()/sessions/{encode_cwd_dirname(cwd)}`.
///
...
pub fn ensure_sessions_cwd_dir(cwd: &str) -> std::io::Result<PathBuf> {
    ...
    let dir = xvora_home().join("sessions").join(&encoded_name);
...
mod tests {
    ...
    #[test]
    fn default_xvora_home_has_no_verbatim_prefix() {
