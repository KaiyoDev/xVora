...
/// [`fail_closed_flag_from_str`]. Env tightening (file vs `XVORA_MANAGED_CONFIG_FAIL_CLOSED`)
...
/// with `XVORA_MANAGED_CONFIG_URL`; only applies to `requirements.toml`.
pub(crate) const FAIL_CLOSED_ENV: &str = "XVORA_MANAGED_CONFIG_FAIL_CLOSED";
...
pub struct RequirementsLayer {
    ...
    /// not re-derive from the source (`XVORA_HOME`-influenced, could carry `..`).
    pub is_system: bool,
}
...
pub fn requirements_layers() -> Vec<RequirementsLayer> {
    let mut out = Vec::new();
    if let Some(user_path) = user_xvora_home().map(|g| g.join("requirements.toml"))
...
pub(crate) fn load_requirements() -> Option<toml::Value> {
    load_user_requirements(user_xvora_home().as_deref())
}

/// User requirements layer from `<home>/requirements.toml`, or `None` with no
/// resolvable user home (rather than reading a cwd-relative `.xvora`).
...
fn validate_requirements_value(
    ...
    let Ok(version) = xai_xvora_version::installed_semver() else {
        return Ok(());
    };
...
pub fn validate_requirements() -> Result<(), RequirementsError> {
    validate_user_requirements(user_xvora_home().as_deref())?;
    if let Some(dir) = system_config_dir() {
...
/// no-op (no cwd-relative `.xvora/requirements.toml` is read or enforced).
