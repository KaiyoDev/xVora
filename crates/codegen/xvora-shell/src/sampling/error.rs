...
//!
//! The canonical error types now live in `xvora_sampling_types::error`.
...
/// This stays in xvora-shell because it depends on `agent_client_protocol::Error`.
...
pub fn terminal_error_data(
    ...
    if kind != xvora_sampler::SamplingErrorKind::MaxTokensTruncation {
...
pub fn stop_reason_for_turn_error(err: &acp::Error) -> &'static str {
    let is_max_tokens = err
        ...
        .and_then(|v| v.as_str())
        .is_some_and(|k| k == xvora_sampler::SamplingErrorKind::MaxTokensTruncation.as_str());
