...
pub(crate) fn local_sandbox_telemetry() -> Option<LocalSandboxTelemetry> {
    let profile = xvora_sandbox::configured_profile_name()?;
    Some(LocalSandboxTelemetry {
        profile: profile.to_owned(),
        applied: xvora_sandbox::is_active(),
    })
}
...
pub(crate) fn build_chat_history_session_state(
    _messages: &[xvora_sampling_types::conversation::ConversationItem],
...
pub(crate) fn spawn_startup_spill_reconcile(
    ...
    tokio::spawn(async move {
        match queue {
            Some(queue) => {
                let report =
                    xvora_workspace::recovery::run_startup_recovery(&xvora_home, &queue).await;
            ...
            None => {
                let purged = tokio::task::spawn_blocking(move || {
                    xvora_workspace::recovery::purge_spilled_items(&xvora_home)
                })
                .await;
                match purged {
                    Ok(removed) => {
