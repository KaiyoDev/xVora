//! Vendor-compat resolution for `xvora inspect`.
//!
...
/// Returns `Some("cursor")` or `Some("claude")` when the path passes through a vendor config directory; `None` for native `.xvora`/`.agents` paths.
