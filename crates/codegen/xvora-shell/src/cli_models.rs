...
/// Status for the `xvora models` banner (display order ≠ sampling priority; see [`AuthStatus::resolve`]).
...
mod tests {
    ...
    ///
    /// Uses `XVORA_AUTH_PATH` (not `XVORA_HOME`) so a OnceLock-cached real home
    ...
    fn isolate_auth_sources() -> (tempfile::TempDir, [EnvGuard; 7]) {
        ...
        let guards = [
            ...
            EnvGuard::unset("XVORA_AUTH"),
            EnvGuard::set("XVORA_AUTH_PATH", auth_path.to_str().unwrap()),
            EnvGuard::unset("XVORA_DEPLOYMENT_KEY"),
            EnvGuard::unset("XVORA_WS_ORIGIN"),
            EnvGuard::unset("XVORA_DISABLE_API_KEY_AUTH"),
        ];
        (dir, guards)
    }
