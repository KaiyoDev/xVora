...
const CLIENT_VERIFICATION_GATE_TIMEOUT: Duration =
    Duration::from_secs(xvora_hooks::config::DEFAULT_VERIFICATION_GATE_TIMEOUT_SECS);

fn default_client_gate_timeout(gate: xvora_hooks::event::GateKind) -> Duration {
...
impl SessionActor {
    ...
    > {
        ...
        groups
            .iter()
            .filter(move |group| {
                xvora_hooks::matcher::matcher_allows(group.matcher.as_ref(), tool_name)
            })
            .flat_map(move |group| {
    ...
    pub(super) async fn run_stop_client_hooks(
        &self,
        envelope: &HookEventEnvelope,
    ) -> xvora_hooks::dispatcher::StopDispatchResult {
        use xvora_hooks::result::HookRunResult;

        let mut out = xvora_hooks::dispatcher::StopDispatchResult::default();
        let Some(groups) = self
            .client_hooks
            .borrow()
        ...
        } in ordered
        {
            ...
            let detail = xvora_hooks::dispatcher::stop_detail(
                stop_reason.is_some(),
            ...
            out.absorb(
                &hook_name,
                xvora_hooks::dispatcher::StopSignals {
                    block_reason,
                    stop_reason,
    ...
    pub(super) async fn run_post_tool_use_client_hooks(
        &self,
        envelope: &HookEventEnvelope,
    ) -> xvora_hooks::dispatcher::PostToolUseResult {
        use xvora_hooks::result::HookRunResult;

        let mut out = xvora_hooks::dispatcher::PostToolUseResult::default();
        let Some(groups) = self
            .client_hooks
            .borrow()
            .get(&envelope.hook_event_name.canonical())
        ...
        } in ordered
        {
            ...
            if let Some(context) = response.additional_context.filter(|c| !c.trim().is_empty()) {
                out.additional_context
                    .push(xvora_hooks::dispatcher::AdditionalContext {
                        hook_name,
        ...
        local
            .run_until(async {
                ...
                *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(
                    xvora_hooks::discovery::HookRegistry::default(),
                ));
                assert!(actor.may_have_hooks_for(HookEventName::Stop));
