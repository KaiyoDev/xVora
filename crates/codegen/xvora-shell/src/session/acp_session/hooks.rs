...
//! both matched by the agent ([`xvora_hooks::matcher::HookMatcher`], shared with
//! file hooks):
...
use xvora_hooks::event::{HookEventEnvelope, HookEventName, HookPayload};
use xvora_telemetry::events::ClientHookGateOutcome;
...
impl SessionActor {
    ...
    pub(super) async fn run_pre_tool_use_client_hook(
        ...
        let tool_name = xvora_hooks::dispatcher::extract_tool_name(envelope)
