...
//! [`xvora_agent::Agent`] for a given session.
//!
//! INVARIANT: This is the **only** place in the shell crate that calls
//! [`xvora_agent::AgentBuilder::new`]. Both initial session spawn
...
//! ## Why this exists
//!
//! [`xvora_agent::Agent`] owns an [`xvora_tools::bridge::ToolBridge`]
...
impl AgentRebuildSpec {
    ...
    async fn build_agent_inner(
        ...
        agent
            .tool_bridge()
            .update_resource(xvora_tools::types::resources::RespectGitignore(
                *respect_gitignore,
            ))
            .await;
        agent
            .tool_bridge()
            .update_resource(xvora_tools::types::resources::PathNotFoundHints(
                *path_not_found_hints,
            ))
            .await;
        ...
        {
            use xvora_tools::implementations::xvora::ask_user_question::UserQuestionSender;
            agent
                .tool_bridge()
