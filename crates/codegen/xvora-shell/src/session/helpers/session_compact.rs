...
fn classify_response_event_error(code: Option<&str>, message: &str) -> CompactFailure {
    ...
    if code.is_some_and(xvora_sampling_types::is_size_overflow_error_code)
                ...
                match chunk_result {
                    Ok(event) => {
                        ...
                        xvora_sampling_types::messages::MessageStreamEvent::ContentBlockDelta {
                            delta: xvora_sampling_types::messages::StreamDelta::TextDelta { text },
                            ..
                        } => {
                            timing.record_delta();
                            content.push_str(&text);
                        }
                        xvora_sampling_types::messages::MessageStreamEvent::MessageDelta { delta, .. } => {
