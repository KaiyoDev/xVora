...
async fn workflow_run_manifest_round_trips_and_clear_tombstone_wins() {
    ...
    assert_eq!(loaded_with_effort.workflow_runs[0].effort,
            Some(xvora_sampling_types::ReasoningEffort::High));
...
async fn test_xai_session_update_round_trip() {
    ...
    let info = create_test_info();
    adapter.init_session(&info, default_model_id()).await.unwrap();
    let xvora_notification = XaiSessionNotification {
    ...
    adapter
        .append_update(&info, &SessionUpdate::Xai(Box::new(xvora_notification.clone())))
        .await
        .unwrap();
...
/// Exercises the real on-read upgrade path end-to-end (loader, serde, and xvora_sampling_types::upgrade_legacy_reasoning).
