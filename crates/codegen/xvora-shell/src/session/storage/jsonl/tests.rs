...
async fn copy_session_data_copies_compaction_segments_when_enabled() {
    ...
    assert!(
        ...
        .join(chat_state::compaction_transcript::COMPACTION_DIR).exists()
    );
}
#[tokio::test]
...
async fn init_session_stamps_configured_profile_on_new_session() {
    let tmp = TempDir::new().unwrap();
    let adapter = JsonlStorageAdapter::with_root(tmp.path().to_path_buf());
    xvora_sandbox::set_configured_profile("workspace");
    let expected = xvora_sandbox::configured_profile_name().map(String::from);
    let info = Info {
...
fn fork_filter_keeps_multi_tool_cycle_turn_with_reasoning() {
    ...
    let mut items = vec![
        ConversationItem::system("sys"), ConversationItem::user("q"),
        ConversationItem::Reasoning(xvora_sampling_types::synthesized_reasoning_item("plan",)),
...
/// path end-to-end (loader + serde + xvora_sampling_types::
...
fn read_chat_history_upgrades_legacy_singular_reasoning_to_sibling() {
    ...
    match &items[3] {
        ConversationItem::Reasoning(r) => {
            ...
            let xvora_sampling_types::rs::SummaryPart::SummaryText(s) = &r.summary[0];
...
fn read_chat_history_handles_hybrid_legacy_and_post_pr_lines() {
    ...
    let xvora_sampling_types::rs::SummaryPart::SummaryText(s) = &reconstructed
        .summary[0];
