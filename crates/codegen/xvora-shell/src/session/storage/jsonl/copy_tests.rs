...
async fn copy_session_data_transforms_xai_updates() {
    ...
    let source_info = Info {
        id: acp::SessionId::new("source-xvora"),
    ...
    let xvora_notification = XaiSessionNotification {
        session_id: acp::SessionId::new("source-xvora"),
    ...
    adapter
        .append_update(
            &source_info,
            &SessionUpdate::Xai(Box::new(xvora_notification)),
        )
        .await
        .unwrap();

    let target_info = Info {
        id: acp::SessionId::new("fork-source-xvora-abcd1234"),
