...
impl JsonlStorageAdapter {
    ...
    fn session_dir(&self, info: &Info) -> PathBuf {
        match &self.dir_mode {
            SessionDirMode::FromRoot(root) => {
                crate::util::xvora_home::sessions_cwd_dir_in(root, &info.cwd)
    ...
    fn create_session_dir_owner_only(&self, info: &Info) -> io::Result<PathBuf> {
        ...
        super::create_dir_all_durable(&dir, |dir| match &self.dir_mode {
            SessionDirMode::FromRoot(root) => {
                if let Ok(cwd_dir) =
                    crate::util::xvora_home::ensure_sessions_cwd_dir_in(root, &info.cwd)
                {
                    super::sync_cwd_marker_if_present(&cwd_dir)?;
                }
                crate::util::xvora_home::create_dir_all_owner_only(dir)
            }
            SessionDirMode::Explicit(_) => crate::util::xvora_home::create_dir_all_owner_only(dir),
        })?;
        Ok(dir)
    }
            ...
            let effort = match read_bounded_nofollow(
                ...
                Ok(bytes) => {
                    match String::from_utf8(bytes)
                        ...
                        .and_then(|effort| {
                            let parsed = effort
                                .parse::<xvora_sampling_types::ReasoningEffort>()
                                .map_err(|error| {
    ...
    /// We pre-extract them via [`xvora_sampling_types::upgrade_legacy_reasoning`].
    ...
    fn read_chat_history_sync(
        ...
        for (line_idx, line) in contents.split(|b| *b == b'\n').enumerate() {
            ...
            let siblings =
                xvora_sampling_types::upgrade_legacy_reasoning(&raw, &mut sibling_btc_ids_seen);
            for sib in siblings {
                match &sib {
...
impl StorageAdapter for JsonlStorageAdapter {
    async fn init_session(&self, info: &Info, model_id: acp::ModelId) -> io::Result<Summary> {
        ...
        if Path::new(&summary_path).exists() {
            ...
            summary.sandbox_profile = xvora_sandbox::configured_profile_name().map(String::from);
    ...
    async fn update_current_model_and_agent(
        ...
        reasoning_effort: Option<Option<xvora_sampling_types::ReasoningEffort>>,
    ) -> io::Result<()> {
        self.apply_summary_patch(
            info,
            super::summary_write::SummaryPatch {
