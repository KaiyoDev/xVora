...
use xvora_tools::implementations::xvora::update_goal::UpdateGoalInput;
...
async fn answer_summarizer(
    ...
    req: Box<xvora_tools::implementations::xvora::task::types::SubagentRequest>,
) {
    match behaviour {
...
fn configure_goal_actor(
    ...
    actor.tool_context.cwd =
        xvora_paths::AbsPathBuf::new(tmp.path().to_path_buf()).expect("abs cwd");
