    ...
    local
        .run_until(async {
            ...
            *actor.goal_update_rx.borrow_mut() = Some(rx);
            tx.send(
                xvora_tools::implementations::xvora::update_goal::envelope_for_test(
...
async fn drain_goal_updates_completes_after_blocked_does_not_leak_pause_message() {
    ...
    local
            .run_until(async {
                ...
                let (tx, rx) = tokio::sync::mpsc::unbounded_channel();
                *actor.goal_update_rx.borrow_mut() = Some(rx);
                tx.send(xvora_tools::implementations::xvora::update_goal::envelope_for_test(xvora_tools::implementations::xvora::update_goal::UpdateGoalInput {
                        completed: None,
                        message: None,
