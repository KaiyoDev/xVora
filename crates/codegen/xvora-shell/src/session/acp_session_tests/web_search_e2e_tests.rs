...
async fn web_search_uses_model_override_from_config_end_to_end() {
    ...
    let ctx = SessionContext {
        ...
        session_folder: std::env::temp_dir().join("xvora-web-search-e2e"),
        ...
        skills: vec![],
        state_path: std::env::temp_dir().join("xvora-web-search-e2e/state.json"),
        memory_backend: None,
...
async fn web_search_errors_when_configured_model_cannot_be_resolved() {
    ...
    let ctx = SessionContext {
        ...
        fs,
        cwd: std::env::temp_dir(),
        session_folder: std::env::temp_dir().join("xvora-web-search-disabled"),
        session_env: std::sync::Arc::new(std::collections::HashMap::new()),
        notification_handle: ToolNotificationHandle::noop(),
        owner_session_id: None,
        subagent: None,
        parent_scheduler_handle: None,
        skills: vec![],
        state_path: std::env::temp_dir().join("xvora-web-search-disabled/state.json"),
        memory_backend: None,
