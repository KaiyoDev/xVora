...
use xvora_hooks::event::{HookEventName, StopCancelledReason, StopFailureKind};
...
async fn a_subagent_session_end_names_the_child() {
    run(async {
        ...
        child.listen(&events);
        let child_timer = xvora_telemetry::session_end::SessionEndTimer::new_shared();
...
async fn a_long_assistant_message_is_clipped() {
    run(async {
        let over = xvora_hooks::event::MAX_ASSISTANT_MESSAGE_CHARS + 500;
        ...
        let assert_clipped = |text: &str| {
            ...
            assert!(
                text.chars().count() <= xvora_hooks::event::MAX_ASSISTANT_MESSAGE_CHARS + 32,
                "{} chars",
                text.chars().count()
            );
        };
        ...
        let xvora_hooks::event::HookPayload::SubagentStop {
    ...
    run(async {
        ...
        for (field, prefix, max) in [
            (
                "cancelTrigger",
                "ctrl_c",
                xvora_hooks::event::MAX_CANCEL_TRIGGER_CHARS,
            ),
            (
                "reasonDetails",
                "read_file: ",
                xvora_hooks::event::MAX_STOP_ENTRY_TEXT_CHARS,
            ),
        ] {
