...
fn assert_messages_rides_parent_prefix(
    ...
    let expected = serde_json::to_value(xvora_sampling_types::build_messages_request(&request))
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            cfg.api_backend = xvora_sampling_types::ApiBackend::Responses;
            ...
            let body = requests
                ...
                .to_string();
            assert!(body.contains(xvora_chat_state::compaction_utils::AGENT_MESSAGE_MODEL_LABEL));
            assert_eq!(
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            cfg.reasoning_effort = Some(xvora_sampling_types::ReasoningEffort::Low);
            actor.chat_state_handle.update_sampling_config(cfg);
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            let mut cfg = actor.chat_state_handle.get_sampling_config().await.unwrap();
            cfg.base_url = server.url();
            cfg.api_backend = xvora_sampling_types::ApiBackend::ChatCompletions;
            actor.chat_state_handle.update_sampling_config(cfg);
        ...
        .run_until(async {
            ...
            while let Ok(msg) = persistence_rx.try_recv() {
                if let PersistenceMsg::RecapRequest(artifact) = msg {
                    ...
                    assert!(
                        artifact.x_grok_req_id.starts_with("xvora-recap-"),
                        "req id: {}",
...
fn over_budget_recap_serializes_to_well_formed_messages_request() {
    ...
    use xvora_sampling_types::messages::{ContentBlock, MessageContent, MessageRole};
    use xvora_sampling_types::{ConversationRequest, ToolCall, rs};
        ...
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            // Backend-search fixture: agent carries hosted tools and both gates are on.
            {
                ...
                *agent_slot = xvora_agent::Agent::new(
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            let mut cfg = actor.chat_state_handle.get_sampling_config().await.unwrap();
            cfg.base_url = server.url();
            cfg.api_backend = xvora_sampling_types::ApiBackend::Messages;
