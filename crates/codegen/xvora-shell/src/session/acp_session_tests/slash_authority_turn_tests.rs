...
use crate::test_support::sse::responses_api_script_exact;
...
async fn actor_with_sampler(
    ...
    config.api_backend = xvora_sampling_types::ApiBackend::Responses;
                        ...
                        matches!(
                            ...
                            ConversationItem::User(user)
                                if user.synthetic_reason
                                    == Some(
                                        xvora_sampling_types::SyntheticReason::AgentMessage
                                    )
            ...
            let parent_turns = conversation
                ...
                .filter(|item| {
                    matches!(
                        ...
                        ConversationItem::User(user)
                            if user.synthetic_reason
                                == Some(xvora_sampling_types::SyntheticReason::AgentMessage)
                    )
                })
                .count();
            assert_eq!(
