...
use xvora_tools::implementations::grok_build::image_gen::ImageGenTool;
use xvora_tools::media_gen_limits::DEFAULT_MAX_PARALLEL_IMAGE_GEN;
...
async fn tool_result_text(actor: &SessionActor, call_id: &str) -> String {
    ...
    conv.iter()
        ...
        .find_map(|item| match item {
            xvora_sampling_types::ConversationItem::ToolResult(tr)
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_tools(vec![
            ...
            let calls: Vec<xvora_sampling_types::ToolCall> = (0..over)
            ...
            let under: Vec<xvora_sampling_types::ToolCall> = (0..DEFAULT_MAX_PARALLEL_IMAGE_GEN)
                .map(|i| xvora_sampling_types::ToolCall {
