...
fn post_tool_use_envelope(actor: &SessionActor) -> xvora_hooks::event::HookEventEnvelope {
        ...
        .run_until(async {
            ...
            *actor.agent.borrow_mut() =
            ...
            client_hooks.insert(
                ...
                vec![crate::extensions::hooks::ClientHookGroup {
                    matcher: Some(
                        xvora_hooks::matcher::HookMatcher::new("linear__save_issue").unwrap(),
                    ),
...
pub(super) fn file_registry_with_spec(
    event: xvora_hooks::event::HookEventName,
    script: &str,
) -> xvora_hooks::discovery::HookRegistry {
...
fn file_registry(
    ...
    let (mut registry, _) = xvora_hooks::discovery::load_hooks(None, None);
    registry.append_specs(vec![xvora_hooks::config::HookSpec {
...
async fn subagent_session_gates_on_subagent_stop() {
    ...
    local
        .run_until(async {
            ...
            *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(file_registry_with_spec(
                xvora_hooks::event::HookEventName::SubagentStop,
...
async fn alias_envelope_serializes_canonical_event_name() {
    ...
    local
        .run_until(async {
            ...
            let envelope = actor.make_hook_envelope(
                xvora_hooks::event::HookEventName::SubagentEnd,
                None,
                xvora_hooks::event::HookPayload::SubagentStop {
                    phase: xvora_hooks::event::SubagentStopPhase::Observe,
                ...
                *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(file_registry(
                ...
                let reported = actor.claim_and_queue(
                    ...
                    super::turn_end_hooks::TurnEnd::Failed {
                        error: xvora_hooks::event::StopFailureKind::Unknown,
                        error_details: None,
                        last_assistant_message: None,
                    },
                );
