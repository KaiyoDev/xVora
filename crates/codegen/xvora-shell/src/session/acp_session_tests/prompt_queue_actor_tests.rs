...
fn x_search_cutoff_update() -> xvora_sampling_types::ToolOverridesUpdate {
    xvora_sampling_types::ToolOverridesUpdate {
        x_search: Some(Some(xvora_sampling_types::XSearchOptions {
            date_bound: Some(
                xvora_sampling_types::SearchDateBound::new(None, Some("2024-03-15".to_string()))
                    .unwrap(),
            ),
        })),
        web_search: None,
...
async fn tool_overrides_update_applies_at_promotion_never_at_enqueue() {
    ...
    local
        .run_until(async {
            let (actor, _rx) = build_actor().await;
            let options = xvora_sampling_types::XSearchOptions {
                date_bound: Some(
                    xvora_sampling_types::SearchDateBound::new(
                        None,
            ...
            let set_update = || xvora_sampling_types::ToolOverridesUpdate {
                x_search: Some(Some(options.clone())),
                web_search: None,
            };
            let expected = xvora_sampling_types::ToolOverrides {
            ...
            actor.apply_tool_overrides_update(Some(xvora_sampling_types::ToolOverridesUpdate {
                x_search: Some(None),
                web_search: None,
            }));
            assert_eq!(
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() =
                test_agent_backend_search(vec![xvora_sampling_types::HostedTool::XSearch {
                    options: None,
                }])
                .await;
            ...
            *actor.tool_overrides.borrow_mut() = Some(expected.clone());
            ...
            assert_eq!(
                ...
                vec![xvora_sampling_types::HostedTool::XSearch {
...
async fn unsupported_backend_search_sends_no_hosted_tool_on_either_channel() {
    ...
    local
        .run_until(async {
            let (actor, _rx) = build_actor().await;
            let configured = xvora_sampling_types::WebSearchOptions {
                allowed_domains: None,
                excluded_domains: Some(vec!["reddit.com".to_string()]),
            };
            *actor.agent.borrow_mut() =
                test_agent_backend_search(vec![xvora_sampling_types::HostedTool::WebSearch {
            ...
            assert_eq!(
                xvora_sampling_types::extra_tool_entries(&actor.hosted_tools_for_turn()).len(),
                1
            );
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() =
            ...
            assert_eq!(
                actor.effective_hosted_tools(),
                vec![xvora_sampling_types::HostedTool::WebSearch {
                    options: Some(configured.clone()),
                }],
            );

            let per_turn = xvora_sampling_types::WebSearchOptions {
            ...
            assert_eq!(
                actor.effective_tool_overrides(),
                Some(xvora_sampling_types::ToolOverrides {
                    x_search: None,
...
async fn agent_rebuild_republishes_the_configured_cutoff() {
    ...
    local
        .run_until(async {
            ...
            let seed = xvora_sampling_types::ToolOverrides {
                x_search: Some(xvora_sampling_types::XSearchOptions {
                    date_bound: Some(
                        xvora_sampling_types::SearchDateBound::new(
                            None,
...
async fn set_tool_overrides_publishes_the_inheritance_cell_before_any_turn() {
    ...
    local
        .run_until(async {
            ...
            let cutoff = xvora_sampling_types::ToolOverrides {
                x_search: Some(xvora_sampling_types::XSearchOptions {
                    date_bound: Some(
                        xvora_sampling_types::SearchDateBound::new(
                            None,
