...
//!
//! Canonical definitions live in `prompt_queue`.
//! This re-export keeps every existing `crate::session::prompt_queue::*` and `shell::session::prompt_queue::*` path resolving without edits.
...
mod tests {
    ...
    fn queue_changed_serializes_camel_case_with_session_id() {
        let payload = QueueChanged {
            ...
            entries: vec![QueueEntryWire {
                id: "p1".to_string(),
                version: 0,
                owner: Some("xvora-tui".to_string()),
                last_editor: None,
    ...
    fn queue_entry_wire_round_trips_last_editor() {
        let entry = QueueEntryWire {
            ...
            version: 3,
            owner: Some("xvora-tui".to_string()),
            last_editor: Some("xvora-vscode".to_string()),
            kind: "prompt".to_string(),
            text: "hello".to_string(),
            position: 0,
            combined_texts: None,
        ...
        assert_eq!(json["lastEditor"], "xvora-vscode");
