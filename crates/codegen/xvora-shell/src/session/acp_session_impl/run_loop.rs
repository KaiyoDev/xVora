...
pub(super) async fn run_session(
    ...
    loop {
        tokio::select! {
            ...
            xvora_telemetry::memory_log::TARGET,
            ...
            } else { tracing::debug!(target : xvora_telemetry::memory_log::TARGET,
            ...
            xvora_telemetry::memory_log::TARGET, "MEMORY_DREAM_CHECK: timer fired");
            ...
            Some(chat_state::ChatStateEvent::ImageBudget { body_bytes, trigger_bytes,
            reclaim_target_bytes, inline_images, needs_image_compaction, evicted,
            body_bytes_after, }) => {
            xvora_telemetry::unified_log::info("shell.image_budget", Some(session
            ...
            Some(chat_state::ChatStateEvent::TokensUpdated { .. }) => {} None => {} }
            ...
            .fire_hook(xvora_hooks::event::HookEventName::SessionEnd, None,
            xvora_hooks::event::HookPayload::SessionEnd { reason : "channel_closed"
            ...
            xvora_hooks::dispatcher::dispatch_non_blocking(& registry,
            xvora_hooks::event::HookEventName::SessionEnd, & envelope, & ctx,). await;
            session.send_hook_execution("session_end", None, None, & results). await; }
            let envelope = session.fire_hook(xvora_hooks::event::HookEventName::Stop,
            None, xvora_hooks::event::HookPayload::Stop { reason : "channel_closed"
            .to_string(), },); if let Some(registry) = session.hook_registry.borrow()
            .clone() { let ctx = session.hook_run_ctx(); let results =
            xvora_hooks::dispatcher::dispatch_non_blocking(& registry,
            xvora_hooks::event::HookEventName::Stop, & envelope, & ctx,). await;
            ...
            xvora_telemetry::memory_log::TARGET, result = ? result, tool_searches =
            ...
            file_utils::trace_context::link_current_span_to_meta(& meta); } let
            ...
            .update_credentials(chat_state::Credentials { api_key : r.api_key,
            ...
            .send(hooks_plugins_types::HooksListResponse { hooks, project_trusted,
            ...
            .push_capped(xvora_tools::implementations::xvora::task::types::MonitorEventNotification
            ...
            xvora_agent::prompt::skills::list_skills_with_plugins(Some(& cwd), &
            ...
            .send(xvora_mcp::servers::McpClientEvent::ConfigDiff { added : diff.added
            ...
            .emit(file_utils::events::Event::McpServerToggled { server_name :
            ...
            session.fire_hook(xvora_hooks::event::HookEventName::SessionStart, None,
            xvora_hooks::event::HookPayload::SessionStart { source, model_id : None,
            agent_type : None, },); if let Some(registry) = session.hook_registry
            .borrow().clone() { let ctx = session.hook_run_ctx(); let results =
            xvora_hooks::dispatcher::dispatch_non_blocking(& registry,
            xvora_hooks::event::HookEventName::SessionStart, & envelope, & ctx,).
            ...
            .fire_hook(xvora_hooks::event::HookEventName::SessionEnd, None,
            xvora_hooks::event::HookPayload::SessionEnd { reason : "shutdown"
            .to_string(), turn_count : None, tool_call_count : None, },); if let
            Some(registry) = session.hook_registry.borrow().clone() { let ctx = session
            .hook_run_ctx(); let results =
            xvora_hooks::dispatcher::dispatch_non_blocking(& registry,
            ...
            session.send_hook_execution("session_end", None, None, & results). await; }
            let envelope = session.fire_hook(xvora_hooks::event::HookEventName::Stop,
            None, xvora_hooks::event::HookPayload::Stop { reason : "shutdown"
            .to_string(), },); if let Some(registry) = session.hook_registry.borrow()
            .clone() { let ctx = session.hook_run_ctx(); let results =
            xvora_hooks::dispatcher::dispatch_non_blocking(& registry,
            xvora_hooks::event::HookEventName::Stop, & envelope, & ctx,). await;
...
pub(super) fn turn_texts_for_feedback(
    conversation: &[xvora_sampling_types::ConversationItem],
    turn_number: usize,
