...
pub(super) async fn fire_session_end_hooks(
    ...
    let envelope = session.fire_hook(
        xvora_hooks::event::HookEventName::SessionEnd,
        None,
        xvora_hooks::event::HookPayload::SessionEnd {
    ...
    if let Some(registry) = session.hook_registry.borrow().clone() {
        ...
        let results = xvora_hooks::dispatcher::dispatch_non_blocking(
            &registry,
            xvora_hooks::event::HookEventName::SessionEnd,
            &envelope,
            &ctx,
                ...
                maybe_cmd = cmd_rx.recv() => {
                    ...
                    match cmd {
                        ...
                        SessionCommand::UpdateMcpServers { mcp_servers, respond_to } => {
                            ...
                            // also fires for `<cwd>/.xvora/config.toml` edits,
                        ...
                        SessionCommand::DispatchSessionStartHook { source } => {
                            let envelope = session.fire_hook(
                                xvora_hooks::event::HookEventName::SessionStart,
                                None,
                                xvora_hooks::event::HookPayload::SessionStart {
                                    source,
                                    model_id: None,
                                    agent_type: None,
                                },
