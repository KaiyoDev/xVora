...
impl SessionActor {
    ...
    pub(super) async fn handle_prompt(
        ...
        *self.tool_context.prompt_index.lock().await = current_prompt_index;
        ...
        let model_id = self
            ...
            .map(|c| c.model)
            .unwrap_or_default();
        if self.telemetry_enabled || xvora_telemetry::external::is_active() {
        ...
        self.dispatch_hook(
            xvora_hooks::event::HookEventName::UserPromptSubmit,
        ...
        self.dispatch_hook(
            ...
            xvora_hooks::event::HookPayload::Stop {
    ...
    pub(crate) async fn inject_pending_monitor_events(&self) {
        ...
        let mine = xvora_tools::implementations::xvora::task::types::drain_owned(
            buffer,
            Some(self.session_info.id.0.as_ref()),
        );
        if mine.is_empty() {
    ...
    pub(crate) async fn first_turn_memory_reminder(&self) -> Option<String> {
        ...
        tracing::info!(
            target : xvora_telemetry::memory_log::TARGET, configured_min_score,
            "MEMORY_INJECT_SEARCH: results={result_count}"
        );
        xvora_telemetry::session_ctx::log_event(
            xvora_telemetry::memory_telemetry::MemoryInjection {
    ...
    async fn process_conversation_turn(
        ...
        loop {
            ...
            for item in response.items {
                match item {
                    xvora_sampling_types::ConversationItem::Assistant(_) => {
