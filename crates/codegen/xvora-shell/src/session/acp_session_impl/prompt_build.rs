...
fn partition_rules_by_scope(
    ...
    for f in files {
        let entry = xvora_agent::prompt::user_message::RuleEntry::from(f);
...
impl SessionActor {
        ...
        if drop_startup_skill_reminder {
            conversation.retain(|item| {
                !matches!(
                    ...
                    Some(xvora_sampling_types::SyntheticReason::SystemReminder)
                )
            });
        }
    }
    ...
    async fn build_templated_user_message(
        ...
        use xvora_agent::prompt::agents_md::read_agents_config_with_paths;
    ...
    async fn gather_vcs_for_prefix(
        ...
        use xvora_workspace::file_system::{git_status_short, jj_status};
