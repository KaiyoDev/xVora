...
struct PreToolUseGate {
    ...
    additional_context: Vec<xvora_hooks::dispatcher::AdditionalContext>,
}
struct GateRewrite {
...
impl SessionActor {
    ...
    async fn execute_tool_calls_batch(
        ...
        while let Some((idx, result, duration_ms)) = dispatch_rx.recv().await {
            ...
            let mut post_tool_use_failure_contexts: Vec<
                xvora_hooks::dispatcher::AdditionalContext,
            > = Vec::new();
    ...
    async fn apply_pre_tool_use_gate(
        ...
        if let Some(registry) = hook_registry_snapshot {
            ...
            let pre_result =
                xvora_hooks::dispatcher::dispatch_pre_tool_use(&registry, &envelope, &ctx).await;
    ...
    async fn validate_hook_rewrite(
        ...
        resolved_tool_name: &str,
        dispatch_target_name: &Option<String>,
        rewrite: xvora_hooks::dispatcher::InputRewrite,
            ...
            .await
        {
            Ok(input) => input,
            Err(err) => {
                let schema_error = xvora_hooks::event::clip_reason(&err.to_string());
                return Ok(Err(self
    ...
    pub(crate) async fn prepare_tool_call(
        ...
        if self.may_have_hooks_for(xvora_hooks::event::HookEventName::PreToolUse) {
            let gate = match self
    ...
    ) -> xvora_hooks::event::HookEventEnvelope {
        let (tool_input, tool_input_truncated) =
            xvora_hooks::event::truncate_payload(input.clone());
