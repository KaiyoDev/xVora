...
impl PlanApprovalOutcome {
    fn from_response(
        resp: &xvora_tools::implementations::xvora::exit_plan_mode::ExitPlanModeExtResponse,
    ) -> Self {
...
impl SessionActor {
        ...
        while let Some((idx, mut result)) = dispatch_rx.recv().await {
                ...
                && server.starts_with(crate::session::managed_mcp::MANAGED_MCP_PREFIX)
            {
                let auth_rejected = match &result {
                    Err(err) => xvora_mcp::servers::is_auth_rejection_message(&err.to_string()),
                    Ok(tool_result) => {
                        tool_result.output.is_error()
                            && xvora_mcp::servers::is_auth_rejection_message(
            ...
            let tool_loop = match result {
                Ok(tool_result) => {
                    ...
                    post_tool_use_result = self
                        .hook_event_active(xvora_hooks::event::HookEventName::PostToolUse)
                        .then(|| {
                ...
                Err(err) => {
                    ...
                    if self.hook_event_active(xvora_hooks::event::HookEventName::PostToolUseFailure)
                    {
                        ...
                        let (tool_input_value, tool_input_truncated) =
                            xvora_hooks::event::truncate_payload(raw_input);
                        let hook_tool_name = prepared.hook_tool_name();
                        self.dispatch_hook(
                            xvora_hooks::event::HookEventName::PostToolUseFailure,
                            xvora_hooks::event::HookPayload::PostToolUseFailure {
            ...
            if let Some(tool_result_value) = post_tool_use_result {
                ...
                let (tool_result_val, tool_result_truncated) =
                    xvora_hooks::event::truncate_payload(tool_result_value);
                let hook_tool_name = prepared.hook_tool_name();
                self.dispatch_hook(
                    xvora_hooks::event::HookEventName::PostToolUse,
                    xvora_hooks::event::HookPayload::PostToolUse {
                        tool_name: hook_tool_name.to_owned(),
                        tool_use_id: prepared.call_id.clone(),
                        tool_input: tool_input_value,
    ...
    pub(crate) async fn prepare_tool_call(
        ...
        if self.hook_event_active(xvora_hooks::event::HookEventName::PreToolUse) {
            let (hook_tool_input, hook_tool_input_truncated) =
                xvora_hooks::event::truncate_payload(raw_input.clone());
            let envelope = self.make_hook_envelope(
                xvora_hooks::event::HookEventName::PreToolUse,
                None,
                xvora_hooks::event::HookPayload::PreToolUse {
            ...
            if let Some(registry) = hook_registry_snapshot {
                ...
                let pre_result =
                    xvora_hooks::dispatcher::dispatch_pre_tool_use(&registry, &envelope, &ctx)
                        .await;
                ...
                .await;
                if let xvora_hooks::result::HookDecision::Deny { reason, hook_name } =
                    pre_result.decision
                {
                    return Ok(Err(self
                        .deny_tool(
        ...
        if !plan_file_auto_approve {
            ...
            let (decision_outcome, _reject_reason) = match &decision {
                Decision::Allow | Decision::Ask => {
                    (xvora_telemetry::events::PermissionOutcome::Allow, None)
                }
            ...
            xvora_telemetry::session_ctx::log_event(
                xvora_telemetry::events::PermissionDecisionPayload {
                    tool_name: call.function.name.clone(),
                    access_kind: telemetry_access_kind,
            ...
            match decision {
                Decision::PolicyDeny(ref reason) | Decision::Reject(ref reason) => {
                    ...
                    let (tool_input_value, tool_input_truncated) =
                        xvora_hooks::event::truncate_payload(raw_input.clone());
                    self.dispatch_hook(
                        xvora_hooks::event::HookEventName::PermissionDenied,
                        xvora_hooks::event::HookPayload::PermissionDenied {
                            tool_name: resolved_tool_name.clone(),
    ...
    pub(super) async fn request_plan_approval(
        ...
        xvora_tools::implementations::xvora::exit_plan_mode::ExitPlanModeExtResponse,
        acp::Error,
    > {
        use agent_client_protocol::Client as _;
        use xvora_tools::implementations::xvora::exit_plan_mode::{
    ...
    /// `xvora_tools::reminders::task_completion::consumed_completion_ids`,
    ...
    /// Translate one [`xvora_sampler::SamplingEvent`] from the
...
mod plan_mode_edit_gate_tests {
    ...
    fn search_replace(path: &str) -> ToolInput {
        use xvora_tools::implementations::xvora::search_replace::SearchReplaceInput;
...
mod plan_approval_helper_tests {
    ...
    use xvora_tools::implementations::xvora::exit_plan_mode::ExitPlanModeExtResponse;
