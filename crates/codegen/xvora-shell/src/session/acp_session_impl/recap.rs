...
use xvora_sampling_types::SamplingError;
...
fn build_side_question_attempt(base: &ConversationRequest) -> ConversationRequest {
    ...
    request.x_grok_req_id = Some(format!("xvora-btw-{}", uuid::Uuid::new_v4()));
    request
}
impl SessionActor {
    ...
    pub(super) async fn handle_side_question(
        ...
        let base_request = self.parent_cached_request(AuxCall {
            ...
            req_id: format!("xvora-btw-{}", uuid::Uuid::new_v4()),
        });
    ...
    pub(super) async fn handle_recap(&self, auto: bool) {
        ...
        let x_grok_req_id = format!("xvora-recap-{}", uuid::Uuid::new_v4());
        let request = self
        ...
        // Applies to manual `/recap` too: spinner-less clients (e.g. xvora Desktop) would otherwise append the late recap mid-turn.
    ...
    pub(super) async fn handle_suggest_prompt(
        ...
        let suggest_reasoning = prompt_suggest::resolve_suggest_reasoning(
            ...
            self.models_manager.model_supports_reasoning_effort_value(
                &model,
                xvora_sampling_types::ReasoningEffort::None,
            ),
        );
        ...
        let request_id = format!("xvora-promptsuggest-{}", uuid::Uuid::new_v4());
        let request = ConversationRequest {
            items,
            tools: vec![],
...
mod tests {
    ...
    fn side_question_attempts_get_fresh_request_ids() {
        ...
        assert!(a_id.starts_with("xvora-btw-"));
