...
//!
//! Token counts come from `POST {xvora_api_base_url}/tokenize-text` (the model's tokenizer), not the bytes/4 `/context` heuristic.
...
impl SessionActor {
    ...
    pub(super) async fn emit_session_context_snapshot(&self) {
        ...
        let tokens = tokenize_texts_parallel(
            &tokenize_text_url(&xvora_api_base_url()),
            &api_key,
            model,
            texts.jobs(),
        )
...
fn xvora_api_base_url() -> String {
    crate::config::load_effective_config()
        .ok()
        .and_then(|effective| crate::agent::config::Config::new_from_toml_cfg(&effective).ok())
        .map(|cfg| cfg.endpoints.xvora_api_base_url)
        .filter(|url| !url.is_empty())
        .unwrap_or_else(|| crate::agent::config::EndpointsConfig::default().xvora_api_base_url)
}
