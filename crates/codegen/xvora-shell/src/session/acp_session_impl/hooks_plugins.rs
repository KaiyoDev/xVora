    ...
    pub(super) async fn handle_hooks_action(
        ...
        match action {
            ...
            HooksAction::Disable { hook_name } => {
                match xvora_hooks::trust::disable_hook(&hook_name) {
                    Ok(()) => ActionOutcome {
                        status: OutcomeStatus::Success,
            ...
            HooksAction::Enable { hook_name } => {
                match xvora_hooks::trust::enable_hook(&hook_name) {
            ...
            HooksAction::ToggleSource {
                ...
                for name in &hook_names {
                    let ok = if disable {
                        xvora_hooks::trust::disable_hook(name).is_ok()
                    } else {
                        xvora_hooks::trust::enable_hook(name).is_ok()
                    };
                    if ok {
                        toggled += 1;
                    }
        ...
        match action {
            ...
            PluginsAction::Update { plugin_id } => {
                ...
                for (key, repo) in &repos_to_update {
                    match xvora_agent::plugins::git_install::update_repo(key, repo) {
                        Ok(status) => {
        ...
        if let Some(ref new_registry) = new_registry_snapshot {
            ...
            hooks_reloaded = new_specs.len();
            {
                ...
                if let Some(ref mut arc_reg) = *reg {
                    ...
                    let (mut new_reg, _) =
                        xvora_hooks::discovery::load_hooks_from_sources(&[], &[]);
