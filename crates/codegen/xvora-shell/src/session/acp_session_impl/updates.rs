...
impl SessionActor {
    ...
    fn log_outbound_xai_buffered(&self, notification: &XaiSessionNotification) {
        ...
        tracing::info!(
            target: "acp_event",
            event = "xvora_buffered_notification_sent",
        ...
        match &notification.update {
            XaiSessionUpdate::SubagentSpawned {
                ...
                let envelope = self.fire_hook(
                    xvora_hooks::event::HookEventName::SubagentStart,
                    None,
                    xvora_hooks::event::HookPayload::SubagentStart {
                ...
                if let Some(registry) = hook_registry_snapshot {
                    ...
                    let _ = xvora_hooks::dispatcher::dispatch_non_blocking(
                        &registry,
                        xvora_hooks::event::HookEventName::SubagentStart,
                        &envelope,
                        &ctx,
    ...
    pub(super) async fn dispatch_notification_hook(
        ...
        let envelope = self.fire_hook(
            xvora_hooks::event::HookEventName::Notification,
            None,
            xvora_hooks::event::HookPayload::Notification {
...
#[cfg(test)]
mod xvora_event_id_stamping_tests {
    use super::support::create_test_actor;
    use super::*;
