...
pub(super) struct DeferredPostToolUseScrollback {
    tool_name: String,
    results: Vec<xvora_hooks::result::HookRunResult>,
}

impl SessionActor {
    pub(super) fn hook_run_ctx(&self) -> xvora_hooks::runner::RunContext<'_> {
        xvora_hooks::runner::RunContext {
    ...
    pub(super) async fn send_hook_execution(
        ...
        results: &[xvora_hooks::result::HookRunResult],
    ) {
        if results.is_empty()
            || results
                .iter()
                .all(|r| matches!(r, xvora_hooks::result::HookRunResult::Skipped { .. }))
        {
            return;
        }
    ...
    pub(super) async fn dispatch_hook(
        &self,
        event: xvora_hooks::event::HookEventName,
        payload: xvora_hooks::event::HookPayload,
        prompt_id: Option<&str>,
        tool_name: Option<&str>,
    ) {
        ...
        let results =
            xvora_hooks::dispatcher::dispatch_non_blocking(&registry, event, &envelope, &ctx).await;
    ...
    pub(super) async fn dispatch_post_tool_use_hook(
        ...
        use xvora_hooks::event::{HookEventName, HookPayload, truncate_payload};
        ...
        let mut dispatch_result = if let Some(registry) = registry {
            let ctx = self.hook_run_ctx();
            xvora_hooks::dispatcher::dispatch_post_tool_use(&registry, &envelope, &ctx).await
        } else {
            xvora_hooks::dispatcher::PostToolUseResult::default()
        };
    ...
    pub(super) async fn emit_post_tool_use_scrollback(
        ...
        let event = xvora_hooks::event::HookEventName::PostToolUse.to_string();
    ...
    pub(super) async fn dispatch_tool_failure(
        ...
        error_text: String,
        duration_ms: u64,
    ) -> Vec<xvora_hooks::dispatcher::AdditionalContext> {
        if !self.may_have_hooks_for(xvora_hooks::event::HookEventName::PostToolUseFailure) {
            return Vec::new();
        }
        let raw_input: serde_json::Value =
            serde_json::from_str(&prepared.raw_arguments).unwrap_or(serde_json::Value::Null);
        let (tool_input, tool_input_truncated) = xvora_hooks::event::truncate_payload(raw_input);
    ...
    async fn dispatch_post_tool_use_failure_hook(
        ...
        let event = xvora_hooks::event::HookEventName::PostToolUseFailure;
        ...
        let ctx = self.hook_run_ctx();
        let result =
            xvora_hooks::dispatcher::dispatch_post_tool_use_failure(&registry, &envelope, &ctx)
                .await;
    ...
    pub(super) async fn dispatch_prompt_submit_hook(
        ...
        payload: xvora_hooks::event::HookPayload,
        prompt_id: Option<&str>,
    ) -> xvora_hooks::result::PromptDecision {
        let event = xvora_hooks::event::HookEventName::UserPromptSubmit;
        if !self.may_have_hooks_for(event) {
            return xvora_hooks::result::PromptDecision::Allow;
        }
        // Fires observe-only client hooks before (and independent of) the on-disk registry guard below.
        let envelope = self.fire_hook(event, prompt_id.map(|s| s.to_string()), payload);
        let Some(registry) = self.hook_registry.borrow().clone() else {
        ...
        let ctx = self.hook_run_ctx();
        let gate = xvora_hooks::dispatcher::dispatch_prompt_gate(&registry, &envelope, &ctx).await;
