...
/// `SamplingError::is_auth_error` in xvora-sampling-types: 403 is
...
impl SessionActor {
    ...
    pub(super) async fn reconstruct_full_config(&self) -> SamplingConfig {
        ...
        impl xvora_sampler::BearerResolver for AuthManagerBearerResolver {
        ...
        SamplingConfig {
            ...
            bearer_resolver: if use_bearer_resolver {
                self.auth_manager
                    .as_ref()
                    .map(|am| -> xvora_sampler::SharedBearerResolver {
    ...
    async fn resolve_auto_classifier_sampler(
        &self,
        slug: &str,
    ) -> Option<(xvora_sampler::SamplingClient, String)> {
    ...
    /// `xvora-sampler` instead of constructing a new
    /// `OaiCompatClient`.
    ///
    ...
    pub(crate) async fn handle_sampling_failure(
        ...
        if auth_mode == crate::auth::AuthMode::WebLogin {
            let msg = format!(
                "{detailed_message}\n\n\
                 ...
                 To fix: run `grok logout` then `xvora login` to re-authenticate with OAuth2.\n\n\
        ...
        let error_type = if xvora_sampling_types::is_context_length_error(&error.message) {
            "context_length"
        } else {
            error.kind.as_str()
        };
    ...
    pub(crate) async fn run_turn_via_sampler(
            ...
            .await
        {
            ...
            Err(rich_err) => {
                self.turn_stream_drained.lock().take();
                let info = xvora_sampler::SamplingErrorInfo::from(&rich_err);
