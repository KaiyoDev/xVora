...
pub(super) fn transient_retry_eligible(error: &sampler::SamplingErrorInfo) -> bool {
    ...
    if error.should_retry == Some(false)
        ...
        || matches!(
            error.error_code,
            Some(xvora_sampling_types::ApiErrorCode::InvalidImage)
        )
    {
        return false;
    }
        ...
        SamplingErrorKind::Api => error.status_code.is_some_and(|status| {
            status != 429
                && reqwest::StatusCode::from_u16(status)
                    .is_ok_and(xvora_sampling_types::is_retryable_api_status)
        }),
...
/// Matches strictly on HTTP 401 when the error carries a structured status code, mirroring `SamplingError::is_auth_error` in xvora-sampling-types.
...
fn revoked_sampling_info() -> sampler::SamplingErrorInfo {
    sampler::SamplingErrorInfo {
        ...
        credential: xvora_sampling_types::SentCredential::Unknown,
    }
}
...
impl SessionActor {
    ...
    fn resolve_hosted(
        ...
        Vec<xvora_sampling_types::HostedTool>,
        xvora_sampling_types::ToolOverrides,
    ) {
        let mut tools = self.agent.borrow().hosted_tools().to_vec();
        let applied = xvora_sampling_types::apply_tool_overrides(
            &mut tools,
    ...
    pub(crate) fn effective_hosted_tools(&self) -> Vec<xvora_sampling_types::HostedTool> {
        self.resolve_hosted().0
    }

    pub(crate) fn hosted_tools_for_turn(&self) -> Vec<xvora_sampling_types::HostedTool> {
    ...
    pub(crate) fn set_tool_overrides(&self, overrides: xvora_sampling_types::ToolOverrides) {
    ...
    pub(crate) fn apply_tool_overrides_update(
        &self,
        update: Option<xvora_sampling_types::ToolOverridesUpdate>,
    ) {
    ...
    pub(super) async fn reconstruct_full_config(&self) -> SamplingConfig {
        ...
        let cfg = self
            ...
            .get_sampling_config()
            .await
            .unwrap_or_else(|| xvora_sampling_types::SamplingConfig {
                base_url: String::new(),
                model: String::new(),
    ...
    pub(crate) async fn handle_sampling_failure(
        ...
        if auth_mode == crate::auth::AuthMode::WebLogin {
            let msg = format!(
                "{detailed_message}\n\n\
                 ...
                 To fix: run `xvora update`, then `xvora logout`, then `xvora login` to re-authenticate with OAuth2.\n\n\
        ...
        let error_type = if error
            .error_code
            .as_ref()
            .is_some_and(xvora_sampling_types::ApiErrorCode::is_size_overflow)
            || xvora_sampling_types::is_context_length_error(&error.message)
        {
...
fn resolve_configured_cutoff(
    seed: Option<xvora_sampling_types::ToolOverrides>,
    base: Option<&xvora_sampling_types::ToolOverrides>,
) -> xvora_sampling_types::ToolOverrides {
    use xvora_sampling_types::{ToolOverrides, WebSearchOptions, XSearchOptions};
    let ToolOverrides {
        x_search: seed_x,
        web_search: seed_w,
