...
//!   ([`xvora_mcp::liveness`]),
//! - the [`xvora_mcp::servers::GrokClientHandler`] (server-pushed
...
pub enum McpServerStatus {
    /// Client is in [`xvora_mcp::servers::ClientStateKind::Ready`]
...
/// [`xvora_mcp::servers::McpClientEventKind`]). Used as the
    ...
    let (status, reason, detail) = match (kind, event) {
        ...
        (McpClientEventKind::HandshakeFailed, McpClientEvent::HandshakeFailed { reason, .. })
            if source == McpServerSource::Managed
                && xvora_mcp::servers::is_auth_rejection_message(reason) =>
        {
            (
