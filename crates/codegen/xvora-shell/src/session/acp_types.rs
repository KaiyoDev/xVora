...
pub struct ContextInfo {
    ...
    /// always matches the actual trigger (e.g. 65 for xvora in remote settings).
...
pub struct SessionInfoData {
    /// Agent definition name for this session (e.g. `xvora`).
...
mod tests {
    ...
    fn should_show_model_fingerprint_truth_table() {
        ...
        assert!(should_show_model_fingerprint(false, "xvora"));
