...
pub struct SessionHandle {
    ...
    pub terminal_backend:
        Option<std::sync::Arc<dyn xvora_tools::computer::types::TerminalBackend>>,
...
impl SessionHandle {
    ...
    pub async fn get_hooks_list(&self) -> Option<hooks_plugins_types::HooksListResponse> {
        let (tx, rx) = oneshot::channel();
        if self
            .cmd_tx
    ...
    pub(crate) async fn snapshot_tool_definitions(&self) -> Vec<xvora_sampling_types::ToolSpec> {
        let (tx, rx) = oneshot::channel();
        if self
            .cmd_tx
