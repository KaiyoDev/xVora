...
mod tests {
    ...
    fn local(session_id: &str, updated_at: &str) -> MergedSession {
        MergedSession {
            ...
            source_workspace_dir: Some("/Users/me/xvora-src".into()),
    ...
    fn ext_superset_preserves_every_legacy_field_and_adds_title_and_meta() {
        ...
        assert_eq!(value["gitRootDir"], "/Users/me/xvora");
        assert_eq!(value["gitRemotes"][0], "git@github.com:example/repo.git");
        assert_eq!(value["sourceWorkspaceDir"], "/Users/me/xvora-src");
    ...
    fn facets_carry_kind_and_cwd() {
        ...
        assert!(matches!(
            ...
            Some(FacetValue::One(serde_json::Value::String(c))) if c == "/Users/me/xvora"
        ));
    }
    #[test]
    fn bare_session_info_is_minimal_plus_meta() {
        ...
        assert_eq!(value["sessionId"], "s1");
        assert_eq!(value["cwd"], "/Users/me/xvora");
        assert_eq!(value["title"], "a summary");
        assert_eq!(value["_meta"]["x.ai/session"]["kind"], "build");
    ...
    fn xvora_auth_manager(dir: &std::path::Path) -> std::sync::Arc<crate::auth::AuthManager> {
        ...
        am.hot_swap(crate::auth::GrokAuth {
            ...
            oidc_issuer: Some(crate::auth::xvora_oauth2_issuer().to_owned()),
    ...
    fn conversations_lane_active_truth_table() {
        ...
        let _desktop_off =
            xvora_test_support::EnvGuard::unset("GROK_SESSION_LIST_CONVERSATIONS");
        assert!(
    ...
    fn relax_rows_scopes_to_repo_and_requires_messages() {
        ...
        let repo_url = xvora_workspace::session::git::normalize_repo_url(this_repo).unwrap();
        ...
        let rows = relax_rows(
            ...
            vec![
                ...
                summary("theirs", Some("git@github.com:xvora-org/other.git"), 9),
            ],
            30,
            facet_registry(),
            HeadlessPolicy::Exclude,
