...
mod tests {
    ...
    fn conversations_lane_env_gating_matrix() {
        {
            let _off = xvora_test_support::EnvGuard::unset("XVORA_SESSION_LIST_CONVERSATIONS");
            assert!(!conversations_lane_enabled());
        }
        {
            let _on = xvora_test_support::EnvGuard::set("XVORA_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_enabled(), false);
        }
        {
            let _off = xvora_test_support::EnvGuard::set("XVORA_SESSION_LIST_CONVERSATIONS", "0");
            assert!(!conversations_lane_enabled());
        }
    }
    ...
    fn conversations_lane_active_truth_table() {
        ...
        {
            let _desktop =
                xvora_test_support::EnvGuard::set("XVORA_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_active(), false);
        }
        {
            let _chat = xvora_test_support::EnvGuard::set(XVORA_CHAT_MODE_ENV, "1");
            assert_eq!(
                conversations_lane_active(),
                false,
    ...
    fn parse_list_req_forces_kind_under_process_chat_mode_only() {
        ...
        .to_string();
        {
            let _off = xvora_test_support::EnvGuard::unset(XVORA_CHAT_MODE_ENV);
        ...
        {
            let _on = xvora_test_support::EnvGuard::set(XVORA_CHAT_MODE_ENV, "1");
            let req = parse_list_req(&raw).expect("parse");
            let parsed = ParsedMeta::parse(req.meta.as_ref());
