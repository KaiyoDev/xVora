//! AcpTerminalAdapter: implements `xvora-tools::TerminalBackend` using ACP gateway calls.
//!
//! This adapter enables bash tool execution over ACP (remote execution).
//! It translates xvora-tools' `TerminalBackend` trait into ACP protocol calls:
...
/// Wraps xvora-shell's ACP gateway to satisfy xvora-tools' TerminalBackend.
