...
/// Resolved absolute path to bash. On Unix uses the `xvora_config` shell
...
/// Delegates to the canonical implementation in `xvora-tools`.
...
/// xvora-pager which renders its own UI and doesn't need raw ANSI codes).
///
