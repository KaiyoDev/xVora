//! Probes the first-party env key (`GET {xvora_api_base_url}/api-key`) before `initialize` advertises `xvora.api_key`.
//! BYOK keys are never probed.
//!
//! The base URL is the caller's effective `endpoints.xvora_api_base_url`, so the probe hits the same host turn traffic uses.
//! That value comes from `GROK_XAI_API_BASE_URL` or `[endpoints] xvora_api_base_url`.
//!
...
///
/// `api_base_url` must be the endpoint the env key is actually sent to (`endpoints.xvora_api_base_url`), not a hardcoded public default.
...
///
/// `api_base_url` is the caller's effective `endpoints.xvora_api_base_url`, so the probe follows the same endpoint as turn traffic.
...
mod tests {
    ...
    async fn local_server_invalid_key_is_unusable() {
        ...
        let v = probe_xai_api_key_at_url("xvora-bad", &url, Duration::from_secs(2)).await;
    ...
    async fn local_server_blocked_key_is_unusable() {
        ...
        let v = probe_xai_api_key_at_url("xvora-blocked", &url, Duration::from_secs(2)).await;
        assert_eq!(v, ApiKeyProbeVerdict::Unusable);
    }
    ...
    async fn local_server_ok_key_is_usable() {
        ...
        let v = probe_xai_api_key_at_url("xvora-good", &url, Duration::from_secs(2)).await;
    ...
    async fn probes_joined_base_url_path() {
        ...
        let v = probe_xai_api_key("xvora-good", base, Duration::from_secs(2)).await;
        assert_eq!(v, ApiKeyProbeVerdict::Usable);
    }
    ...
    async fn retries_429_then_succeeds() {
        ...
        let v = probe_xai_api_key_at_url("xvora-retry", &url, Duration::from_secs(2)).await;
        assert_eq!(v, ApiKeyProbeVerdict::Usable);
    }
    ...
    async fn timeout_is_unknown_fail_open() {
        ...
        let v = probe_xai_api_key_at_url("xvora-slow", &url, Duration::from_millis(80)).await;
