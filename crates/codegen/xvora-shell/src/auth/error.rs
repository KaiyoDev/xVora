...
#[non_exhaustive]
pub enum AuthError {
    #[error("Not logged in. Run `xvora login`.")]
    NotLoggedIn,

    /// The token expired and no refresh authority is available.
    #[error("Token expired. Run `xvora login` to re-authenticate.")]
    TokenExpiredNoRefresh,

    /// Server rejected the token (401) with no recovery path.
    #[error("Authentication rejected by server. Run `xvora login` to re-authenticate.")]
    ...
    #[error("{message} Run `xvora login` to sign in with the required team.")]
    PinnedTeamMismatch { message: String },

    /// The cached API-key session was rejected because API-key auth is disabled.
    #[error("API-key auth is disabled by your administrator. Run `xvora login` to authenticate.")]
    ApiKeyAuthDisabled,
    ...
    pub(crate) fn user_message(self) -> Cow<'static, str> {
        match self {
            Self::RefreshTokenRejected => {
                "Your session has expired. Run `xvora login` to sign in again.".into()
            }
            Self::ClientRejected => {
                "Authentication is temporarily unavailable. Run `xvora login` if this persists."
                    .into()
            }
            Self::ProviderInteractiveRequired => provider_login_message(None),
            Self::Other => {
                "Authentication could not be refreshed. Run `xvora login` to sign in again.".into()
            }
        }
    }
}
