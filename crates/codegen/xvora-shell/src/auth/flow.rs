...
mod tests {
    ...
    use chrono::Utc;
    use std::path::Path;
    /// `os_error` and the reqwest classification are covered in `xvora-http`.
    ...
    /// That breaks unrelated auth tests, so classification is covered in `xvora-http`.
    #[test]
    ...
    async fn mint_session_noninteractive_uses_external_provider() {
        ...
        let cfg = GrokComConfig {
            auth_provider_command: Some("printf '%s' xvora-ext-token".to_string()),
        ...
        assert_eq!(auth.map(|a| a.key), Some("xvora-ext-token".to_string()));
    }
    #[tokio::test]
    ...
    async fn enterprise_oidc_never_uses_device_flow() {
        ...
        let xvora = GrokComConfig::default();
        assert!(xvora.oauth2.is_some() && xvora.oidc.is_none());
        assert!(cli_should_use_device(&xvora, LoginTransportOverride::ForceDevice).await);
    }
    #[test]
