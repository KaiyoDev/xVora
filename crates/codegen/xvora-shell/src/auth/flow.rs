...
pub async fn run_cli_login(
    ...
    // supports the device flow. Without this guard, `xvora login` on an
    ...
    // config was actually applied; `xvora setup` reports the no-config case.
...
mod tests {
    ...
    async fn enterprise_oidc_never_uses_device_flow() {
        // oidc=Some, oauth2=None: `xvora login` must use loopback, not device —
    ...
    /// CLI `xvora login` passes `on_stderr=None`; stderr must be inherited so
