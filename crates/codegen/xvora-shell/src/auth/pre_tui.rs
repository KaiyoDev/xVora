...
mod tests {
    ...
    async fn provider_echo_token_signs_in() {
        let (_dir, mgr) = isolated_manager(Some("printf '%s' xvora-ext-token"));
        let outcome = run_pre_tui_external_login_with(&mgr, "printf '%s' xvora-ext-token", false)
            .await
            .expect("provider must mint");
        match outcome {
            PreTuiLoginOutcome::SignedIn(auth) => {
                assert_eq!(auth.key, "xvora-ext-token");
                assert!(mgr.current().is_some());
            }
