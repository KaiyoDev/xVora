...
impl AuthManager {
    ...
    async fn auth_dispatch(self: &Arc<Self>) -> Result<GrokAuth, AuthError> {
        ...
        let result = match token_type {
            ...
            TokenType::LegacySession => {
                ...
                // assumption that a sibling process (`xvora login` from
    ...
    fn check_refresh_deferral(&self, reason: RefreshReason) -> Result<(), AuthError> {
        ...
        // Dark wake (see `system_power::PowerState` for the canonical
...
/// Build a refreshing [`ApiKeyProvider`](xvora_tools::types::ApiKeyProvider)
...
/// [`current_api_key_async`](xvora_tools::types::ApiKeyProvider::current_api_key_async),
