...
pub struct GrokComConfig {
    ...
    /// Admin kill switch: when `Some(true)`, the `xvora.api_key` auth method is neither advertised nor accepted.
...
impl GrokComConfig {
    /// Pinning a team (`force_login_team_uuid`) disables `xvora.api_key` auth: team membership can't be verified from a bare API key.
        ...
        let oauth2 = if oidc.is_some() {
            ...
            Some(
                OAuth2ProviderConfig::from_env().unwrap_or_else(|| OAuth2ProviderConfig {
                    issuer: xvora_oauth2_issuer().to_owned(),
