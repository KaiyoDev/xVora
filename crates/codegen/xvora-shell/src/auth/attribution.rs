...
//!
//! [`xvora_sampler`] is intentionally decoupled from this crate. It
//! invokes the trait [`xvora_sampler::Auth401AttributionCallback`] at
//! its six 401 arms; this module provides [`ShellAttribution`], the
//! concrete impl that the shell wires into
//! [`xvora_sampler::SamplerConfig::attribution_callback`] at every
...
impl ShellAttribution {
    ...
    /// [`xvora_sampler::SamplerConfig::attribution_callback`].
    ///
    /// (Returns `Arc<dyn Trait>` rather than `Self` because the
    /// `xvora_sampler::SamplerConfig` field expects exactly that;
    ...
    /// `Arc<dyn xvora_tools::Auth401AttributionCallback>` for the
...
impl Auth401AttributionCallback for ShellAttribution {
    fn record_401(&self, consumer: SamplingConsumer, sent_bearer_prefix: Option<&str>) {
        ...
        // to `xvora_sampler::SENT_BEARER_PREFIX_LEN` characters
...
/// in `xvora-tools` emits a 401 attribution event through this
...
/// All 401 emit sites in `xvora-shell` go through this helper -- the
...
mod tests {
    ...
    /// `ShellAttribution` implements `xvora_tools::Auth401AttributionCallback`
