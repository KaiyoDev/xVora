...
/// Tool-side hook: each tool client (image_gen, video_gen, web_search) in `xvora-tools` emits a 401 attribution event through this
...
/// All 401 emit sites in `xvora-shell` go through this helper.
