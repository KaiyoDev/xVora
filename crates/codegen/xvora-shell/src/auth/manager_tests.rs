...
async fn cached_api_key_session_rejected_when_api_key_auth_disabled() {
    ...
    assert_eq!(
        ...
        Some("xvora-cached-key".to_string()),
...
/// With no OAuth session, voice/tools fall back to env or the auth.json `xvora::api_key`.
#[tokio::test]
#[serial_test::serial]
...
fn manual_auth_reason_maps_terminal_and_skips_non_forcing() {
    ...
    use telemetry::events::ManualAuthReason as R;
...
async fn manual_auth_capture_attributes_and_recorder_debounces() {
    ...
    use telemetry::events::{AuthTokenKind, ManualAuthSurface};
    let auth = GrokAuth {
        key: "dead-token".into(),
        user_id: "user-1".into(),
        auth_mode: AuthMode::Oidc,
