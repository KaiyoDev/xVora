...
pub(crate) fn assemble_hooks(
    ...
    let (mut specs, mut errors) =
        xvora_hooks::config::parse_hooks_from_config_layers(config_layers);
    ...
    let (file_specs, file_errors) =
        xvora_hooks::discovery::collect_specs_from_sources(&global_sources, &project_sources);
    specs.extend(file_specs);
    errors.extend(file_errors);

    (
        xvora_hooks::discovery::registry_from_specs_deduped(specs),
        errors,
    )
}
...
mod tests {
    use super::*;
    use xvora_hooks::config::HookProvenance;
...
"#,
        ...
        let (specs, errors) = xvora_hooks::config::parse_hooks_from_config_layers(&layers);
        assert!(errors.is_empty(), "errors: {errors:?}");
