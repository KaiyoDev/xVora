...
pub use xvora_config_types::{McpConfig, RelaySyncConfig};
...
/// Walks from cwd up to the git repo root checking `.xvora/config.toml` at each level.
/// Project-scoped `.xvora/config.toml` entries override global `~/.xvora/config.toml`
...
/// Scope an MCP server resolves at: project when any project-scoped `.xvora/config.toml` defines it, otherwise user.
...
/// Load MCP servers with project-scoped overrides from `.xvora/config.toml`.
///
/// Merge strategy:
/// 1. Load MCP servers from global `~/.xvora/config.toml`
/// 2. Walk from git repo root down to `cwd`, loading `.xvora/config.toml` at each level (matching skills and AGENTS.md discovery)
        ...
        if let Ok(root) = crate::config::load_config_file(config_path) {
            ...
            if !project_servers.is_empty() {
                tracing::info!(
                    ...
                    "Loaded project-scoped MCP servers from .xvora/config.toml"
                );
...
pub(crate) fn mcp_preferences_path() -> PathBuf {
    config::xvora_home().join("mcp_preferences.json")
}
...
/// Upsert an MCP server entry in `~/.xvora/config.toml`.
...
///
/// Same behavior as [`save_mcp_server_config`] but targets an explicit config file, e.g. a project-scoped `.xvora/config.toml`.
...
/// Delete an MCP server entry from `~/.xvora/config.toml`.
///
...
///
/// Same behavior as [`delete_mcp_server_config`] but targets an explicit config file, e.g. a project-scoped `.xvora/config.toml`.
...
/// The transport-less case is steered to `disabled_mcp_servers`, xvora's real disable mechanism.
    ...
    let message = if !has_command && !has_url {
        format!(
            "`mcp_servers.{name}` has no transport. To run it, set `command = \"...\"` or \
             ...
             See ~/.xvora/docs/user-guide/07-mcp-servers.md"
        )
    } else {
        format!(
                    ...
                    problems.push(McpServerConfigProblem {
                        ...
                        message: format!(
                            "`mcp_servers.{name}` has an unrecognized field `{field}`; it is \
                             ignored. See ~/.xvora/docs/user-guide/07-mcp-servers.md"
                        ),
                    });
                }
                if config.enabled
                    ...
                    problems.push(McpServerConfigProblem {
                        ...
                        message: format!(
                            "`mcp_servers.{name}` is enabled but its `{field}` is blank. \
                             ...
                             off. See ~/.xvora/docs/user-guide/07-mcp-servers.md"
                        ),
                    });
                    continue;
                }
...
///
/// Overlays project-scoped `.xvora/config.toml` files from `cwd` up to the repo root onto the user-tier config, nearest definition winning.
...
/// MCP config problems across the same layers as [`load_mcp_server_configs_with_project`], for `xvora inspect`.
...
/// Names `xvora mcp enable`/`disable` may target.
...
fn config_path() -> PathBuf {
    crate::util::xvora_home::xvora_home().join("config.toml")
}

/// Path to the user-level config file (`~/.xvora/config.toml`).
pub fn user_config_path() -> PathBuf {
    config_path()
}

/// Path to a project-level config file (`<dir>/.xvora/config.toml`).
pub fn project_config_path(dir: &std::path::Path) -> PathBuf {
    dir.join(".xvora").join("config.toml")
}
...
/// Overrides `[cli] session_registry`; usable before `~/.xvora/config.toml` exists.
...
"#;
    ...
    fn skills_config_parses_paths_and_ignore() {
        let root = toml::from_str::<TomlValue>(
            r#"
[skills]
paths = ["~/.xvora/skills", "~/.xvora/skills/special/SKILL.md"]
ignore = ["~/.xvora/skills/noisy/SKILL.md"]
"#,
        )
        .unwrap();
        ...
        assert_eq!(
            cfg.paths,
            vec!["~/.xvora/skills", "~/.xvora/skills/special/SKILL.md"]
        );
        assert_eq!(cfg.ignore, vec!["~/.xvora/skills/noisy/SKILL.md"]);
    }
...
"#,
    ...
    async fn enable_unstick_only_touches_nearest_project_definition() {
        ...
        std::fs::create_dir_all(nested.join(".xvora")).unwrap();
...
enabled = false
"#;
        let ancestor = tmp.path().join(".xvora").join("config.toml");
        let nearer = nested.join(".xvora").join("config.toml");
    ...
    async fn restore_mcp_server_enabled_after_enable_scopes_tiers() {
        ...
        // Do not call save_mcp_server_enabled_in (that reads ambient config_path / xvora_home)
