...
/// `xvora-telemetry`; re-exported here so existing call sites continue
/// to work.
...
mod tests {
    ...
    // definition in `xvora_workspace::permission::claude_compat`.

    #[test]
    fn resolve_launch_yolo_policy_pin_neutralizes_requested_bypass() {
        let warning = xvora_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
