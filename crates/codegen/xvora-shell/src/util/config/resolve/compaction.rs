...
pub(crate) const ENV_AUTO_COMPACT_THRESHOLD_PERCENT: &str = "XVORA_AUTO_COMPACT_THRESHOLD_PERCENT";
...
///   1. env `XVORA_AUTO_COMPACT_THRESHOLD_PERCENT`
...
///      (populated from `xvora_models[i].auto_compact_threshold_percent`;
...
///      (populated from `xvora_settings.auto_compact_threshold_percent`)
...
const ENV_COMPACTION_WALL_CLOCK_BUDGET_SECS: &str = "XVORA_COMPACTION_WALL_CLOCK_SECS";

/// Resolve the per-compaction wall-clock budget (seconds). Precedence: env
/// `XVORA_COMPACTION_WALL_CLOCK_SECS` > remote settings global
...
mod compaction_wall_clock_budget_tests {
    ...
    // Assumes XVORA_COMPACTION_WALL_CLOCK_SECS is unset in the test env.
    #[test]
