...
/// [`xvora_config::env_bool`] parser (`XVORA_TOOLS_FIND_BFS` /
...
/// [`xvora_tools::computer::local::SearchShadowConfig`] on the local terminal
/// backend.
...
/// the shared [`xvora_config::env_bool`] via `BoolFlag`). The secs env var
...
fn resolve_ask_user_question_timeout_secs(
    ...
    resolve_ask_user_question_timeout_secs_from_tiers(
        ...
        xvora_tools::implementations::xvora::ask_user_question::response_timeout_env_secs(),
...
) -> xvora_tools::implementations::xvora::ask_user_question::AskUserQuestionParams {
    let requirements = crate::config::load_merged_requirements();
    let layers = match crate::config::ConfigLayers::load() {
        Ok(l) => Some(l),
        Err(e) => {
...
mod ask_user_question_timeout_tests {
    ...
    use xvora_tools::implementations::xvora::ask_user_question::RESPONSE_TIMEOUT_ENV;
