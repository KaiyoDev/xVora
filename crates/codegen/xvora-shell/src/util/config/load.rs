...
pub fn load_relay_sync_enabled_sync() -> bool {
    toml_bool_sync(Some("XVORA_RELAY_SYNC_ENABLED"), "relay", "enabled")
}
...
/// 1. Nearest project `.xvora/config.toml` with `[permission]` section (from cwd upward)
/// 2. Global `~/.xvora/config.toml` `[permission]` section
///
