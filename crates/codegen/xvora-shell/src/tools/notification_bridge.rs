//! Translates `xvora-tools` `ToolNotification` events into `xvora-shell`'s native systems (ACP gateway, hunk tracker, file state tracker).
...
pub(crate) struct NotificationBridgeConfig {
    ...
    /// xvora-build leaves this `false`: its exit-plan tool result already informs the model, and a deferred reminder would arrive stale.
