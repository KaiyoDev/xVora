//! Notification bridge: translates `xvora-tools` `ToolNotification` events
//! into `xvora-shell`'s native systems (ACP gateway, hunk tracker, file state tracker).
...
pub struct NotificationBridgeConfig {
    ...
    pub auto_wake_delivered: xvora_tools::reminders::task_completion::AutoWakeDeliveredIds,
...
mod tests {
    ...
    async fn bash_output_chunk_persists_and_broadcasts_one_event_id() {
        ...
        let notification = ToolNotification::BashOutputChunk(
            xvora_tools::notification::types::BashOutputChunk {
    ...
    async fn scheduled_task_removed_is_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskRemoved(
            xvora_tools::notification::types::ScheduledTaskRemoved {
                task_id: "loop-1".into(),
            },
        );
        let mut offsets = HashMap::new();
    ...
    async fn plan_mode_entered_emits_current_mode_update_plan() {
        ...
        let notification = ToolNotification::PlanModeEntered(
            xvora_tools::notification::types::PlanModeEntered {
