...
use crate::auth::{XaiAuth, read_auth_json};
...
/// `ConfigUpdate::Skills`) and `xvora inspect` (via the `crate::config`
...
mod tests {
    ...
    async fn reloader_detects_new_auth_key() {
        ...
        assert!(
            matches!(update, ConfigUpdate::Auth(a) if a.key == "new-key"), // a is Box<XaiAuth>, Deref coercion
