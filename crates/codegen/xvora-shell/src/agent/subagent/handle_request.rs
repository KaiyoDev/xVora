...
pub(crate) async fn handle_subagent_request(
    ...
    }
    {
        ...
        if effective_runtime.isolation == SubagentIsolationMode::None
            && definition.isolation
                == Some(xvora_agent::config::IsolationMode::Worktree)
        {
    ...
    } else if effective_runtime.isolation != tool_types::SubagentIsolationMode::None
    {
        ...
        match tokio::task::spawn_blocking(move || {
                let mut builder = fast_worktree::WorktreeBuilder::new(
                        &source_clone,
                        &dest,
                    )
                    .working_tree_mode(
                        fast_worktree::WorkingTreeMode::PreserveWorkingTree,
                    )
    ...
    {
        use xvora_tools::implementations::xvora::task::MAX_SUBAGENT_DEPTH;
    ...
    let child_cwd_abs = xvora_paths::AbsPathBuf::new(child_cwd)
        .unwrap_or_else(|_| {
    ...
    let initial_child_tokens = chat_state::estimate_conversation_tokens(
        &forked_conversation,
    );
    ...
    let attribution_callback: Option<xvora_sampler::SharedAttributionCallback> = effective_sampling_config
    ...
    if let Some(scope) = agent_memory_scope {
        ...
        if memory_md.is_file() && let Ok(content) = std::fs::read_to_string(&memory_md) {
            ...
            let truncated: String = content
                ...
                .collect::<Vec<_>>()
                .join("\n");
            let truncated = xvora_tools::util::truncate::truncate_str(
                    &truncated,
                    MAX_BYTES,
                )
                .to_string();
    ...
    xvora_telemetry::session_ctx::log_event(xvora_telemetry::events::SubagentCompleted {
        subagent_id: request.id.clone(),
        parent_session_id: request.parent_session_id.clone(),
        outcome,
