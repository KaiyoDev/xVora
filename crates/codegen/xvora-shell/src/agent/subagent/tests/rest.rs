...
fn normalize_forked_context_strips_project_layout() {
    ...
    let (conv, _) = xvora_subagent_resolution::context::normalize_forked_context(
        items,
    );
...
fn end_to_end_normalized_conversation_shape() {
    ...
    let (mut conv, prefix_len) = xvora_subagent_resolution::context::normalize_forked_context(
        parent_conv,
    );
    assert_eq!(prefix_len, 2);
...
fn last_user_message_is_task_after_normalization() {
    ...
    let (mut conv, _) = xvora_subagent_resolution::context::normalize_forked_context(
        parent_conv,
    );
            ...
            if let ConversationItem::User(u) = i {
                u.content
                    ...
                    .any(|p| {
                        matches!(
                    p,
                    xvora_sampling_types::conversation::ContentPart::Text { text } if text.contains("<background_context>")
                )
                    })
            } else {
                false
...
fn resume_identity_does_not_gate_on_model() {
    ...
    assert!(
            xvora_subagent_resolution::validate_resume_identity(
                "general-purpose",
                None,
                &source,
            )
...
async fn read_parent_sampling_config_keeps_auto_when_catalog_has_slug_key_only() {
    ...
    ctx.parent_chat_state
        .as_ref()
        .unwrap()
        .update_sampling_config(xvora_sampling_types::SamplingConfig {
...
async fn read_parent_sampling_config_live_never_strips_a_fallback_key() {
    ...
    assert_eq!(config.api_key.as_deref(), Some("xvora-env-fallback"));
}
...
async fn read_parent_sampling_config_fallback_never_strips_a_fallback_key() {
    ...
    ctx.sampling_config.model = "grok-4.5".to_string();
    ctx.sampling_config.base_url = "https://api.x.ai/v1".to_string();
    ctx.sampling_config.api_key = Some("xvora-env-fallback".to_string());
    let (config, _) = read_parent_sampling_config(&ctx).await;
...
fn non_cursor_persona_injected_as_system_reminder() {
    use xvora_sampling_types::conversation::{ConversationItem, SyntheticReason};
