...
fn subagent_bypass_permission_mode_gated_by_policy_pin() {
    ...
    const PIN: &str = xvora_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    assert_eq!(
...
fn subagent_inherits_parent_lsp_via_context() {
    let parent: std::sync::Arc<dyn xvora_tools::implementations::lsp::LspBackend> = Arc::new(
        DummyLspDispatch,
    );
...
fn inject_subagent_completed_prompt_sends_prompt_and_marks_delivered() {
    ...
    let auto_wake = xvora_tools::reminders::task_completion::AutoWakeDeliveredIds::default();
...
fn dummy_tracker(
    ...
    let cwd = xvora_paths::AbsPathBuf::new(PathBuf::from("/tmp")).unwrap();
    let fs: Arc<dyn xvora_workspace::file_system::AsyncFileSystem> = Arc::new(
...
fn explicit_override_takes_precedence_over_role() {
    ...
    let role = xvora_subagent_resolution::config::SubagentRole {
    ...
    assert_eq!(
        resolved.capability_mode, Some(tool_types::SubagentCapabilityMode::All)
    );
}
#[test]
fn role_default_used_when_no_explicit_override() {
    ...
    assert_eq!(
        resolved.capability_mode, Some(tool_types::SubagentCapabilityMode::ReadOnly)
    );
}
#[test]
...
fn partial_override_fills_from_role() {
    ...
    assert_eq!(resolved.model.as_deref(), Some("explicit-model"));
    assert_eq!(
        resolved.capability_mode, Some(tool_types::SubagentCapabilityMode::Execute)
    );
}
#[test]
...
fn persona_resolved_from_config() {
    ...
    personas
        .insert(
            ...
            xvora_subagent_resolution::config::SubagentPersona {
...
fn model_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_model = xvora_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
...
fn reasoning_effort_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_re = xvora_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
    let r = resolve_effective_overrides(
    ...
    if let ConversationItem::User(ref u) = ctx.conversation[1] {
        let text: String = u
            ...
            .filter_map(|p| match p {
                xvora_sampling_types::conversation::ContentPart::Text { text } => {
                    Some(text.as_ref())
                }
                _ => None,
            })
...
fn forked_initial_context_inherits_parent_across_reasoning() {
    ...
    let items = vec![
        ...
        ConversationItem::Reasoning(xvora_sampling_types::synthesized_reasoning_item("deliberating",)),
...
fn resume_vs_fork_helper_shapes_differ() {
    ...
    assert!(
        ...
        xvora_sampling_types::conversation::ContentPart::Text { text } if text
...
fn verbatim_fork_falls_back_to_summary_when_oversize() {
    use xvora_sampling_types::conversation::{ContentPart, ConversationItem};
    let items = vec![
        ConversationItem::system("parent system"),
                    ...
                    ConversationItem::User(u) => {
                        Some(
                            u
                                ...
                                .iter()
                                .filter_map(|p| match p {
                                    xvora_sampling_types::conversation::ContentPart::Text {
                                        text,
                                    } => Some(text.as_ref()),
                                    _ => None,
                                })
...
fn normalize_forked_context_empty_parent() {
    ...
    let (conv, prefix_len) = xvora_subagent_resolution::context::normalize_forked_context(
        items,
    );
...
fn test_sampling_config(model_slug: &str) -> xvora_sampling_types::SamplingConfig {
