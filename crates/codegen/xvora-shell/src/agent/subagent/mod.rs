...
//!
//! Lifecycle state and command scheduling live in the shared `xvora-tools` coordinator actor.
...
pub(crate) struct SubagentSpawnContext {
    ...
    pub inherited_tool_overrides: Option<xvora_sampling_types::ToolOverrides>,
    pub yolo_mode: bool,
...
pub(crate) fn strip_ask_user_question_tool(tools: &mut Vec<xvora_sampling_types::ToolSpec>) {
...
fn resolve_agent_definition(
    ...
    let cli_agents = ctx
        ...
        .unwrap_or_default();
    let resolution_context = xvora_subagent_resolution::DefinitionResolutionContext {
        cwd: &ctx.parent_cwd,
...
fn available_agent_names(ctx: &SubagentSpawnContext) -> Vec<String> {
    let cli_agents = ctx
        ...
        .map(|config| config.cli_agents.as_slice())
        .unwrap_or_default();
    xvora_subagent_resolution::available_agent_names(
        &xvora_subagent_resolution::DefinitionResolutionContext {
            cwd: &ctx.parent_cwd,
            plugins: ctx.plugin_registry.as_deref(),
            cli_agents,
            toggles: &ctx.subagent_toggle,
...
pub(crate) fn validate_subagent_type(
    ...
    let context = xvora_subagent_resolution::DefinitionValidationContext {
        cwd: &ctx.parent_cwd,
        plugins: ctx.plugin_registry.as_deref(),
    ...
    match xvora_subagent_resolution::validate_agent_name(subagent_type, &context) {
        Ok(()) => SubagentValidateTypeOutcome::Ok,
        Err(xvora_subagent_resolution::ResolutionError::Unknown { available, .. }) => {
            SubagentValidateTypeOutcome::Unknown { available }
        }
        Err(xvora_subagent_resolution::ResolutionError::Disabled { .. }) => {
            SubagentValidateTypeOutcome::Disabled
        }
        Err(xvora_subagent_resolution::ResolutionError::NotAllowed { allowed, .. }) => {
            SubagentValidateTypeOutcome::NotAllowed { allowed }
        }
        Err(
            xvora_subagent_resolution::ResolutionError::PersonaResolution(_)
            | xvora_subagent_resolution::ResolutionError::ResumeValidation(_),
...
fn gate_subagent_type(
    ...
    match xvora_subagent_resolution::gate_agent_definition(subagent_type, &resolution_context) {
        Ok(()) => SubagentValidateTypeOutcome::Ok,
        Err(xvora_subagent_resolution::ResolutionError::Disabled { .. }) => {
            SubagentValidateTypeOutcome::Disabled
        }
        ...
        Err(
            xvora_subagent_resolution::ResolutionError::Unknown { .. }
            | xvora_subagent_resolution::ResolutionError::PersonaResolution(_)
            | xvora_subagent_resolution::ResolutionError::ResumeValidation(_),
        ) => SubagentValidateTypeOutcome::ValidationUnavailable,
    }
...
pub(crate) fn subagent_harness_flavor_is_representable(agent_type: &str) -> bool {
    xvora_subagent_resolution::subagent_harness_flavor_is_representable(agent_type)
}
...
fn resolve_subagent_toolset(
    ...
    xvora_subagent_resolution::apply_harness_toolset(
        subagent_type,
        &resolution_context,
        definition,
    );
