...
pub(crate) struct SubagentTracker {
    ...
    pub color: Option<xvora_agent::config::AgentColor>,
}
...
pub(crate) struct SubagentSpawnContext {
    ...
    pub auth: Option<crate::auth::XaiAuth>,
    ...
    pub web_fetch_config: xvora_tools::implementations::xvora::web_fetch::WebFetchConfig,
    /// Image generation config (parent-inherited).
    pub image_gen_config: xvora_tools::implementations::xvora::image_gen::ImageGenConfig,
    /// Resolved config for video generation.
    pub video_gen_config: xvora_tools::implementations::xvora::video_gen::VideoGenConfig,
    ...
    pub parent_tool_snapshot: Option<Vec<xvora_sampling_types::ToolSpec>>,
...
pub(crate) fn resume_inherited_prefix_len(
    conversation: &[xvora_sampling_types::conversation::ConversationItem],
) -> usize {
...
struct InitialContext {
    ...
    conversation: Vec<xvora_sampling_types::conversation::ConversationItem>,
...
fn forked_initial_context(
    mut items: Vec<xvora_sampling_types::conversation::ConversationItem>,
) -> InitialContext {
    ...
    let (conversation, prefix_len) =
        xvora_subagent_resolution::context::normalize_forked_context(items);
    if prefix_len < 2 {
        return InitialContext {
            source: InitialContextSource::New,
...
fn conversation_tail_is_complete(
    items: &[xvora_sampling_types::conversation::ConversationItem],
) -> bool {
    use xvora_sampling_types::conversation::ConversationItem;
    matches!(
...
fn verbatim_or_normalize_fork(
    items: Vec<xvora_sampling_types::conversation::ConversationItem>,
    ...
    let (conversation, prefix_len) =
        xvora_subagent_resolution::context::normalize_forked_context(filtered);
    InitialContext {
        source: InitialContextSource::Forked,
        copy_error: None,
        prefix_len: Some(prefix_len),
...
fn filter_pool_by_inheritance(
    ...
    use xvora_agent::config::McpInheritance;
    match inheritance {
...
fn resolve_agent_definition(
    ...
    let mut def = xvora_agent::discovery::by_name_in_cwd_with_plugins(
        subagent_type,
        &ctx.parent_cwd,
...
fn inject_subagent_completed_prompt(
    ...
    request: &SubagentRequest,
    auto_wake_delivered: &Option<xvora_tools::reminders::task_completion::AutoWakeDeliveredIds>,
