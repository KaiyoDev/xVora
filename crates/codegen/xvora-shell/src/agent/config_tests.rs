...
fn default_models_dual_endpoint_routing() {
    ...
    for (model_id, entry) in default_model_entries(&endpoints) {
        ...
        assert_eq!(
            api_key_creds.base_url, endpoints.xvora_api_base_url,
...
fn enforce_disable_api_key_auth_blocks_first_party_only() {
    ...
    assert_eq!(creds.auth_type, AuthType::ApiKey);
    assert_eq!(creds.api_key.as_deref(), Some("xvora-secret"));
    let mut creds = api_key_creds("https://api.x.ai/v1");
...
fn try_resolve_model_credentials_swaps_first_party_own_key_under_kill_switch() {
    ...
    assert_eq!(creds.api_key.as_deref(), Some("xvora-model-key"));
    enforce_disable_api_key_auth(&mut creds, true, Some("session-jwt"));
    assert_eq!(
        creds.auth_type,
...
/// Without this, `--reasoning-effort` is silently dropped in xvora-shell/src/agent/models.rs:857 for any BYOK Claude config.
#[test]
...
fn e2e_config_toml_model_overrides_default() {
    ...
    unsafe { std::env::set_var("XAI_API_KEY", "xvora-key") };
    let sampling = resolve_sampling(model, None);
    assert_eq!(sampling.base_url, "https://inference.example.com/v1");
...
fn e2e_default_model_with_external_api_key_routes_to_api_xai() {
    ...
    let model = models
        .get(crate::models::default_model())
        .expect("default model should exist");
    unsafe { std::env::set_var("XAI_API_KEY", "xvora-external-key") };
    let sampling = resolve_sampling(model, None);
    assert_eq!(sampling.api_key.as_deref(), Some("xvora-external-key"));
    assert_eq!(
...
fn e2e_enterprise_endpoints_plus_partial_model_override() {
    ...
    let (_, models) = resolve_models_from_toml(
        &format!(
            ...
            xvora_api_base_url = "https://enterprise-api.acme.com/v1"
...
fn e2e_enterprise_endpoints_only_no_model_override() {
    ...
    assert_eq!(
        model.api_base_url.as_deref(),
        Some("https://enterprise-api.acme.com/v1"),
        "default model should use enterprise xvora_api_base_url"
    );
}
...
/// INVARIANT: auxiliary-service resolvers resolve to the cli-chat-proxy, never `xvora_api_base_url`.
...
fn aux_endpoints_resolve_to_proxy_never_inference() {
    ...
    let inference = "https://inference.acme-corp.example/xvora/v1";
    let cfg = EndpointsConfig {
        xvora_api_base_url: inference.to_string(),
    ...
    assert_eq!(cfg.xvora_api_base_url, inference);
...
/// REGRESSION: the managed-config URL never follows `xvora_api_base_url` through the full loader `Config::new_from_toml_cfg`.
...
fn config_warns_on_section_typo() {
    let raw: toml::Value = toml::from_str(
        r#"
            [endpoint]
            deployment_key = "xvora-token-test"
        "#,
    )
    .unwrap();
