...
/// ([`xvora_fsnotify::shared`]) run their event loops on a runtime that outlives
...
pub async fn run_stdio_agent(
    ...
    // Log the client that launched us (set by xvora-desktop when spawning `xvora agent stdio`).
    // This appears early in unified.jsonl and is extremely useful for auth diagnostics.
    if let Ok(version) = std::env::var("XVORA_CLIENT_VERSION") {
        crate::unified_log::info(
            "XVORA_CLIENT_VERSION",
            None,
    ...
    let result = local_set
        .run_until(async move {
            ...
            // `xvora agent stdio` is a local/interactive entrypoint (spawned by
...
async fn run_headless_inner(
    ...
    register_fs_watch_runtime();
    xvora_telemetry::unified_log::set_version(xvora_version::VERSION);
    // `xvora agent [headless]` serves non-TUI automation; stamp proxy requests
    // as headless. IDE-facing `xvora agent stdio` stays interactive.
    ...
    let (mut auth, did_browser_flow) = if no_browser {
        ...
        match auth_manager.current() {
            ...
            None if auth_manager.is_expired() => {
                anyhow::bail!("Session expired. Please run 'xvora login' to re-authenticate.")
            }
            None => anyhow::bail!("No cached credentials found. Run `xvora login`."),
        }
...
///
/// * `relay_on_demand == false` (default — explicit `xvora agent leader`
...
pub async fn run_leader(
    ...
    local_set
        .run_until(async move {
            ...
            // `xvora agent leader` (devbox / systemd) has no local IPC clients
            ...
            // Gated on user_xvora_home() so a cwd-relative .xvora/auth.json is never
            // read as the user auth store when no home resolves.
            let initial_auth_key_hash = xvora_config::user_xvora_home()
                ...
                while let Some(update) = config_update_rx.recv().await {
                    match update {
                        ...
                        ConfigUpdate::ModelsCacheChanged => {
                            // External write to ~/.xvora/models_cache.json
...
mod tests {
    ...
    /// `xvora agent leader` (devbox/systemd — no local IPC clients,
        ...
        local
            .run_until(async {
                let slot = spawn_leader_relay(
                    config,
                    false, // eager: explicit `xvora agent leader` invocation
                    demand_rx,
                    ws_to_agent_tx,
                    agent_to_ws_tx.clone(),
                    cancel.clone(),
