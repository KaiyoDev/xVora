...
async fn attempt_auth_recovery(
    ...
    let Some(ref am) = config.auth_manager else {
        teprintln!(
            "Authentication required. Run `xvora login` to re-authenticate, or set XVORA_NO_AUTH=1 to skip authentication."
        );
        cancel.cancel();
        return false;
    };
...
mod tests {
    ...
    fn for_session_builds_only_for_xai_issuer() {
        ...
        let xvora = GrokAuth {
            ...
            ..test_auth("xvora-bearer")
        };
        assert!(xvora.is_xai_auth(), "precondition: is_xai_auth");
        assert!(builds(&xvora));
