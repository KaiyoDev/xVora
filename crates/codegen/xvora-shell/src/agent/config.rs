...
impl EndpointsConfig {
    ...
    /// NEVER falls back to `xvora_api_base_url`: that is the inference endpoint (API-key auth) only.
    ...
    /// Feedback endpoint, an auxiliary service, so it defaults to the cli-chat-proxy, never `xvora_api_base_url`.
    pub(crate) fn resolve_feedback_base_url(&self) -> String {
        blank_as_unset(&self.feedback_base_url).unwrap_or_else(|| self.proxy_url())
    }
    /// Trace upload endpoint, an auxiliary service, so it defaults to the cli-chat-proxy, never `xvora_api_base_url`.
    ...
    /// Uses the proxy default (not the `xvora_api_base_url` fallback) so telemetry reports to xAI even when inference is overridden.
...
impl Default for EndpointsConfig {
    fn default() -> Self {
        Self {
            ...
            xvora_api_base_url: std::env::var("GROK_XAI_API_BASE_URL")
    ...
    entries
        ...
        .map(|m| {
            ...
            let config = ModelEntryConfig {
                ...
                api_base_url: Some(endpoints.xvora_api_base_url.clone()),
                name: m.name,
...
/// codebase_indexing = ["/Users/*/xvora*", "!/Users/*/old-*"]           # globs, ! to exclude
/// ```
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(untagged)]
