...
impl EndpointsConfig {
    ...
    /// Managed deployment-config URL (`xvora setup`): explicit `managed_config_url`,
...
pub use xvora_config_types::{BoolFlag, ConfigSource, LazinessDetectorPerModelConfig, Resolved};
...
impl PluginsConfig {
    ...
    pub fn to_discovery_config(&self) -> xvora_agent::plugins::discovery::DiscoveryConfig {
...
pub struct Config {
    ...
    /// Warnings from `[model.*]` parsing; surfaced by `xvora inspect`.
    #[serde(skip)]
    ...
    /// `[marketplace]` — also read by `xvora_plugin_marketplace::load_sources()`.
    #[serde(default, skip_serializing)]
    ...
    #[serde(skip)]
    pub subagent_roles:
        std::collections::HashMap<String, xvora_subagent_resolution::config::SubagentRole>,
    #[serde(skip)]
    ...
    pub zdr_video_output_s3:
        Option<xvora_tools::implementations::xvora::video_gen::ZdrVideoOutputS3Config>,
...
impl CliAgentOverrides {
    ...
    pub fn apply_to_definition(&self, def: &mut xvora_agent::config::AgentDefinition) {
    ...
    pub fn apply_to_subagent_definition(&self, def: &mut xvora_agent::config::AgentDefinition) {
...
impl Config {
    ...
    pub(crate) fn resolve_doom_loop_recovery(
        &self,
    ) -> Option<xvora_sampling_types::DoomLoopRecoveryPolicy> {
        use xvora_sampling_types::DoomLoopRecoveryPolicy as Policy;
        let remote = self
            .remote_settings
            .as_ref()
    ...
    pub(crate) fn resolve_image_edit(&self) -> Resolved<bool> {
        use xvora_tools::implementations::xvora::IMAGE_EDIT_TOOL_NAME;
...
/// by `xvora setup`) sit under the env vars, requirements pins are applied on
...
pub(crate) fn resolve_external_otel_config_with(
    ...
    let file_cfg: Option<xvora_telemetry::external::ExternalOtelFileConfig> = effective_config
        ...
        .map(|t| xvora_telemetry::external::ExternalOtelFileConfig {
    ...
    let getenv_pinned = |name: &str| -> Option<String> {
        let pin = match name {
            xvora_telemetry::external::config::ENV_MASTER_SWITCH => req_enabled,
...
/// Effective catalog provider id for a model (`xai`, `openai`, …).
///
/// Prefers the stored `info.provider`, else re-infers from URLs/slug so
/// credential fallthrough stays correct even if ensure was skipped.
pub fn model_provider_id(model: &ModelEntry) -> String {
    model
        .info
        .provider
        .as_deref()
...
/// True when this catalog entry is an xAI-provider model (OAuth / `XAI_API_KEY`
/// fallthrough applies). Other providers require their own BYOK credentials.
pub fn is_xai_provider_model(model: &ModelEntry) -> bool {
...
pub fn resolve_credentials(model: &ModelEntry, session_key: Option<&str>) -> ResolvedCredentials {
    let info = model.info();
    let provider = model_provider_id(model);
    ...
    tracing::debug!(
        model = % info.model, provider = % provider, auth_type = ? auth_type, "resolved credentials"
    );
    ResolvedCredentials {
        api_key,
        base_url,
...
pub fn inject_url_derived_headers(
    ...
    if crate::util::is_cli_chat_proxy_url(base_url) {
        headers
            ...
            .or_insert_with(|| "xvora-cli".to_string());
        headers
...
"#;
    ...
    fn resolve_credentials_empty_env_key_falls_through_to_session() {
        ...
        // First-party xAI URL → session fallthrough allowed.
    ...
    fn resolve_credentials_sets_auth_type() {
        use chat_state::AuthType;
        // xAI endpoint: session token is valid fallthrough.
    ...
    #[test]
    #[serial]
    fn resolve_credentials_non_xai_ignores_session_and_xai_env() {
        use crate::agent::auth_method::{LEGACY_XAI_API_KEY_ENV_VAR, XAI_API_KEY_ENV_VAR};
        use chat_state::AuthType;
        use xvora_test_support::EnvGuard;
    ...
    fn user_override_parses_compaction_at_tokens_from_toml() {
        use xvora_sampling_types::CompactionAtTokens;
        let dm = crate::models::default_model();
        let raw_config: toml::Value = toml::from_str(&format!(
            r#"
            [model."{dm}"]
    ...
    fn user_override_parses_compactions_remaining_from_toml() {
        use xvora_sampling_types::CompactionsRemaining;
        let dm = crate::models::default_model();
        let raw_config: toml::Value = toml::from_str(&format!(
            r#"
            [model."{dm}"]
    ...
    /// xvora-shell/src/agent/models.rs:857 for any BYOK Claude config.
    #[test]
