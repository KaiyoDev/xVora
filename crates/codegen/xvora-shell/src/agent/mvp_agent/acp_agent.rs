...
impl acp::Agent for MvpAgent {
    ...
    async fn initialize(
        ...
        *self.buffering_settings.borrow_mut() = buffering_settings;
        ...
        let xvora_api_base_url = self.cfg.borrow().endpoints.xvora_api_base_url.clone();
        let has_byok = self
            .models_manager
            .models()
            .values()
        ...
        debug_assert!(
            ...
            "BYOK invariant violated: xvora.api_key MUST be auth_methods.first() \
    ...
    async fn prompt(
            ...
            .and_then(|m| m.get("toolOverrides"))
        {
            None => None,
            Some(value) => {
                match xvora_sampling_types::ToolOverridesUpdate::parse(value) {
