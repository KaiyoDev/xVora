...
impl acp::Agent for MvpAgent {
            ...
            acp::InitializeResponse::new(acp::ProtocolVersion::V1)
                ...
                .meta({
                    ...
                    serde_json::json!(
                        ...
                        default_auth_method_id_wire, (xvora_mcp::wire::MCP_SDK) :
                        ...
                        xvora_version::VERSION, "agentId" : agent_id(),
    ...
    async fn new_session(
        ...
        xvora_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::SessionStarted {
        ...
        let (git_root, is_git_repo, discovery_failed) = match xvora_workspace::session::git::discover_git_root(
            cwd.as_path(),
        ) {
    ...
    async fn load_session(
            ...
            Some(s) => {
                match s.state {
                    crate::session::plan_mode::PlanModeState::Inactive => {
                        xvora_telemetry::events::PlanModeState::Inactive
                    }
                    crate::session::plan_mode::PlanModeState::Pending => {
                        xvora_telemetry::events::PlanModeState::Pending
                    }
                    crate::session::plan_mode::PlanModeState::Active
                    | crate::session::plan_mode::PlanModeState::ExitPending => {
                        xvora_telemetry::events::PlanModeState::Active
                    }
                }
            }
            None => xvora_telemetry::events::PlanModeState::Inactive,
        };
        ...
        let restore_checkout_allowed = xvora_workspace::session::git::restore_code_checkout_allowed(
            cwd.as_path(),
        ...
        {
            tracing::warn!(
                target : xvora_workspace::session::git::RESTORE_CODE_LOG, session_id =
        ...
        let preloaded_envrc = xvora_workspace::envrc::load_envrc_or_empty_when_trusted(
            cwd.as_path(),
        ...
        if summary.head_commit.is_some() && let Some(ref cwd) = session_cwd
            && summary
                ...
                .as_deref()
                .is_none_or(|root| {
                    xvora_workspace::session::git::find_git_root_from_path(
                ...
                })
        {
            ...
            let current_head = xvora_workspace::session::git::git_cli(
                    cwd_path,
                    &["rev-parse", "HEAD"],
                )
                .await
