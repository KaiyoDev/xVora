...
fn auth_with_mode(mode: crate::auth::AuthMode, key: &str) -> crate::auth::XaiAuth {
    crate::auth::XaiAuth {
        key: key.into(),
        auth_mode: mode,
...
fn harness_pair(id: &str) -> Vec<xvora_sampling_types::conversation::ConversationItem> {
    use xvora_sampling_types::ToolCall;
...
fn resolve_agent_definition_acp_profile_wins_when_model_agent_type_is_default() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let acp_profile = xvora_agent::AgentDefinition::from_json(&serde_json::json!(
...
fn resolve_agent_definition_agent_profile_with_model_override() {
    ...
    assert_eq!(
        def.model,
        xvora_agent::config::ModelOverride::Override("test-model-123".to_string()),
...
async fn file_toolset_override_e2e_to_finalized_toolset() {
    ...
    let builder = xvora_tools::registry::types::ToolRegistryBuilder::new();
    let ctx = SessionContext {
        ...
        memory_backend: None,
        web_search_config: xvora_tools::implementations::web_search::WebSearchConfig::default(),
        web_fetch_config: Default::default(),
        lsp: None,
        image_gen_config: xvora_tools::implementations::xvora::image_gen::ImageGenConfig::default(),
        video_gen_config: xvora_tools::implementations::xvora::video_gen::VideoGenConfig::default(),
        app_builder_deployer_config: xvora_tools::implementations::xvora::deploy_app::AppBuilderDeployerConfig::default(),
...
fn make_test_handle(
    ...
    crate::session::SessionHandle {
        ...
        tool_context: crate::tools::ToolContext::new_local_context(
            xvora_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap(),
...
async fn model_state_prefers_session_reasoning_effort_over_global() {
    ...
    use xvora_sampling_types::{REASONING_EFFORT_META_KEY, ReasoningEffort};
    let agent = build_minimal_agent_for_tests();
...
async fn ext_method_routes_auth_cleared_and_refreshes_resident_sessions() {
    ...
    local
        .run_until(async {
            let agent = build_agent_with_auth(crate::auth::XaiAuth {
                key: "eligible".into(),
...
fn build_agent_with_auth(auth: crate::auth::XaiAuth) -> MvpAgent {
    use crate::agent::config::Config as AgentConfig;
    use crate::auth::{AuthManager, GrokComConfig};
    let temp_dir = tempfile::tempdir().unwrap();
    let auth_manager =
...
async fn auth_type_session_based_with_current_returns_session_token() {
    ...
    agent.auth_manager.hot_swap(XaiAuth::test_default());
...
async fn prepare_video_gen_config_disabled_when_zdr_flag_set() {
    use xvora_tools::implementations::xvora::video_gen::{
...
async fn data_collection_enabled_for_normal_user() {
    let agent = build_agent_with_auth(crate::auth::XaiAuth::test_default());
    assert!(
...
fn chat_session_spawn_options_matches_thin_profile() {
    ...
    let cwd = xvora_paths::AbsPathBuf::new(std::env::temp_dir()).expect("temp cwd");
...
fn ann(id: &str) -> xvora_announcements::RemoteAnnouncement {
...
fn announcements_push_gate_ignores_expired_only_addition() {
    let now = test_now();
    let expired = xvora_announcements::RemoteAnnouncement {
...
fn announcements_push_gate_emits_on_expiry_crossing() {
    let expiring = xvora_announcements::RemoteAnnouncement {
