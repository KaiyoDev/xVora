...
impl MvpAgent {
    ...
    /// Mirrors the `resolve_config` gate: only upgrades from `Local`, writeback needs xvora auth.
    ...
    /// Both BYOK and session (OAuth) users go direct to `xvora_api_base_url`.
    ...
    pub(super) fn prepare_image_gen_config(
        ...
        let cfg = self.cfg.borrow();
        let base_url = cfg.endpoints.xvora_api_base_url.clone();
        let version = cfg
            .client_version
            .clone()
