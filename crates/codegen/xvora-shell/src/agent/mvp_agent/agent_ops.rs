...
impl MvpAgent {
    ...
    pub(super) fn prepare_image_gen_config(
        &self,
    ) -> xvora_tools::implementations::xvora::image_gen::ImageGenConfig {
    ...
    pub(super) fn prepare_video_gen_config(
        &self,
    ) -> xvora_tools::implementations::xvora::video_gen::VideoGenConfig {
    ...
    pub(super) fn prepare_web_fetch_config(
        &self,
    ) -> xvora_tools::implementations::xvora::web_fetch::WebFetchConfig {
    ...
    pub fn with_models(
        ...
        if relay_sync_enabled {
            ...
            tracing::info!(
                "[grok] Relay sync: DISABLED (no auth - run 'xvora login' first)"
            );
        ...
        let instance = Self {
            ...
            monitor_event_buffer: xvora_tools::implementations::xvora::task::types::MonitorEventBuffer::default(),
    ...
    pub fn cancel_subagent(
        &self,
        subagent_id: &str,
    ) -> xvora_tools::implementations::xvora::task::types::SubagentCancelOutcome {
    ...
    /// [`xvora_agent::config::is_strict_harness_agent_type`].
    ///
    ...
    pub(super) async fn spawn_and_register_session(
        ...
        let envrc = match preloaded_envrc {
            Some(env) => env,
            None => {
                xvora_workspace::envrc::load_envrc_or_empty_when_trusted(
                    cwd.as_path(),
        ...
        if lsp_tools_enabled && tool_ctx.lsp.is_none() {
            ...
            if servers.is_empty() {
                let user_path = xvora_tools::util::xvora_home::xvora_home()
                    .join("lsp.json");
