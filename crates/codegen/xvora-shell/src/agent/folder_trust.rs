...
//! interactive prompt, and the durable [`xvora_workspace::trust::TrustStore`]
//! reads/writes — lives in `xvora-workspace` (client-side); this module keeps
...
//! on [`xvora_workspace::folder_trust::decide`]; the consume-side nuance is
...
// Decision-side (scan/decide/prompt/store) relocated to `xvora-workspace`
// (client crate). `grant_folder_trust` is the ONLY moved item referenced from
// OUTSIDE this module (shell call sites + the pager's
// `xvora_shell::agent::folder_trust::grant_folder_trust`), so only it is
...
// (`xvora_agent::plugins::TrustStore` at `~/.xvora/trusted-plugins`, plus the
...
/// [`xvora_workspace::folder_trust::revoke_folder_trust_store`].
///
...
/// ([`xvora_workspace::folder_trust::repo_config_kinds`] /
...
mod tests {
    ...
    fn discover_hooks_gates_then_loads_project_hook_via_trust_verdict() {
        ...
        let has_project_hook = |reg: &xvora_hooks::discovery::HookRegistry| {
            reg.all_hooks()
                .iter()
