...
//! Those files include `.mcp.json`, project `.xvora/config.toml` (`[permission]` / `[mcp_servers]` / `[plugins].paths`), and `.xvora/lsp.json`.
...
// (`agent::plugins::TrustStore` at `~/.xvora/trusted-plugins`, plus the
...
/// So an `.xvora/lsp.json`-only repo still has a non-empty reason list.
...
/// Every other call site (per-session cwd, leader-served sessions whose cwd differs from the launch dir, `xvora mcp doctor`) passes `false`.
...
///   A `xvora --trust` grant issued AFTER this workspace was first resolved writes the store, so honor it on the next session without a restart.
...
///
/// SINGLE SOURCE OF TRUTH for "project-scoped MCP names" across ALL gate sites (session merge, the session-less agent pool, `xvora mcp doctor`).
...
///
/// Sources: project `.xvora/config.toml [mcp_servers]` (NOT the user-tier global config).
...
pub(crate) fn project_scoped_mcp_names(cwd: &Path) -> HashSet<String> {
    ...
    // `.xvora/config.toml [mcp_servers]` entries tagged project (the loader's key is the display name, matching `mcp_server_name` of the merged server)
...
mod tests {
    ...
    fn revoke_folder_trust_downgrades_cache() {
        ...
        // xvora_home-isolated so the seed can't touch the real user file, and `#[serial]` because xvora_home is process-global
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("xvora_home", home.path());
        let tmp = repo_tmp();
    ...
    fn revoke_on_unrecordable_home_root_records_no_deny() {
        ...
        // HOME overridden so workspace_key sees the tempdir as home; xvora_home-isolated store
    ...
    fn envrc_gate_drops_untrusted_then_loads_when_store_trusted() {
        ...
        // xvora_home-isolated so the trust store is empty; GROK_FOLDER_TRUST unset so the default-on feature flag applies
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("xvora_home", home.path());
        let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
        let tmp = repo_tmp();
    ...
    fn claude_env_gate_drops_subdir_project_env_when_untrusted() {
        ...
        // xvora_home-isolated so the trust store is empty
        use workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("xvora_home", home.path());
        let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
    ...
    fn project_agent_inline_hooks_gated_when_untrusted_but_user_kept() {
        ...
        // xvora_home-isolated (empty store)
    ...
    fn project_scope_allowed_denies_untrusted_repo_with_configs() {
        ...
        // The setup: a release-stamped build, the feature on by default, an untrusted folder shipping code-exec config (here `.xvora/hooks`)
        ...
        // xvora_home-isolated (empty store); GROK_FOLDER_TRUST unset so the default-on flag applies
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("xvora_home", home.path());
        let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
    ...
    fn project_scope_allowed_allows_repo_without_configs() {
        ...
        // Release-stamped and xvora_home-isolated so the verdict comes from `decide` rule 4 (no repo configs), not the inert short-circuit
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("xvora_home", home.path());
        let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
    ...
    fn project_scope_allowed_allows_store_trusted_repo() {
        ...
        // xvora_home-isolated so the seeded store is the temp one; GROK_FOLDER_TRUST unset so the default-on flag applies
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("xvora_home", home.path());
        let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
    ...
    fn project_scope_allowed_denies_untrusted_plugin_only_repo() {
        // A plugin-only untrusted repo (just `.xvora/plugins/<x>/`, no hooks/MCP/LSP, no store grant) is repo-controlled code-exec
        // It must be DENIED
    ...
    fn project_scope_allowed_denies_untrusted_permission_only_repo() {
        // Bridge: a clone whose ONLY repo-local config is `.xvora/config.toml` `[permission]` (no MCP/hooks/plugins) must still gate
    ...
    fn kill_switch_allows_untrusted_repo_after_authoritative_resolve() {
        ...
        // xvora_home-isolated (empty store); GROK_FOLDER_TRUST unset so the kill-switch is the only signal
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("xvora_home", home.path());
        let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
    ...
    fn build_for_cwd_with_trust_verdict_gates_active_project_plugin() {
        ...
        //
        // xvora_home-isolated so both the folder-trust store and the plugin trust store start empty (deterministic untrusted)
    ...
    fn discover_hooks_gates_then_loads_project_hook_via_trust_verdict() {
        ...
        // xvora_home-isolated so the grant writes to a temp store; GROK_FOLDER_TRUST unset so the default-on flag applies
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("xvora_home", home.path());
        let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
        let tmp = repo_tmp();
    ...
    fn load_servers_sourced_tags_project_lsp_json() {
        ...
        // A `<cwd>/.xvora/lsp.json` server must be tagged `Project` so the gate can distinguish it from user/plugin servers
    ...
    fn untrusted_workspace_drops_loaded_project_lsp() {
        ...
        // A project server loaded from `<cwd>/.xvora/lsp.json` is dropped once the workspace is untrusted
        let tmp = repo_tmp();
        let xvora = tmp.path().join(".xvora");
        std::fs::create_dir_all(&xvora).unwrap();
        std::fs::write(
    ...
    /// A git-init'd repo declaring two project-scoped MCP servers: `projjson` (`.mcp.json`) and `projtoml` (`.xvora/config.toml [mcp_servers]`).
    ...
    /// A project server declared in each of `.xvora/config.toml`, `.mcp.json`, and `.cursor/mcp.json` must appear in the returned set.
    ...
    fn cached_untrusted_is_upgraded_by_a_later_grant() {
        ...
        record(&key, false);
        assert!(!project_scope_allowed(tmp.path()));
        // Simulate a `xvora --trust` grant landing in the store after the untrusted verdict was cached
    ...
    fn resolve_launch_dir_trust_matches_resolve_and_record() {
        ...
        // Isolate xvora_home so the store is empty/seeded in temp; `#[serial]` because both vars are process-global
    ...
    fn local_build_is_inert_launch_trust_auto_trusts() {
        ...
        // GROK_TEST_VERSION is unset so `is_local_build()` is genuinely true; xvora_home-isolated so the real store is never touched
    ...
    fn prompt_warranted_true_for_untrusted_repo_with_configs() {
        ...
        // xvora_home-isolated so the store starts empty; `#[serial]` because xvora_home is process-global
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("xvora_home", home.path());
        let tmp = repo_tmp();
    ...
    fn prompt_warranted_false_when_feature_disabled() {
        ...
        // xvora_home-isolated and GROK_FOLDER_TRUST unset so the kill-switch is the only signal
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("xvora_home", home.path());
        let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
        let _sim = simulate_release_build();
    ...
    fn detected_config_kinds_reports_lsp_only_repo() {
        // Regression for the "empty configKinds" bug: a repo gated SOLELY by `.xvora/lsp.json` must still produce a non-empty reason list
        let tmp = repo_tmp();
