...
//!
//! Fetches the authenticated user's Xvora billing configuration
...
/// Current billing configuration for Xvora coding credits.
///
/// Carries both the newer credits-config fields (`credit_usage_percent`,
/// `current_period`) and the deprecated `XvoraBillingConfig` fields
...
/// `GetXvoraBillingConfig` backend responses.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct BillingConfig {
    ...
    /// absent (legacy `GetXvoraBillingConfig` shape or older servers).
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
async fn handle_get_billing(agent: &MvpAgent) -> ExtResult {
    let auth = super::auth_gate::require_xai_auth(
        ...
        "Billing data requires auth with grok.com. Run `xvora login` to authenticate.",
    )?;
...
async fn handle_get_auto_topup_rule(agent: &MvpAgent) -> ExtResult {
    let auth = super::auth_gate::require_xai_auth(
        ...
        "Auto top-up data requires auth with grok.com. Run `xvora login` to authenticate.",
    )?;
    ...
    fn billing_config_deserializes_credits_config_shape() {
        ...
        let json = serde_json::json!({
            "config": {
                ...
                "productUsage": [
                    {"product": "PRODUCT_XVORA_BUILD", "usagePercent": 61.2}
                ],
                "history": [
                    {
                        "period": {
