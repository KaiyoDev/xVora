...
//
// Implements the `McpResourceProvider` trait from xvora-tools
// The `ListMcpResources` and `FetchMcpResource` tools can then access MCP servers without depending on `xvora-mcp` directly
