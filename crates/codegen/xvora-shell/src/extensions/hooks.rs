...
pub(crate) fn current_hook_infos(
    registry: Option<&xvora_hooks::discovery::HookRegistry>,
) -> Vec<HookInfo> {
    let Some(registry) = registry else {
        return Vec::new();
    };
    let disabled = xvora_hooks::trust::DisabledHooks::load();
...
fn hook_specs_to_infos(
    specs: &[&xvora_hooks::config::HookSpec],
    disabled: &xvora_hooks::trust::DisabledHooks,
...
fn hook_spec_to_info_with(
    spec: &xvora_hooks::config::HookSpec,
    disabled: &xvora_hooks::trust::DisabledHooks,
    registered_dirs: &HashSet<String>,
    ...
    HookInfo {
        ...
        source_dir,
        disabled: xvora_hooks::trust::hook_disabled_for_display_with(spec, disabled),
...
pub(crate) const ADVERTISED_BLOCKING_EVENTS: &[xvora_hooks::event::HookEventName] = &[
...
mod tests {
    ...
    fn hook_spec_to_info_raw_display_wins_so_secrets_never_reach_dto() {
        let no_disabled = xvora_hooks::trust::DisabledHooks::from_names([]);
    ...
    fn hook_specs_to_infos_pins_removable_at_source_level() {
        ...
        policy.layer = xvora_hooks::config::HookProvenance::SystemManaged;
        ...
        req.layer = xvora_hooks::config::HookProvenance::Requirements;
    ...
    fn advertised_blocking_events_are_gates() {
        use xvora_hooks::event::GateKind;
    ...
    fn client_hook_dispatch_serializes_envelope() {
        use xvora_hooks::event::{HookEventEnvelope, HookPayload};
