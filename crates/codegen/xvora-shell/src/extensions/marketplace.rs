...
fn load_filtered_marketplace_sources() -> Vec<xvora_plugin_marketplace::MarketplaceSource> {
    ...
    let source_names: Vec<String> = sources
        ...
        .map(|s| {
            let url = match &s.kind {
                xvora_plugin_marketplace::SourceKind::Git { url, .. } => url.as_str(),
...
async fn handle_update(
    ...
    let scan = xvora_plugin_marketplace::scan_marketplace(&marketplace_root);
...
fn scan_source(
    source: &xvora_plugin_marketplace::MarketplaceSource,
    ...
    let scan = xvora_plugin_marketplace::scan_marketplace(&root);
    ...
    let plugins = discovered
        ...
        .map(|p| {
            let (install_status, installed_version) =
                match xvora_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    &registry,
                    &source_url_or_path,
                    &p.relative_path,
                ) {
...
fn to_plugin_entry(
    p: xvora_plugin_marketplace::MarketplaceEntry,
...
async fn handle_add_source(url: &str) -> hooks_plugins_types::ActionOutcome {
    ...
    let name = if is_official {
        xvora_plugin_marketplace::OFFICIAL_SOURCE_NAME.to_string()
    } else {
        match &input {
...
pub(crate) fn ensure_official_marketplace_source(grok_home: &std::path::Path) {
    ...
    let already_present = toml_sources.iter().chain(json_sources.iter()).any(|s| {
        matches!(&s.kind, xvora_plugin_marketplace::SourceKind::Git { url, .. }
            if xvora_plugin_marketplace::is_official_source_url(url))
    });
    ...
    let write_result = if already_present {
        ...
        add_marketplace_source(
            &config_path,
            xvora_plugin_marketplace::OFFICIAL_SOURCE_NAME,
            &crate::plugin::MarketplaceAddInput::GitUrl(
                xvora_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.to_string(),
            ),
            true,
        )
    };
    ...
    match write_result {
        Ok(()) if !already_present => {
            tracing::info!(
                url = xvora_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
...
mod official_source_tests {
    ...
    fn read_sources(
        config_path: &std::path::Path,
    ) -> Vec<xvora_plugin_marketplace::MarketplaceSource> {
        let raw = std::fs::read_to_string(config_path).unwrap_or_default();
        let parsed: toml::Value =
            toml::from_str(&raw).unwrap_or_else(|_| toml::Value::Table(Default::default()));
        xvora_plugin_marketplace::load_sources(&parsed)
    }
    ...
    fn add_marketplace_source_local_path_writes_path_entry() {
        ...
        assert!(matches!(
            &sources[0].kind,
            xvora_plugin_marketplace::SourceKind::Local { path } if path == &dir
        ));
    ...
    fn removing_last_nonofficial_source_preserves_flag_and_blocks_readd() {
        ...
        assert!(
            !sources.iter().any(|s| matches!(&s.kind,
                xvora_plugin_marketplace::SourceKind::Git { url, .. }
                    if xvora_plugin_marketplace::is_official_source_url(url))),
    ...
    fn first_run_creates_source_and_sets_flag() {
        ...
        assert!(matches!(
            &sources[0].kind,
            xvora_plugin_marketplace::SourceKind::Git { url, .. }
                if url == xvora_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL
        ));
...
mod conversion_tests {
    ...
    fn to_plugin_entry_carries_homepage_and_keywords() {
        let entry = xvora_plugin_marketplace::MarketplaceEntry {
            name: "demo".into(),
