...
/// files (TUI tick vs `xvora login` vs prefetch). `None` on contention — the
...
#[derive(Clone, Copy)]
enum SyncBudget {
    /// Background loop and explicit `xvora setup`; runs retries to completion.
    Standard,
...
/// Base is 1s; `XVORA_DEPLOYMENT_CONFIG_BACKOFF_MS` overrides it for tests.
fn retry_backoff(attempt: u32) -> std::time::Duration {
    let base = std::env::var("XVORA_DEPLOYMENT_CONFIG_BACKOFF_MS")
        .ok()
...
/// Map a `reqwest` send failure to a `ManagedConfigError` via the shared `xvora-http` classifier.
...
/// Override with `XVORA_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn managed_config_sync_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("XVORA_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS")
...
/// snapshots, and OTel: the **server** XvoraDeployment UUID (the id
...
/// Resolve deployment key from `XVORA_DEPLOYMENT_KEY` env var, then config files.
...
pub fn is_fetch_enabled() -> bool {
    if let Some(v) = crate::agent::config::env_bool("XVORA_MANAGED_CONFIG") {
        return v;
    }
...
/// Fetch managed config + requirements and write to `~/.xvora/`, trying the
...
struct SyncOutcome {
    ...
    /// `xvora setup` doesn't misreport a lock skip as "no config".
    served: bool,
...
/// Post-login hook for `xvora login` and the ACP/TUI authenticate flow: clear any
...
/// Whether a credential exists that `xvora setup` could install config for.
...
/// `xvora setup`); ceasing to serve `fail_closed` rolls back.
pub fn managed_policy_gate() -> Result<(), String> {
    // Skip under the lib unit-test build only: `bootstrap` reaches this without a staged
    // `XVORA_HOME` and would flake on the dev machine's real marker/auth. The pure decision
...
/// Outcome of the `xvora setup` sync. The caller renders it — CLI presentation
/// and exit codes stay out of the library.
#[derive(Debug)]
pub enum SetupOutcome {
    /// Config was written to `~/.xvora`.
    Installed,
...
/// exactly as `xvora setup` would write it to disk.
...
pub struct SetupReport {
    ...
    /// TOML documents exactly as `xvora setup` would install them.
...
pub async fn fetch_setup_report() -> Result<SetupReport, ManagedConfigError> {
    ...
    // Match the installer's trust decision: a payload `xvora setup` would refuse
...
/// Run the `xvora setup` sync for the current principal. The caller must check
