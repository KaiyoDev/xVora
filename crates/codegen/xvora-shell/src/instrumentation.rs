//! Shim; see `telemetry::instrumentation` for the implementation.
//!
//! Two pieces stay here:
//! - The [`instrumentation_timer!`] macro is `#[macro_export]`-ed from this crate.
//!   Call sites spell it `crate::instrumentation_timer!` or `shell::instrumentation_timer!`.
...
pub use telemetry::instrumentation::{
...
///
/// The macro stays in shell so `$crate` continues to resolve to `shell` for the 12+ existing call sites.
/// Those sites spell it `crate::instrumentation_timer!(...)` or `shell::instrumentation_timer!(...)`.
/// The macro body delegates to types and functions in `telemetry::instrumentation`.
#[macro_export]
