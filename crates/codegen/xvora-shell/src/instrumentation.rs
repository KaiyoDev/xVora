...
//!   (i.e. `xvora_shell::instrumentation_timer!`). Keeping the macro here
...
///
/// Macro stays in shell so `$crate` continues to resolve to `xvora_shell`
/// for the 12+ existing call sites that spell it as
/// `crate::instrumentation_timer!(...)` or `xvora_shell::instrumentation_timer!(...)`.
/// The macro body delegates to types and functions in
/// `xvora_telemetry::instrumentation`.
#[macro_export]
