//! Shim; see `xvora_telemetry::instrumentation` for the implementation.
//!
//! Two pieces stay here:
//! - The [`instrumentation_timer!`] macro is `#[macro_export]`-ed from this crate.
//!   Call sites spell it `crate::instrumentation_timer!` or `xvora_shell::instrumentation_timer!`.
...
pub use xvora_telemetry::instrumentation::{
...
///
/// Stays in shell so callers can keep calling `xvora_shell::instrumentation::finalize_and_exit`.
...
///
/// The macro stays in shell so `$crate` continues to resolve to `xvora_shell` for the 12+ existing call sites.
/// Those sites spell it `crate::instrumentation_timer!(...)` or `xvora_shell::instrumentation_timer!(...)`.
/// The macro body delegates to types and functions in `xvora_telemetry::instrumentation`.
#[macro_export]
