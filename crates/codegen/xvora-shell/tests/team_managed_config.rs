...
use xvora_shell::config::ServingIdentity;
...
async fn team_sync_writes_files() {
    ...
    let wrote = xvora_shell::managed_config::sync()
        .await
...
async fn team_switch_evicts_prior_teams_policy() {
    ...
    xvora_shell::managed_config::sync()
        .await
    ...
    assert!(!xvora_shell::config::is_managed_config_stale_for(
...
async fn withdrawn_artifact_is_removed_on_next_sync() {
    ...
    assert!(
        !xvora_shell::config::is_managed_config_stale_for(&team_identity("team-007")),
...
async fn empty_dk_response_with_failing_team_leaves_team_policy_intact() {
    ...
    let err = xvora_shell::managed_config::sync()
        .await
...
async fn expired_refreshable_team_token_heals_after_auth_refresh() {
    ...
    std::fs::remove_file(home.join("requirements.toml")).unwrap();
    assert!(
        !xvora_shell::managed_config::has_principal(),
...
async fn managed_policy_gate_fails_closed_on_deleted_policy_offline() {
    ...
    assert!(
        xvora_shell::managed_config::managed_policy_gate().is_ok(),
    ...
    std::fs::remove_file(home.join("requirements.toml")).unwrap();
    assert!(
        xvora_shell::managed_config::managed_policy_gate().is_err(),
    ...
    write_config(&home, &url);
    write_team_auth(&home, "team-007");
    xvora_shell::config::mark_managed_config_synced(xvora_shell::config::SyncMarker {
...
async fn bootstrap_fails_closed_when_managed_policy_compromised() {
    ...
    let err = match xvora_shell::agent::init::bootstrap(&cfg, &auth_manager, None) {
        Err(e) => e,
        Ok(_) => {
...
async fn managed_policy_gate_fails_closed_on_deployment_key_switch_offline() {
    ...
    let gate_same_key = xvora_shell::managed_config::managed_policy_gate();
    ...
    let gate_switched_key = xvora_shell::managed_config::managed_policy_gate();
...
async fn fail_closed_env_cannot_disarm_the_gate() {
    ...
    let gate = xvora_shell::managed_config::managed_policy_gate();
    unsafe {
        std::env::remove_var("XVORA_MANAGED_CONFIG");
...
async fn setup_lock_skip_is_not_reported_as_no_config() {
    ...
    let outcome = xvora_shell::managed_config::run_setup().await;
    lock.unlock().unwrap();

    assert!(
        matches!(
            outcome,
            xvora_shell::managed_config::SetupOutcome::Installed
        ),
...
async fn shared_client_reuses_pooled_connection() {
    ...
    let client = xvora_shell::http::shared_client();
...
async fn managed_config_opt_out_makes_no_requests() {
    ...
    unsafe { std::env::set_var("XVORA_MANAGED_CONFIG", "0") };
    let outcome = xvora_shell::managed_config::post_login_sync(None).await;
    unsafe { std::env::remove_var("XVORA_MANAGED_CONFIG") };

    assert_eq!(
        outcome,
        xvora_shell::managed_config::ManagedConfigSync::Skipped
    );
...
async fn post_login_pins_authenticated_team_over_disk() {
    ...
    let outcome = xvora_shell::managed_config::post_login_sync(Some(pinned)).await;
    assert_eq!(
        outcome,
        xvora_shell::managed_config::ManagedConfigSync::Updated { is_team: true }
    );
    assert_eq!(
        auths.lock().unwrap().last().map(String::as_str),
...
async fn post_login_sync_is_latency_bounded() {
    ...
    assert_eq!(
        outcome,
        xvora_shell::managed_config::ManagedConfigSync::Failed
    );
    assert_eq!(
        *count.lock().unwrap(),
        2,
