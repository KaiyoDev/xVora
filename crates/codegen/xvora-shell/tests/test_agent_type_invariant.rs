...
//! Run locally:
//! ```bash
//! cargo test -p xvora-shell --test test_agent_type_invariant -- --ignored
//! ```
...
/// - `model-a`: no agent_type (→ "xvora")
/// - `model-b`: no agent_type (→ "xvora")
...
/// `xvora` harness. The system prompt sent to the LLM should contain
/// the xvora identity string.
#[tokio::test]
#[ignore]
async fn test_default_model_uses_xvora_harness() {
    with_local_set(|| async {
        ...
        assert!(
            ...
            "default model should use xvora harness\nsystem prompt preview: {}",
...
/// default to `"xvora"`. This exercises the serde default.
#[tokio::test]
#[ignore]
...
async fn test_grok_agent_env_overrides_model_agent_type() {
    with_local_set(|| async {
            ...
            cmd.args(["agent", "stdio"])
                ...
                .kill_on_drop(true);
            xvora_test_support::env::test_env_cmd_tokio(
                &mut cmd,
                &server.url(),
                home.path(),
            );
            cmd.env("XVORA_AGENT", "xvora");
            ...
            let incoming = acp_lib::LineBufferedRead::spawn_local(incoming);
            ...
            assert!(
                sys_prompt.contains("Grok") || sys_prompt.contains("grok"),
                "XVORA_AGENT=xvora should override cursor model's agent_type\nsystem prompt preview: {}",
                & sys_prompt[..sys_prompt.len().min(500)]
            );
        })
        .await;
