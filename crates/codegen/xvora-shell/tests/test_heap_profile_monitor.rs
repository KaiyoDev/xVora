...
use xvora_shell::auth::{AuthManager, AuthMode, GrokComConfig, XaiAuth};
...
use xvora_test_support::{EnvGuard, MockInferenceServer};
use xvora_version::TEST_VERSION_ENV;
...
fn assert_storage_auth(uploads: &[xvora_test_support::mock_server::StorageUpload]) {
    for u in uploads {
        assert_eq!(
