...
/// The agent emitting it from `model_switch::apply` is covered by the `extensions::notification` tests in `xvora-shell`.
/// The pager applying it silently on followers is covered by the `model_changed_*` tests in `xvora-pager`'s `acp_handler` tests.
