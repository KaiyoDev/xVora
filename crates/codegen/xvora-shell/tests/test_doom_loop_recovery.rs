...
use xvora_sampler::RetryPolicy;
use xvora_sampling_types::doom_loop::{DoomLoopSignalKind, SAMPLE_CHECK_EVENT_DATA_CUMULATIVE};
...
/// ```bash
/// cargo test -p xvora-shell --test test_doom_loop_recovery -- --ignored
/// ```
#[tokio::test]
...
async fn headless_config_enables_doom_loop_check_header() {
    ...
    let workdir = xvora_test_support::git_workdir();
    ...
    let mut cmd = tokio::process::Command::new(xvora_test_support::grok_binary());
    cmd.args(["-p", "say hi", "--yolo", "--output-format", "json"])
        ...
        .kill_on_drop(true);
    xvora_test_support::env::test_env_cmd_tokio(&mut cmd, &server.url(), home.path());
    ...
    let result = xvora_test_support::run_headless_with_cmd(cmd).await;
    xvora_test_support::assert_headless_success(&result, "doom-loop header e2e", Some(&server));
