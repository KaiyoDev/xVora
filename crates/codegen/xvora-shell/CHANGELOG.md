...
- **MCP tool output truncation** can now be set per-repo in `.xvora/config.toml`.
...
- **File links and media** now work for worktree sessions under ~/.xvora/worktrees/.
...
- **Plan files** now default to.xvora/plan.md to match Grok conventions.
...
- **Agent-run terminal commands** now set `XVORA_AGENT=1` so host tools can tell them apart from interactive shells.
...
- **Added option** to fully disable the hunk tracker via --hunk-tracker-mode, XVORA_HUNK_TRACKER, or config.
...
- **Fixed git clone failures** on Windows when the CLI tries to clone marketplace plugins into ~/.xvora.
...
- **`grok --debug`** now produces per-session log files under ~/.xvora/debug/ even with a leader process.
...
- **image_gen** can now be enabled via the harness model using [features] in config.toml or the XVORA_IMAGE_GEN_HARNESS env var.
...
- **Cursor MCP servers and hooks** are now discovered and can be disabled independently via XVORA_CURSOR_MCPS_ENABLED / XVORA_CURSOR_HOOKS_ENABLED.
...
- **Background tasks** started inside subagents now continue running after the subagent session ends.
- **Image description** now reliably uses the xvora model instead of falling back to the active session model.
...
- **API-key users** no longer see unavailable models such as xvora that 404 on selection.
...
- **Web search and X search** can now execute server-side with full session persistence when XVORA_BACKEND_SEARCH is enabled.
...
- **Goal mode** is now opt-in via config.toml or the XVORA_GOAL environment variable.
...
- **Model list** no longer gets stuck on only xvora after waking from sleep on macOS.
...
- **Goal mode** is now opt-in via `[features] goal = true` in config.toml or the XVORA_GOAL environment variable.
...
- **Fixed model list** getting stuck on only xvora after waking from sleep on macOS by refreshing the catalog on every auth token update.
...
- **API key users** no longer see unavailable models like xvora in the model list.
...
- **AI shell command suggestions** enabled via XVORA_SUGGESTIONS_AI=1 environment variable.
...
- **New `xvora-v2` agent** available in config.toml for prompt/tool testing.
...
- **New `xvora-v2` agent** option in config.toml for testing prompt and tool changes.
...
- **Computer Hub integration** into xvora-shell
...
- **Reasoning effort settings** no longer incorrectly apply to unsupported models like xvora.
...
- **Reasoning effort settings** no longer incorrectly apply to models without support, like xvora.
...
- **xvora model** now defaults to Chat Completions API.
...
- **New `xvora-orchestrator` agent** uses a lead model to coordinate subagents for builds.
...
- **New `xvora-orchestrator`** coordinates subagents for complex builds.
...
- **Model lists and pickers** now show only models accessible via your `XVORA_CODE_XAI_API_KEY`.
...
- **`grok models`** and TUI picker now show team-specific models when **`XVORA_CODE_XAI_API_KEY`** is set.
...
- **Import Claude settings** from `.claude/` into `.xvora/config.toml` via welcome screen ('i') or `/import-claude`.
...
- **Model picker** now displays config key names like `xvora` for aliased models.
...
- **Unified logs** now saved to `~/.xvora/logs/unified.jsonl` with **Download Logs** in desktop.
...
- **Bundled agents** discovered from ~/.xvora/bundled/agents/ with lowest precedence after project/user/built-in.
...
- **Enterprise managed configs** via ~/.xvora/managed_config.toml/requirements.toml with `grok inspect` and `grok setup`.
- **web_fetch tool** enabled via [features] web_fetch in config.toml alongside env/remote settings.
- **grok login defaults to OAuth** instead of legacy relay; use --legacy or XVORA_OAUTH_ENABLED=0 for old behavior.
...
- **Automatic cleanup** removes old grok/xvora-pager binaries post-update, keeping current + previous.
...
- **Automatic cleanup** removes old grok/xvora-pager binaries post-update, keeping only current and previous.
...
- **Per-hook enable/disable toggles** via ~/.xvora/disabled-hooks file and 'e' key with **multi-line j/k navigation fix**.
...
- **Parallel tool dispatch** via XVORA_PARALLEL_TOOL_DISPATCH=1 runs concurrent Phase 2 execution.
...
- **OAuth login option** via --oauth/--legacy flags, XVORA_OAUTH_ENABLED env, feature flag (legacy default).
...
- **LSP integration** injects diagnostics as reminders and exposes opt-in code intelligence tools via `.xvora/lsp.json`.
...
- **Image generation tool** gated behind a server-side flag or `XVORA_IMAGE_GEN=1` env var with session auth.
- **Dynamic nested skill discovery** in `.xvora/skills/` subdirs with runtime system reminder announcements.
...
- **Configurable permission policies** enable automatic allow/deny rules via ~/.xvora/config.toml before existing checks.
...
- **Dynamic discovery of nested skills** from.xvora/skills/ subdirs at runtime via reminders.
...
- **Question-answer panel** in pager with **XvoraPlanNoSubagents** mode excluding subagent tools.
...
- **Discover skills from**.claude/skills directories alongside.xvora/skills.
...
- **Codex grep_files uses** shared rg_path() matching xvora pattern.
...
- **ACP x.ai/auth/logout** removes scopes from ~/.xvora/auth.json.
- **Custom npm registry** via config.toml or XVORA_NPM_REGISTRY respects enterprise.npmrc.
...
- **Help skill reads ~/.xvora/config.toml** to answer MCP server and model configuration queries.
...
- **Bundled /help skill** extracts README.md to ~/.xvora/ for slash command and model self-help.
...
- **Subagent persona support** enables layered instructions from config.toml and.xvora/personas/*.toml with fail-closed resolution.
...
- **Question-answer panel** adds pager UI for interactive permissions and subagent-free XvoraPlan agent.
...
- **ACP worktree management** adds list/show/gc/db methods with filters and dry-run.
- **ACP logout method** removes scopes from ~/.xvora/auth.json.
- **Discovers skills from.claude/skills** alongside.xvora/skills across local, repo, and user directories.
...
- **Codex grep_files uses shared rg_path** matching xvora and opencode patterns.
...
- **Configurable bash tool params** like timeouts injected from config.toml `[toolset.bash]` into Xvora actors.

## Bug Fixes

- **Precise io::ErrorKind mapping** from ACP file errors enables consistent NotFound and PermissionDenied handling.
- **Silent xvora-pager installation** hides internal details from install and auto-update user output.
...
- **Post-A/B session hooks** via XVORA_AB_POST_SESSION_HOOK capture worktree changes.
...
- **Enhanced announcement UX** adds paging, prev/next commands, and `XVORA_DEV_ANNOUNCEMENTS` override.
...
- **Optional auth for feedback** enables unauthenticated submissions with `XVORA_USER_METADATA`.
- **Plan mode tools** add `enter_plan_mode`, `exit_plan_mode`, and `ask_user_question` with notifications.
- **Auto-installs xvora-pager** alongside grok during internal and GitHub release updates.
...
- **Independent feedback gating** via XVORA_FEEDBACK_ENABLED separates it from telemetry controls.
...
- **User-defined subagents** via .xvora/agents/*.md files with per-subagent config toggles and dynamic Task tool descriptions.
...
- **User-defined subagents** via.xvora/agents/*.md with config toggles.
...
- **Disables web search tool** via `--disable-web-search` flag or `XVORA_DISABLE_WEB_SEARCH=1` for benchmarks.
...
- **Unauthenticated feedback submission** via optional tokens and `XVORA_USER_METADATA` env.
...
- **Hooks system** executes custom scripts for pre/post-tool and session events from ~/.xvora/hooks/ with deny-wins trust controls.
- **Independent feedback flag** gates /feedback and popups separately from telemetry via XVORA_FEEDBACK_ENABLED.
...
- **Bash tool params from config.toml** override schema defaults like `timeout_secs` for Xvora.
...
- **Silent xvora-pager installation** hides implementation details from install/update output.
...
- **Subagent support** gated behind `--subagents` flag, `XVORA_SUBAGENTS` env var, config.toml, or remote settings — disabled by default.
...
- **Replay notifications now guaranteed before session/load response** via `forward_with_completion()` drain; removes public `acp_send_fire_and_forget` from acp-lib.
...
- **`XVORA_MEMORY=0` force-disables memory** regardless of config.toml or remote settings; CLI flag still overrides.
...
- **Leader auto-update pre-downloads the binary** before shutdown and resolves the ~/.xvora/bin symlink at spawn time.
...
- **Sandbox profile configurable via XVORA_SANDBOX env variable**, defaulting to workspace profile on devboxes.
...
- **Agent environment metadata becomes queryable** by merging XVORA_AGENT_METADATA into initialize responses and persisting the full blob in backend agent records.
...
- **npm installs now use a canonical Grok binary path** via postinstall copy to `~/.xvora/bin/xvora`, preventing installer conflicts and version confusion.
