//! MCP server configuration value types, extracted from xvora-shell so crates the shell depends on can use them.
...
/// A problem found loading an `[mcp_servers.*]` entry. It is reported (never fatal) and shown by `xvora inspect`.
