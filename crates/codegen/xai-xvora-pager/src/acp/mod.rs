...
pub struct AcpConnection {
    ...
    // When available, add field: startup_announcements: Option<Vec<xai_xvora_announcements::RemoteAnnouncement>>
    ...
    pub auth_manager: std::sync::Arc<xai_xvora_shell::auth::AuthManager>,
}
...
pub struct ConnectFlags {
    ...
    pub remote_settings: Option<xai_xvora_shell::util::config::RemoteSettings>,
...
pub async fn connect_via_leader(
    ...
    xai_xvora_shell::agent::init::update_telemetry_config(&agent_config, &auth_manager);
...
fn apply_config_writes(flags: &ConnectFlags) {
    ...
    let config_path = xai_xvora_shell::util::xvora_home::xvora_home().join("config.toml");
...
async fn initialize(
    ...
    // Check if this is a xvora-shell agent
...
mod tests {
    ...
    fn shell_built_auth_methods_for_byok_user_skip_login_screen() {
        use xai_xvora_shell::agent::auth_method::{AuthMethodsBuildInputs, build_auth_methods};
    ...
    fn startup_auth_xai_api_key_not_first_still_requires_login() {
        use xai_xvora_shell::agent::auth_method::{XVORA_COM_METHOD_ID, XAI_API_KEY_METHOD_ID};
