...
pub async fn spawn_grok_shell(
    ...
    memory_config: Option<xai_xvora_shell::config::MemoryConfig>,
) -> Result<SpawnedAgent> {
    ...
    // skills to ~/.xvora/skills/), and model catalog construction.
    let (agent_config, models_manager) =
        xai_xvora_shell::agent::init::bootstrap(&agent_config, &auth_manager, None)
