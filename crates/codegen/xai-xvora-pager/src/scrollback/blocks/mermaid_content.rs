...
//! the generic [`CodeBlockSpan`](xai_xvora_markdown::CodeBlockSpan) API) and
...
use xai_xvora_markdown::MarkdownRenderView;
...
pub struct MermaidBlock {
    ...
    /// from [`CodeBlockSpan::body`](xai_xvora_markdown::CodeBlockSpan::body). For
    ...
    /// [`CodeBlockSpan::output_line_range`](xai_xvora_markdown::CodeBlockSpan::output_line_range).
...
) -> impl Iterator<Item = &'a xai_xvora_markdown::CodeBlockSpan> {
    view.code_blocks
        .iter()
...
/// dark. The render worker maps this to `xai_xvora_mermaid::MermaidTheme`; it
