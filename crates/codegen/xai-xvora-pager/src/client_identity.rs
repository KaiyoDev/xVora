...
pub const PAGER_CLIENT_VERSION: &str = xai_xvora_version::VERSION;

/// `User-Agent` for pager-owned direct-to-`api.x.ai` clients (voice STT).
///
/// Matches the sampler's `xvora-shell/<version> (os; arch)` shape so server-side
...
mod tests {
    ...
    fn client_user_agent_has_expected_shape() {
        // e.g. "xvora-shell/1.2.3 (macos; aarch64)". The pieces are wire
