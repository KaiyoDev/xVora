...
use xai_xvora_shell::extensions::notification::RetryState;
use xai_xvora_shell::extensions::notification::SessionUpdate as XaiSessionUpdate;
...
pub(super) fn compressed_entry(
    index: usize,
) -> xai_xvora_shell::extensions::notification::ImageCompressedEntry {
    xai_xvora_shell::extensions::notification::ImageCompressedEntry {
        index,
...
pub(super) fn xai_hook_execution_notif_for_prompt(
    ...
    use xai_xvora_shell::extensions::notification::{HookRunEntryDto, HookRunStatusDto};
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
...
/// Shared temp `XVORA_HOME` for disk-replay tests. `xvora_home()` uses a
/// process-wide `OnceLock`, so parallel tests must not each set `XVORA_HOME`
...
/// depend on process-wide `xvora_home()` cache order when the full suite runs.
pub(super) fn with_replay_disk_home<R>(f: impl FnOnce(&std::path::Path) -> R) -> R {
    let home = replay_disk_test_home();
    crate::app::subagent::set_replay_xvora_home_for_tests(Some(home.to_path_buf()));
    let out = f(home);
    crate::app::subagent::set_replay_xvora_home_for_tests(None);
    out
}
...
pub(super) fn make_git_head_changed_notif(
    ...
    let payload = xai_xvora_workspace::session::git::GitHeadChanged {
...
pub(super) fn make_server_status_notif(
    session_id: &str,
    name: &str,
    status: xai_xvora_shell::extensions::mcp::McpServerStatus,
    tools: Option<serde_json::Value>,
) -> acp::ExtNotification {
    use xai_xvora_shell::extensions::mcp::{
...
pub(super) fn make_tools_changed_notif_post_h2(
    ...
    let payload = xai_xvora_shell::extensions::mcp::McpToolsChanged {
        session_id: session_id.to_string(),
