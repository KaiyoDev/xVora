...
use super::*;
use xai_xvora_shell::extensions::billing::{BillingConfig, Cent, UsagePeriod};
...
fn parse_kill_outcome_reads_result_envelope() {
    use xai_xvora_tools::types::KillOutcome;
...
fn parse_worktree_restore_payload_full() {
    use xai_xvora_workspace::session::git::RestoreDegree;
...
fn setup_xvora_home_in_tempdir() -> tempfile::TempDir {
...
fn register_session_in(root: &std::path::Path, id: &str) -> acp::SessionId {
    use xai_xvora_shell::active_sessions::{ActiveSession, register_in};
...
fn unregister_best_effort_removes_entry_when_lock_free() {
    ...
    assert!(
        xai_xvora_shell::active_sessions::list_in(dir.path()).expect("list").is_empty(),
...
fn unregister_best_effort_is_nonblocking_under_lock_contention() {
    ...
    assert_eq!(
        xai_xvora_shell::active_sessions::list_in(dir.path()).expect("list").len(), 1,
...
async fn persist_permission_mode_acp_notification_fires_once_on_best_effort() {
    ...
    let _guard = setup_xvora_home_in_tempdir();
...
async fn foreign_scan_task_echoes_sequence_without_enabled_sources() {
    ...
    execute(
        Effect::ScanForeignSessions {
            ...
            compat: xai_xvora_workspace::foreign_sessions::EnabledForeignSessionSources::default(),
...
/// Default flags produce no agent profile (uses xvora default).
#[test]
fn default_flags_produce_no_profile() {
    let flags = SessionFlags::default();
    assert_eq!(flags.agent_profile(), None);
}
/// --subagents alone produces no profile (xvora already has TaskTool).
#[test]
...
/// Neutralize `XVORA_AGENT` for the profile-matrix tests below: agent-driven
...
/// Callers must be `#[serial_test::serial(XVORA_AGENT)]` (process-global env).
fn without_grok_agent() -> crate::test_util::EnvVarGuard {
    crate::test_util::EnvVarGuard::set("XVORA_AGENT", "")
}
...
#[serial_test::serial(XVORA_AGENT)]
#[test]
fn runtime_default_flags_produce_plan_meta() {
    ...
    assert_eq!(meta["agentProfile"], "xvora-plan");
...
fn plan_only_meta() {
    ...
    let meta = flags.to_meta().unwrap();
    assert_eq!(meta["agentProfile"], "xvora-plan-no-subagents");
...
/// --ask-user alone selects the xvora-ask-user profile.
#[serial_test::serial(XVORA_AGENT)]
#[test]
fn ask_user_alone_meta() {
    ...
    let meta = flags.to_meta().unwrap();
    assert_eq!(meta["agentProfile"], "xvora-ask-user");
    assert!(meta.get("askUserQuestion").is_none());
    assert_eq!(meta["yoloMode"], false);
}
...
/// does not need an `agentProfile` (default `xvora` already has it).
#[test]
...
/// All three flags on at the runtime default produce xvora-plan
...
fn make_session_info(
    ...
    used: u64,
    total: u64,
) -> xai_xvora_shell::session::SessionInfoResponse {
    use xai_xvora_shell::session::acp_types::{ContextInfo, SessionInfoData};
    xai_xvora_shell::session::SessionInfoResponse {
...
fn format_session_info_shows_model_hash_for_coding_slug_without_flag() {
    let mut info = make_session_info("xvora", None, 1000, 10000);
    info.data.model_fingerprint = Some("abc123".into());
    info.data.show_model_fingerprint = false;
    let text = format_session_info(&info, None, false);
    assert!(text.contains("Model Hash: abc123"));
