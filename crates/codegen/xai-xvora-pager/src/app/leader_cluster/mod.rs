...
//!
//! Env sandboxing follows this crate's `serial(XVORA_HOME)` idiom; note
//! `xvora_home()` is process-cached (OnceLock), so disk assertions always go
//! through [`effective_xvora_home`] rather than assuming the temp dir won.
//!
//! The scenarios are `#[ignore]`d in the shared lib test binary: the harness
//! mutates process-global env (proxy URLs, `XAI_API_KEY`,
//! `XVORA_LEADER_SOCKET`, `XVORA_HOME`) for a real agent's whole lifetime, and
//! in a several-thousand-test process that poisons concurrently-running tests
//! (and `xvora_home()`'s OnceLock is usually already pinned). Run on demand:
//!
//! ```bash
//! cargo test -p xai-xvora-pager --lib -- app::leader_cluster --ignored --test-threads=1
//! ```
//!
...
/// The grok home the agent actually persisted under: `xvora_home()` is
/// process-cached, so an earlier test in this binary may have pinned it.
fn effective_xvora_home() -> PathBuf {
    xai_xvora_config::xvora_home()
}
...
struct PagerLeaderCluster {
    ...
    /// agent on the same XVORA_HOME (two agents on one updates.jsonl is the
...
impl PagerLeaderCluster {
    /// Stand up the cluster. Callers MUST be `#[serial_test::serial(XVORA_HOME)]`
    ...
    async fn start() -> Self {
        ...
        let env = vec![
            ...
            crate::test_util::EnvVarGuard::set("XVORA_CLI_CHAT_PROXY_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("XVORA_XAI_API_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("XAI_API_KEY", "test-key-for-ci"),
            crate::test_util::EnvVarGuard::set("XVORA_TELEMETRY_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("XVORA_FEEDBACK_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("XVORA_TRACE_UPLOAD", "false"),
    ...
    async fn spawn_leader_generation(&mut self) {
        ...
        let control_state = LeaderServerControlState::new(LeaderServerMetadata {
            ...
            // MUST be the client-side comparison source (xai_xvora_version), not
            // this crate's version: a reconnecting client evicts strictly-older
            // leaders, and "evict" here would signal THIS test process.
            leader_binary_version: xai_xvora_version::VERSION.to_string(),
        });
        ...
        generation_tasks.push(tokio::task::spawn_local(async move {
            let _ = run_leader_server(
                ...
                xai_xvora_shell::agent::activity::AgentActivity::default(),
                tokio::sync::watch::channel(true).1,
                tokio::sync::watch::channel(false).0,
                tokio::sync::watch::channel(xai_xvora_shell::leader::ShutdownReason::Manual).0,
                None,
                control_state,
            )
            .await;
        ...
        // xai-xvora-shell/src/agent/app.rs (inside its LocalSet body) — a
        // deliberate copy so production stays untouched. Second copy of the
        // same wiring: xai-xvora-shell/tests/test_leader_soak.rs ("Real agent
    ...
    async fn kill_leader(&mut self) {
        ...
        // be running against the same XVORA_HOME when the next generation's
    ...
    /// `LeaderReconnector` (socket pinned via `XVORA_LEADER_SOCKET`, flock held
