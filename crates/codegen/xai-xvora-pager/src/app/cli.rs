...
fn version_with_channel() -> &'static str {
    ...
    V.get_or_init(|| {
        let label = xai_xvora_update::channel_label();
        xai_xvora_version::display_version_with_commit(env!("VERSION_WITH_COMMIT"), label)
    })
}
...
pub struct PagerArgs {
    ...
    #[arg(
        ...
        value_parser = clap::builder::PossibleValuesParser::new(
            xai_xvora_shell::agent::config::PermissionMode::VALID_VALUES
        )
    )]
...
impl PagerArgs {
    ...
    pub fn saved_resume_profile(&self) -> Option<String> {
        ...
        match self.resume_target() {
            ResumeTarget::SessionId(id) => {
                xai_xvora_shell::session::persistence::resumed_session_sandbox_profile(
                    Some(&id),
                    cwd,
                )
            }
            ResumeTarget::MostRecentForCwd => {
                xai_xvora_shell::session::persistence::resumed_session_sandbox_profile(None, cwd)
            }
    ...
    fn resolve_startup_sandbox(explicit: Option<&str>, saved: Option<String>) -> SandboxStartup {
        match (explicit, saved) {
            (Some(x), Some(s))
                if x.parse::<xai_xvora_sandbox::ProfileName>().ok()
                    != s.parse::<xai_xvora_sandbox::ProfileName>().ok() =>
            {
