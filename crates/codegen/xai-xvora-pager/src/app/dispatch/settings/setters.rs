...
pub(super) fn set_voice_stt_language_inner(app: &mut AppView, canonical: &str) {
    ...
    if language_changed && let Some(tx) = app.voice_cmd_tx.take() {
        app.voice_reset();
        let _ = tx.try_send(xai_xvora_voice::VoiceCommand::Shutdown);
    }
}
...
pub(in crate::app::dispatch) fn set_voice_stt_language(
    ...
    refresh_open_settings_modals(app);
    let effective = xai_xvora_voice::language_for_api(canonical);
    tracing::info!(
        target: "settings",
    ...
    let toast = if canonical == xai_xvora_voice::STT_LANGUAGE_AUTO {
        format!("\u{2713} Voice language: System ({effective})")
    } else {
        let name = xai_xvora_voice::stt_language_by_code(canonical).map_or(canonical, |l| l.name);
...
/// `XVORA_PROMPT_SUGGESTIONS` env var overrides the effective value.
...
pub(super) fn set_contextual_hint_inner(
    app: &mut AppView,
    write: fn(&mut xai_xvora_shell::agent::config::ContextualHints, Option<bool>),
    new: bool,
) {
    write(&mut app.current_ui.contextual_hints, Some(new));
    let resolved = xai_xvora_shell::util::config::resolve_contextual_hints(
...
pub(in crate::app::dispatch) fn set_default_model(
    ...
    // so persisting the human-readable name (e.g. "Xvora")
    // would silently fail to resolve on the next startup.
    //
    // Chat (`--chat` / XVORA_CHAT_MODE) catalogs use opaque `/rest/modes`
    // slugs that must not become the global Build `default_model`.
    let mut effects: Vec<Effect> = Vec::new();
    if !xai_xvora_shell::agent::chat_modes::process_chat_mode_enabled() {
...
pub(in crate::app::dispatch) fn clear_fork_secondary_model(app: &mut AppView) -> Vec<Effect> {
    let baseline = xai_xvora_shell::models::default_model().to_string();
    let prev_id_str = app.current_ui.fork_secondary_model.clone();
