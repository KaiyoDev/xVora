...
use xai_xvora_telemetry::events::{SuperGrokUpsell, SuperGrokUpsellClicked};
...
pub(crate) const UPSELL_URL_UPGRADE: &str = "https://grok.com/supergrok?referrer=xvora";
...
pub(crate) fn acp_error_is_free_usage_exhausted(err: &agent_client_protocol::Error) -> bool {
    err.data
        .as_ref()
        .and_then(xai_xvora_shell::sampling::error::error_detail_from_data)
        .as_deref()
...
pub(crate) const FREE_USAGE_USER_MESSAGE: &str = "You\u{2019}ve reached your free xvora usage limit for now. Get SuperGrok for much higher limits, or try again later: https://grok.com/supergrok?referrer=xvora";
...
pub(super) fn open_credit_limit_upsell(
    ...
    let (
        ...
        xai_xvora_telemetry::events::CreditLimitChoice,
        bool,
    ) = match mode {
        CreditLimitUpsellMode::UnifiedCredits => (
            ...
            "Buy more credits",
            "Purchase credits to keep using xvora",
            CreditLimitCardAction::PurchaseCredits,
            xai_xvora_telemetry::events::CreditLimitChoice::PurchaseCredits,
            false,
        ),
        CreditLimitUpsellMode::LegacyPayg { enabled: true } => (
            ...
            xai_xvora_telemetry::events::CreditLimitChoice::PayAsYouGo,
            true,
        ),
    ...
    if max_tier {
        use crate::scrollback::block::RenderBlock;
        log_event(xai_xvora_telemetry::events::CreditLimitUpsellShown {
            surface: xai_xvora_telemetry::events::CreditLimitUpsellSurface::InlineCard,
            max_tier: true,
    ...
    log_event(xai_xvora_telemetry::events::CreditLimitUpsellShown {
        surface: xai_xvora_telemetry::events::CreditLimitUpsellSurface::QuestionModal,
        max_tier: false,
        pay_as_you_go: payg_telemetry,
        unified_billing,
    });
    ...
    .with_local_kind(LocalQuestionKind::CreditLimitUpsell {
        choices: vec![
            xai_xvora_telemetry::events::CreditLimitChoice::UpgradeTier,
            second_choice,
        ],
    })
    .with_no_freeform();
...
fn open_supergrok_upsell(
    ...
    log_event(xai_xvora_telemetry::events::SuperGrokUpsellShown {
        source,
        auth_method,
    });
    ...
    let options = vec![
        ...
        QuestionOption {
            ...
            description: "Get the most out of xvora. Highest usage limits.".into(),
            preview: None,
...
pub(super) fn handle_gate_refreshed(
    app: &mut AppView,
    settings: Option<xai_xvora_shell::util::config::RemoteSettings>,
) -> Vec<Effect> {
...
pub(super) fn handle_check_subscription_complete(
    ...
    let applied = match meta {
        Some(meta_val) => {
            match serde_json::from_value::<xai_xvora_shell::auth::AuthMeta>(meta_val) {
                Ok(auth_meta) => {
...
pub(super) fn handle_credit_limit_recheck_complete(
    ...
    if let Some(meta_val) = meta
        && let Ok(auth_meta) = serde_json::from_value::<xai_xvora_shell::auth::AuthMeta>(meta_val)
    {
        app.apply_auth_meta(&auth_meta);
    }
...
pub(super) fn dispatch_open_supergrok_url(app: &mut AppView) -> Vec<Effect> {
    ...
    let url = app
        ...
        .unwrap_or("https://grok.com/supergrok?referrer=xvora");
    // Funnel attribution: tag CLI-originated SuperGrok upsell clicks
    // with `referrer=xvora`, matching the OAuth consent flow and
    ...
    let url = crate::app::link_opener::ensure_query_param(url, "referrer", "xvora");
