...
fn plugin_cta_catalog_loaded_sanitizes_components_at_ingestion() {
    ...
    let response = xai_hooks_plugins_types::MarketplaceListResponse {
        sources: vec![xai_hooks_plugins_types::MarketplaceScanResult {
            source_name: xai_xvora_plugin_marketplace::OFFICIAL_SOURCE_NAME.into(),
            source_kind: "git".into(),
            source_url_or_path: xai_xvora_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.into(),
            plugins: vec![entry],
            error: None,
        }],
    };
...
mod cta_e2e {
    ...
    fn dismissed_plugin_suppressed_on_next_recompute() {
        ...
        let dismissed = xai_xvora_shell::config::dismissed_plugin_ctas_in_file(&config_path);
        let matched =
        ...
        xai_xvora_shell::config::add_dismissed_plugin_cta_to_file("figma", &config_path)
