...
/// Plant a Build session under the process `xvora_home()` (OnceLock-cached;
/// do not rely on setting `XVORA_HOME` mid-process). Caller must remove `sess_dir`.
fn plant_local_build_session(cwd: &std::path::Path, session_id: &str) -> std::path::PathBuf {
    let home = xai_xvora_shell::util::xvora_home::xvora_home();
    let encoded = xai_xvora_shell::util::xvora_home::encode_cwd_dirname(&cwd.to_string_lossy());
...
fn make_ask_user_question_args(
    ...
    let req = AskUserQuestionExtRequest {
        ...
        mode: xai_xvora_tools::implementations::xvora::ask_user_question::AskUserQuestionMode::Default,
        questions: vec![
        ...
        vec![
            acp::PermissionOption::new(
                acp::PermissionOptionId::new(Arc::from(
                    xai_xvora_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID,
                )),
...
const POLICY_WARNING: &str =
    xai_xvora_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
