...
//! inside [`xai_xvora_mermaid::render_checked`] is a no-op there: a panic in the
...
//!
//! 1. The pager re-execs itself as `xai-xvora-pager __mermaid-render` (see
...
//!    via [`xai_xvora_mermaid::run_with_timeout`], which **kills and reaps** the
...
//!    ([`xai_xvora_mermaid::RenderLimits`]) and the raster megapixel/height caps
//!    (see `xai-xvora-mermaid`). The parent also rejects obviously-oversized
...
fn render_source_to_png(
    ...
    target_width_px: u32,
    quality: MermaidRenderQuality,
) -> Result<RenderedDiagram, xai_xvora_mermaid::MermaidError> {
...
thread_local! {
    ...
    /// `subagent::REPLAY_XVORA_HOME` test seam. Production never sets this.
...
impl AgentView {
    ...
    fn mermaid_out_path(&self, key: &MermaidCacheKey) -> Option<PathBuf> {
        // Test seam: a hermetic per-test cache dir (no `XVORA_HOME` mutation).
        #[cfg(test)]
...
mod tests {
    ...
    /// hermetic, with no process-global `XVORA_HOME` mutation. The `TempDir` lives
    ...
    /// so there is no cross-test contamination and no `xvora_home()` cache race.
