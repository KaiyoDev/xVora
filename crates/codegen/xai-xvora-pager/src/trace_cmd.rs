...
use xai_xvora_shell::session::repo_changes::UploadMethod;
...
pub(crate) fn find_session_dir(session_id: &str) -> Result<PathBuf> {
    xai_xvora_shell::session::persistence::find_session_dir_by_id(session_id).with_context(|| {
        format!(
...
async fn run_upload(
    ...
    if bucket_url.is_none()
        && matches!(
            upload_method,
            xai_xvora_shell::session::repo_changes::UploadMethod::Direct { .. }
        )
    {
        anyhow::bail!(
    ...
    let upload_config = xai_xvora_shell::session::repo_changes::TraceExportConfig {
...
async fn upload_with_retries(
    config: &xai_xvora_shell::session::repo_changes::TraceExportConfig,
    object_path: &str,
    archive: &[u8],
...
pub async fn resolve_upload_method(agent_config: &AgentConfig) -> Option<UploadMethod> {
    ...
    let auth_token = xai_xvora_shell::auth::ensure_authenticated_or_noninteractive(
