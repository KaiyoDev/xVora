...
//! render mode.
//!
//! `xai-xvora-pager` cannot depend on `xai-xvora-pager-minimal`: that crate reads
...
//!
//! The composition-root binary (`xai-xvora-pager-bin`) wires it up once at
//! startup by calling `xai_xvora_pager_minimal::install()`. When nothing has
