...
/// `~/.xvora/config.toml`. Lenient — corrupted fields fall back to
...
pub struct PendingDispatchModel {
    ...
    pub effort: Option<xai_xvora_shell::sampling::types::ReasoningEffort>,
    pub display: String,
}
    ...
    fn handle_mouse(&mut self, mouse: &crossterm::event::MouseEvent) -> InputOutcome {
        ...
        if matches!(mouse.kind, MouseEventKind::Down(MouseButton::Left)) {
            ...
            if self.upgrade_cta_hit.contains(mouse.column, mouse.row) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    xai_xvora_telemetry::events::AnnouncementCtaSurface::Dashboard,
                ));
            }
...
/// `~/.xvora/config.toml` (`[ui]`, `[hints]`, `[mcpServers]`, …).
///
...
mod tests {
    ...
    /// per-key serial lock. The `XVORA_AGENT_DASHBOARD` key means this
    /// test runs serially with any other test that decorates itself
    /// with `#[serial_test::serial(XVORA_AGENT_DASHBOARD)]` — see the
    ...
    fn env_var_force_disables() {
        ...
        // same `XVORA_AGENT_DASHBOARD` key reads it concurrently.
