...
use xai_xvora_tools::implementations::skills::types::SkillInfo;
...
impl ExtensionsTab {
    ...
    pub fn telemetry_tab(self) -> xai_xvora_telemetry::events::ExtensionsModalTab {
        use xai_xvora_telemetry::events::ExtensionsModalTab;
        match self {
...
pub enum ButtonAction {
    ...
    AddMcpServer {
        name: String,
        config: Box<xai_xvora_shell::util::config::McpServerConfig>,
    },
...
fn parse_mcp_add_fields(name: &str, url_or_cmd: &str) -> Option<ButtonAction> {
    use xai_xvora_shell::util::config::{McpServerConfig, McpServerTransportConfig};
...
pub fn derive_source_label(source_dir: &str) -> (String, bool) {
    let grok = xai_xvora_config::xvora_home();
    let source_path = std::path::Path::new(source_dir);
    // Plugin / installed-plugin dirs, under the user grok home (XVORA_HOME-aware)
    // or a project-scoped `{cwd}/.xvora/<subdir>/`. Returns the first path
    ...
    let plugin_name = |subdir: &str| -> Option<String> {
        ...
        // User grok home (XVORA_HOME-aware).
    ...
    let display = {
        if let Ok(rest) = source_path.strip_prefix(&grok) {
            let prefix = crate::util::display_xvora_home_prefix();
...
fn skill_source_str(skill: &SkillInfo) -> String {
    if let Some(ref cs) = skill.config_source {
        match cs {
            xai_xvora_tools::types::config_source::ConfigSource::User { path } => {
            ...
            xai_xvora_tools::types::config_source::ConfigSource::Project { path } => {
            ...
            xai_xvora_tools::types::config_source::ConfigSource::Plugin { plugin_name, .. } => {
...
mod tests {
    ...
    fn derive_source_label_detects_project_scoped_plugins() {
        // Regression: project-scoped `{cwd}/.xvora/plugins/<name>/` must label as
        // a (non-removable) plugin, not a removable "Custom" source. The user
        // xvora-home branch is XVORA_HOME-aware; this covers the project fallback.
    ...
    fn make_skill(
        ...
        xai_xvora_tools::implementations::skills::types::SkillInfo {
            ...
            metadata: None,
            path: "test".to_string(),
            scope: xai_xvora_tools::implementations::skills::types::SkillScope::User,
            config_source: None,
            plugin_name: None,
            plugin_version: None,
            plugin_root: None,
    ...
    fn make_plugin_skill(
        ...
        skill.scope = xai_xvora_tools::implementations::skills::types::SkillScope::Plugin;
    ...
    fn mcp_add_url_derives_name() {
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                assert!(matches!(
                    config.transport,
                    xai_xvora_shell::util::config::McpServerTransportConfig::StreamableHttp { .. }
                ));
            }
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                match config.transport {
                    xai_xvora_shell::util::config::McpServerTransportConfig::Stdio {
                        command,
                        args,
                        ..
                    } => {
    ...
    fn local_plugins_source() -> xai_hooks_plugins_types::MarketplaceScanResult {
        xai_hooks_plugins_types::MarketplaceScanResult {
            ...
            source_url_or_path: "/home/user/.xvora/marketplace/local".into(),
            plugins: vec![
                TestPlugin {
                    name: "my-linter",
                    version: Some("0.1.0"),
