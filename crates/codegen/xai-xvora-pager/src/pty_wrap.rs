...
pub(crate) fn run_wrapped_command(program: &str, args: &[String]) -> Result<i32> {
    ...
    // reads this (see `xai_xvora_pager_render::clipboard::osc52_sink_active`) to
...
///
/// Delegates to [`xai_xvora_shell::util::clipboard::set_text`] which uses
...
fn set_local_clipboard(data: &[u8]) {
    ...
    if let Err(e) = xai_xvora_shell::util::clipboard::set_text(text) {
...
fn host_clipboard_image_frame() -> Vec<u8> {
    let image = xai_xvora_pager_render::clipboard::system_clipboard_get_image();
