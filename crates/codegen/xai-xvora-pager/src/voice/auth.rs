...
use xai_xvora_voice::{SharedVoiceAuth, VoiceAuthProvider};
...
pub fn build_voice_auth(auth_manager: Arc<xai_xvora_shell::auth::AuthManager>) -> SharedVoiceAuth {
    Arc::new(AuthManagerVoiceAuth(
        xai_xvora_shell::auth::shared_api_key_provider(auth_manager),
    ))
}
