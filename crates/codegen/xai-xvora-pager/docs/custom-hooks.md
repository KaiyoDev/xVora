...
2. Create a simple hook file, e.g. `~/.xvora/hooks/session-start.json`:
   ```json
   {
     "hooks": {
       "SessionStart": [
...
|-----------|-----------------------------------|--------------|-------|
| Global    | `~/.xvora/hooks/*.json`            | Always       | Best for personal hooks |
...
**Trusting a project**: Open the hooks modal (`Ctrl+L` on non–VS Code family, or `/hooks` on any terminal including VS Code family) or run `/hooks-trust` (the same folder-trust gate as `--trust`, recorded in `~/.xvora/trusted_folders.toml`) the first time you open a project with hooks. This prevents untrusted repos from running arbitrary code.
...
- `XVORA_HOOK_EVENT` — the event name (e.g. `pre_tool_use`, `session_start`, `post_tool_use`)
- `XVORA_HOOK_NAME` — the full configured name of this hook
- `XVORA_SESSION_ID` — the current session identifier
- `XVORA_WORKSPACE_ROOT` — absolute path to the workspace root

For hooks provided by plugins, the following are also set:

- `XVORA_PLUGIN_ROOT` — absolute path to the plugin's installation directory
- `XVORA_PLUGIN_DATA` — absolute path to the plugin's writable data directory

These runner- and plugin-injected variables always take precedence. Attempts to override the reserved runner keys via the `env` field are stripped at load time (with a warning logged). For plugin hooks, `XVORA_PLUGIN_ROOT` and `XVORA_PLUGIN_DATA` similarly override any user-supplied values for those keys.
...
`XVORA_PLUGIN_ROOT` and `XVORA_PLUGIN_DATA`. These keys override any user-declared
...
`${XVORA_PLUGIN_ROOT}/check` resolve against the plugin's actual path.
...
Hooks from `~/.xvora/hooks/` appear under **Global**, project ones under **Project**, etc.
...
- Global hooks (`~/.xvora/...`) run with your user permissions — treat them like shell scripts.
...
- **See errors?** → Check the pager logs (usually in the tracing pane or `~/.xvora/logs`).

## More Examples

See the built-in examples in the `xai-xvora-hooks` crate:

- [Safe Shell Guard](../../../xai-xvora-hooks/examples/hooks/safe-shell.json)
- [No Recursive Grep](../../../xai-xvora-hooks/examples/hooks/no-recursive-grep.json) — hard-blocks `grep -r`/`grep -R`/`rgrep` (OOM guard)
- [Session Audit Log](../../../xai-xvora-hooks/examples/hooks/session-log.json)
- [Tool Activity Logger](../../../xai-xvora-hooks/examples/hooks/tool-logger.json)

Copy them to `~/.xvora/hooks/` and customize.
