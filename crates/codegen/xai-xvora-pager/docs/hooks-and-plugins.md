...
Xvora supports **hooks** (event-driven shell commands) and **plugins** (bundles of skills, agents, hooks, and MCP servers). Both are managed through a unified modal interface.
...
- **Global hooks** — from `~/.xvora/hooks/`
- **Project hooks** — from `.xvora/hooks/` in your repo
...
Plugin hooks automatically receive `XVORA_PLUGIN_ROOT` and `XVORA_PLUGIN_DATA` environment variables (see the [Plugins guide](../user-guide/09-plugins.md#environment-variables-in-plugin-hooks)).
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.xvora/settings.json` or `~/.claude/settings.json`
...
Sources land in `~/.xvora/config.toml`:
...
Or in `~/.xvora/settings.json` / `~/.claude/settings.json`:
...
- [Hook Examples](../../../xai-xvora-hooks/examples/README.md) — ready-to-use sample hooks
