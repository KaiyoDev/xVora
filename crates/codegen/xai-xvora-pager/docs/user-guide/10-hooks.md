...
| Project | `<project>/.xvora/hooks/*.json` | Requires trust | Per-repo automation |
...
The Claude and Cursor hook sources are scanned by default. To disable scanning for a specific vendor, set `[compat.<vendor>] hooks = false` in `~/.xvora/config.toml` or the corresponding environment variable. See [Configuration](05-configuration.md#harness-compatibility) for details.
...
Because hooks are unified under folder-trust, a `--trust` / `/hooks-trust` grant trusts the whole folder for **MCP, LSP, and hooks** together, and cascades to subdirectories. Conversely, disabling folder-trust (`XVORA_FOLDER_TRUST=0` or `[folder_trust] enabled = false`) ungates project hooks along with MCP/LSP.
...
| `XVORA_SESSION_ID`     | The unique identifier of the current Grok session. |
...
5. **Version control project hooks** -- commit `.xvora/hooks/` (but never secrets).
...
- **See errors?** Capture logs by launching with `RUST_LOG=debug XVORA_LOG_FILE=/tmp/grok.log grok`, then check `/tmp/grok.log`.
