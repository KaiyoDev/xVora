...
3. **config.toml** (`~/.xvora/config.toml`)
...
The setting can also be overridden with the `XVORA_DEFAULT_SELECTED_PERMISSION`
...
`[ui] vim_mode` in `~/.xvora/config.toml` immediately and applies it to every
...
`xvora` launches. Configure it from `/settings` → **Default screen mode**
...
`XVORA_SCROLL_SPEED`, `XVORA_SCROLL_MODE`, `XVORA_INVERT_SCROLL`
(`1`/`true`/`0`/`false`), and `XVORA_SCROLL_LINES`. Precedence: env var →
...
(`XVORA_ASK_USER_QUESTION_TIMEOUT_ENABLED` /
`XVORA_ASK_USER_QUESTION_TIMEOUT_SECS`) → user config → managed →
...
MCP servers can also be configured per-project in `.xvora/config.toml`. Project-scoped config contributes `[mcp_servers]`, `[plugins]`, and `[permission]` rules; other sections load only from `~/.xvora/config.toml`.

Priority for `[mcp_servers]` and `[plugins]`: `.xvora/config.toml` (current dir) > `<repo-root>/.xvora/config.toml` > `~/.xvora/config.toml`. `[permission]` rules are not overridden by priority; they merge across all files with `deny` > `ask` > `allow` (see [22-permissions-and-safety.md](22-permissions-and-safety.md)).

### Memory

Persist knowledge across sessions (requires `--experimental-memory` or `XVORA_MEMORY=1`).
...
[subagents.models]
explore = "xvora"               # route to different models
```
...
`xvora inspect` reports cells that still need session-start resolution as
...
`[hints]` is read from the **effective config merge** (same precedence as other settings): system managed → user `managed_config.toml` → user `config.toml` → user `requirements.toml` → system `requirements.toml`. Higher-priority layers override lower ones. The TUI only **writes** opt-outs to user `~/.xvora/config.toml`.
...
`$XVORA_EVENT`, `$XVORA_MESSAGE`, and `$XVORA_SESSION_ID`.

```toml
# macOS native notification
[[ui.notifications.hooks]]
command = "terminal-notifier -title 'Grok' -message '$XVORA_MESSAGE'"
events = ["turn_complete", "approval_required"]
only_unfocused = true
timeout_secs = 10
...
| Variable | Description |
|----------|-------------|
| `XVORA_LOG_FILE` | Write logs to this file path (the value is used verbatim as the path) |
| `RUST_LOG` | Log level filter (for example `debug`); controls the `XVORA_LOG_FILE` log and headless stderr output |
...
| `~/.xvora/logs/` | Internal log files (for example `unified.jsonl`, MCP server logs) |
| `.xvora/config.toml` | Project-scoped MCP servers, plugins, and permission rules |
...
Some configuration can be set per-project by placing files in `.xvora/` within your repository:

| File | What it configures |
|------|--------------------|
| `.xvora/config.toml` | MCP servers, plugins, permission rules, and the `[mcp] max_output_bytes` tool-result cap (other sections load only from `~/.xvora/config.toml`) |
| `.xvora/skills/` | Project-specific skills |
| `.xvora/hooks/` | Project-specific lifecycle hooks |
| `.xvora/agents/` | Project-specific agent definitions |
| `.xvora/lsp.json` | LSP server configuration |
| `.xvora/sandbox.toml` | Custom sandbox profiles |
...
| Project | `.xvora/lsp.json` | Current repository |
