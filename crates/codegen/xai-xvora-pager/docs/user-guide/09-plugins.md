...
- **Hooks** -- a `hooks/hooks.json` file of lifecycle hooks. Plugin hooks also receive `XVORA_PLUGIN_ROOT` and `XVORA_PLUGIN_DATA` (see the [Hooks guide](10-hooks.md) for every environment variable passed to hooks).
...
| `XVORA_PLUGIN_DATA`   | Absolute path to the plugin's writable data directory, for plugin state, caches, and logs. |
...
| `.xvora/plugins/` | Project -- shared with the team through version control | Requires trust |
| `~/.xvora/plugins/` | User -- personal plugins for every project | Trusted automatically |
...
Configure plugin directories and per-plugin state in `~/.xvora/config.toml`:
...
To hide the hooks and plugins UI — the `/hooks` and `/plugins` commands and the scrollback annotations — set this in `~/.xvora/pager.toml`:
...
Grok trusts plugins from `~/.xvora/plugins/` automatically. Project plugins in `.xvora/plugins/` require explicit trust. To trust a plugin, install it with `--trust`:
