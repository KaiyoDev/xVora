...
|----------|-------|----------|-------|
| `./.xvora/skills/`, `./.xvora/commands/` | Local (CWD) | Highest | Current directory skills / legacy command markdown |
| `<repo_root>/.xvora/skills/`, `…/commands/` | Repo | Medium | Shared across the repo |
...
Grok deduplicates skills by name -- a higher-priority location overrides a lower one. Grok also scans `.agents/skills/` (and `commands/`) at each tier (alongside `.xvora/`) and walks every directory between your working directory and the repo root.
...
Skill and command discovery does **not** use `.gitignore`. Paths under known skill roots (`.xvora/`, `.agents/`, `.claude/`, `.cursor/`) always load when present on disk — teams often ignore `.claude/**` as local-only config while still expecting `/frontend`-style project commands to work. To hide a skill, use `[skills] ignore` in config (not repo ignore rules).

Grok scans the Claude and Cursor skill directories by default. To stop scanning a vendor, set its `skills` cell to `false` under `[compat.cursor]` or `[compat.claude]` in `~/.xvora/config.toml`, or set the `XVORA_CURSOR_SKILLS_ENABLED` or `XVORA_CLAUDE_SKILLS_ENABLED` environment variable to `false`. See [Configuration](05-configuration.md#harness-compatibility) for details. Grok always filters out known vendor-shipped default skills (such as Cursor's `shell`, `canvas`, and `statusline`), regardless of these settings.
...
3. **Creates the skill directory.** Grok creates the `<scope>/.xvora/skills/<name>/` directory, plus `scripts/` or `references/` subdirectories when the skill needs them.
...
- **Project** (`<repo_root>/.xvora/skills/<name>/`) -- available only in this repository and shareable with teammates through version control. Grok recommends this scope inside a git repository.
...
To browse your skills, type `/` to open the slash-command menu. Grok lists every built-in command and skill and filters them as you type. To list skills from the command line instead, run `xvora inspect` (see [Viewing Skill Details](#viewing-skill-details)).
...
```
/local:commit        # The "commit" skill from ./.xvora/skills/
...
Run `xvora inspect` to see every skill Grok discovers, along with the rest of your configuration:
...
Grok ships with built-in skills and extracts them to `~/.xvora/skills/` on startup -- among them `/create-skill`, `/help`, and `/check-work`. Bundled skills behave like user skills, and a same-named skill in a higher-priority location (local or repo) overrides the bundled copy; `xvora inspect` labels the extracted copies `bundled` so they stay distinguishable from skills you authored yourself. (A plugin skill of the same name does not override it; it stays available under its qualified `plugin:name` form.)
...
5. **Version-control project skills.** Commit `.xvora/skills/` to your repository so the whole team benefits. User skills in `~/.xvora/skills/` stay personal and unshared.
