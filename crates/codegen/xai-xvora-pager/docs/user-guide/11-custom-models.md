...
4. The `XAI_API_KEY` environment variable (global fallback; Grok also accepts `XVORA_CODE_XAI_API_KEY` for backward compatibility)
...
|----------|----------|-------------|
| `XVORA_MODELS_BASE_URL` | Yes | Base URL for inference. Grok fetches the model list from `{base_url}/models`. |
| `XAI_API_KEY` | Yes | API key sent as `Authorization: Bearer`. Grok also accepts `XVORA_CODE_XAI_API_KEY`. |
...
When you set `models_base_url`, Grok uses API key auth (`Authorization: Bearer`) instead of session auth. You do not need `xvora login` -- the API key is enough.
...
```bash
export XVORA_WEB_SEARCH_MODEL="grok-4.20-multi-agent"
```
