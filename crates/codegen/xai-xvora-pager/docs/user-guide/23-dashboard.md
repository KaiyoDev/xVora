...
- **Ctrl+\\** — same as the slash command, two keystrokes. Configurable
  in `~/.xvora/config.toml` under `[keybindings]` like every other shortcut.
...
```
 Xvora · Dashboard — 4 agents · 2 awaiting
...
The state icon matches Xvora's sibling views (
`tasks_pane`):
...
Set `XVORA_AGENT_DASHBOARD=0` to force-disable the feature for a single
