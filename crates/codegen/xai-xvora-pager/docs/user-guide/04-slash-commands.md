...
- **Shell builtins** -- handled by the agent backend (xai-xvora-shell)
- **Pager builtins** -- handled by the TUI frontend (xai-xvora-pager)
...
`--fullscreen` CLI flags are likewise session-scoped. To make plain `xvora` open
...
The `/flush`, `/dream`, and `/memory` commands require `--experimental-memory` or `XVORA_MEMORY=1`. `/remember` is always available.
...
Any enabled skill with `user-invocable: true` in its SKILL.md frontmatter appears as a slash command. (A skill turned off via `/skills` is not advertised.) For example, if you have a skill at `~/.xvora/skills/commit/SKILL.md`, you can invoke it with:
