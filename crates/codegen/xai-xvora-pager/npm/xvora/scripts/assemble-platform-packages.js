...
// (`@xai-official/xvora`) lists all six as `optionalDependencies` pinned to
...
const NOTICES_SOURCE = path.resolve(
    npmRoot, '..', '..', 'xai-xvora-tools', 'THIRD_PARTY_NOTICES.md');
...
async function main() {
    const targets = [
        {
            platform: 'darwin', arch: 'arm64', binName: 'xvora',
            envVar: 'XVORA_DARWIN_ARM64',
            defaultSource: path.join(xaiRoot, 'target', 'release', 'xvora'),
        },
        {
            platform: 'darwin', arch: 'x64', binName: 'xvora',
            envVar: 'XVORA_DARWIN_X64',
            defaultSource: path.join(xaiRoot, 'target', 'x86_64-apple-darwin', 'release', 'xvora'),
        },
        {
            platform: 'linux', arch: 'x64', binName: 'xvora',
            envVar: 'XVORA_LINUX_X64',
            defaultSource: path.join(xaiRoot, 'target',
                'explorer_cross_x86_64-unknown-linux-gnu',
                'x86_64-unknown-linux-gnu', 'release', 'xvora'),
        },
        {
            platform: 'linux', arch: 'arm64', binName: 'xvora',
            envVar: 'XVORA_LINUX_ARM64',
            defaultSource: path.join(xaiRoot, 'target',
                'explorer_cross_aarch64-unknown-linux-gnu',
                'aarch64-unknown-linux-gnu', 'release', 'xvora'),
        },
        {
            platform: 'win32', arch: 'x64', binName: 'xvora.exe',
            envVar: 'XVORA_WIN32_X64',
            defaultSource: path.join(xaiRoot, 'target', 'x86_64-pc-windows-msvc', 'release', 'xvora.exe'),
        },
        {
            platform: 'win32', arch: 'arm64', binName: 'xvora.exe',
            envVar: 'XVORA_WIN32_ARM64',
            defaultSource: path.join(xaiRoot, 'target', 'aarch64-pc-windows-msvc', 'release', 'xvora.exe'),
        },
    ];
