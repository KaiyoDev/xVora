...
// matching per-platform optional dependency (@xai-official/xvora-<platform>)
// and installs it to ~/.xvora/bin/ using versioned filenames:
//
//   Unix:    xvora-<version>  +  grok  (symlink)
//   Windows: xvora-<version>.exe  +  grok.exe  (copy)
//
...
const CANONICAL_DIR = path.join(os.homedir(), '.xvora', 'bin');
...
if (!SUPPORTED.has(key)) {
    console.error(`@xai-official/xvora: unsupported platform ${key}`);
    process.exit(0);
}
...
function resolvePlatformPackageDir() {
    const platformPkg = `@xai-official/xvora-${key}`;
    try {
...
if (!version) {
    console.error('@xai-official/xvora: unable to determine version');
    process.exit(0);
}
...
function installBinary(binName, sourceDir, vendorSubpath) {
    ...
    if (fs.existsSync(brPath)) {
        ...
        console.error(`@xai-official/xvora: missing binary at ${brPath}`);
        return false;
    }
    ...
    if (IS_WINDOWS) {
        ...
        try {
            ...
            try {
                ...
                console.error(`@xai-official/xvora: failed to update ${canonicalPath}: ${e2.message}`);
...
// Uses an exact prefix match + hyphen + digit to avoid grok-* matching xvora-pager-*.
...
if (!platformDir) {
    console.error(`@xai-official/xvora: platform package @xai-official/xvora-${key} not installed.`);
    console.error('  This usually means npm was invoked with --no-optional, or the install failed.');
    console.error('  Try: npm install -g @xai-official/xvora');
    process.exit(0);
}

installBinary('xvora', platformDir, `grok${EXE}`);
cleanupOldVersions('xvora');
cleanupOldVersions('xvora-pager');

// Write installer config
const configDir = path.join(os.homedir(), '.xvora');
...
const npmRegistry = process.env.XVORA_NPM_REGISTRY
    || (() => {
        try {
...
// Set XVORA_INSTALL_COMPLETIONS=1 to auto-generate to ~/.xvora/completions.
const XVORA_PATH = path.join(CANONICAL_DIR, `grok${EXE}`);
if (process.env.XVORA_INSTALL_COMPLETIONS === '1' && !IS_WINDOWS) {
    try {
        const { spawnSync } = require('child_process');
        const completionsDir = path.join(os.homedir(), '.xvora', 'completions');
        ...
        const bashRes = spawnSync(XVORA_PATH, ['completions', 'bash'], { encoding: 'utf8' });
        if (bashRes.status === 0) fs.writeFileSync(bashPath, bashRes.stdout);
        const zshRes = spawnSync(XVORA_PATH, ['completions', 'zsh'], { encoding: 'utf8' });
        if (zshRes.status === 0) fs.writeFileSync(zshPath, zshRes.stdout);
        console.log('Completions generated to ~/.xvora/completions (bash/zsh)');
    } catch {}
} else if (!IS_WINDOWS) {
