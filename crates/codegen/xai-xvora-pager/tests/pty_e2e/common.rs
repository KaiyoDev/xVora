...
pub(crate) use xai_xvora_pager_pty_harness::{
...
// `xai_xvora_pager_pty_harness::flows` (re-exported above).
...
pub(crate) fn folder_is_trusted(content: &ContentController, repo: &std::path::Path) -> bool {
    let store_path = content
        .home()
        .join(".xvora")
        .join(xai_xvora_workspace::trust::TRUST_FILE_NAME);
    let store = xai_xvora_workspace::trust::TrustStore::load_from(store_path);
    store.is_trusted(&xai_xvora_workspace::trust::workspace_key(repo))
}
...
// `wait_for_labels_absent` lives in `xai_xvora_pager_pty_harness::flows`
// (re-exported above).
...
// unit tests in xai-xvora-hooks::discovery::tests. The PTY E2E test requires
...
pub(crate) fn spawn_esc_double_press_pager(content: &ContentController) -> PtyHarness {
    ...
    env.push((
        ...
        xai_xvora_pager::app::app_view::ESC_DOUBLE_PRESS_TEST_MS.to_string(),
    ));
    let env_refs: Vec<(&str, &str)> = env.iter().map(|(k, v)| (k.as_str(), v.as_str())).collect();
    PtyHarness::new(&binary, DEFAULT_ROWS, DEFAULT_COLS, &[], &env_refs).expect("spawn pager")
}
...
pub(crate) use xai_xvora_pager_pty_harness::host_clipboard::{
...
pub(crate) use xai_xvora_pager_pty_harness::host_clipboard::clipboard_roundtrip_works;
