...
async fn prompt_suggestion_ghost_tab_accepts() {
    // The suggestion model (`xvora-0.1`, the shell's built-in default)
    ...
    env.retain(|(k, _)| k != "XVORA_PROMPT_SUGGESTIONS");
    env.push(("XVORA_PROMPT_SUGGESTIONS".into(), "true".into()));
