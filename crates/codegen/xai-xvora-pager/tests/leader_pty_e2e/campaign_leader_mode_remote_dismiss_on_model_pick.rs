...
#[ignore = "PTY e2e; run with cargo test -p xai-xvora-pager --test leader_pty_e2e -- --ignored --test-threads=1"]
async fn campaign_leader_mode_remote_dismiss_on_model_pick() {
    ...
    // socket under the shared XVORA_HOME so every spawn elects/attaches to the
