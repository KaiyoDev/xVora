...
//!
//! These tests exercise the real `xai-xvora-pager` binary through a PTY using
...
use xai_xvora_pager_pty_harness::{
...
/// enterprise deploy report: with `XVORA_GOAL=1`, `/goal` must
...
/// explicitly off (`XVORA_GOAL=0`; goal mode defaults on), `/goal` must stay
...
/// Full folder-trust session: with `XVORA_FOLDER_TRUST=1` and a git repo that
...
/// also pins `XVORA_CONTEXTUAL_HINTS=0` (belt-and-suspenders); the same planning
