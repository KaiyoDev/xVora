...
fn xvora_home_override_path_helpers() {
    ...
    assert_eq!(
        xai_xvora_pager::util::pager_toml_path(),
        xvora_home.join("pager.toml")
    );
    assert_eq!(
        xai_xvora_pager::util::display_xvora_home_prefix(),
        "$XVORA_HOME"
    );
    assert_eq!(
        xai_xvora_pager::util::display_user_grok_path("config.toml"),
        "$XVORA_HOME/config.toml"
    );

    let memory_path = xvora_home.join("memory/MEMORY.md");
    assert_eq!(
        xai_xvora_pager::util::abbreviate_path(&memory_path.display().to_string()),
        "$XVORA_HOME/memory/MEMORY.md"
    );

    assert!(xai_xvora_pager::util::is_under_user_xvora_home(&memory_path));
    assert!(!xai_xvora_pager::util::is_under_user_xvora_home(
