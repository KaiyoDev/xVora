...
//! Row/input details are unit-tested in `xvora-pager` (`views/settings_modal/tests.rs`, `locked_coding_*`).
//! This suite covers the auth-to-render pipeline.
//!
//! ```bash
//! cargo test -p xvora-pager-pty-harness --test settings_locked_row_e2e \
//!   -- --ignored --nocapture
//! ```
