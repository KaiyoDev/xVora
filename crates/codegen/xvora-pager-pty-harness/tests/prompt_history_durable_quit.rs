...
//!
//! ```bash
//! cargo test -p xvora-pager-pty-harness --test prompt_history_durable_quit \
//!   -- --ignored --nocapture
//! ```
...
use xvora_pager_pty_harness::{ContentController, PtyExitPoll, PtyHarness, keys, pager_binary};
...
/// The per-CWD history file lives at `<home>/.xvora/sessions/<enc-cwd>/prompt_history.jsonl`.
fn find_prompt_history(home: &Path) -> Result<PathBuf> {
    let root = home.join(".xvora").join("sessions");
