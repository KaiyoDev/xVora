...
//!
//! ```bash
//! cargo test -p xvora-pager-pty-harness --test privacy_banner_e2e \
//!   -- --ignored --nocapture
//! ```
...
/// Poll `<home>/.xvora/config.toml` for the async `privacy_banner_acked` write.
...
fn wait_for_ack_on_disk(pager: &mut PtyHarness, home: &Path, timeout: Duration) -> Result<()> {
    let path = home.join(".xvora").join("config.toml");
