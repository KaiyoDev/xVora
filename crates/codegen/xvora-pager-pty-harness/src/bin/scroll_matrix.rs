//! `scroll-matrix` — scroll validation matrix sweep for xvora-pager.
//!
...
use xvora_pager_pty_harness::pager_binary;
use xvora_pager_pty_harness::scroll_matrix::{
...
#[command(
    name = "scroll-matrix",
    about = "Run the scroll validation matrix against xvora-pager",
    long_about = None,
)]
struct Cli {
    /// Cell tier to run.
    ...
    /// Pager binary. Defaults to PAGER_BINARY, CARGO_BIN_EXE_xvora-pager,
