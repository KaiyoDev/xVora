...
//!
//! Wire schema source of truth: the pager's `ScrollLogRecord` in `xvora-pager/src/input/scroll_log.rs`.
...
mod tests {
    ...
    // The lines are copied from `scroll_log_records_flood_flushes_and_capped_finalize_drop` in `xvora-pager/src/input/mouse/tests.rs`
