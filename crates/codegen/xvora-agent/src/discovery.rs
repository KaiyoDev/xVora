...
/// Project-level agent directories to scan (`.xvora/agents/` plus `.claude/agents/` for compat).
const PROJECT_AGENT_SUBDIRS: &[&str] = &[".xvora/agents", ".claude/agents"];

/// Existing project-level agent dirs (`.xvora/agents` / `.claude/agents`), walked from `cwd` up to the git worktree root (inclusive).
...
/// Existing project agent dirs (`.xvora/agents` / `.claude/agents`) under each dir of a cwd-to-git-root chain ([`crate::repo::RepoDirChain`]).
///
...
pub fn all_subagents(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<SubagentEntry> {
    ...
    all_subagents_with_home(cwd, toggle, dirs::home_dir().as_deref(), xvora.as_deref())
}
...
/// User-level agent directories in priority order: user xvora agents, `.claude`
...
pub fn discover(cwd: &Path) -> Vec<AgentDefinition> {
    let xvora = config::user_grok_home();
    discover_with_home(cwd, dirs::home_dir().as_deref(), xvora.as_deref())
}
...
pub fn by_name(name: &str) -> Option<AgentDefinition> {
    let xvora = config::user_grok_home();
    by_name_with_home(name, dirs::home_dir().as_deref(), xvora.as_deref())
}
...
/// Project-level `.xvora/agents/` has highest priority, then falls back to built-ins, user-level, and finally bundled definitions.
pub fn by_name_in_cwd(name: &str, cwd: &Path) -> Option<AgentDefinition> {
    let xvora = config::user_grok_home();
    by_name_in_cwd_with_home(name, cwd, dirs::home_dir().as_deref(), xvora.as_deref())
}
...
/// Expand `${CLAUDE_PLUGIN_ROOT}` / `${CLAUDE_PLUGIN_DATA}` (and the xvora aliases) in a plugin agent's body so the model receives absolute paths.
...
/// Load project agent definitions from every `.xvora/agents` / `.claude/agents` dir that [`project_agent_dirs`] finds along the cwd-to-git-root walk.
...
mod tests {
    ...
    fn user_agent_dirs_includes_legacy_grok_when_grok_home_differs() {
        ...
        let xvora = Path::new("/custom/grokhome");
        let paths: Vec<_> = user_agent_dirs(Some(home), Some(xvora))
            .into_iter()
            .map(|(p, _)| p)
            .collect();
        assert!(paths.contains(&xvora.join("agents")));
        assert!(paths.contains(&home.join(".xvora").join("agents")));
        assert!(paths.contains(&home.join(".claude").join("agents")));
        assert!(paths.contains(&xvora.join("bundled").join("agents")));
    ...
    fn user_agent_dirs_dedups_legacy_when_grok_home_is_dot_grok() {
        let home = Path::new("/home/u");
        let xvora = home.join(".xvora");
        let count = user_agent_dirs(Some(home), Some(&xvora))
            .into_iter()
            .filter(|(p, _)| *p == xvora.join("agents"))
            .count();
        assert_eq!(
            count, 1,
    ...
    fn test_by_name_builtin_grok_build() {
        let def = by_name("xvora-build");
        assert!(def.is_some());
        assert_eq!(def.unwrap().name, "xvora-build");
    }
    ...
    fn test_by_name_in_cwd_project_shadows_builtin() {
        ...
        // Create a project-level "xvora-build" that shadows the built-in
        write_agent_file(
            &agents_dir,
            "xvora-build.md",
            "xvora-build",
        ...
        let def = by_name_in_cwd("xvora-build", tmp.path());
        assert!(def.is_some());
        let def = def.unwrap();
        assert_eq!(def.name, "xvora-build");
        assert_eq!(def.description, "Custom xvora-build");
    }

    #[test]
    fn test_by_name_in_cwd_falls_back_to_builtin() {
        let tmp = tempfile::tempdir().unwrap();
        // No .xvora/agents/ directory, so lookup falls back to the built-in
    ...
    fn test_orchestrator_from_str_resolves() {
        use std::str::FromStr;
        let variant = BuiltinAgentName::from_str("xvora-build-orchestrator")
            .expect("from_str must resolve xvora-build-orchestrator");
        assert_eq!(variant, BuiltinAgentName::GrokBuildOrchestrator);
        let def = variant.definition();
        assert_eq!(def.name, "xvora-build-orchestrator");
        assert!(
    ...
    fn test_orchestrator_by_name_in_cwd() {
        let tmp = tempfile::tempdir().unwrap();
        let def = by_name_in_cwd("xvora-build-orchestrator", tmp.path())
            .expect("by_name_in_cwd must find xvora-build-orchestrator");
        assert_eq!(def.name, "xvora-build-orchestrator");
