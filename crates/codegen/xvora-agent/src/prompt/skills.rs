...
//! live in `xvora_tools::implementations::skills::discovery`.
...
/// vendor-compat config without reaching into `xvora_tools` directly.
...
pub async fn list_skills_with_plugins(
    ...
    let mut skills = list_skills_with_options(
        ...
        &xvora_tools::util::xvora_home::xvora_home(),
        compat,
    )
    .await;
...
mod tests {
    ...
    // these functions now live in xvora-tools::implementations::skills::discovery
    // and xvora-tools::types::skill_discovery_tracker, tested there.
