...
//! `TemplateRenderer` in `xvora-tools`. This struct does NOT own a
...
pub struct PromptContext {
    ...
    /// Identity in the primary xvora system prompt (`You are <label>…`).
    /// Not the UI picker name. Defaults to [`DEFAULT_SYSTEM_PROMPT_LABEL`].
    #[serde(default = "default_system_prompt_label")]
    pub system_prompt_label: String,
}
/// Default identity for the main system prompt (`You are xVora, … by KaiyoDev`).
...
mod tests {
    ...
    fn trimmed_prompts_no_redundant_identity() {
        ...
        for (name, prompt) in [
            ...
            assert!(
                !prompt.contains("You are a Xvora agent"),
