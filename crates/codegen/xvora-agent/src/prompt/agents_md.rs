...
async fn read_agents_config_with_options(
    ...
    let global_dir = xvora_tools::util::xvora_home::xvora_home();
...
/// Used by `xvora-shell` to structurally detect legacy untagged AGENTS.md
...
fn render_agents_md(configs: &[AgentConfigFile]) -> Option<String> {
    ...
    for config in configs {
        ...
        let content = if is_rules_file {
            xvora_tools::implementations::skills::skill::extract_skill_body(&config.content)
        } else {
