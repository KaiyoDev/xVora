//! Plugin hooks adapter: pre-filter plugin hook JSON, then feed it to `xvora-hooks`' parser and inject plugin env vars.
...
fn process_hooks_content(
    ...
    for spec in &mut specs {
        ...
        spec.layer = xvora_hooks::config::HookProvenance::Plugin;
        spec.name = format!(
            "{}{}/{}",
            xvora_hooks::config::PLUGIN_HOOK_PREFIX,
            plugin_name,
            spec.name
        );
        ...
        if let Some(cmd) = &spec.command {
            ...
            let expanded = xvora_hooks::config::expand_env_skipping_runner_vars(&substituted);
            if expanded != cmd_str {
