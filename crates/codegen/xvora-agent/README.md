...
This crate extracts a first-class `Agent` type from `xvora-shell`.
...
`xvora-shell`, another in-process host, or a headless batch runner.
...
```rust
use xvora_agent::{AgentDefinition, AgentBuilder};
...
└────────▲─────────┘
         │ depends on
┌────────┴─────────┐
│  xvora-shell  │  uses AgentBuilder to create
│  (session host)  │  Agent during session setup
└──────────────────┘
```

- **`xvora-tools`**: Provides `ToolBridge`, `ToolRegistry`,
  `ToolState`, `SystemReminderLayer`, and tool implementations.
  `xvora-agent` depends on it for tool setup.
- **`xvora-shell`**: The application shell. Uses `AgentBuilder`
  to construct an `Agent` during session creation. The shell
  re-exports some modules from `xvora-agent` (AGENTS.md
...
```bash
# Check
cargo check -p xvora-agent

# Test
cargo test -p xvora-agent

# Clippy
cargo clippy -p xvora-agent --fix --allow-dirty
