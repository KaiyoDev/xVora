...
// engine in `crates/common/xai-xvora-compaction` operate over xvora's
...
impl xai_xvora_compaction::CompactionItem for ConversationItem {
    fn role(&self) -> xai_xvora_compaction::CompactionRole {
        use xai_xvora_compaction::CompactionRole;
        // xvora has no distinct `Developer` role; everything maps onto
    ...
    fn is_compaction_summary(&self) -> bool {
        // xvora has no structural marker that uniquely identifies a prior
        ...
        // (add a dedicated marker) before routing xvora history through
        // the shared `history`/`inter` filter.
        false
    }

    fn attachment_refs(&self) -> Vec<xai_xvora_compaction::CompactionFileRef> {
        // xvora `UserItem`s carry only `Text`/`Image { url }` content
...
impl xai_xvora_compaction::CompactionItemFactory for ConversationItem {
...
/// ([stream/messages.rs:340](crates/codegen/xai-xvora-sampler/src/stream/messages.rs))
...
///
/// This is `pub` so downstream crates (`xai-xvora-shell`) can re-use it
...
impl From<ChatResponseMessage> for ConversationItem {
    fn from(msg: ChatResponseMessage) -> Self {
        ...
        // ([crates/codegen/xai-xvora-sampler/src/stream/chat_completions.rs])
...
pub fn build_messages_request(req: &ConversationRequest) -> crate::messages::MessagesRequest {
    ...
    // output without tools. The xvora-shell agent does NOT use this path — a
...
/// ([crates/codegen/xai-xvora-sampler/src/stream/messages.rs]) instead
...
mod compaction_item_bridge_tests {
    use super::*;
    use xai_xvora_compaction::{CompactionItem, CompactionItemFactory, CompactionRole};
    ...
    fn metadata_accessors_are_conservative() {
        // xvora has no structural compaction-summary marker, and no
...
mod tests {
    ...
    //      (earlier xvora / chat-completions written as v1)
    ...
    #[test]
    fn upgrade_legacy_reasoning_singular_xvora_shape() {
