...
These options belong to the `xvora agent` command and apply to every mode. Pass them before the mode name, for example `xvora agent --model grok-build stdio`. The `stdio` subcommand itself takes no options.
