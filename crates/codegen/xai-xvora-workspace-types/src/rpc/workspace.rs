...
/// over `xai_xvora_tools`'s `TaskSnapshot`). `tool_name`, when set, is the
...
/// needs (a slim DTO over `xai_xvora_tools`'s `TaskSnapshot`).
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
...
/// One TODO list item (slim DTO over `xai_xvora_tools`'s `TodoState`). `status`
...
/// in `xai-xvora-workspace/src/hub_server.rs`.
