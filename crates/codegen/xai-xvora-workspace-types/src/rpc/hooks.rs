...
//!
//! `xai_xvora_hooks` pulls in `git2`/`reqwest`/`xai-xvora-tools`, too heavy for
...
/// Wire mirror of `xai_xvora_hooks::discovery::HookRegistry`.
///
...
/// Wire mirror of `xai_xvora_hooks::config::HookSpec`.
///
...
/// `xai-xvora-hooks`, so a server-side test (`xai-xvora-workspace`'s
...
/// Wire mirror of `xai_xvora_hooks::event::HookEventName`.
///
