//! Wire types for the `xai-xvora-workspace` API.
//!
...
//! `xai-xvora-workspace-grpc` crate. The Rust types defined here are
//! the source of truth.
...
///
/// Lives in `xai-xvora-workspace-types` (instead of `xai-xvora-workspace` or
/// `xai-xvora-mcp`) so both the permission-validation layer and the MCP
/// transport layer can depend on it without dragging the full workspace or
/// rmcp into each other. Re-exported by `xai_xvora_workspace::permission`
