...
//! agent-session config types in `xai-xvora-config` and friends.
...
/// `xai-xvora-tools` once the wire surface is firm.
...
///
/// TODO(workspace): align with `xai_xvora_config::ProjectConfig`.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
