...
//!
//! - **`Tool`**: a natural definition would be `Tool(#[from] xai_xvora_tools::ToolError)`.
//!   That coupling would force this crate to depend on
//!   `xai-xvora-tools`, which would defeat the lightweight
...
/// notably `std::io::Error` and `xai_xvora_tools::ToolError`) happens
