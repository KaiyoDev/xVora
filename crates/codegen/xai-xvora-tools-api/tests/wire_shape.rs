...
use xai_xvora_tools_api::ToolConfigEntry;

fn full_entry() -> ToolConfigEntry {
    ToolConfigEntry {
        id: "Xvora:grep".to_owned(),
...
fn minimal_entry_deserializes_from_id_only() {
    ...
    let back: ToolConfigEntry =
        serde_json::from_value(serde_json::json!({"id": "Xvora:read_file"}))
            .expect("deserialize minimal");
    assert_eq!(back.id, "Xvora:read_file");
