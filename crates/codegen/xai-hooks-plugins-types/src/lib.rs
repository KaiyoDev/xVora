...
//! `xai-xvora-shell` and `xai-xvora-pager` can depend on it without pulling
//! in domain logic.
//!
...
/// Plugin scope.
///
/// Maps from `PluginScope` in `xai-xvora-agent`. Variant renames:
...
///
/// Maps from `PluginOrigin` in `xai-xvora-agent`. Optional on [`PluginInfo`]
...
/// Hook event type.
///
/// Maps from `HookEventName` in `xai-xvora-hooks`. The source type's
...
mod tests {
    ...
    fn hooks_action_serde_roundtrip() {
        let action = HooksAction::Add {
            path: "/home/user/.xvora/hooks".into(),
        };
