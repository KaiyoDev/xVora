...
use crate::storage_client::{Auth401AttributionCallback, StaticXaiAuth, StorageClient};
...
/// `StaticXaiAuth` carrying the inline user / deployment keys from
...
fn build_proxy_client_with_fallback(
    ...
    let provider = credentials.unwrap_or_else(|| {
        let mut creds = StaticXaiAuth::new(Some(user_token.to_owned()));
