...
fn ensure_local_pager_binary(binary: &std::path::Path) -> Result<()> {
    ...
    let output = cmd
        .output()
        .with_context(|| format!("failed to spawn {cargo} to build xai-xvora-pager"))?;

    if !output.status.success() {
        bail!(
            "failed to build xai-xvora-pager (exit {:?})\nstdout:\n{}\nstderr:\n{}",
            output.status.code(),
...
/// 3. Build locally via `cargo build -p xai-xvora-pager-bin` (the composition-
///    root package that owns the `xai-xvora-pager` binary)
