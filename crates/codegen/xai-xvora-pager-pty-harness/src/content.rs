...
//! [`MockInferenceServer`] from `xai-xvora-test-support` and provides the
...
pub use xai_xvora_test_support::mock_server::LogEntry;
pub use xai_xvora_test_support::mock_server::MockModelEntry as MockModel;
pub use xai_xvora_test_support::mock_server::StorageUpload;
// SSE event builders for `enqueue_response` scripts (reasoning turns etc.).
pub use xai_xvora_test_support::sse;
pub use xai_xvora_test_support::{ScriptedResponse, SseEvent};
...
impl ContentController {
    ...
    ///
    /// Mirrors `xai_xvora_test_support::env::test_env_cmd_tokio`.
    pub fn env_for_pager(&self) -> Vec<(String, String)> {
        ...
        vec![
            ("HOME".into(), home),
            // Explicit XVORA_HOME prevents leaking the real user's
            // config.toml when $HOME alone isn't sufficient (e.g. if
            // XVORA_HOME is set in the test runner's env).
            ("XVORA_HOME".into(), xvora_home),
            ("XVORA_CLI_CHAT_PROXY_BASE_URL".into(), self.url()),
            ("XVORA_XAI_API_BASE_URL".into(), self.url()),
            ...
            ("XVORA_PROMPT_SUGGESTIONS".into(), "false".into()),
...
mod tests {
    ...
    async fn env_for_pager_shape() {
        ...
        assert_eq!(
            get("XVORA_HOME").as_deref(),
            content.home().join(".xvora").to_str()
        );
        assert_eq!(get("XVORA_CLI_CHAT_PROXY_BASE_URL"), Some(content.url()));
        assert_eq!(get("XVORA_XAI_API_BASE_URL"), Some(content.url()));
        assert_eq!(get("XAI_API_KEY").as_deref(), Some("test-key-for-ci"));
        assert_eq!(get("XVORA_TELEMETRY_ENABLED").as_deref(), Some("false"));
        assert_eq!(get("XVORA_FEEDBACK_ENABLED").as_deref(), Some("false"));
        assert_eq!(get("XVORA_TRACE_UPLOAD").as_deref(), Some("false"));
        assert_eq!(get("XVORA_PROMPT_SUGGESTIONS").as_deref(), Some("false"));
        assert_eq!(get("XVORA_MAX_RETRIES").as_deref(), Some("0"));
