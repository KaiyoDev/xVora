//! Unified PTY harness for xai-xvora-pager.
//!
...
//!    `xai-xvora-pager/tests/scenarios/`) — assert screen contents and
...
//! - **`scroll_matrix`**  — `XVORA_SCROLL_LOG` JSONL ingestion for the scroll validation matrix.
...
impl PtyHarness {
    ...
    /// # use xai_xvora_pager_pty_harness::{PtyHarness, ContentController, pager_binary};
