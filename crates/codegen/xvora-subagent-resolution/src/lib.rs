//! Extracts the pure-logic "resolution" phase of subagent spawning from `xvora-shell` into a reusable library.
...
//! It serves local hosts (e.g. `xvora-shell`) and any future remote spawn path that needs only pure resolution logic.
//!
