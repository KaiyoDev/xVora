...
//!
//! Extracted from `xvora-shell/src/agent/subagent/` `normalize_forked_context()`.
...
///
/// See also: `xvora-chat-state::compaction_utils::strip_system_tags`, which strips a related (but different) tag set for compaction.
...
/// The other is `fork_filter_chat` in `xvora-shell/src/session/storage/jsonl.rs`; it truncates to the last complete turn before this counts them.
...
///
/// See also: `xvora-chat-state::compaction_utils::strip_system_tags`, which also leaves unclosed tags untouched for a different tag set.
...
mod tests {
    use super::*;
    use xvora_sampling_types::conversation::{ConversationItem, ToolCall, ToolResultItem};
    ...
    fn reasoning_item(text: &str) -> ConversationItem {
        ConversationItem::Reasoning(xvora_sampling_types::synthesized_reasoning_item(text))
    }
