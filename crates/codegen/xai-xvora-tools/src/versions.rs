...
mod tests {
    ...
    fn unknown_override_version_errors() {
        let err =
            resolve_version("current", "Xvora:run_terminal_cmd", Some("v999")).unwrap_err();
        assert!(
    ...
    fn active_version_produces_no_warning() {
        let res =
            resolve_version_with_warnings("current", "Xvora:run_terminal_cmd", None).unwrap();
        assert!(
    ...
    fn active_bundle_produces_no_bundle_level_warning() {
        let res =
            resolve_version_with_warnings("legacy-0.4.10", "Xvora:run_terminal_cmd", None)
                .unwrap();
        assert!(
