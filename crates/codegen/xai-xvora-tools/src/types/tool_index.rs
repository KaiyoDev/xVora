...
//!
//! `ToolSearchIndex` is defined in `xai-xvora-tools` to keep the tool crate
//! backend-agnostic. The concrete implementation lives in `xai-xvora-shell`
...
/// in `xai-xvora-shell` maps `mcp_initialized` to `is_ready`.
...
/// serialized), injected by `xai-xvora-shell` after MCP initialization.
#[derive(Clone)]
