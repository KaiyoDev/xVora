...
//!
//! Two consumers read it independently. The hook matcher (`xai-xvora-hooks`) needs the
//! Grok tool **names** an external settings term maps to (and the reverse, for regex
//! matchers); the agent builder (`xai-xvora-agent`) needs the [`ToolKind`] a `tools:`
...
const CLAUDE_TOOLS: &[ClaudeTool] = &[
    ...
    k("Skill",           Read,                 &["skill"]),                           // matcher: opencode's `skill` tool; allowlist Read (xvora reads SKILL.md)
...
/// Every distinct Grok name the table references, for the `xai-xvora-agent` drift-check
