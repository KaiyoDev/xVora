...
//!
//! `MemoryBackend` is defined in `xai-xvora-tools` to keep the tool crate
...
///
/// Use `tracing::info!(target: MEMORY_LOG_TARGET, ...)` in `xai-xvora-tools`.
/// Mirrors `xai_xvora_shell::session::memory_log::TARGET`.
