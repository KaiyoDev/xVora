...
impl<'a> SkillEntry<'a> {
    ...
    // ── Budgeted XML rendering (xvora harness) ─────────────
...
/// Used by the budgeted (xvora) XML rendering path.
...
pub enum XmlRenderMode {
    ...
    Verbatim,
    /// Xvora harness: budget-capped three-tier rendering with full
    /// XML entity escaping.
    Budgeted {
...
mod tests {
    ...
    fn verbatim_bypasses_is_listable_filter() {
        // xvora drops a description-less skill; compat mode renders all.
    ...
    // ── budgeted mode: xvora harness (budgeted XML) ───────────
