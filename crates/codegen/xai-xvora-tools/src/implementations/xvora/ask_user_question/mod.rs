...
/// params are injected, `XVORA_ASK_USER_QUESTION_TIMEOUT_SECS` (positive
/// integer seconds) still overrides this default directly —
/// e.g. `XVORA_ASK_USER_QUESTION_TIMEOUT_SECS=8` for tests / TUI repro.
...
pub const RESPONSE_TIMEOUT_ENV: &str = "XVORA_ASK_USER_QUESTION_TIMEOUT_SECS";
...
crate::register_resource!("xvora", "AskUserQuestion", AskUserQuestionParams);
...
/// session-owned coordinator (in xai-xvora-shell), which performs an ACP
