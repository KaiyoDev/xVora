...
//!
//! - **`xai-xvora-tools`** — tool blocks on a oneshot, formats the result.
//! - **`xai-xvora-shell`** — coordinator receives requests over mpsc, calls the
//!   client via ACP `ext_method`, sends results back over the oneshot.
//! - **`xai-xvora-pager`** — handles the `ExtMethod`, renders UI, returns a
//!   typed response.
//!
//! All three crates import these types from `xai-xvora-tools`.
...
register_resource!("xvora", "UserQuestionSender", UserQuestionSender);
