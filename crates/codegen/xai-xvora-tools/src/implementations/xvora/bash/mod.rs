...
use regex::Regex;
use xai_xvora_config::shell::AmpersandSemantics;
...
pub struct BashParams {
    ...
    /// xvora opts up to 10h via xai-xvora-shell's `BashToolConfig`.
    ///
    /// When set:
...
impl crate::types::resources::ResourceType for BashParams {
    const ID: &'static str = "xvora.Bash";
...
// particular production xvora sets it to 10h in xai-xvora-shell's
...
/// [`xai_xvora_config::shell::ampersand_semantics`] reports POSIX `&` semantics
/// (Unix + Git Bash).
...
/// should use `background: true`. Env override: `XVORA_MAX_FOREGROUND_BLOCK_MS`.
const MAX_FOREGROUND_BLOCK: Duration = Duration::from_secs(300); // 5 minutes

fn max_foreground_block() -> Duration {
    std::env::var("XVORA_MAX_FOREGROUND_BLOCK_MS")
        .ok()
...
// ───────────────────────────────────────────────────────────────────────────
// Bare `echo "<msg>"` detection (for statistics + hints in xvora bash)
// ───────────────────────────────────────────────────────────────────────────
...
/// communication. We want statistics on this pattern for the xvora
...
impl BashTool {
    ...
    ///   the terminal backend default applies (`XVORA_FOREGROUND_BLOCK_BUDGET_MS`
    ...
    /// still honors `XVORA_FOREGROUND_BLOCK_BUDGET_MS` via `None` on the request.
    ///
...
${%- endif %}"#
    ...
    fn get_prefixed_command(cmd_prefix: &Option<String>, command: &str) -> String {
        match cmd_prefix {
            Some(prefix) => {
                let sep = xai_xvora_config::shell::chain_separator();
...
impl xai_tool_runtime::Tool for BashTool {
    ...
    async fn run(
        ...
        let ampersand = xai_xvora_config::shell::ampersand_semantics();
        ...
        if input.is_background {
            ...
            let request = TerminalRunRequest {
                ...
                // existing xvora callers that never opted in are
                // unaffected.
            ...
            // We tag it for statistics (xvora backend) and can surface an educational
