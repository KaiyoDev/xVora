...
//! (in `xai-xvora-tools`) and the subagent coordinator (in `xai-xvora-shell`).
//!
//! ## Resource types
//!
...
pub struct SubagentRuntimeOverrides {
    ...
    /// `"xvora-plan"`) whose `AgentDefinition` decides the child's harness
...
/// defense-in-depth check in `xai-xvora-shell`'s subagent coordinator.
...
fn is_background_capable_bash_tool(tc: &crate::registry::types::ToolConfig) -> bool {
    match tc.id.as_str() {
        "Xvora:run_terminal_cmd" | "XvoraConcise:run_terminal_cmd" => tc
            .params
            .as_ref()
...
/// the cross-crate drain loop in `xai-xvora-shell` relies on
...
register_resource!("xvora", "SubagentEventSender", SubagentEventSender);
...
pub struct MonitorEventNotification {
    ...
    /// events inside another session's turn. `None` for legacy / non-xvora
...
register_resource!("xvora", "MonitorEventBuffer", MonitorEventBuffer);
...
/// xai-xvora-shell produces it, the channel protocol carries it, and the
...
register_resource!("xvora", "SubagentDepthCounter", SubagentDepthCounter);
...
register_resource!("xvora", "TaskModelValidator", TaskModelValidator);
...
register_resource!("xvora", "SessionIdResource", SessionIdResource);

/// Carries the current parent prompt/turn ID for TaskTool subagent scoping.
///
/// Set by xai-xvora-shell immediately before a prompt turn begins executing so
...
/// True while a `/goal` loop is active. Set by xai-xvora-shell at turn start.
...
register_resource!("xvora", "GoalLoopActive", GoalLoopActive);
...
mod tests {
    ...
    fn read_only_filter_prunes_orphaned_background_task_tools() {
        let mut config = ToolServerConfig {
            tools: vec![
                tc("Xvora:run_terminal_cmd", ToolKind::Execute),
                tc("Xvora:read_file", ToolKind::Read),
                tc("Xvora:list_dir", ToolKind::List),
                tc("Xvora:grep", ToolKind::Search),
                tc("Xvora:kill_task", ToolKind::KillTaskAction),
                tc("Xvora:get_task_output", ToolKind::BackgroundTaskAction),
            ],
    ...
    fn read_write_filter_keeps_background_capable_bash_when_explicitly_enabled() {
        let mut bash = tc("Xvora:run_terminal_cmd", ToolKind::Execute);
        bash.params = Some(
