...
impl AutoWakeDeliveredIds {
    ...
    ///
    /// Used by the between-turn completion drain in `xai-xvora-shell` to
...
crate::register_resource!("xvora", "AutoWakeDeliveredIds", AutoWakeDeliveredIds);
...
pub fn format_monitor_events(
    events: &[crate::implementations::xvora::task::types::MonitorEventNotification],
    task_output_name: Option<&str>,
) -> Option<String> {
    use std::fmt::Write as _;
    ...
    match events {
        ...
        _ => {
            type Event = crate::implementations::xvora::task::types::MonitorEventNotification;
...
/// scope only when it has no recorded owner (legacy / non-xvora backends)
...
/// - `xai-xvora-shell`'s `SessionActor` to sweep matching synthetic
...
mod tests {
    ...
    fn shared_with_reminders_disabled(tasks: Vec<TaskSnapshot>) -> SharedResources {
        ...
        res.insert(Terminal(backend));
        res.register_state::<ReportedTaskCompletions>();
        let params = crate::implementations::xvora::bash::BashParams {
    ...
    fn format_monitor_events_single_vs_batched() {
        use crate::implementations::xvora::task::types::MonitorEventNotification;
        ...
        let bare = crate::implementations::xvora::task::types::MonitorEventNotification {
            task_id: "task-9".into(),
    ...
    fn wrap_monitor_event_round_trips_through_split() {
        use crate::implementations::xvora::monitor::event::wrap_monitor_event;
