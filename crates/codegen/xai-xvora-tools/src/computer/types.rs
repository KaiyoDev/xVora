...
/// (`xai-xvora-shell::extensions::task::KillTaskResponse`) and deserialized
/// by clients (xai-xvora-pager), so it derives both serde directions.
...
///
/// Implemented by:
/// - `LocalTerminalBackend` (in xai-xvora-tools, spawns processes)
/// - `AcpTerminalBackend` (in xai-xvora-shell, calls ACP protocol)
#[async_trait::async_trait]
