...
//!
//! Extracted from `xvora-shell` so binaries (TUI, sampler) can reuse them without the shell's HTTP product-analytics client.
...
/// `method` is "xvora" or "api_key"; `mode` is "device", "loopback", or "api_key".
#[derive(Serialize)]
...
/// `method` is "xvora" or "api_key"; `mode` is the resolved auth mode.
