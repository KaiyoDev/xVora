...
///
/// Lives here (not in `xvora-pager`) so the firehose directives below and
...
/// safe. See `xvora-pager/src/tracing.rs` for the consumer side.
...
// until added here.
const FIREHOSE_BASE_DIRECTIVES: &str = "info,xvora_pager=debug,xvora_shell=debug,xvora_tools=debug,xvora_telemetry=debug,xvora_agent=debug,xvora_mcp=debug,acp_lib=debug,sampling_log=off";
...
mod tests {
    ...
    fn layer_routes_under_real_firehose_filter() {
        ...
        // `xvora_telemetry::session_ctx`) survives the real filter so
