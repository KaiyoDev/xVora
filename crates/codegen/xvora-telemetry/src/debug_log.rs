...
///
/// Lives here (not in `xvora-pager`) so the firehose directives below and the pager's own filter are built from the same constants.
...
/// See `xvora-pager/src/tracing.rs` for the consumer side.
