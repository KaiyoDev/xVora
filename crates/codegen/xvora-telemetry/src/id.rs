...
//!
//! Extracted from `xvora-shell::agent::unique_identifier` so the
//! telemetry engine can stamp events without depending on shell internals.
//! `$XVORA_HOME` is resolved through `xvora-config::xvora_home`.
...
fn load_or_compute_agent_id() -> String {
    let cache_path = xvora_config::xvora_home().join("agent_id");
