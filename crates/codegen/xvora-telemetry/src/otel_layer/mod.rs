...
//!
//! `xvora-pager` uses this module to set up OTLP
...
///
/// Built by the binaries (`xvora-pager`) from their own
/// configuration. The credentials provider is constructed by shell's
/// `xvora_shell::auth::credential_provider::build_otel_credential_provider`.
pub struct OtelLayerConfig {
    ...
    /// Value for the `X-XAI-Token-Auth` header (typically `"xvora-cli"`).
...
mod tests {
    ...
    use std::sync::Mutex;
    use xvora_auth::{AuthCredentialProvider, CredentialSnapshot, HttpAuth};
    ...
    fn make_exporter(
        ...
        RefreshableSpanExporter {
            ...
            token_header_value: Arc::from("xvora-cli"),
