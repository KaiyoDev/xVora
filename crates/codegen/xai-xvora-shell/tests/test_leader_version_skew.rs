...
//! - `XVORA_BINARY_LEADER` — the binary that elects the initial leader
//!   (typically the latest released stable, e.g. fetched from
//!   `https://storage.googleapis.com/xvora-public-artifacts/cli/grok-<ver>-linux-x86_64`).
//! - `XVORA_BINARY_CLIENT` — the second client (typically a freshly built main).
//!
...
//!
//! ```bash
//! XVORA_BINARY_LEADER=/path/to/grok-old XVORA_BINARY_CLIENT=/path/to/grok-new \
//!   cargo test -p xai-xvora-shell --test test_leader_version_skew -- --ignored --nocapture
//! ```
...
use xai_xvora_test_support::leader::{
...
fn skew_binaries() -> Option<(std::path::PathBuf, std::path::PathBuf)> {
    ...
    if old == new {
        eprintln!(
            "SKIP: XVORA_BINARY_LEADER/XVORA_BINARY_CLIENT resolve to the same binary ({})",
            old.display()
        );
        return None;
    }
...
fn sandbox_unified_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".xvora").join("logs").join("unified.jsonl"))
        .unwrap_or_default()
}
...
#[tokio::test]
#[ignore = "two-binary version-skew test; set XVORA_BINARY_LEADER/XVORA_BINARY_CLIENT and run with --ignored"]
...
/// `signal_leaders_to_relaunch` in `xai-xvora-pager-bin/src/main.rs` (which is
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let control = LeaderClient::connect(
                home.path().join(".xvora").join("leader.sock"),
                "xvora-pager-update",
                ClientMode::Stdio,
...
async fn eviction_leaves_single_leader_and_single_auth_owner() {
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let auth_path = home.path().join(".xvora").join("auth.json");
