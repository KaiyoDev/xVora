...
//! `.xvora`, `.cursor`, and `.claude` vendor dirs, spawns a real `grok agent
//! stdio` process against the mock inference server (toggling the
//! `XVORA_<VENDOR>_<SURFACE>_ENABLED` env vars via `cmd.env`), sends one prompt,
...
//! ```bash
//! cargo test -p xai-xvora-shell --test test_vendor_compat -- --ignored
//! ```
...
const MARKER_XVORA_SKILL: &str = "ZZ_XVORA_SKILL_MARKER";
...
fn seed_fixtures(home: &Path, cwd: &Path) {
    ...
    write_skill(home, ".xvora", "grok-skill", MARKER_XVORA_SKILL);
...
async fn vendor_compat_defaults_include_vendor_skills_but_drop_denylisted() {
    with_local_set(|| async {
        ...
        assert!(
            body.contains(MARKER_XVORA_SKILL),
...
/// `XVORA_CURSOR_SKILLS_ENABLED=false` drops the cursor-vendor skill; grok stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_skills_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("XVORA_CURSOR_SKILLS_ENABLED", "false")]).await;
        assert!(
            body.contains(MARKER_XVORA_SKILL),
...
/// `XVORA_CLAUDE_SKILLS_ENABLED=false` drops the claude-vendor skill; grok stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_skills_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("XVORA_CLAUDE_SKILLS_ENABLED", "false")]).await;
        assert!(
            body.contains(MARKER_XVORA_SKILL),
            "grok-skill always present"
        );
...
/// `XVORA_CURSOR_RULES_ENABLED=false` drops cursor-vendor rules; claude-vendor rules stay.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_rules_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("XVORA_CURSOR_RULES_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CURSOR_RULE),
...
/// `XVORA_CLAUDE_RULES_ENABLED=false` drops claude-vendor rules; cursor-vendor rules stay.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_rules_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("XVORA_CLAUDE_RULES_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CLAUDE_RULE),
...
/// `XVORA_CURSOR_AGENTS_ENABLED=false` drops cursor-vendor AGENTS.md; claude-vendor stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_agents_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("XVORA_CURSOR_AGENTS_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CURSOR_AGENTS),
...
/// `XVORA_CLAUDE_AGENTS_ENABLED=false` drops claude-vendor AGENTS.md; cursor-vendor stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_agents_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("XVORA_CLAUDE_AGENTS_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CLAUDE_AGENTS),
...
async fn vendor_compat_all_cursor_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[
            ("XVORA_CURSOR_SKILLS_ENABLED", "false"),
            ("XVORA_CURSOR_RULES_ENABLED", "false"),
            ("XVORA_CURSOR_AGENTS_ENABLED", "false"),
        ])
        .await;
        ...
        assert!(body.contains(MARKER_XVORA_SKILL), "grok always present");
...
async fn vendor_compat_all_claude_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[
            ("XVORA_CLAUDE_SKILLS_ENABLED", "false"),
            ("XVORA_CLAUDE_RULES_ENABLED", "false"),
            ("XVORA_CLAUDE_AGENTS_ENABLED", "false"),
        ])
        .await;
