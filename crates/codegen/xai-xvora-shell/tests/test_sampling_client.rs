...
use xai_xvora_shell::sampling::{
    ApiBackend, Client, ConversationItem, ConversationRequest, ConversationToolChoice,
    SamplingError, ToolCall, ToolSpec, rs,
};
use xai_xvora_shell::session::storage::JsonlStorageAdapter;
use xai_xvora_test_support::sse::responses_api_reasoning_and_text_events;
use xai_xvora_test_support::{MockInferenceServer, ScriptedResponse, SseEvent};
...
/// Upgrade path, xvora / Responses API: a legacy session whose
...
async fn responses_upgrade_roundtrips_reconstructed_reasoning_as_typed_input() {
    // 1. Seed a legacy xvora chat_history.jsonl (inline reasoning
    ...
    std::fs::write(
        ...
        concat!(
            ...
            r#"{"type":"user","content":[{"type":"text","text":"q1"}]}"#,
            "\n",
            r#"{"type":"assistant","content":"a1","reasoning":{"text":"legacy xvora reasoning","encrypted":"ENC_BLOB_xyz","id":"rs_grokbuild_legacy"},"model_id":"xvora"}"#,
            "\n",
        ),
    )
    .unwrap();
...
async fn test_responses_api_streaming_text() {
    ...
    while let Some(event_result) = stream.next().await {
        ...
        use xai_xvora_shell::sampling::rs::ResponseStreamEvent;
        match event {
    ...
    while let Some(event_result) = stream.next().await {
        ...
        if let ResponseStreamEvent::ResponseCompleted(completed) = event {
            for output in completed.response.output {
                use xai_xvora_shell::sampling::rs::OutputItem;
                ...
                match output {
                    ...
                    OutputItem::Message(msg) => {
                        for content in &msg.content {
                            use xai_xvora_shell::sampling::rs::OutputMessageContent;
...
async fn test_doom_loop_check_enabled_sends_header_and_absorbs_check_event() {
    use xai_xvora_sampling_types::doom_loop::{DOOM_LOOP_CHECK_EVENT_TYPE, SAMPLE_CHECK_EVENT_DATA};
...
async fn test_responses_backend_hits_responses_endpoint_not_chat_completions() {
    ...
    // The session-level dispatch in `xai-xvora-sampler` selects the
