//! Built-binary end-to-end tests for the xvora (xai-xvora-pager) binary.
//!
...
//! ```bash
//! cargo test -p xai-xvora-shell --test test_built_binary_e2e -- --ignored
//! ```
//!
//! In CI, set `XVORA_BINARY` to point at the release artifact:
//! ```bash
//! XVORA_BINARY=./artifacts/grok-0.1.159-linux-x86_64 \
//!   cargo test -p xai-xvora-shell --test test_built_binary_e2e -- --ignored
//! ```
...
use xai_xvora_test_support::env::test_env_cmd_tokio;
...
async fn xvora_server() -> MockInferenceServer {
    MockInferenceServer::start_with_models(vec![
        MockModelEntry::with_agent_type("grok-4.5", "xvora")
...
async fn test_version_with_crash_handler_exits_zero() {
    ...
    let output = Command::new(&binary)
        .arg("--version")
        .env("XVORA_CRASH_HANDLER", "1")
        .output()
        .unwrap_or_else(|e| panic!("failed to run {}: {e}", binary.display()));

    assert!(
        output.status.success(),
        "grok --version with XVORA_CRASH_HANDLER=1 failed (exit {:?}):\n{}",
        output.status.code(),
        String::from_utf8_lossy(&output.stderr)
    );
}
...
async fn test_headless_json_reports_server_cost() {
    use xai_xvora_test_support::scripted::SseEvent;
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        xai_xvora_test_support::scripted::ScriptedResponse::sse(vec![
...
async fn test_headless_json_reports_usage_on_max_turns() {
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        xai_xvora_test_support::scripted::ScriptedResponse::sse(
            xai_xvora_test_support::sse::chat_completions_reasoning_then_tool_call_events(
                "let me look",
                "call-1",
                "read_file",
                r#"{"path":"README.md"}"#,
...
/// Isolated headless run with a custom `~/.xvora/`. Clean env (no leaked
...
impl ConfigTestHarness {
    fn new(server: &MockInferenceServer) -> Self {
        ...
        Self {
            ...
            workdir: git_workdir(),
            env: vec![
                ("XVORA_CLI_CHAT_PROXY_BASE_URL".into(), server.url()),
                ("XVORA_TELEMETRY_ENABLED".into(), "false".into()),
                ("XVORA_FEEDBACK_ENABLED".into(), "false".into()),
                ("XVORA_TRACE_UPLOAD".into(), "false".into()),
                ("XVORA_INSTRUMENTATION".into(), "disabled".into()),
                ("XVORA_DISABLE_AUTOUPDATER".into(), "1".into()),
            ],
        }
    }

    fn grok_dir(&self) -> std::path::PathBuf {
        self.home.path().join(".xvora")
    }
...
/// Enterprise BYOK: managed_config.toml overrides xvora with a custom
...
"#,
    ...
    .unwrap();
    h.env("XVORA_TEST_BYOK_TOKEN", "test-byok-secret-token");
...
#[cfg(unix)]
use xai_xvora_test_support::sse::{
