...
//!
//! Knobs: XVORA_PERF_GIT_FILES (default 300), XVORA_PERF_GIT_PICKS (default 6).
...
async fn run_storm(
    ...
    let files = env_usize("XVORA_PERF_GIT_FILES", 300);
    let picks = env_usize("XVORA_PERF_GIT_PICKS", 6);
...
fn git_rebase_refresh_storm_e2e() {
    ...
    let server = mock_rt
        ...
        .expect("mock server");
    let xvora_home = TempDir::new().expect("grok home");
