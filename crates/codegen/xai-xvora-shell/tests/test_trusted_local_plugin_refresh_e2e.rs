...
//! 4. Smoke-validate session JSON under `$XVORA_HOME/sessions/` after exit.
//!
//! Requires a built `grok` binary (`XVORA_BINARY` or cargo-built pager) for the
//! ignored headless test.
//!
//! ```bash
//! cargo test -p xai-xvora-shell --test test_trusted_local_plugin_refresh_e2e
//! cargo test -p xai-xvora-shell --test test_trusted_local_plugin_refresh_e2e -- --ignored
//! ```
...
use xai_xvora_agent::plugins::SharedPluginRegistryHandle;
use xai_xvora_agent::plugins::discovery::DiscoveryConfig;
use xai_xvora_agent::plugins::git_install::{InstallSource, install_from_source};
use xai_xvora_agent::plugins::install_registry::{
...
fn trusted_local_refresh_surfaces_new_agent_via_discovery() {
    ...
    // xvora_home() caches the first XVORA_HOME per process; a separate test
    ...
    assert_eq!(
        plugin.scope,
        xai_xvora_agent::plugins::PluginScope::CliOverride
    );
...
async fn headless_session_refreshes_trusted_local_plugin_and_writes_session_json() {
    ...
    // The spawned binary gets HOME/XVORA_HOME via `cmd.env` below; this global env
    // is only for the in-process post-run discovery assertion (which resolves the
    // registry via xvora_home()). `#[serial]` keeps it from racing other tests.
    let _home_guard = EnvVarGuard::set("HOME", &home);
    let _grok_guard = EnvVarGuard::set("XVORA_HOME", &xvora_home);
    ...
    // Smoke: session storage under XVORA_HOME/sessions has JSON artifacts.
