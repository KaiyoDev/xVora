...
/// All tests share this as XVORA_HOME (the `OnceLock` in xai-xvora-config
...
async fn update_config_does_not_leak_requirements_into_user_config() {
    ...
    let effective = xai_xvora_shell::config::load_effective_config().unwrap();
    let effective_cfg = xai_xvora_shell::util::config::load_config_from_toml(&effective);
    assert_eq!(
    ...
    xai_xvora_shell::util::config::update_config(|cfg| {
    ...
    let user_cfg = xai_xvora_shell::util::config::load_config_from_toml(&user_toml);
