...
//! caller-owned `$XVORA_HOME`, then inspects `~/.xvora/debug/`:
...
//!   `~/.xvora/debug/<sessionId>.txt` with first-party content, and does NOT enable
//!   sampling/instrumentation. Regression for the master switch having bundled
//!   `XVORA_LOG_SAMPLING`/`XVORA_INSTRUMENTATION`, whose global `TargetFilterLayer`
...
//! - a live `agent` session (explicit `XVORA_DEBUG_LOG=1`) writes a per-session
//!   `~/.xvora/debug/<sessionId>.txt` with real first-party content + `latest.txt`.
//! - `--debug-file <path>` writes one explicit file and bypasses per-session
//!   routing entirely (no `~/.xvora/debug/` files).
//! - `XVORA_LOG_FILE=<path>` writes that explicit file (back-compat single file).
//!
...
//! ```bash
//! cargo test -p xai-xvora-shell --test test_debug_logging -- --ignored
//! ```
...
/// The per-session firehose directory under a pinned `$XVORA_HOME`.
fn debug_dir(home: &Path) -> PathBuf {
    home.join(".xvora").join("debug")
}

/// List firehose `*.txt` files under `~/.xvora/debug` (excluding the `latest.txt`
...
/// Build a headless `grok -p` command with a pinned `$XVORA_HOME` so the firehose
/// lands under `<home>/.xvora/debug`. Firehose env knobs are cleared so the test
...
fn debug_cmd(
    ...
    cmd.args(["-p", "say hi", "--yolo", "--output-format", "json"])
        ...
        .kill_on_drop(true);
    xai_xvora_test_support::env::test_env_cmd_tokio(&mut cmd, &server.url(), home);
    // Pin the home location and drop inherited firehose toggles for determinism.
    cmd.env("XVORA_HOME", home.join(".xvora"));
    cmd.env_remove("XVORA_DEBUG_LOG");
    cmd.env_remove("XVORA_LOG_FILE");
    cmd.env_remove("XVORA_LOG_SAMPLING");
    cmd.env_remove("XVORA_HOOKS_LOG");
    cmd
}
...
/// A live `agent` session writes `~/.xvora/debug/<sessionId>.txt` with real
...
async fn agent_session_writes_named_session_file() {
    with_local_set(|| async {
        ...
        let xvora_home_str = xvora_home.to_string_lossy().into_owned();

        let client = GrokStdioClient::spawn_with_home_and_env(
            &server,
            workdir.path(),
            home,
            &[("XVORA_DEBUG_LOG", "1"), ("XVORA_HOME", &xvora_home_str)],
        )
        .await;
        ...
        let session_file = xvora_home.join("debug").join(format!("{sid}.txt"));
        ...
        // file, so `tail -f ~/.xvora/debug/latest.txt` follows the live session.
        #[cfg(unix)]
        {
            let link = xvora_home.join("debug").join("latest.txt");
...
/// The `--debug` FLAG (not `XVORA_DEBUG_LOG` directly) drives the firehose end to
/// end through the master switch. Regression: the master switch used to also set
/// `XVORA_LOG_SAMPLING`/`XVORA_INSTRUMENTATION`, whose `TargetFilterLayer` globally
...
async fn debug_flag_master_switch_enables_firehose() {
    with_local_set(|| async {
        ...
        // clears XVORA_LOG_SAMPLING (hermetic), so the sampling layer stays off and
        // `~/.xvora/logs/sampling.jsonl` is never written — the `--debug`
        ...
        // XVORA_INSTRUMENTATION=disabled, so that assertion would be vacuous.
        let sampling = xvora_home.join("logs").join("sampling.jsonl");
...
/// `--debug-file <path>` writes one explicit file and bypasses per-session
/// routing entirely (no `~/.xvora/debug/` files created).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
...
/// `XVORA_LOG_FILE=<path>` (no `--debug`) writes that exact file (back-compat).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
async fn grok_log_file_explicit_path_is_written() {
    ...
    cmd.env("XVORA_LOG_FILE", &custom);
    let result = run_headless_with_cmd(cmd).await;

    assert_headless_success(&result, "grok XVORA_LOG_FILE=path", Some(&server));
    assert_no_crashes(&result.stderr);
    assert!(
        custom.exists(),
        "explicit XVORA_LOG_FILE path not written: {custom:?}\nstderr tail:\n{}",
        stderr_tail(&result.stderr, 800)
    );
    // Single-file mode bypasses per-session routing.
    assert!(
        firehose_txt_files(home.path()).is_empty(),
        "XVORA_LOG_FILE must bypass per-session routing, found: {:?}",
        firehose_txt_files(home.path())
    );
}
