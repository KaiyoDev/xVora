...
use xai_xvora_shell::auth::{AuthManager, AuthMode, GrokAuth, GrokComConfig};
use xai_xvora_shell::heap_profile::{
...
use xai_xvora_test_support::{EnvGuard, MockInferenceServer};
use xai_xvora_version::TEST_VERSION_ENV;
...
fn assert_storage_auth(uploads: &[xai_xvora_test_support::mock_server::StorageUpload]) {
    for u in uploads {
        assert_eq!(
