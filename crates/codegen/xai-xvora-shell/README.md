...
- **Reference**
  - [Introspection (`xvora inspect`)](#introspection)
...
- [Building with Xvora](#building-with-grok) — headless API, ACP SDK integration
...
export XVORA_OIDC_ISSUER="https://acme.okta.com"
export XVORA_OIDC_CLIENT_ID="0oa1b2c3d4e5f6g7h8i9"
```

Customers typically also override the API endpoint to point at their own proxy:
```bash
export XVORA_CLI_CHAT_PROXY_BASE_URL="https://grok-proxy.acme.com/v1"
```

**3. Run `xvora`.** The CLI discovers endpoints via `{issuer}/.well-known/openid-configuration`, opens the IdP login page, and stores tokens in `~/.xvora/auth.json`. The OIDC token is sent as `Authorization: Bearer` to the configured proxy. Tokens auto-refresh silently via the stored `refresh_token`.
...
4. **stdout** → captured by Grok and saved to `~/.xvora/auth.json` as the access token
...
```bash
# Or via environment variables
export XVORA_AUTH_PROVIDER_COMMAND="/usr/local/bin/my-auth-provider"
export XVORA_AUTH_PROVIDER_LABEL="Acme Corp"   # optional
export XVORA_AUTH_TOKEN_TTL=3600               # optional
```
...
When `auth_provider_label` is set, the TUI welcome screen shows **"Login with Acme Corp"** instead of "Login with grok.com". In headless mode (`xvora -p`), the label has no effect — stderr from your binary is printed directly to the terminal.
...
When Grok needs to refresh an expired token, it re-runs your binary with `XVORA_AUTH_EXPIRED=1` set in the environment. Your binary can use this to take a faster silent-refresh path:

```bash
#!/bin/sh
if [ "$XVORA_AUTH_EXPIRED" = "1" ]; then
...
`XVORA_AUTH_EXPIRED` is optional — if your binary ignores it, Grok still works. It just runs the same flow for both login and refresh.
...
export XVORA_AUTH_EARLY_INVALIDATION_SECS=300
```

**Keep in mind:**
- When using `auth_provider_command`, you don't need to run `xvora login` before starting — Grok runs your binary automatically on first launch. You _can_ run `xvora login` to explicitly hydrate `auth.json` ahead of time if you prefer.
...
If you've authenticated with `xvora login`, you can use the stored credentials to call the CLI chat proxy directly via curl. The proxy requires specific headers that mirror what the Xvora CLI sends internally:

```bash
curl -s -N -X POST "https://cli-chat-proxy.grok.com/v1/chat/completions" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $(jq -r '."https://accounts.x.ai/sign-in".key' ~/.xvora/auth.json)" \
  -H "X-XAI-Token-Auth: xai-xvora-cli" \
  -H "x-grok-model-override: xvora" \
  -d '{
    "model": "xvora",
...
| -------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Authorization: Bearer <token>`  | Yes      | Session token from `~/.xvora/auth.json` (set by `xvora login`)                                                                                                                              |
| `X-XAI-Token-Auth: xai-xvora-cli` | Yes      | Tells the auth middleware to validate as a CLI session token                                                                                                                              |
| `x-grok-model-override: <model>` | Yes\*    | The proxy uses this header (not the JSON body) to route to the correct backend. \*Can be omitted for `xvora` which is on the default route, but always safe to include. |
...
| --------------------- | -------------- | ------------ |
| `xvora`    | ✅ Supported   | ✅ Supported |

> **Note:** `auth.json` tokens expire after 7 days. Run `xvora login` to refresh.
...
| `-m, --model <MODEL>`   | Model to use (e.g., `xvora`)               |
...
xvora -p "Explain this codebase" --tools "read_file,grep,list_dir"

# Remove web access and file editing
xvora -p "Review this code" --disallowed-tools "web_search,web_fetch,search_replace"

# Remove shell access
xvora -p "Review this code" --disallowed-tools "run_terminal_cmd"
```
...
xvora -p "Fix this bug" --disallowed-tools "Agent"

# Block only the explore subagent
xvora -p "Refactor this module" --disallowed-tools "Agent(explore)"
```
...
xvora -p "Clean up this project" --deny "Bash(rm*)"

# Allow npm commands, deny everything else dangerous
xvora -p "Set up the project" --allow "Bash(npm*)" --deny "Bash(sudo*)"

# Deny edits outside src/
xvora -p "Refactor the code" --deny "Edit(/etc/**)"

# Allow all bash commands (auto-approve without prompting)
xvora -p "Build the project" --allow "Bash"
...
```bash
# Simple question
xvora -p "What does this project do?"

# Use a specific model
xvora -p "Optimize this function" -m xvora

# Get JSON output for parsing
xvora -p "List all TODO comments in the codebase" --output-format json

# Streaming JSON for real-time processing
xvora -p "Explain the architecture" --output-format streaming-json

# Multi-turn conversation (session ID is returned in JSON output)
xvora -p "Remember: the secret number is 42" --output-format json
xvora -p "What's the secret number?" --resume <sessionId>

# Resume most recent session
xvora -p "Continue where we left off" -c

# Run in a different directory
xvora -p "Run the tests" --cwd ~/projects/other-app --always-approve
```
...
xvora -p "Review the changes in this PR" -s "critique-myrepo-pr-123"

# Continue in the same session
xvora -p "Now check for security issues" -s "critique-myrepo-pr-123"
```
...
xvora -p "Generate a README" > README.md

# Parse JSON output with jq
xvora -p "List files" --output-format json | jq -r '.text'

# CI/CD: automated code review
xvora -p "Review changes for bugs and security issues." \
  --output-format json --always-approve | jq -r '.text' > review.md

# Pipeline: chain with other tools
git diff --staged | xvora -p "Write a concise commit message for these changes"

# Batch: process multiple files
for file in src/*.js; do
  xvora -p "Migrate $file from CommonJS to ES modules." --always-approve
done

# Pre-commit hook
xvora -p "Review staged changes for obvious bugs. Reply OK if fine, or list issues." \
...
| Flag                  | Description                                                                         |
| --------------------- | ----------------------------------------------------------------------------------- |
| `-m, --model <MODEL>` | Override the default model ID (e.g., `xvora`)                           |
...
```bash
xvora agent headless --grok-ws-url wss://your-relay.example.com/ws
```
...
## SSH Passthrough (`xvora ssh`)

Use `xvora ssh` instead of plain `ssh` when connecting to remote hosts in terminals that lack native support (e.g. Apple Terminal) for local OSC 52 clipboard interception.
...
# With SSH flags
xvora ssh -t user@host
xvora ssh -L 8080:localhost:8080 user@host

# With remote command
xvora ssh user@host -- tmux attach
```

On macOS, if the terminal doesn't natively handle OSC 52, `xvora ssh` runs SSH inside a local PTY that intercepts clipboard sequences and writes them to `pbcopy`. Both plain OSC 52 and tmux DCS passthrough are handled. Terminals with native OSC 52 (iTerm2, Ghostty, Kitty, WezTerm, Alacritty) get a plain `ssh` exec with no wrapper.
...
Use headless mode for simple integrations. Spawns `xvora -p` and parses JSON output.
...
class GrokChat:
    ...
    async def create(self, messages, model="xvora", stream=False):
...
Grok reads configuration from `~/.xvora/config.toml`. If the file doesn't exist, Grok uses sensible defaults. You only need to specify values you want to override.
...
[models]
default = "xvora"           # model used for new sessions
...
telemetry = false                      # anonymous usage telemetry (env: XVORA_TELEMETRY_ENABLED)
feedback = false                       # feedback system (env: XVORA_FEEDBACK_ENABLED)
...
[tools]
respect_gitignore = true               # filter gitignored files from tools (env: XVORA_RESPECT_GITIGNORE)
...
```toml
[telemetry]
events_url = "https://example.com/events"  # env: XVORA_TELEMETRY_EVENTS_URL
events_api_key = "..."                      # env: XVORA_TELEMETRY_EVENTS_API_KEY
mixpanel_token = "..."                      # env: XVORA_TELEMETRY_MIXPANEL_TOKEN
mixpanel_enabled = true                     # env: XVORA_TELEMETRY_MIXPANEL_ENABLED
trace_upload = true                         # env: XVORA_TELEMETRY_TRACE_UPLOAD
```

When building from source, defaults can also be baked into the binary at compile time by setting `XVORA_TELEMETRY_BUILD_EVENTS_URL`, `XVORA_TELEMETRY_BUILD_EVENTS_API_KEY`, and `XVORA_TELEMETRY_BUILD_MIXPANEL_TOKEN` in the build environment (providing a Mixpanel token this way also enables Mixpanel by default). Config-file and runtime env values override build-time defaults.
...
- project config: `<repo>/.xvora/lsp.json`
- user config: `~/.xvora/lsp.json`
...
- LSP tools are enabled (`XVORA_LSP_TOOLS=1` or `[features] lsp_tools = true`)
- the merged LSP configuration is non-empty

Enable the tool for one run:

```bash
XVORA_LSP_TOOLS=1 grok
```
...
- Passive diagnostics do **not** require `XVORA_LSP_TOOLS=1`; they run whenever an applicable server is configured and starts successfully.
...
With this config, `xvora` runs your auth binary, stores the token, and routes inference through your corporate proxy. See [Authentication](#authentication) for full auth setup details.
...
| --------------------------- | ----- | -------- |
| `./.xvora/skills/`           | Local | Highest  |
| `<repo_root>/.xvora/skills/` | Repo  | Medium   |
| `~/.xvora/skills/`           | User  | Lowest   |
| `~/.claude/skills/`         | User  | Lowest   |

Skills with the same name are deduplicated — higher priority locations override lower ones.

Repo-scoped skills (Local and Repo) respect `.gitignore` and are filtered out if ignored. User-scoped skills (`~/.xvora/skills/`) are outside the repo and never filtered.
...
Grok discovers agent definitions from `.xvora/agents/` (project), `~/.xvora/agents/` (user), and built-in agents. Priority (highest wins):

1. `--agent-profile <PATH>` CLI flag
2. `[agent]` section in `config.toml`
3. `XVORA_AGENT` env var
4. Default `xvora` agent
...
# or
export XVORA_AGENT="my-custom-agent"
```
...
[subagents.models]
explore = "xvora"              # route explore to a lighter model
```
...
# instructions_file = ".xvora/personas/concise.md"  # or load from file
```

Both are also discovered from `.xvora/roles/*.toml` and `.xvora/personas/*.toml` files respectively. If a requested persona is not found, the spawn fails (fail-closed).
...
Grok discovers hooks from `.xvora/hooks/` in the project directory. Manage them with:
...
**Web search model:** Set `[models] web_search`, `XVORA_WEB_SEARCH_MODEL`, or `--web-search-model` to point the `web_search` tool at a different model. The target endpoint must support the Responses API and web search.
...
|----------|----------|-------------|
| `XVORA_MODELS_BASE_URL` | Yes | Base URL for inference / chat completions (e.g. `https://api.acme.com/v1`). The model list is fetched from `{base_url}/models` automatically |
| `XAI_API_KEY` | Yes | API key sent as `Authorization: Bearer` to the custom endpoint |
| `XVORA_MODELS_LIST_URL` | No | Override the model list URL if it differs from `{base_url}/models` |

**Setup:**

```bash
export XVORA_MODELS_BASE_URL="https://api.acme.com/v1"
export XAI_API_KEY="xai-..."
grok
```

Grok fetches the model list from `{XVORA_MODELS_BASE_URL}/models` on startup and sends inference requests to `XVORA_MODELS_BASE_URL`. This follows the standard OpenAI-compatible convention used by OpenAI, Anthropic, OpenRouter, Groq, Together.ai, and others.

If your model list endpoint differs from `{base_url}/models`, set `XVORA_MODELS_LIST_URL` explicitly.

**Combining with `[endpoints]` config:** You can also set endpoints in `~/.xvora/config.toml`:
...
**Auth behavior:** When `models_base_url` is set, Grok uses API key auth (`Authorization: Bearer`) instead of session auth. `xvora login` is not required — only the API key.
...
MCP servers are configured in `~/.xvora/config.toml`:
...
MCP servers can also be configured per-project in `.xvora/config.toml`. Grok walks from the current directory up to the git repo root, loading `.xvora/config.toml` at each level:

| Location                        | Scope             | Priority |
| ------------------------------- | ----------------- | -------- |
| `~/.xvora/config.toml`           | All projects      | Lowest   |
| `<repo-root>/.xvora/config.toml` | This repository   | ↑        |
...
**Example:** commit a `.xvora/config.toml` in your repo to share MCP servers across the team:
...
If you also have a `linear` server in `~/.xvora/config.toml`, the project version replaces it entirely.

> **Note:** Only `[mcp_servers]` is supported in project-scoped `.xvora/config.toml`. Other config sections (models, shortcuts, etc.) are only read from `~/.xvora/config.toml`.
...
> **Experimental:** requires `--experimental-memory` (or `XVORA_MEMORY=1` / `[memory] enabled = true` in config).
...
- **Global** (`~/.xvora/memory/MEMORY.md`) — facts that apply across all your projects
- **Workspace** (`~/.xvora/memory/<project-slug>-<hash8>/MEMORY.md`) — project-specific conventions and context
- **Session logs** (`~/.xvora/memory/<project-slug>-<hash8>/sessions/`) — per-session summaries
...
For richer capture — decisions, patterns, debugging workflows, API discoveries — use `/flush` in the TUI. This triggers an LLM-generated summary of the current session's most important content and writes it to a dated session log under `~/.xvora/memory/<project-slug>-<hash8>/sessions/`, where it is indexed and searchable in future sessions.
...
Key options under `[memory]` in `~/.xvora/config.toml`:
...
| `watcher.enabled` | `true` | Watch `~/.xvora/memory/` for external edits and reindex on search |
...
When first-turn memory injection runs, Grok emits the `xvora-shell-memory_injection`
...
| `workspace`     | Everywhere         | CWD + `/tmp` + `~/.xvora/` | Allowed       | Normal development       |
| `read-only`     | Everywhere         | `~/.xvora/` only           | Blocked       | Exploration, code review |
| `strict`        | CWD + system paths | CWD + `/tmp` + `~/.xvora/` | Blocked       | Untrusted code           |

Sensitive paths (`~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.xvora/auth/`) are always
write-protected regardless of profile.

### Custom Profiles

Create `~/.xvora/sandbox.toml` (global) or `.xvora/sandbox.toml` (per-project):
...
Sandbox events (profile applied, violations) are logged to `~/.xvora/sandbox-events.jsonl`
...
Use `xvora inspect` to see everything Grok discovers in the current directory:

```bash
xvora inspect          # human-readable output
xvora inspect --json   # machine-readable JSON
```
...
- **Skills** — from `.xvora/skills/`, `~/.xvora/skills/`, plugins, and config paths
...
Grok automatically discovers configuration from Claude Code directories alongside native `.xvora/` paths. No extra setup is needed.

### What is picked up

| Component         | Claude Code location                                 | How Grok uses it                 |
| ----------------- | ---------------------------------------------------- | -------------------------------- |
| **Skills**        | `.claude/skills/`, `~/.claude/skills/`               | Loaded as skills (same as `.xvora/skills/`) |
...
Fetch a specific URL and return its content as markdown. **Disabled by default** — enable with `XVORA_WEB_FETCH=1`. 
...
Sessions are stored under `~/.xvora/sessions/`, organized by URL-encoded working directory:

```
~/.xvora/sessions/<encoded-cwd>/<session-id>/
...
xvora -p "Remember: X=42" -s my-session
xvora -p "What is X?" -s my-session

# Resume existing session (errors if not found)
xvora -p "Continue" -r my-session
...
| Path                  | Description                                         |
| --------------------- | --------------------------------------------------- |
| `~/.xvora/config.toml` | Configuration file                                  |
| `~/.xvora/sessions/`   | Persisted sessions (organized by working directory) |
| `~/.xvora/auth.json`   | Authentication credentials (auto-managed)           |
| `~/.xvora/memory/`     | Cross-session memory files and index                |
| `~/.xvora/skills/`     | User-scoped skill definitions                       |
| `~/.xvora/plugins/`    | User-scoped plugins                                 |
| `~/.xvora/agents/`     | User-scoped agent definitions                       |
| `.xvora/config.toml`   | Project-scoped config (MCP servers)                 |
...
| `.xvora/hooks/`        | Project-scoped hooks                                |
...
| `XVORA_CLI_CHAT_PROXY_BASE_URL`  | Override the cli-chat-proxy URL (default: `https://cli-chat-proxy.grok.com/v1`)                          |
| `XVORA_MODELS_BASE_URL`          | Custom base URL for inference. Model list auto-fetched from `{base_url}/models` (see [Custom Models Endpoint](#custom-models-endpoint)) |
| `XVORA_MODELS_LIST_URL`          | Override the model list URL if it differs from `{XVORA_MODELS_BASE_URL}/models`                                              |
| `XVORA_AUTH_PROVIDER_COMMAND`     | External auth binary (alternative to config file). See [External Auth Provider](#external-auth-provider) |
| `XVORA_AUTH_TOKEN_TTL`            | Token lifetime in seconds for external auth providers that output bare tokens. See [External Auth Provider](#external-auth-provider) |
| `XVORA_AUTH_EARLY_INVALIDATION_SECS` | Seconds before `expires_at` to consider a token expired (default: `300`). See [Automatic Credential Refresh](#automatic-credential-refresh) |
| `XVORA_OIDC_ISSUER`              | OIDC issuer URL (alternative to config file). See [OIDC](#oidc-customer-sso)                             |
| `XVORA_OIDC_CLIENT_ID`           | OIDC client ID (alternative to config file). See [OIDC](#oidc-customer-sso)                              |
| `XVORA_HOME`                     | Override config directory (default: `~/.xvora`)                                                           |
| `XVORA_SUBAGENTS`                | Enable (`1`) or disable (`0`) subagent/task tool support                                                 |
| `XVORA_MEMORY`                   | Enable (`1`) or disable (`0`) cross-session memory                                                       |
| `XVORA_AGENT`                    | Custom agent definition path or name (see [Agent Profiles](#agent-profiles))                             |
| `XVORA_WEB_FETCH`                | Enable (`1`) or disable (`0`) the `web_fetch` tool                                                       |
| `XVORA_WEB_FETCH_PROXY`          | Egress proxy URL for `web_fetch` requests (overridden by `[toolset.web_fetch] proxy_endpoint`)           |
| `XVORA_RESPECT_GITIGNORE`        | Disable `.gitignore` filtering in tools when set to `0`                                                  |
| `XVORA_FEEDBACK_ENABLED`         | Enable (`1`) or disable (`0`) feedback system independently from telemetry                               |
| `XVORA_DEPLOYMENT_KEY`           | Management API key for enterprise deployments                                                            |
| `XVORA_LOG_FILE`                 | Enable file logging by providing a file path (the value is used verbatim as the path)                    |
| `XVORA_DEBUG_LOG`                | Debug firehose (set by `--debug`): truthy routes per-session logs to `~/.xvora/debug/<sessionId>.txt`, a path writes that one file |
| `RUST_LOG`                      | Log filter for stderr (headless `-p` defaults to `off`, other non-TUI modes to `error`; TUI captures stderr) and for the `XVORA_LOG_FILE` log; the `--debug` firehose ignores it |

---

## Shell Completions

Generate completions for your shell and install them to enable tab completion for `xvora` commands and flags.
...
xvora completions bash > ~/.local/share/bash-completion/completions/grok
```
...
```bash
mkdir -p ~/.xvora/completions/bash
xvora completions bash > ~/.xvora/completions/bash/grok.bash
```

Add to `~/.bashrc`:

```bash
[[ -r "$HOME/.xvora/completions/bash/grok.bash" ]] && source "$HOME/.xvora/completions/bash/grok.bash"
```
...
xvora completions zsh > ~/.zsh/completions/_grok
```
...
```bash
mkdir -p ~/.xvora/completions/zsh
xvora completions zsh > ~/.xvora/completions/zsh/_grok
```

Add to `~/.zshrc`:

```zsh
fpath=("$HOME/.xvora/completions/zsh" $fpath)
autoload -Uz compinit
compinit
```
...
Regenerate completions after upgrading `xvora` — the script reflects the CLI of the installed version.
...
Write logs to a file for debugging. The TUI captures stderr, so `RUST_LOG` alone won't produce visible output in production — use `grok --debug` or `XVORA_LOG_FILE` instead:

```bash
# Per-session debug log (~/.xvora/debug/<sessionId>.txt)
grok --debug

# Log to a custom path
XVORA_LOG_FILE=/tmp/grok-debug.log grok

# Tail the most-recently-opened session's log in another terminal (Unix symlink)
tail -f ~/.xvora/debug/latest.txt
```

The `--debug` firehose uses a fixed filter (first-party crates at `debug`) and is not narrowed by `RUST_LOG`. A `XVORA_LOG_FILE` log defaults to `debug` and honors `RUST_LOG`, so you can set module-level filters for targeted debugging:

```bash
# Debug auth, info for everything else
XVORA_LOG_FILE=/tmp/grok-debug.log RUST_LOG="info,xai_xvora_shell::auth=debug" grok
```
...
cat ~/.xvora/sessions/<encoded-cwd>/<session-id>/summary.json | jq .

# View conversation history
cat ~/.xvora/sessions/<encoded-cwd>/<session-id>/updates.jsonl | head -20

# Count turns in a session
wc -l ~/.xvora/sessions/<encoded-cwd>/<session-id>/chat_history.jsonl
```
