...
//!
//! ```ignore
//! use xai_xvora_shell::leader::{connect_or_spawn, ClientCapabilities, ClientMode};
//!
...
mod transport;
use crate::env::XvoraEnvironment;
...
const CLIENT_LEADER_VERSION: &str = xai_xvora_version::VERSION;
...
pub struct LeaderDescriptor {
    ...
    pub environment: Option<XvoraEnvironment>,
...
pub enum LeaderTarget {
    Environment(XvoraEnvironment),
    WsUrl(String),
    Pid(u32),
}
fn known_environment_for_ws_url(ws_url: &str) -> Option<XvoraEnvironment> {
    let environments: &[XvoraEnvironment] = &[XvoraEnvironment::Production];
    environments
        .iter()
        .copied()
...
fn known_environment_for_suffix(ws_url_suffix: &str) -> Option<XvoraEnvironment> {
    let environments: &[XvoraEnvironment] = &[XvoraEnvironment::Production];
    environments
        .iter()
        .copied()
                ...
                Ok(()) => {
                    ...
                    crate::unified_log::warn(
                        ...
                        Some(serde_json::json!(
                            ...
                            "reason" : reason, "killer_ver" : xai_xvora_version::VERSION,
                            }
                        )),
                    );
                }
...
/// `xai_xvora_update::auto_update::resolve_restart_exe`.
///
/// For a **dev / out-of-tree binary** (`cargo run`, integration tests, installs
/// not under `xvora_home`), keep `current_exe()` so the spawned leader matches the
/// calling binary.
///
...
/// Binary file name for the managed grok install (`grok` / `xvora.exe`).
...
fn spawn_leader_subprocess(env_urls: &LeaderEnvUrls) -> Result<u32, ConnectionError> {
    ...
    let leader_log = std::env::var("XVORA_LEADER_LOG")
        .or_else(|_| std::env::var("RUST_LOG"))
        .unwrap_or_else(|_| "xai_xvora_shell=info,xai_acp_lib=warn,xai_xvora_mcp=warn".into());
