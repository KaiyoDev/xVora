...
pub enum ConfigUpdate {
    ...
    ///
    /// Project-scoped changes (`<cwd>/.xvora/config.toml`,
    ...
    /// (`<cwd>/.xvora/config.toml`, `<cwd>/.mcp.json`, or
    ...
    ProjectMcpServersChanged {
        /// The project root whose `.xvora/`, `.mcp.json`, or
...
impl ConfigReloader {
    ...
    fn reload_config(&mut self) -> anyhow::Result<()> {
        ...
        // scoped changes (`<cwd>/.xvora/config.toml`,
...
/// for `.xvora/config.toml`, `find_mcp_json_files` for `.mcp.json`) so
...
pub(crate) fn parse_skills_config(
    config: &toml::Value,
) -> xai_xvora_agent::prompt::skills::SkillsConfig {
    config
        .get("skills")
...
fn parse_compat_config(config: &toml::Value) -> xai_xvora_tools::types::compat::CompatConfigToml {
    config
        .get("compat")
        .and_then(|v| v.clone().try_into().ok())
        .unwrap_or_default()
...
mod tests {
    ...
    /// The hash must reflect ancestor `.xvora/config.toml` and `.mcp.json`
...
"#,
    ...
    /// `ProjectConfigChanged` (`<cwd>/.xvora/config.toml`) and
