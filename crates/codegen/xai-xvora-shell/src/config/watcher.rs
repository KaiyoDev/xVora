...
/// changes, plus any extra paths (project `.xvora/config.toml`, `.mcp.json`,
...
/// `<cwd>/.xvora/` (catches `<cwd>/.xvora/config.toml`). Recursing on `<cwd>`
...
impl ConfigFileWatcher {
    ...
    /// `<cwd>/.xvora/`. Use [`Self::watch_path`] later to register additional
    ...
    pub fn start(
        ...
        let xvora_home_buf = xvora_home.to_path_buf();
        ...
        let mut debouncer = new_filtered_debouncer(debounce, move |res: DebounceEventResult| {
            ...
            for event in events {
                ...
                let change = match name {
                    Some("auth.json") if parent == Some(xvora_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::AuthChanged)
                    }
                    Some("config.toml") if parent == Some(xvora_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::GlobalConfigChanged)
                    }
                    Some("models_cache.json") if parent == Some(xvora_home_buf.as_path()) => {
        ...
        // includes `<cwd>/.xvora/config.toml` so the loop above
        // watches `<cwd>/.xvora/`), the call below installs a
    ...
    /// Register `<cwd>/` and `<cwd>/.xvora/` as **non-recursive** watch
    ...
    /// `<cwd>/.mcp.json` and `<cwd>/.xvora/config.toml` trigger a
    ...
    /// `<cwd>/.xvora/`) previously registered for `cwd` via
...
///
/// **Known limitation:** if `<cwd>/.xvora/` does not yet
/// exist at session-open time, the `.xvora/` watch fails ENOENT and is
/// swallowed at `debug!`. A later `mkdir <cwd>/.xvora/` followed by a
/// write to `<cwd>/.xvora/config.toml` will NOT be observed — the
...
/// expected for a freshly-opened session whose `<cwd>/.xvora/` hasn't
...
/// Watches skill directories (`~/.xvora/skills/`, `<repo>/.xvora/skills/`, etc.)
...
/// recursively: `xvora_home` (`~/.xvora`, or `$XVORA_HOME`) or a known vendor dir
/// directly under `$HOME` ([`HOME_VENDOR_DIRS`]).
///
/// These hold large non-skill trees — `~/.xvora` alone has `worktrees/`,
...
fn is_global_config_dir(dir: &Path, xvora_home: &Path) -> bool {
    #[allow(deprecated)]
    let home = std::env::home_dir();
    is_global_config_dir_impl(dir, xvora_home, home.as_deref())
}
...
const HOME_VENDOR_DIRS: &[&str] = &[".xvora", ".agents", ".claude", ".cursor"];

/// Testable core of [`is_global_config_dir`] with `$HOME` injected.
fn is_global_config_dir_impl(dir: &Path, xvora_home: &Path, home: Option<&Path>) -> bool {
    let canon = |p: &Path| dunce::canonicalize(p).unwrap_or_else(|_| p.to_path_buf());
    if canon(dir) == canon(xvora_home) {
        return true;
    }
...
/// `~/.xvora` these exist at startup; later creation is caught on restart).
...
impl SkillsFileWatcher {
    ...
    ///
    /// Uses [`collect_skill_config_dirs`](xai_xvora_agent::prompt::skills::collect_skill_config_dirs)
    ...
    pub fn start(
        ...
        let dirs_to_watch = xai_xvora_agent::prompt::skills::collect_skill_config_dirs(
            cwd,
            monorepo_user_dir,
            &xvora_home,
            config_paths,
        ...
        for dir in &dirs_to_watch {
            if is_global_config_dir(dir, &xvora_home) {
...
mod tests {
    ...
    /// `is_global_config_dir` must scope down only xvora_home and the known
    ...
    /// recursively watched.
    #[test]
    fn is_global_config_dir_matches_only_xvora_home_and_vendor_dirs() {
        let home = TempDir::new().unwrap();
        let home = home.path();
        let xvora_home = home.join(".xvora");

        let g = |dir: &Path| is_global_config_dir_impl(dir, &xvora_home, Some(home));

        // xvora_home and vendor dirs directly under $HOME: scoped (global).
        assert!(g(&xvora_home));
        ...
        assert!(!g(&home.join("repo").join(".xvora")));
    }

    /// Regression for the ~/.xvora inotify-exhaustion / worktree-noise bug: a
    /// `SKILL.md` under a sibling subtree (e.g. `~/.xvora/worktrees/`) must not
    ...
    /// A write to `<xvora_home>/models_cache.json` must surface as
    ...
    /// A write to `<cwd>/.xvora/config.toml` must surface as
    ...
    fn project_cwd_toml_triggers_reload() {
        ...
        let project_grok = cwd.path().join(".xvora");
        ...
        assert!(
            found,
            "expected ProjectConfigChanged for <cwd>/.xvora/config.toml within 2s"
        );
    }
    ...
    /// `start` must light up `<new_cwd>/.xvora/config.toml` writes
    ...
    fn watch_path_dynamic_registration() {
        ...
        let project_grok = new_cwd.path().join(".xvora");
        fs::create_dir_all(&project_grok).unwrap();
        fs::write(project_grok.join("config.toml"), "").unwrap();
