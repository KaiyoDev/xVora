...
fn expands_env_vars_in_toml_strings() {
    with_env_var(
        "XVORA_TEST_CONFIG_EXPAND",
        "expanded",
        || {
            let toml_str = r#"
[mcp_servers.test]
command = "$XVORA_TEST_CONFIG_EXPAND/bin/server"
args = ["--path", "${XVORA_TEST_CONFIG_EXPAND}/data"]
"#;
...
fn leaves_missing_env_vars_unchanged() {
    let toml_str = r#"
[mcp_servers.test]
command = "$XVORA_TEST_CONFIG_MISSING/bin/server"
"#;
    let mut value = toml::from_str::<toml::Value>(toml_str).unwrap();
    expand_env_vars_in_toml(&mut value);
    let toml::Value::Table(table) = value else {
    ...
    let command = test.get("command").and_then(|v| v.as_str()).unwrap();
    assert_eq!(command, "$XVORA_TEST_CONFIG_MISSING/bin/server");
}
#[test]
...
/// Mutex to serialize tests that touch the XVORA_MEMORY env var.
...
/// Run `f` with XVORA_MEMORY explicitly unset.
fn without_grok_memory<T>(f: impl FnOnce() -> T) -> T {
    let _guard = MEMORY_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("XVORA_MEMORY", None, f)
}
/// Run `f` with XVORA_MEMORY set to a specific value.
fn with_grok_memory<T>(value: &str, f: impl FnOnce() -> T) -> T {
    let _guard = MEMORY_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("XVORA_MEMORY", Some(value), f)
}
#[test]
...
fn memory_config_env_var_zero_does_not_enable() {
    with_grok_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, false, &config, None);
            assert!(! mem.enabled, "XVORA_MEMORY=0 should not enable memory");
        },
    );
}
#[test]
fn memory_config_env_var_false_does_not_enable() {
    with_grok_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, false, &config, None);
            assert!(! mem.enabled, "XVORA_MEMORY=false should not enable memory");
        },
    );
}
#[test]
...
fn memory_config_env_zero_force_disables_toml_enabled() {
    with_grok_memory(
        ...
        || {
            ...
            assert!(
                ! mem.enabled,
                "XVORA_MEMORY=0 should force-disable even when TOML enables memory"
            );
        },
    );
}
...
fn memory_config_env_false_force_disables_toml_enabled() {
    with_grok_memory(
        ...
        || {
            ...
            assert!(
                ! mem.enabled,
                "XVORA_MEMORY=false should force-disable even when TOML enables memory"
            );
        },
    );
}
...
fn memory_config_cli_flag_overrides_env_disable() {
    with_grok_memory(
        ...
        || {
            ...
            let mem = MemoryConfig::resolve(true, false, &config, None);
            assert!(
                mem.enabled, "CLI --experimental-memory should override XVORA_MEMORY=0"
            );
        },
    );
}
...
fn memory_config_no_memory_overrides_all() {
    with_grok_memory(
        ...
        || {
            ...
            assert!(
                ! mem.enabled,
                "--no-memory should override --experimental-memory, XVORA_MEMORY=1, and TOML enabled=true"
            );
        },
    );
}
...
fn memory_config_no_memory_overrides_env_enable() {
    with_grok_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, true, &config, None);
            assert!(! mem.enabled, "--no-memory should override XVORA_MEMORY=1");
        },
    );
}
#[test]
        ...
        || {
            with_env_var(
                ...
                || {
                    let toml_str = r#"
[mcp_servers.test]
command = "$XVORA_TEST_USER $XVORA_TEST_ROOT"
"#;
                    let mut value = toml::from_str::<toml::Value>(toml_str).unwrap();
                    expand_env_vars_in_toml(&mut value);
                    let toml::Value::Table(table) = value else {
...
/// Mutex to serialize tests that touch the XVORA_SUBAGENTS env var.
static SUBAGENTS_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());
/// Run `f` with XVORA_SUBAGENTS explicitly unset.
fn without_grok_subagents<T>(f: impl FnOnce() -> T) -> T {
    let _guard = SUBAGENTS_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("XVORA_SUBAGENTS", None, f)
}
/// Run `f` with XVORA_SUBAGENTS set to a specific value.
fn with_grok_subagents<T>(value: &str, f: impl FnOnce() -> T) -> T {
    let _guard = SUBAGENTS_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("XVORA_SUBAGENTS", Some(value), f)
}
#[test]
    ...
    with_grok_subagents(
        ...
        || {
            let config: toml::Value = toml::from_str("[subagents]\nenabled = true")
                .unwrap();
            let sa = SubagentsConfig::resolve(false, &config, None);
            assert!(! sa.enabled, "XVORA_SUBAGENTS=0 should override config file");
        },
    );
}
#[test]
...
fn subagents_config_env_var_disables_default() {
    with_grok_subagents(
        ...
        || {
            ...
            let sa = SubagentsConfig::resolve(false, &config, None);
            assert!(
                ! sa.enabled, "XVORA_SUBAGENTS=0 should override the enabled default"
            );
        },
    );
}
...
fn subagents_config_cli_flag_overrides_env_var() {
    with_grok_subagents(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let sa = SubagentsConfig::resolve(true, &config, None);
            assert!(sa.enabled, "--subagents CLI flag should override XVORA_SUBAGENTS=0");
        },
    );
}
#[test]
    ...
    with_grok_subagents(
        ...
        || {
            let config: toml::Value = toml::from_str(
                ...
                .unwrap();
            let sa = SubagentsConfig::resolve(false, &config, None);
            assert!(sa.enabled, "XVORA_SUBAGENTS=1 should enable");
            assert_eq!(sa.models.get("explore").unwrap(), "grok-3-fast");
        },
    );
}
...
fn with_managed_mcp_env<T>(
    ...
    with_env_var_opt(
        ...
        managed_mcps,
        || with_env_var_opt("XVORA_MANAGED_MCP_GATEWAY_TOOLS_ENABLED", gateway_tools, f),
    )
}
#[test]
#[serial_test::serial]
...
fn with_model_overrides_env_full<T>(
    ...
    let _guard = LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt(
        "XVORA_WEB_SEARCH_MODEL",
        ws,
        || with_env_var_opt(
            "XVORA_SESSION_SUMMARY_MODEL",
            ss,
            || with_env_var_opt(
                "XVORA_IMAGE_DESCRIPTION_MODEL",
                id,
                || with_env_var_opt("XVORA_PROMPT_SUGGESTIONS_MODEL", ps, f),
            ),
        ),
    )
}
...
#[test]
fn model_overrides_default_image_description_is_xvora() {
    with_model_overrides_env(
        None,
        None,
        None,
...
#[test]
fn model_overrides_default_session_summary_is_xvora() {
    with_model_overrides_env(
        None,
        None,
        None,
...
fn with_tools_env<T>(
    ...
    with_env_var_opt(
        "XVORA_RESPECT_GITIGNORE",
        respect_gitignore,
        || with_env_var_opt("XVORA_DISABLE_ZDR_INCOMPATIBLE_TOOLS", disable_zdr, f),
    )
}
    ...
    with_grok_respect_gitignore(
        ...
        || {
            ...
            assert!(
                ...
                "XVORA_RESPECT_GITIGNORE=false should override config file"
            );
        },
    );
}
...
fn roles_parse_from_toml() {
    ...
    assert_eq!(implementer.prompt_file.as_deref(), Some(".xvora/prompts/impl.md"));
}
#[test]
...
fn personas_parse_from_toml() {
    ...
    assert_eq!(concise.instructions_file.as_deref(), Some(".xvora/personas/concise.md"));
}
#[test]
...
fn enterprise_two_file_merge_routes_deployment_key_to_proxy() {
    for k in [
        ...
        "XVORA_TRACE_UPLOAD_ENDPOINT_URL",
    ] {
...
#[test]
fn validate_hooks_path_rejects_outside_xvora_home() {
...
fn managed_settings_disables_features_and_requirements_overrides() {
    use xai_xvora_workspace::permission::resolution::ManagedSettingsFeatures;
    let mut cfg = crate::agent::config::Config::default();
...
fn simulate_release_build() -> xai_xvora_test_support::EnvGuard {
    xai_xvora_test_support::EnvGuard::set(xai_xvora_version::TEST_VERSION_ENV, "0.0.0-sim")
}
...
/// verdicts untouched). XVORA_HOME-isolated + `#[serial]` for folder-trust
/// store hygiene (empty store ⇒ deterministic untrusted;
/// `EnvGuard` restores XVORA_HOME even on panic). No user-global
/// `$XVORA_HOME/config.toml` is seeded: `xvora_home()` is `OnceLock`-cached,
...
/// in `xai-xvora-agent`. An ABSOLUTE plugin path is used so the merged
...
/// XVORA_HOME-isolated + `#[serial]` (`EnvGuard` restores it even on panic).
#[test]
#[serial_test::serial]
fn discover_plugins_excludes_untrusted_configpath_plugin_end_to_end() {
    use xai_xvora_agent::plugins::{TrustStore, discover_plugins};
    use xai_xvora_test_support::EnvGuard;
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("XVORA_HOME", home.path());
    let _flag = EnvGuard::unset("XVORA_FOLDER_TRUST");
...
/// arm (store-only reconcile) could never lift. XVORA_HOME-isolated (empty
/// store); XVORA_FOLDER_TRUST unset so the kill-switch is the only signal.
#[test]
#[serial_test::serial]
