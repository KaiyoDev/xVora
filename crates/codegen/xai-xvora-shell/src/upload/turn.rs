...
pub(crate) use xai_xvora_telemetry::session_metrics::TraceUploadReason;
...
pub(crate) fn parse_agent_profile_from_meta(
...
) -> Option<xai_xvora_agent::AgentDefinition> {
    let value = meta?.get("agentProfile")?;
    if value.is_object() {
        return match xai_xvora_agent::AgentDefinition::from_json(value) {
            Ok(def) => {
                tracing::info!(
                    agent_name = % def.name,
    ...
    if let Some(name) = value.as_str() {
        ...
        return xai_xvora_agent::discovery::by_name(name);
    }
    tracing::warn!(
...
mod tests {
    ...
    fn parse_ask_user_question_returns_none_when_absent() {
        let meta = serde_json::json!({ "agentProfile" : "xvora-plan" });
