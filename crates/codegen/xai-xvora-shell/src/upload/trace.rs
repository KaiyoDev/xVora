...
use xai_xvora_workspace::permission::PermissionEvent;
...
fn record_upload_failure(ctx: &PromptTraceContext, f: UploadFailure<'_>) {
    ...
    if level == UploadFailureLogLevel::Warn {
        xai_xvora_telemetry::unified_log::warn(&msg, sid, log_ctx);
    } else {
        xai_xvora_telemetry::unified_log::error(&msg, sid, log_ctx);
    }
}
...
pub(crate) fn local_sandbox_telemetry() -> Option<LocalSandboxTelemetry> {
    let profile = xai_xvora_sandbox::configured_profile_name()?;
    Some(LocalSandboxTelemetry {
        profile: profile.to_owned(),
        applied: xai_xvora_sandbox::is_active(),
    })
}
...
pub(crate) async fn upload_plugin_state(
    ctx: &PromptTraceContext,
    registry: Option<&xai_xvora_agent::plugins::PluginRegistry>,
) {
    use xai_xvora_agent::plugins::discovery::PluginScope;
...
pub(crate) async fn upload_unified_log(ctx: &PromptTraceContext, wait: UploadWait) {
    ...
    let log_bytes = match tokio::task::spawn_blocking(move || {
        xai_xvora_telemetry::unified_log::snapshot_session_log(&session_id)
    })
    .await
    {
        Ok(Some(bytes)) => bytes,
    ...
    .await;
    let full_log_bytes =
        tokio::task::spawn_blocking(xai_xvora_telemetry::unified_log::snapshot_log).await;
    let user_id = ctx
        .auth_manager
        .current_or_expired()
        .map(|a| a.user_id)
...
pub(crate) fn build_chat_history_session_state(
    _messages: &[xai_xvora_sampling_types::conversation::ConversationItem],
...
impl TraceExportSource for DynamicResolver {
    ...
    fn proxy_credentials(&self) -> Option<Arc<dyn xai_xvora_auth::AuthCredentialProvider>> {
...
pub(crate) fn spawn_startup_spill_reconcile(
    ...
    tokio::spawn(async move {
        match queue {
            Some(queue) => {
                let report =
                    xai_xvora_workspace::recovery::run_startup_recovery(&xvora_home, &queue).await;
            ...
            None => {
                let purged = tokio::task::spawn_blocking(move || {
                    xai_xvora_workspace::recovery::purge_spilled_items(&xvora_home)
                })
                .await;
                match purged {
                    Ok(removed) => {
