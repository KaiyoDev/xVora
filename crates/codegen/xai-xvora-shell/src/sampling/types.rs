// Re-export all types from the standalone xai-xvora-sampling-types crate.
// This keeps all existing `crate::sampling::types::*` imports working.
pub use xai_xvora_sampling_types::types::*;

// `CreateResponseWrapper` and `MessagesRequestWrapper` previously lived
// here. They were moved into `xai-xvora-sampling-types::types` (and
// are re-exported above via the wildcard) so the new
// `xai-xvora-sampler` crate can reference them without a circular
// dep on `xai-xvora-shell`.

// Tests for the types now live in xai-xvora-sampling-types crate.

use xai_xvora_tools::types::output::ImageContent as ToolsImageContent;
...
///
/// Lives in the shell (rather than `xai-xvora-sampling-types` or
/// `xai-xvora-tools`) so neither of those crates needs to depend on
