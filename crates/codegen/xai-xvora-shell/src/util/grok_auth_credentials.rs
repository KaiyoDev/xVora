...
/// User token (xAI users) sends `Bearer` + `X-XAI-Token-Auth: xai-xvora-cli`.
...
impl GrokAuthCredentials {
    ...
    pub fn auth_error_hint(&self) -> &'static str {
        if self.deployment_key.is_some() {
            "Your XVORA_DEPLOYMENT_KEY is invalid or expired. Please contact a team admin."
...
impl xai_xvora_auth::HttpAuth for GrokAuthCredentials {
