...
// The foundation utilities live in `xai-xvora-shell-base` (upstream of this
// crate so they build in parallel). Re-exported at the original paths so
// existing `crate::util::…` / `xai_xvora_shell::util::…` users compile
// unchanged.
pub use xai_xvora_shell_base::util::*;
