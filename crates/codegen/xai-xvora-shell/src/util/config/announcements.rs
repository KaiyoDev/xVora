...
/// Re-exported from `xai-xvora-announcements` for backward compatibility.
pub use xai_xvora_announcements::RemoteAnnouncement;
...
/// Dev/test override for announcements via `XVORA_ANNOUNCEMENTS_OVERRIDE` (a JSON
...
pub fn announcements_override() -> Option<Vec<RemoteAnnouncement>> {
    let raw = std::env::var("XVORA_ANNOUNCEMENTS_OVERRIDE").ok()?;
    match serde_json::from_str::<Vec<RemoteAnnouncement>>(&raw) {
        Ok(list) => Some(list),
        Err(_) => {
            tracing::warn!("invalid XVORA_ANNOUNCEMENTS_OVERRIDE JSON; ignoring");
            None
        }
    }
}
...
/// `XVORA_ANNOUNCEMENTS_OVERRIDE` env var overrides everything (dev-only escape hatch).
