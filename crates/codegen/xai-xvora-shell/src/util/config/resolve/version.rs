...
///
/// This is a lightweight duplicate of `xai_xvora_update::channel_name()` for
/// use in `xai-xvora-shell` which cannot depend on `xai-xvora-update`.
pub fn channel_name_from_cache() -> Option<&'static str> {
    ...
    *NAME.get_or_init(|| {
        ...
        let current = semver::Version::parse(xai_xvora_version::VERSION).ok()?;
