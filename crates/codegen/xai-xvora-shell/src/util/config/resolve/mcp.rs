...
/// | env          | `XVORA_MCP_LIVENESS_WATCHERS` (handled by `BoolFlag::env`)       |
...
/// | cli          | (none — no CLI flag)                                            |
/// | env          | `XVORA_MCP_AUTO_RESTART` (handled by `BoolFlag::env`)            |
...
/// | cli          | (none — no CLI flag)                                            |
/// | env          | `XVORA_MCP_PUSH_SERVER_STATUS` (handled by `BoolFlag::env`)      |
...
/// | cli          | (none — no CLI flag)                                                |
/// | env          | `XVORA_MCP_RECURSIVE_CONFIG_WATCH` (handled by `BoolFlag::env`)      |
...
/// Kept in sync with `xai_xvora_mcp::servers`'s standalone fallback.
...
const ENV_MCP_STARTUP_TIMEOUT_SECS: &str = "XVORA_MCP_STARTUP_TIMEOUT_SECS";
...
/// `XVORA_MCP_STARTUP_TIMEOUT_SECS` secs) > effective `config.toml [mcp]` >
...
/// `XVORA_MCP_STARTUP_TIMEOUT_SECS` (secs). Unparseable/zero values are ignored.
...
pub const DEFAULT_MAX_MCP_OUTPUT_BYTES: usize = xai_xvora_tools::MCP_MAX_OUTPUT_BYTES;
...
pub fn cache_remote_max_mcp_output_bytes(remote: Option<u64>) {
    xai_xvora_tools::set_mcp_max_output_bytes(resolve_max_mcp_output_bytes(remote));
}
...
///   2. env `XVORA_MAX_MCP_OUTPUT_BYTES` / `MAX_MCP_OUTPUT_BYTES`
...
pub fn resolve_max_mcp_output_bytes(remote: Option<u64>) -> usize {
    ...
    resolve_max_mcp_output_bytes_precedence(
        requirements,
        xai_xvora_tools::mcp_max_output_bytes_from_env(),
...
/// `.xvora/config.toml` chain (`cwd` → git root), deepest file wins.
///
...
fn project_max_mcp_output_bytes(cwd: &std::path::Path) -> Option<usize> {
    ...
    for config_path in crate::config::find_project_configs(cwd) {
        if let Ok(toml_val) = xai_xvora_config::load_config_file(&config_path)
...
pub fn resolve_max_mcp_output_bytes_for_cwd(cwd: &std::path::Path) -> Option<usize> {
    let requirements = crate::config::load_merged_requirements()
        .as_ref()
        .and_then(max_mcp_output_bytes_from_toml);
    if requirements.is_some() || xai_xvora_tools::mcp_max_output_bytes_from_env().is_some() {
        return None;
    }
...
mod max_mcp_output_bytes_tests {
    ...
    fn project_chain_deepest_file_wins() {
        ...
        // No .xvora files with the key anywhere → None.
