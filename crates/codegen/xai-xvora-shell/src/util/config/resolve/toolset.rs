...
/// [`xai_xvora_config::env_bool`] parser (`XVORA_TOOLS_FIND_BFS` /
/// `XVORA_TOOLS_GREP_UGREP`, plus the `XVORA_FIND_BFS` / `XVORA_GREP_UGREP` aliases).
///
...
/// [`xai_xvora_tools::computer::local::SearchShadowConfig`] on the local terminal
/// backend.
pub fn resolve_search_tools_enabled(
    ...
    let disable = xai_xvora_config::env_bool("DISABLE_EMBEDDED_SEARCH_TOOLS");
    ...
    let resolve = |primary: &str, alias: &str, key: &str| -> bool {
        let env = xai_xvora_config::env_bool(primary).or_else(|| xai_xvora_config::env_bool(alias));
    ...
    (
        resolve("XVORA_TOOLS_FIND_BFS", "XVORA_FIND_BFS", "find_bfs"),
        resolve("XVORA_TOOLS_GREP_UGREP", "XVORA_GREP_UGREP", "grep_ugrep"),
    )
}
...
const ENV_PERSISTENT_SHELL: &str = "XVORA_PERSISTENT_SHELL";
...
mod persistent_local_shell_tests {
    ...
    // XVORA_PERSISTENT_SHELL is process-global (the documented kill-switch a dev
...
/// the shared [`xai_xvora_config::env_bool`] via `BoolFlag`). The secs env var
/// lives in the tools crate (`RESPONSE_TIMEOUT_ENV`), parsed once there.
const ENV_ASK_USER_QUESTION_TIMEOUT_ENABLED: &str = "XVORA_ASK_USER_QUESTION_TIMEOUT_ENABLED";
...
///
/// Precedence: requirements > env (`XVORA_ASK_USER_QUESTION_TIMEOUT_ENABLED`)
...
fn resolve_ask_user_question_timeout_secs_from_tiers(
    ...
    requirement
        ...
        .or(remote)
        .unwrap_or(
            xai_xvora_tools::implementations::xvora::ask_user_question::RESPONSE_TIMEOUT
                .as_secs(),
        )
}
...
///
/// Precedence: requirements > env (`XVORA_ASK_USER_QUESTION_TIMEOUT_SECS`,
...
fn resolve_ask_user_question_timeout_secs(
    ...
    resolve_ask_user_question_timeout_secs_from_tiers(
        ...
        xai_xvora_tools::implementations::xvora::ask_user_question::response_timeout_env_secs(),
...
) -> xai_xvora_tools::implementations::xvora::ask_user_question::AskUserQuestionParams {
    let requirements = crate::config::load_merged_requirements();
    let layers = match crate::config::ConfigLayers::load() {
        Ok(l) => Some(l),
        Err(e) => {
...
mod ask_user_question_timeout_tests {
    ...
    use xai_xvora_tools::implementations::xvora::ask_user_question::RESPONSE_TIMEOUT_ENV;
    ...
    fn timeout_secs_tier_precedence() {
        let d = xai_xvora_tools::implementations::xvora::ask_user_question::RESPONSE_TIMEOUT
            .as_secs();
...
mod tests {
    ...
    // Assumes XVORA_TOOLS_* / DISABLE_EMBEDDED_SEARCH_TOOLS are unset in the test env.
    #[test]
