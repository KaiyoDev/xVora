...
use xai_xvora_tools::types::compat::{CompatConfig, CompatConfigToml};

pub use xai_xvora_mcp::oauth_config::{McpOAuthConfig, McpOAuthConfigMap};
...
pub use xai_xvora_config_types::{McpJsonOAuthBlock, McpServerConfig, McpServerTransportConfig};
...
pub use xai_xvora_config_types::{McpConfig, RelaySyncConfig};
// Worktree-pool config value type extracted; re-exported to keep paths stable.
pub use xai_xvora_config_types::PoolConfig;
...
/// `.xvora/config.toml`, otherwise user (global config, `~/.claude.json`,
...
/// 2. Walk from git repo root down to `cwd`, loading `.xvora/config.toml` at each level
...
///
/// Uses the top-level `disabled_mcp_servers` array in `~/.xvora/config.toml`.
...
/// config file, e.g. a project-scoped `.xvora/config.toml`.
...
/// config file, e.g. a project-scoped `.xvora/config.toml`. OAuth credential
...
pub async fn delete_mcp_server_config_at(
    ...
    if let Ok(mut cred_store) = xai_xvora_mcp::credentials::McpCredentialStore::load_default() {
...
/// project-scoped `.xvora/config.toml` files.
...
pub use xai_xvora_workspace::project_config::{
...
/// (`"user"` or `"project"`).
///
/// Overlays project-scoped `.xvora/config.toml` files from `cwd` up to the
    ...
    fn mcp_json_env_var_default_value() {
        ...
        std::fs::write(
            ...
            r#"{
                "mcpServers": {
                    "api": {
                        "url": "${XVORA_TEST_MCP_UNSET_VAR_12345:-https://fallback.example.com}/mcp"
                    }
                }
            }"#,
        )
