...
/// Resolved absolute path to bash. On Unix uses the `xai_xvora_config` shell
/// resolution cascade (`$XVORA_SHELL` > `$SHELL` > `which` > common dirs >
...
pub fn default_shell_path() -> &'static str {
    #[cfg(unix)]
    {
        xai_xvora_config::shell::unix_shell_path(xai_xvora_config::shell::UnixShellKind::Bash)
    }
    #[cfg(not(unix))]
    {
        "/bin/bash"
...
/// Delegates to the canonical implementation in `xai-xvora-tools`.
pub use xai_xvora_tools::util::pager_env;
...
/// xai-xvora-pager which renders its own UI and doesn't need raw ANSI codes).
///
