...
use xai_xvora_config::signed_policy::now_unix;
...
pub enum ManagedConfigError {
    ...
    #[error(
        "The deployment key was rejected. Confirm that XVORA_DEPLOYMENT_KEY is set correctly and hasn't expired."
    )]
    ...
    SignatureRejected,
    #[error(
        "Can't save the configuration to ~/.xvora. Make sure the directory exists and is writable.\n  ({0})"
    )]
...
pub(super) struct ManagedConfigResponse {
    ...
    #[serde(default)]
    pub(super) signatures: Option<Vec<xai_xvora_config::signed_policy::SignatureEnvelope>>,
}
...
impl ManagedConfigResponse {
    ...
    pub(super) fn signature_sidecar(
        &self,
    ) -> Option<xai_xvora_config::signed_policy::SignatureEnvelope> {
        self.signature_sidecar_with(xai_xvora_config::signed_policy::embedded_key_id_trusted)
    }
...
pub(super) struct VerifiedEnvelope {
    pub(super) sidecar: xai_xvora_config::signed_policy::SignatureEnvelope,
    pub(super) payload: xai_xvora_config::signed_policy::SignedPayload,
}
...
pub(super) fn verify_signed_envelope(
    ...
    use xai_xvora_config::signed_policy;
...
mod tests {
    ...
    fn signature_sidecar_picks_trusted_envelope_then_falls_back() {
        use xai_xvora_config::signed_policy::SignatureEnvelope;
