...
///
/// These directories under `~/.xvora/skills/` will be deleted on startup
...
/// `<xvora_home>/skills/<name>/SKILL.md`. Exact-path (not prefix) so a
/// user-authored skill that reuses a bundled name — even elsewhere under
/// `<xvora_home>/skills/` — is never labeled bundled. Lives beside the
...
pub(crate) fn is_extracted_bundled_skill(
    ...
    BUNDLED_SKILLS.iter().any(|&(n, _)| n == name)
        && path == xvora_home.join("skills").join(name).join("SKILL.md")
}

/// Resolve the content for a skill, applying any name-specific transforms.
fn resolve_skill_content(name: &str, raw: &str, xvora_home: &std::path::Path) -> String {
    match name {
        // Help skill needs path substitution so absolute paths work.
        "help" => {
            let xvora_home_str = format!("{}/", xvora_home.to_string_lossy());
            raw.replace("~/.xvora/", &xvora_home_str)
        }
        _ => raw.to_string(),
    }
}

/// Extract bundled files to `~/.xvora/` on startup.
///
...
pub fn extract_bundled_files(xvora_home: &std::path::Path) {
    ...
    remove_legacy_bundled_skills(xvora_home);
        ...
        && existing.trim() == version
    {
        ...
        extract_missing_skills(xvora_home);
        return;
    }
    ...
    for &(name, raw) in BUNDLED_SKILLS {
        let skill_dir = xvora_home.join("skills").join(name);
        let _ = std::fs::create_dir_all(&skill_dir);
        let content = resolve_skill_content(name, raw, xvora_home);
...
fn extract_missing_skills(xvora_home: &std::path::Path) {
    for &(name, raw) in BUNDLED_SKILLS {
        let skill_md = xvora_home.join("skills").join(name).join("SKILL.md");
        if skill_md.exists() {
            continue;
        }
...
fn remove_legacy_bundled_skills(xvora_home: &std::path::Path) {
    remove_legacy_skills(xvora_home, LEGACY_BUNDLED_SKILL_NAMES, BUNDLED_SKILLS);
}
...
mod tests {
    ...
    async fn help_skill_discovered_by_skill_pipeline() {
        ...
        std::fs::create_dir_all(workspace.join(".xvora").join("skills").join("help")).unwrap();
        std::fs::copy(
            home.join("skills/help/SKILL.md"),
            workspace.join(".xvora/skills/help/SKILL.md"),
        )
        .unwrap();
