...
fn remove_managed_config_files(home: &std::path::Path) {
    ...
    for name in [
        "managed_config.toml",
        "requirements.toml",
        xai_xvora_config::signed_policy::SIGNATURE_SIDECAR_FILE,
...
/// Map a `reqwest` send failure to a `ManagedConfigError` via the shared `xai-xvora-http` classifier.
...
fn apply_fetched(
    ...
    let verified = if xai_xvora_config::signed_policy::verification_active() {
    ...
    if let Some(verified) = verified {
        clear_squatting_dir(&home.join(xai_xvora_config::signed_policy::SIGNATURE_SIDECAR_FILE));
        xai_xvora_config::signed_policy::write_sidecar(&home, &verified.sidecar)?;
    }
...
pub async fn fetch_setup_report() -> Result<SetupReport, ManagedConfigError> {
    ...
    if source.is_some()
        && xai_xvora_config::signed_policy::verification_active()
