...
use serde::Serialize;
use xai_xvora_tools::types::compat::{COMPAT_CELLS, CompatCell, CompatConfig};
...
/// `None` for native `.xvora`/`.agents` paths.
...
pub(super) fn resolve_inspect_compat(
    ...
    resolve_inspect_compat_with_env(effective_config, |cell| {
        xai_xvora_config::env_bool(cell.env_var())
    })
}
...
mod tests {
    ...
    fn inspect_compat_uses_env_config_default_precedence() {
        ...
        let report = resolve_inspect_compat_with_env(Ok(&effective_config), |cell| {
            (cell.env_var() == "XVORA_CURSOR_SKILLS_ENABLED").then_some(true)
        });
