...
use xai_xvora_tools::util::truncate::estimate_tokens;
...
pub struct LoginPolicyReport {
    /// Raw `disable_api_key_auth` knob (env `XVORA_DISABLE_API_KEY_AUTH`).
...
pub struct ConfigSources {
    ...
    /// .xvora/config.toml files). Driven from the same resolvers used at runtime
...
async fn build_report(cwd: &Path) -> InspectReport {
    ...
    let trust_store = xai_xvora_agent::plugins::TrustStore::load();
    ...
    let discovered_plugins = xai_xvora_agent::plugins::discover_plugins(
        Some(cwd),
        &plugin_config,
        &trust_store,
        project_trusted,
    ...
    let plugin_registry = xai_xvora_agent::plugins::PluginRegistry::from_discovered(
    ...
    InspectReport {
        grok_version: xai_xvora_version::VERSION.to_string(),
...
fn instruction_file_type(
    ...
    if has_rules_directory(file_path, ".xvora")
...
async fn list_instructions(cwd: &Path) -> Vec<InstructionFile> {
    ...
    let configs = xai_xvora_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd.display().to_string(),
        xai_xvora_agent::prompt::skills::CompatConfig::default(),
    )
    .await;

    let xvora_home = Some(crate::util::xvora_home::xvora_home());
    ...
    // `xai_xvora_agent::prompt::agents_md::read_agents_config_with_paths`
...
async fn list_permissions(cwd: &Path) -> PermissionsReport {
    use xai_xvora_workspace::permission::resolution;
...
fn list_hooks(
    ...
    discovered_plugins: &[xai_xvora_agent::plugins::DiscoveredPlugin],
) -> Vec<HookEntry> {
    let all_on = xai_xvora_tools::types::compat::CompatConfig::default();
    ...
    let (registry, _errors) =
        xai_xvora_hooks::discovery::load_hooks_from_sources(&global_sources, &project_sources);
    ...
    let mut entries: Vec<HookEntry> = registry
        ...
        .into_iter()
        .map(|h| {
            let is_user_scope = h.source_dir.starts_with(&xvora_home)
...
async fn list_skills(
    cwd: &Path,
    plugin_registry: &xai_xvora_agent::plugins::PluginRegistry,
    skills_config: &xai_xvora_agent::prompt::skills::SkillsConfig,
) -> Vec<SkillEntry> {
    // Discover with all vendors ON so inspect shows the full set.
    let skills = xai_xvora_agent::prompt::skills::list_skills_with_plugins(
    ...
    skills
        .into_iter()
        .map(|s| {
            let source = skill_entry_source(&s, &xvora_home);
...
/// `<xvora_home>/skills/<name>/SKILL.md` and discovered as user skills, so a
...
fn skill_entry_source(
    s: &xai_xvora_agent::prompt::skills::SkillInfo,
    xvora_home: &Path,
) -> ConfigSource {
    use xai_xvora_tools::implementations::skills::types::SkillScope;
    ...
    match s.scope {
        ...
        SkillScope::User => {
            if crate::builtin::is_extracted_bundled_skill(&s.name, &path, xvora_home) {
...
fn list_agents(
    ...
    let agents = xai_xvora_agent::discovery::all_subagents_with_plugins(
        cwd,
        &HashMap::new(),
        Some(plugin_registry),
    );
...
fn list_plugins(discovered: &[xai_xvora_agent::plugins::DiscoveredPlugin]) -> Vec<PluginEntry> {
    discovered
        .iter()
        .map(|p| {
            let scope = match p.scope {
                xai_xvora_agent::plugins::PluginScope::CliOverride => Scope::Cli,
                xai_xvora_agent::plugins::PluginScope::Project => Scope::Project,
                xai_xvora_agent::plugins::PluginScope::User => Scope::User,
                xai_xvora_agent::plugins::PluginScope::ConfigPath => Scope::Config,
            };
            PluginEntry {
                ...
                provides: PluginProvides {
                    ...
                    skills: xai_xvora_agent::plugins::registry::skill_md_paths(&p.skill_dirs).len(),
...
fn list_marketplaces(git_root: Option<&Path>) -> Vec<MarketplaceEntry> {
    ...
    xai_xvora_agent::plugins::marketplace::resolve(root)
        .into_iter()
...
fn list_lsp_servers(
    ...
    let servers = xai_xvora_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
        cwd,
        &plugin_lsp_paths,
        &inline_values,
        &plugin_names,
...
/// `.xvora/config.toml` files (via `find_project_configs`). The macOS MDM
...
fn list_config_sources(cwd: &Path) -> ConfigSources {
    ...
    // User managed
    if let Some(home) = crate::config::user_xvora_home() {
...
mod tests {
    use super::*;
    use xai_xvora_agent::prompt::skills::{SkillInfo, SkillsConfig};
    use xai_xvora_tools::implementations::skills::types::SkillScope;
    ...
    fn vendor_rule_paths_select_rules_compatibility_cells() {
        ...
        for path in ["/repo/.xvora/rules/team.md", r"C:\repo\.grok\rules\team.md"] {
    ...
    fn skill_entry_source_maps_scopes() {
        ...
        let s = skill_fixture("e", "/home/u/.xvora/bundled/e/SKILL.md", SkillScope::Bundled);
        assert!(matches!(
            skill_entry_source(&s, home),
    ...
    /// path `<xvora_home>/skills/<name>/SKILL.md`; a same-named skill anywhere
    ...
    fn skill_entry_source_relabels_extracted_bundled_skills() {
        ...
        let s = skill_fixture(
            "help",
            "/home/u/.xvora/skills/help/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        let s = skill_fixture("help", "/repo/.xvora/skills/help/SKILL.md", SkillScope::Repo);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::Project { .. }
        ));

        // Bundled name in a user dir outside <xvora_home>/skills: stays user.
        let s = skill_fixture(
            "help",
        ...
        // Bundled frontmatter name in a different dir under <xvora_home>/skills:
        // not the extracted copy — stays user.
        let s = skill_fixture(
            "help",
            "/home/u/.xvora/skills/my-tools/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        // Non-bundled name under <xvora_home>/skills: stays user.
        let s = skill_fixture(
            "my-skill",
            "/home/u/.xvora/skills/my-skill/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
    ...
    async fn list_skills_honors_skills_config() {
        ...
        let registry = xai_xvora_agent::plugins::PluginRegistry::from_discovered(vec![], &[], &[]);
