...
//!       [--api-key <key> | $XAI_API_KEY | <xvora-home>/auth.json] \
//!       [--min-confidence 0.7] \
...
use clap::Parser;
use xai_xvora_shell::trace_classifier::{RunArgs, run, validate_min_confidence};
...
struct Cli {
    ...
    /// `$XAI_API_KEY`, then `<xvora-home>/auth.json` (`xai::api_key`
    ...
    /// shell uses (`$XVORA_HOME` or `~/.xvora`). Exposed primarily for
    /// tests / sandboxed invocations.
    #[arg(long)]
    xvora_home: Option<PathBuf>,
}
...
mod tests {
    ...
    #[test]
    fn cli_xvora_home_override_parses() {
        ...
        .expect("parse");
        assert_eq!(cli.xvora_home, Some(PathBuf::from("/tmp/scratch-grok")));
    }
