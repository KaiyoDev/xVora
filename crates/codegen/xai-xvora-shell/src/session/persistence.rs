...
pub enum PersistenceMsg {
    ...
    CurrentModel {
        ...
        /// The active agent definition name (e.g. `"xvora"`).
...
pub use xai_xvora_shared::session::session_dir;
...
fn resolve_local_session_any_cwd_in_root(session_id: &str, sessions_root: &Path) -> Option<String> {
    ...
    for entry in entries.flatten() {
        ...
        if is_persisted_session_dir(&session_path) {
            ...
            if let Some(decoded) = crate::util::xvora_home::decode_cwd_from_dirname(&path) {
                return Some(decoded);
            }
        }
    }
...
pub fn xvora_home_string() -> Option<String> {
...
impl Summary {
    pub fn new(info: &Info, model_id: acp::ModelId) -> std::io::Result<Self> {
        let git_metadata =
            xai_xvora_workspace::session::git::resolve_persisted_session_git_metadata_sync(
...
fn collect_mcp_stderr_logs(files: &mut Vec<CopiedSessionFile>) {
    let mcp_log_dir = xai_xvora_config::xvora_home().join("logs").join("mcp");
...
pub fn cleanup_stale_sessions(skip_session_dir: Option<&Path>) {
    CLEANUP_SESSIONS_ONCE.call_once(|| {
        ...
        tracing::info!(
            target: "xai_xvora_shell::session::persistence",
...
mod session_exists_for_cwd_tests {
    ...
    fn session_under_different_cwd_is_not_considered_present() {
        ...
        let encoded_a = crate::util::xvora_home::encode_cwd_dirname("/project/alpha");
    ...
    fn resolve_local_session_any_cwd_skips_stub_and_finds_real() {
        ...
        let encoded_a = crate::util::xvora_home::encode_cwd_dirname(cwd_a);
        let dir_a = root.join(&encoded_a).join(session_id);
        fs::create_dir_all(&dir_a).unwrap();
        fs::write(dir_a.join("summary.json"), b"{}").unwrap();
        ...
        let encoded_b = crate::util::xvora_home::encode_cwd_dirname(cwd_b);
...
mod find_local_child_tests {
    ...
    fn returns_none_when_no_child_exists() {
        let tmp = TempDir::new().unwrap();
        let root = tmp.path().join("sessions");
        let encoded = crate::util::xvora_home::encode_cwd_dirname("/work");
...
mod resolve_local_session_tests {
    ...
    fn exact_match_returns_original_id() {
        ...
        // (We can't call resolve_local_session directly because it uses xvora_home(),
    ...
    fn no_match_returns_none() {
        ...
        fs::create_dir_all(root.join(crate::util::xvora_home::encode_cwd_dirname(cwd))).unwrap();
...
mod repo_wide_resolution_tests {
    ...
    fn skips_images_only_stub_and_resolves_real_sibling() {
        ...
        let encoded = crate::util::xvora_home::encode_cwd_dirname(exact_cwd);
