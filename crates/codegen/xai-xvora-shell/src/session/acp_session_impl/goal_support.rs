...
pub(super) fn try_send_ack(
    ack_tx: Option<
        ...
        >,
    >,
    ack: xai_xvora_tools::implementations::xvora::update_goal::UpdateGoalAck,
) {
...
/// `Some` only when the planner is enabled (`XVORA_GOAL_PLANNER`) and a plan
...
/// Override with `XVORA_GOAL_REVERIFY_AFTER` (floored at 1).
...
pub(crate) fn goal_slash_and_harness_available(goal_enabled: bool, tool_names: &[String]) -> bool {
    use xai_xvora_tools::implementations::xvora::UPDATE_GOAL_TOOL_NAME;
...
impl SessionActor {
    ...
    pub(super) async fn set_goal_loop_active_resource(&self, active: bool) {
        ...
        self.agent
            ...
            .tool_bridge()
            .update_resource(
                xai_xvora_tools::implementations::xvora::task::types::GoalLoopActive(active),
            )
            .await;
    }
}
