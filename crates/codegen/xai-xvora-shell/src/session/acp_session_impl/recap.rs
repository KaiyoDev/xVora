...
impl SessionActor {
    ...
    /// (`XVORA_PROMPT_SUGGESTIONS_MODEL`) > `[models] prompt_suggestion`
    /// (config.toml) > remote `prompt_suggestion_model` (remote settings) >
    /// (config.toml) > remote `prompt_suggestion_model` (remote settings) >
    /// [`prompt_suggest::DEFAULT_SUGGEST_MODEL`] (`xvora-0.1`). Every
    ...
    /// `xvora-0.1` for OAuth users) the request is **skipped
    ...
    /// a reasoning model's response, and some models (e.g. `xvora`)
    ...
    pub(super) async fn handle_suggest_prompt(
        ...
        tracing::debug!(
            raw_preview = %xai_xvora_tools::util::truncate_str(raw.trim(), 60),
