...
impl SessionActor {
    pub(super) fn hook_run_ctx(&self) -> xai_xvora_hooks::runner::RunContext<'_> {
        xai_xvora_hooks::runner::RunContext {
    ...
    pub(super) async fn send_hook_execution(
        ...
        results: &[xai_xvora_hooks::result::HookRunResult],
    ) {
        if results.is_empty()
            || results
                .iter()
                .all(|r| matches!(r, xai_xvora_hooks::result::HookRunResult::Skipped { .. }))
        {
            return;
        }
    ...
    pub(super) async fn dispatch_hook(
        &self,
        event: xai_xvora_hooks::event::HookEventName,
        payload: xai_xvora_hooks::event::HookPayload,
        prompt_id: Option<&str>,
        tool_name: Option<&str>,
    ) {
        ...
        let results =
            xai_xvora_hooks::dispatcher::dispatch_non_blocking(&registry, event, &envelope, &ctx)
                .await;
        ...
        for r in results {
            let (hook_name, elapsed, outcome) = match r {
                xai_xvora_hooks::result::HookRunResult::Success {
                    ...
                    hook_name,
                    elapsed,
                    xai_xvora_telemetry::events::HookOutcome::Success,
                ),
                xai_xvora_hooks::result::HookRunResult::Failed {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                    xai_xvora_telemetry::events::HookOutcome::Error,
                ),
                xai_xvora_hooks::result::HookRunResult::Skipped { .. } => continue,
            };
            xai_xvora_telemetry::session_ctx::log_event(xai_xvora_telemetry::events::HookExecuted {
...
mod notification_hook_filter_tests {
    ...
    fn task_completed_does_not_fire_via_filter() {
        let update = XaiSessionUpdate::TaskCompleted {
            task_snapshot: xai_xvora_tools::types::TaskSnapshot {
                task_id: "task-1".into(),
