...
impl RoleCapability {
    ...
    fn is_satisfied(
        self,
        summary: &xai_xvora_tools::implementations::xvora::task::types::SubagentTypeSummary,
    ) -> bool {
        match self {
...
impl SessionActor {
    ...
    pub(super) async fn drain_goal_updates_with_extra(
        ...
        extra: Vec<xai_xvora_tools::implementations::xvora::update_goal::UpdateGoalEnvelope>,
    ) {
    ...
    pub(crate) async fn resolve_goal_role_override(
        ...
        //    silently run xvora flavor. Non-strict names (xvora
        //    family) run the default flavor and pass; unresolvable names fall
        //    through to the describe `Unknown` arm below.
        if xai_xvora_agent::config::is_strict_harness_agent_type(&pair.agent_type)
    ...
    /// agrees with `from_summary` — `search_replace` on the default xvora
    /// host, not the literal `write`). The `{TOOLSET_TOOLS}` block is empty on
    /// this path (no per-role toolset enumeration); on the default xvora
    ...
    pub(super) async fn prune_prior_goal_continuation_directives(&self) {
        use xai_xvora_sampling_types::conversation::SyntheticReason;
