...
/// - `file_path` — xvora (`search_replace`), opencode (`EditTool`,
///   `WriteTool`, `ReadTool`), codex (`read_file`), xvora_hashline
///   (`hashline_edit`)
/// - `path` — alternate edit/read tools
/// - `target_file` — xvora (`read_file`, via `#[serde(rename)]`)
///
...
impl SessionActor {
    ...
    pub(super) async fn handle_direct_bash_command(
        ...
        let title_command = xai_xvora_tools::util::strip_redundant_session_cd(
            &command,
        ...
        let exec_wire = {
            ...
            agent
                .tool_bridge()
                .toolset()
                .tool_name_for_kind(xai_xvora_tools::types::tool::ToolKind::Execute)
        };
...
// Re-use the UTF-8-safe truncation helper from xai-xvora-sampling-types rather
...
///    [`MAX_ARGS_IN_ERROR`] bytes).  Without this, xvora-shell would sanitize
