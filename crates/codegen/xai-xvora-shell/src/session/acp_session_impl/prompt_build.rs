...
use super::*;
/// Partition the AGENTS.md / Claude.md / `.xvora/rules/*.md` files returned
/// by `read_agents_config_with_paths` into "workspace" (cwd / repo root /
/// extra workspace user dir) and "user" (`~/.xvora/`, `~/.claude/`,
/// `~/.xvora/bundled/`) buckets, mirroring the split between
...
fn partition_rules_by_scope(
    files: Vec<xai_xvora_agent::prompt::agents_md::AgentConfigFile>,
) -> (
    Vec<xai_xvora_agent::prompt::user_message::RuleEntry>,
    Vec<xai_xvora_agent::prompt::user_message::RuleEntry>,
) {
    let home = dirs::home_dir().map(|p| p.to_string_lossy().to_string());
    let user_prefixes: Vec<String> = match home {
        Some(h) => vec![format!("{h}/.xvora/"), format!("{h}/.claude/")],
        None => vec![],
    };
    let mut workspace = Vec::new();
    let mut user = Vec::new();
    for f in files {
        let entry = xai_xvora_agent::prompt::user_message::RuleEntry::from(f);
...
impl SessionActor {
        ...
        if drop_startup_skill_reminder {
            conversation.retain(|item| {
                !matches!(
                    ...
                    Some(xai_xvora_sampling_types::SyntheticReason::SystemReminder)
                )
            });
        }
    }
    ...
    async fn build_templated_user_message(
        &self,
        cwd: &std::path::Path,
        template: xai_xvora_agent::prompt::user_message::UserMessageTemplate,
    ) -> Option<String> {
        use xai_xvora_agent::prompt::agents_md::read_agents_config_with_paths;
        use xai_xvora_agent::prompt::user_message::UserMessageContext;
    ...
    async fn gather_vcs_for_prefix(
        ...
        use xai_xvora_workspace::file_system::{git_status_short, jj_status};
        use xai_xvora_workspace::session::git::VcsKind;
    ...
    ) -> Vec<xai_xvora_agent::prompt::user_message::McpServerEntry> {
        use xai_xvora_agent::prompt::user_message::McpServerEntry;
    ...
    pub(super) async fn transcribe_user_images(
        ...
        let client = xai_xvora_sampler::SamplingClient::new(sampler_config).map_err(|e| {
