        ...
        match item {
            ...
            ConversationItem::User(user) => {
                ...
                for part in &user.content {
                    if let xai_xvora_sampling_types::ContentPart::Text { text: t } = part {
                        if !text.is_empty() {
                            text.push(' ');
                        }
                        text.push_str(t);
            ...
            ConversationItem::Reasoning(r) => {
                if include_reasoning {
                    let text = xai_xvora_sampling_types::reasoning_item_text(r);
...
const CLASSIFIER_TURN_MAX_LEN: usize = xai_xvora_workspace::permission::CLASSIFIER_TURN_MAX_LEN;
...
pub(crate) fn build_classifier_turns(
    items: &[ConversationItem],
    max_items: usize,
) -> Vec<xai_xvora_workspace::permission::ClassifierTurn> {
        ...
        match item {
            ConversationItem::User(user) => {
                ...
                if !text.is_empty() {
                    ...
                    let text = xai_xvora_tools::util::truncate_str_with_marker(
                        &text,
            ...
            ConversationItem::Assistant(assistant) => {
                for tc in &assistant.tool_calls {
                    ...
                    let args = xai_xvora_tools::util::truncate_str_with_marker(
                        &args,
                        CLASSIFIER_TURN_MAX_LEN,
                    )
                    .into_owned();
