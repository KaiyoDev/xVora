...
pub fn conversations_lane_enabled() -> bool {
    ...
    std::env::var("XVORA_SESSION_LIST_CONVERSATIONS")
        .ok()
        .is_some_and(|v| {
            !matches!(
...
/// Env lane (desktop `XVORA_SESSION_LIST_CONVERSATIONS`) OR process-wide
/// `--chat` (`XVORA_CHAT_MODE`); hard-off in release builds.
...
mod tests {
    ...
    async fn forced_kind_serves_conversations_only() {
        ...
        .await;
        let _env = xai_xvora_test_support::EnvGuard::set(
    ...
    fn conversations_lane_env_gating_matrix() {
        {
            let _off = xai_xvora_test_support::EnvGuard::unset("XVORA_SESSION_LIST_CONVERSATIONS");
            assert!(!conversations_lane_enabled());
        }
        {
            let _on = xai_xvora_test_support::EnvGuard::set("XVORA_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_enabled(), false);
        }
        {
            let _off = xai_xvora_test_support::EnvGuard::set("XVORA_SESSION_LIST_CONVERSATIONS", "0");
            assert!(!conversations_lane_enabled());
        }
    }
    ...
    fn conversations_lane_active_truth_table() {
        ...
        let _chat_off = xai_xvora_test_support::EnvGuard::unset(XVORA_CHAT_MODE_ENV);
        let _desktop_off =
            xai_xvora_test_support::EnvGuard::unset("XVORA_SESSION_LIST_CONVERSATIONS");
        assert!(
        ...
        {
            let _desktop =
                xai_xvora_test_support::EnvGuard::set("XVORA_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_active(), false);
        }
        {
            let _chat = xai_xvora_test_support::EnvGuard::set(XVORA_CHAT_MODE_ENV, "1");
            assert_eq!(
                conversations_lane_active(),
                false,
    ...
    fn parse_list_req_forces_kind_under_process_chat_mode_only() {
        ...
        .to_string();
        {
            let _off = xai_xvora_test_support::EnvGuard::unset(XVORA_CHAT_MODE_ENV);
        ...
        {
            let _on = xai_xvora_test_support::EnvGuard::set(XVORA_CHAT_MODE_ENV, "1");
            let req = parse_list_req(&raw).expect("parse");
            let parsed = ParsedMeta::parse(req.meta.as_ref());
