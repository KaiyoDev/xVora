...
//!
//! Tests mutate `XVORA_GOAL_CLASSIFIER` so they carry `serial`.
...
use xai_xvora_tools::implementations::xvora::update_goal::UpdateGoalInput;
...
async fn answer_summarizer(
    ...
    req: Box<xai_xvora_tools::implementations::xvora::task::types::SubagentRequest>,
) {
    match behaviour {
...
fn configure_goal_actor(
    ...
    actor.tool_context.cwd =
        xai_xvora_paths::AbsPathBuf::new(tmp.path().to_path_buf()).expect("abs cwd");
