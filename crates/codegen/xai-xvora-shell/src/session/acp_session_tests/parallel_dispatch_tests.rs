...
//!
//! These tests verify the parallel dispatch path (XVORA_PARALLEL_TOOL_DISPATCH):
...
/// JSON key. The compat toolset input types use `path`, and xvora's
...
#[test]
fn lock_path_for_args_matches_xvora_file_path() {
    // xvora search_replace / opencode EditTool / WriteTool / etc.
...
#[test]
fn lock_path_for_args_matches_xvora_target_file() {
    // xvora read_file uses #[serde(rename = "target_file")].
    let args = serde_json::json!({
...
#[test]
fn lock_path_for_args_buckets_xvora_and_compat_to_same_lock_for_same_file() {
    // A mixed batch (e.g. xvora search_replace + StrReplace
...
fn test_skill_discovery_deferred_during_parallel_batch() {
    use xai_xvora_sampling_types::{ConversationItem, SyntheticReason};
