...
/// attribution.
fn auth_error() -> xai_xvora_sampler::SamplingErrorInfo {
    xai_xvora_sampler::SamplingErrorInfo {
        kind: xai_xvora_sampler::SamplingErrorKind::Auth,
...
fn model_not_found_error() -> xai_xvora_sampler::SamplingErrorInfo {
    xai_xvora_sampler::SamplingErrorInfo {
            kind: xai_xvora_sampler::SamplingErrorKind::Api,
            message: "API error (status 404 Not Found): The model xvora does not exist or your team does not have access".into(),
...
fn unauthorized_401_error() -> xai_xvora_sampler::SamplingErrorInfo {
    xai_xvora_sampler::SamplingErrorInfo {
            kind: xai_xvora_sampler::SamplingErrorKind::Api,
            message: "Unauthorized (401) from https://cli-chat-proxy.grok.com/v1/responses: {\"error\":\"Invalid or expired credentials (auth_kind=bearer, x_xai_token_auth=xai-xvora-cli, upstream=Unauthenticated, reason=no auth context)\"}".into(),
            status_code: Some(401),
            is_retryable: false,
            retry_after_secs: None,
            model_metadata: None,
