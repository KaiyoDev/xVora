...
#[cfg(test)]
pub(crate) async fn test_agent_default() -> xai_xvora_agent::Agent {
...
#[cfg(test)]
pub(crate) async fn test_agent_with_goal_tool() -> xai_xvora_agent::Agent {
    use xai_xvora_tools::implementations::xvora::update_goal::UpdateGoalTool;
...
#[cfg(test)]
pub(crate) async fn test_xvora_agent_with_todo() -> xai_xvora_agent::Agent {
    use xai_xvora_tools::implementations::xvora::todo::TodoWriteTool;
    use xai_xvora_tools::registry::types::ToolConfig;
...
#[cfg(test)]
pub(crate) async fn test_agent_with_plan_tools() -> xai_xvora_agent::Agent {
    use xai_xvora_tools::implementations::xvora::enter_plan_mode::EnterPlanModeTool;
    use xai_xvora_tools::implementations::xvora::exit_plan_mode::ExitPlanModeTool;
    use xai_xvora_tools::registry::types::ToolConfig;
...
pub(crate) async fn test_agent_with_tools(
    tools: Vec<xai_xvora_tools::registry::types::ToolConfig>,
) -> xai_xvora_agent::Agent {
    test_agent_from_config(
        xai_xvora_tools::registry::types::ToolServerConfig {
            tools,
            behavior_preset: None,
        },
        xai_xvora_agent::AgentDefinition::default_xvora(),
        std::sync::Arc::new(xai_xvora_tools::computer::local::LocalTerminalBackend::new()),
    )
    .await
}
#[cfg(test)]
async fn test_agent_from_config(
    ...
    definition: xai_xvora_agent::AgentDefinition,
    backend: std::sync::Arc<dyn xai_xvora_tools::computer::types::TerminalBackend>,
) -> xai_xvora_agent::Agent {
    use xai_xvora_tools::computer::local::LocalFs;
    use xai_xvora_tools::computer::types::AsyncFileSystem;
    ...
    xai_xvora_agent::Agent::new(
        definition,
        xai_xvora_agent::PromptContext::default(),
        String::new(),
        tool_bridge,
        xai_xvora_agent::ReminderPolicy::default(),
        xai_xvora_agent::CompactionPolicy::default(),
        vec![],
        false,
    )
}
...
pub(crate) async fn create_test_actor_ex(
    ...
    let cwd = xai_xvora_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap();
    let fs = Arc::new(xai_xvora_workspace::file_system::MockFs::new(
        cwd.to_path_buf(),
    ));
    ...
    let actor = SessionActor {
        ...
        permissions: xai_xvora_workspace::permission::PermissionHandle::allow_all(),
        tool_context,
