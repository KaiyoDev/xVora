...
pub(crate) fn new_submission(
    ...
    s.shell_version = Some(xai_xvora_version::VERSION.to_string());
    s
}

/// Pipeline: persist → strip → submit → telemetry (telemetry fields captured
/// before strip). Callers merge `XVORA_USER_METADATA` and set
...
pub(crate) async fn submit_feedback_workflow(
    ...
    if telemetry_enabled {
        ...
        xai_xvora_telemetry::session_ctx::log_event(xai_xvora_telemetry::events::UserFeedback {
...
pub struct FeedbackManagerConfig {
    ...
    /// ratings). Gated by `XVORA_FEEDBACK_ENABLED`.
    pub feedback_enabled: bool,
    /// Whether session analytics (signal sync, turn deltas) are enabled.
    /// Gated by `XVORA_TELEMETRY_ENABLED`. These are analytics data that
...
impl FeedbackManager {
    ...
    pub(crate) async fn submit_text_feedback(
        ...
        if let Some(user_meta) =
            crate::agent::mvp_agent::parse_json_object_env("XVORA_USER_METADATA")
        {
