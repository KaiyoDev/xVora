...
pub enum SessionCommand {
    ...
    ReloadPlugins {
        registry: Option<std::sync::Arc<xai_xvora_agent::plugins::PluginRegistry>>,
    },
    /// Re-discover the session's own project hooks (`.xvora/hooks`,
    ...
    SnapshotToolDefinitions {
        respond_to: oneshot::Sender<Vec<xai_xvora_sampling_types::ToolSpec>>,
    },
    ...
    KillBackgroundTask {
        task_id: String,
        respond_to: oneshot::Sender<Result<xai_xvora_tools::types::KillOutcome, String>>,
    },
    DeleteScheduledTask {
        task_id: String,
    ...
    ListTasks {
        respond_to: oneshot::Sender<Option<Vec<xai_xvora_tools::types::TaskSnapshot>>>,
    },
    ...
    PluginsList {
        respond_to:
            oneshot::Sender<Option<std::sync::Arc<xai_xvora_agent::plugins::PluginRegistry>>>,
    },
    ...
    /// tool-free model call (default `xvora-0.1` when available via
    ...
    /// `xvora` model, low temperature, and capped output tokens.
    RewriteMemoryNote {
        raw_text: String,
    ...
    TakeHarnessTraceTurns {
        respond_to:
            oneshot::Sender<Vec<Vec<xai_xvora_sampling_types::conversation::ConversationItem>>>,
    },
