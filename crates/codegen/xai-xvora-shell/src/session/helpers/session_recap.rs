...
/// `max_prompt_length` for current `xvora` / `grok-4.5` product backends
...
/// that unlikely for normal xvora sessions).
///
...
mod tests {
    ...
    fn main_turn_count_counts_real_users_only() {
        use std::sync::Arc;
        use xai_xvora_sampling_types::{ContentPart, SyntheticReason, ToolCall, UserItem};
    ...
    fn mk_tool_call(id: &str, args: &str) -> xai_xvora_sampling_types::ToolCall {
        use std::sync::Arc;
