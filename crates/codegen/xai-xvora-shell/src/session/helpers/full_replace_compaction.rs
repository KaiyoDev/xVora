//! xvora's L5 wiring onto the shared full-replace engine
//! (`xai_xvora_compaction::code_compaction`).
//!
//! The shared engine drives the sample → retry → degenerate/failure
//! classification loop via [`sample_full_replace_summary`](xai_xvora_compaction::sample_full_replace_summary);
//! this module adapts xvora's transport and telemetry to its two seams:
//!
...
//! [`FullReplaceError`](xai_xvora_compaction::FullReplaceError).
...
use xai_xvora_sampling_types::{ConversationItem, HostedTool, ToolSpec};
use xai_xvora_telemetry::events::{CompactionRetryDegraded, CompactionTrigger};
...
/// [`CompactionSampler`] for xvora's full-replace pass.
///
...
/// else the structured xvora prompt), so the shared `CompactionPrompt`
/// the engine passes is ignored — the engine builds the xvora prompt,
...
impl CompactionSampler for ShellCompactionSampler {
    ...
    async fn sample_compaction(
        ...
        // message (compat short vs structured xvora), ignoring the shared
...
/// Map xvora's [`CompactFailure`] onto the shared engine's
...
/// [`FullReplaceObserver`] that reproduces xvora's per-attempt telemetry:
    ...
    fn on_attempt(&self, _attempt: u32, outcome: &FullReplaceAttemptOutcome<'_>) {
        ...
        match outcome {
            ...
            FullReplaceAttemptOutcome::Degenerate {
                ...
                if *will_retry {
                    xai_xvora_telemetry::session_ctx::log_event(CompactionRetryDegraded {
