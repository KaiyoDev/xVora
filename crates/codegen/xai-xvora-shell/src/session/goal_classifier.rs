...
/// `XVORA_GOAL_CLASSIFIER_MAX` or remote `goal_classifier_max_runs` to
/// raise it arbitrarily (only the `GOAL_CLASSIFIER_MAX_RUNS_MIN` floor
/// is enforced).
pub(crate) const GOAL_CLASSIFIER_MAX_RUNS_DEFAULT: u32 = 10;

/// Floor for `XVORA_GOAL_CLASSIFIER_MAX` / remote `goal_classifier_max_runs`.
...
/// attempt. Override via `XVORA_GOAL_VERIFIER_N` (clamped 1..=5) or the
...
/// Lower/upper bounds for `XVORA_GOAL_VERIFIER_N` / remote
...
pub(crate) fn build_subagent_trace_items(
    ...
    prompt: &str,
    output: &str,
) -> Vec<xai_xvora_sampling_types::conversation::ConversationItem> {
    use xai_xvora_sampling_types::conversation::{ConversationItem, ToolCall};
...
mod tests {
    ...
    fn verifier_template_renders_per_agent_type_and_falls_back() {
        use xai_xvora_tools::implementations::xvora::task::types::SubagentTypeSummary;
        let mut tool_names = std::collections::HashMap::new();
        tool_names.insert(
            xai_xvora_tools::types::tool::ToolKind::Read,
            "cursor_read".to_string(),
        );
        tool_names.insert(
            xai_xvora_tools::types::tool::ToolKind::ListDir,
            "cursor_ls".to_string(),
        );
        tool_names.insert(
            xai_xvora_tools::types::tool::ToolKind::Search,
        ...
        // xvora explicit render: no leftover placeholder either.
        let grok = RoleToolNames::from_summary(&summary_with(&[
            (xai_xvora_tools::types::tool::ToolKind::Read, "read_file"),
            (xai_xvora_tools::types::tool::ToolKind::ListDir, "list_dir"),
            (xai_xvora_tools::types::tool::ToolKind::Search, "grep"),
        ]))
    ...
    fn cold_and_resume_renders_are_symmetric_for_an_index() {
        let summary = summary_with(&[
            (xai_xvora_tools::types::tool::ToolKind::Read, "cursor_read"),
            (xai_xvora_tools::types::tool::ToolKind::ListDir, "cursor_ls"),
            (xai_xvora_tools::types::tool::ToolKind::Search, "cursor_grep"),
        ]);
    ...
    fn resolve_goal_verifier_count_env_clamps() {
        unsafe { std::env::set_var("XVORA_GOAL_VERIFIER_N", "0") };
        assert_eq!(
            cfg_verifier(None, None).resolve_goal_verifier_count().value,
            GOAL_VERIFIER_SKEPTIC_MIN
        );
        unsafe { std::env::set_var("XVORA_GOAL_VERIFIER_N", "99") };
        assert_eq!(
            cfg_verifier(None, None).resolve_goal_verifier_count().value,
            GOAL_VERIFIER_SKEPTIC_MAX
        );
        unsafe { std::env::set_var("XVORA_GOAL_VERIFIER_N", "garbage") };
        assert_eq!(
            cfg_verifier(None, None).resolve_goal_verifier_count().value,
            GOAL_VERIFIER_SKEPTIC_COUNT,
            "invalid env falls through to the default"
        );
        unsafe { std::env::remove_var("XVORA_GOAL_VERIFIER_N") };
    }
    ...
    fn resolve_goal_verifier_count_precedence_and_clamp() {
        ...
        // env > config.
        unsafe { std::env::set_var("XVORA_GOAL_VERIFIER_N", "2") };
    ...
    fn resolve_goal_classifier_max_runs_env_clamps_and_no_ceiling() {
        unsafe { std::env::set_var("XVORA_GOAL_CLASSIFIER_MAX", "0") };
        assert_eq!(
        ...
        unsafe { std::env::set_var("XVORA_GOAL_CLASSIFIER_MAX", "999") };
        assert_eq!(
            cfg_max_runs(None, None)
                .resolve_goal_classifier_max_runs()
                .value,
        ...
        unsafe { std::env::set_var("XVORA_GOAL_CLASSIFIER_MAX", "garbage") };
        assert_eq!(
            cfg_max_runs(None, None)
                .resolve_goal_classifier_max_runs()
                .value,
        ...
        unsafe { std::env::remove_var("XVORA_GOAL_CLASSIFIER_MAX") };
    }
    ...
    fn resolve_goal_classifier_max_runs_precedence_and_floor() {
        ...
        // env > config.
        unsafe { std::env::set_var("XVORA_GOAL_CLASSIFIER_MAX", "4") };
    ...
    fn resolve_strategist_every_default_tracks_cap_floored_at_one() {
        unsafe { std::env::remove_var("XVORA_GOAL_STRATEGIST_EVERY") };
    ...
    fn resolve_strategist_every_precedence_and_floor() {
        ...
        // env > config + remote.
        unsafe { std::env::set_var("XVORA_GOAL_STRATEGIST_EVERY", "7") };
        let r = cfg_strategist(Some(3), Some(4)).resolve_goal_strategist_every(10);
        assert_eq!(r.value, 7);
        assert_eq!(r.source, ConfigSource::Env);
        // invalid env falls through to the default (cap/2).
        unsafe { std::env::set_var("XVORA_GOAL_STRATEGIST_EVERY", "not-a-number") };
        assert_eq!(
            cfg_strategist(None, None)
                .resolve_goal_strategist_every(10)
                .value,
