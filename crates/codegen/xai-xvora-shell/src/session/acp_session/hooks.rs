...
//! both matched by the agent ([`xai_xvora_hooks::matcher::HookMatcher`], shared with
//! file hooks):
...
use xai_xvora_hooks::event::{HookEventEnvelope, HookEventName, HookPayload};
use xai_xvora_telemetry::events::ClientHookGateOutcome;
...
impl SessionActor {
    ...
    pub(super) async fn deny_tool(
        ...
        xai_xvora_telemetry::session_ctx::log_event(xai_xvora_telemetry::events::HookBlocked {
    ...
    pub(super) async fn run_pre_tool_use_client_hook(
        ...
        let tool_name = xai_xvora_hooks::dispatcher::extract_tool_name(envelope)
            ...
            .map(|(callback_id, timeout)| {
                ...
                async move {
                    ...
                    xai_xvora_telemetry::session_ctx::log_event(
                        xai_xvora_telemetry::events::ClientHookGate {
...
mod tests {
    ...
    async fn hook_event_active_inert_vs_active() {
        use xai_xvora_hooks::event::HookEventName;
        ...
        local
            .run_until(async {
                ...
                *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(
                    xai_xvora_hooks::discovery::HookRegistry::default(),
                ));
                assert!(actor.hook_event_active(HookEventName::Stop));
    ...
    fn matching_callback_ids_filters_by_matcher() {
        use xai_xvora_hooks::matcher::HookMatcher;
