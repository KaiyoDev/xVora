...
//!   under `~/.xvora/worktree_pool/<instance_id>/` with a `.pid` liveness
...
/// `~/.xvora/worktree_pool/<instance_id>/` with a `.pid` file recording
...
impl WorktreePool {
    ...
    /// `~/.xvora/worktree_pool/<instance_id>/`. A `.pid` file is written so
...
pub struct AdoptableWorktree {
    /// Current path: `~/.xvora/worktree_pool/<old_instance>/<pool_id>/`
...
/// `CleanAll` mode. Placed under `~/.xvora/worktree_pool/<instance_id>/<uuid>/`.
