//! ACP conversion functions for `xai-xvora-tools`'s `ToolOutput`.
//!
//! These standalone functions convert `xai_xvora_tools::types::output::ToolOutput`
...
pub fn acp_tool_update(
    ...
    match output {
        ToolOutput::ReadFile(read_file_output) => {
            let (content, status) = match read_file_output {
                ...
                ReadFileOutput::ImageContent(image_content) => {
                    ...
                    // `xai-xvora-tools` stay free of an
        ...
        ToolOutput::WebFetch(web_fetch_output) => {
            use xai_xvora_tools::types::output::WebFetchOutput;
        ...
        ToolOutput::Todo(todo_output) => {
            use xai_xvora_tools::types::output::TodoWriteOutput;
        ...
        ToolOutput::AskUserQuestion(ask) => {
            let message = match ask {
                xai_xvora_tools::types::output::AskUserQuestionOutput::UserAnswered { message } => {
                    message.clone()
                }
                xai_xvora_tools::types::output::AskUserQuestionOutput::QuestionsSent {
                    message,
                    ..
                } => message.clone(),
            };
        ...
        ToolOutput::EnterPlanMode(enter) => {
            let message = match enter {
                xai_xvora_tools::types::output::EnterPlanModeOutput::Entered { message, .. } => {
                    message.clone()
                }
            };
            Some(acp::ToolCallUpdate::new(
        ...
        ToolOutput::ExitPlanMode(exit) => {
            let message = match exit {
                xai_xvora_tools::types::output::ExitPlanModeOutput::PlanReady {
                    message, ..
                } => message.clone(),
                xai_xvora_tools::types::output::ExitPlanModeOutput::EmptyPlan {
                    message, ..
                } => message.clone(),
            };
            Some(acp::ToolCallUpdate::new(
...
///
/// This converts `xai-xvora-tools`' TodoItem (which has `id`, `content: Option<String>`,
...
mod tests {
    ...
    fn test_acp_tool_update_todo_returns_completed() {
        let output = ToolOutput::Todo(TodoWriteOutput::TodosUpdated(TodoWriteSuccess {
            ...
            todos: vec![],
            state: xai_xvora_tools::implementations::xvora::todo::TodoState::default(),
        }));
        let update = acp_tool_update(&output, "call-1", None, None).unwrap();
        assert_eq!(update.fields.status, Some(acp::ToolCallStatus::Completed));
    }
    ...
    fn test_turn_end_plan_cleanup_preserves_semantics_and_priority() {
        use crate::tools::todo::plan_entry_from_todo_item;
        use xai_xvora_tools::implementations::xvora::todo::{
    ...
    fn test_acp_plan_update_todo() {
        let output = ToolOutput::Todo(TodoWriteOutput::TodosUpdated(TodoWriteSuccess {
            summary_for_prompt: "tasks".to_string(),
            todos: vec![
                xai_xvora_tools::implementations::xvora::todo::TodoItem {
                    content: "Task 1".to_string(),
                    priority:
                        xai_xvora_tools::implementations::xvora::todo::TodoPriority::Medium,
                    status:
                        xai_xvora_tools::implementations::xvora::todo::TodoStatus::Completed,
                    meta: None,
                },
            ],
            state: xai_xvora_tools::implementations::xvora::todo::TodoState::default(),
    ...
    fn test_rewrite_handles_url_encoded_paths() {
        ...
        let encoded_overlay = urlencoding::encode("/root/.xvora/worktrees/project/ab-123-a-overlay");
        let input = format!(
