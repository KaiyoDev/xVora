...
//! verdict is [`xai_xvora_workspace::folder_trust::TrustOutcome::Prompt`]. No
...
//! `.xvora/lsp.json` servers start on the NEXT session open (the durable grant
...
impl MvpAgent {
    ...
    pub(crate) fn maybe_spawn_interactive_trust_prompt(
        ...
        let key = xai_xvora_workspace::trust::workspace_key(cwd);
        ...
        let targets: Vec<ReloadTarget> = self
            ...
            .values()
            .filter(|h| {
                xai_xvora_workspace::trust::workspace_key(std::path::Path::new(&h.info.cwd)) == key
            })
            .map(|h| ReloadTarget {
...
struct ReloadAfterGrant<'a> {
    ...
    plugin_handle: &'a xai_xvora_agent::plugins::SharedPluginRegistryHandle,
    ...
    proxy_url: &'a str,
    compat: &'a xai_xvora_tools::types::CompatConfig,
...
async fn reload_project_servers_after_grant(ctx: ReloadAfterGrant<'_>) {
    ...
    for target in ctx.targets {
        ...
        // The session's OWN project hooks (`.xvora/hooks`, `.cursor/hooks.json`),
