...
use xai_xvora_telemetry::id::{agent_id, agent_instance_id};
use xai_xvora_telemetry::session_ctx::log_event;
use xai_xvora_workspace::file_system::{AcpSessionFs, CodebaseIndexManager, LocalFs};
use xai_xvora_workspace::permission::{ClientType, PermissionEvent};
...
use xai_xvora_workspace::session::git::GitDiscoveryResult;
...
pub(crate) struct PromptResponseMetaArgs<'a> {
    ...
    pub total_tokens: u64,
    pub model_id: &'a str,
    pub last_turn_usage: Option<&'a xai_xvora_sampling_types::TokenUsage>,
...
fn announcements_push_payload(
    stored: Option<&[xai_xvora_announcements::RemoteAnnouncement]>,
    last_emitted: &[xai_xvora_announcements::RemoteAnnouncement],
    now: chrono::DateTime<chrono::Utc>,
    mode: AnnouncementsPushMode,
) -> Option<Vec<xai_xvora_announcements::RemoteAnnouncement>> {
    let current = xai_xvora_announcements::filter_expired_at(
...
/// Override with `XVORA_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn announcements_refresh_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("XVORA_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS")
...
pub struct MvpAgent {
    ...
    /// **non-recursive** watch on `<cwd>/` and `<cwd>/.xvora/`).
    ///
    ...
    subagent_event_tx: tokio::sync::mpsc::UnboundedSender<
        xai_xvora_tools::implementations::xvora::task::types::SubagentEvent,
    >,
    ...
    monitor_event_buffer: xai_xvora_tools::implementations::xvora::task::types::MonitorEventBuffer,
    ...
    subagent_roles: std::collections::HashMap<
        String,
        xai_xvora_subagent_resolution::config::SubagentRole,
    >,
    subagent_personas: std::collections::HashMap<
        String,
        xai_xvora_subagent_resolution::config::SubagentPersona,
    >,
    ...
    pub(crate) plugin_registry_handle: xai_xvora_agent::plugins::SharedPluginRegistryHandle,
    ...
    workspace_ops: RefCell<Option<xai_xvora_workspace::WorkspaceOps>>,
    ...
    last_emitted_announcements: RefCell<Vec<xai_xvora_announcements::RemoteAnnouncement>>,
...
pub(crate) fn inherited_harness_template(
    current: &xai_xvora_agent::prompt::user_message::UserMessageTemplate,
    pinned_model_agent_type: Option<&str>,
    cwd: &std::path::Path,
) -> Option<xai_xvora_agent::prompt::user_message::UserMessageTemplate> {
    ...
    if !xai_xvora_agent::config::is_strict_harness_agent_type(agent_type) {
        return None;
    }
    let harness = xai_xvora_agent::discovery::by_name_in_cwd(agent_type, cwd)?;
...
/// unchanged — compatible stock switches (e.g. `xvora` →
/// `xvora-plan`) intentionally preserve the session's original ACP
/// `agentProfile`.
...
/// default wire format and toolset, so switching e.g. `xvora` →
/// `xvora-plan` doesn't require rebuilding the harness and would
...
pub(crate) fn harnesses_are_compatible(active: &str, required: &str) -> bool {
    use xai_xvora_agent::config::is_strict_harness_agent_type;
    match (
...
impl MvpAgent {
    ...
    pub(super) fn reconcile_stale_background_tasks(
        ...
        for task in &orphaned {
            let snapshot = xai_xvora_tools::types::TaskSnapshot {
    ...
    pub(super) async fn enforce_grok_code_access(&self, auth: &crate::auth::GrokAuth) {
        ...
        if !allow {
            tracing::info!(
                "auth: user blocked by allow_access (remote settings xvora_access_gate)"
            );
            ...
            .map(|auth| {
                let gate = if !self.tier_allowed.get() && gate.is_none() {
                    ...
                    Some(crate::auth::GateInfo {
                        message,
                        url: Some(
                            "https://grok.com/supergrok?referrer=xvora".to_string(),
                        ),
...
/// absent — matching the `xvora_access_gate` flag's server-side default.
///
