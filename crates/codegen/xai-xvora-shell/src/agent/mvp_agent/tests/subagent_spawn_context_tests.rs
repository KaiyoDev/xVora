...
async fn subagent_spawn_context_inherits_parent_permission_handle() {
    use xai_xvora_workspace::permission::types::{
    ...
    local
        .run_until(async {
            ...
            let cwd = xai_xvora_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp"))
                .expect("absolute cwd");
            let (permission_handle, _events_rx) =
                xai_xvora_workspace::permission::spawn_permission_manager(
                    sid.clone(),
                    gateway,
                    cwd,
                    xai_xvora_workspace::permission::types::ClientType::Generic,
            ...
            for access in [
                xai_xvora_workspace::permission::AccessKind::Read(Some(".env".into())),
                xai_xvora_workspace::permission::AccessKind::Bash("cat .env".into()),
            ] {
                ...
                assert!(
                    matches!(
                        decision,
                        xai_xvora_workspace::permission::Decision::PolicyDeny(_)
                    ),
