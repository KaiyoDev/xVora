...
fn harness_pair(id: &str) -> Vec<xai_xvora_sampling_types::conversation::ConversationItem> {
    use xai_xvora_sampling_types::ToolCall;
...
#[test]
#[serial_test::serial]
fn resolve_agent_definition_defaults_to_xvora() {
    let prev = std::env::var("XVORA_AGENT").ok();
    unsafe {
        std::env::remove_var("XVORA_AGENT");
    }
    ...
    if let Some(v) = prev {
        unsafe { std::env::set_var("XVORA_AGENT", v) }
    }
}
/// When model_agent_type = Some("codex"), the codex agent is selected even
/// though the default chain would return xvora.
#[test]
#[serial_test::serial]
...
fn resolve_agent_definition_acp_profile_wins_when_model_agent_type_is_default() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let acp_profile = xai_xvora_agent::AgentDefinition::from_json(&serde_json::json!(
...
/// `xvora-plan`, models in the catalog that still declare
/// `agent_type = "xvora"` explicitly must NOT preempt an ACP
/// profile. Any value in the `xvora*` family is the stock harness
/// with no strict requirement.
#[test]
#[serial_test::serial]
fn resolve_agent_definition_acp_profile_wins_for_explicit_xvora_family() {
    let prev = std::env::var("XVORA_AGENT").ok();
    unsafe {
        std::env::remove_var("XVORA_AGENT");
    }
    ...
    .expect("agent definition must parse");
    for family_variant in ["xvora", "xvora-plan", "xvora-concise"] {
        let def = MvpAgent::resolve_agent_definition(
            tmp.path(),
            None,
            &config::AgentSelectionConfig::default(),
        ...
        assert_eq!(
            def.name, "custom-devbox-profile",
            "ACP profile must win for xvora family variant `{family_variant}`"
        );
    }
    if let Some(v) = prev {
        unsafe { std::env::set_var("XVORA_AGENT", v) }
...
fn inherited_harness_template_skips_nonstrict_model() {
    use xai_xvora_agent::prompt::user_message::UserMessageTemplate;
    let tmp = tempfile::tempdir().unwrap();
    assert!(
...
fn resolve_agent_definition_agent_profile_with_model_override() {
    ...
    assert_eq!(
        def.model,
        xai_xvora_agent::config::ModelOverride::Override("test-model-123".to_string()),
        "agent profile model override must be preserved through resolution"
    );
    match prev {
        Some(v) => unsafe { std::env::set_var("XVORA_AGENT", v) },
        None => unsafe { std::env::remove_var("XVORA_AGENT") },
    }
}
#[test]
...
/// flow: a zero-turn switch from `xvora` (a client profile name) to
/// `xvora-plan` (the default model agent_type) must be
...
fn harnesses_are_compatible_for_stock_family_pairs() {
    assert!(harnesses_are_compatible("xvora", "xvora-plan"));
    assert!(harnesses_are_compatible("xvora-plan", "xvora"));
    assert!(harnesses_are_compatible("xvora", "xvora"));
...
fn harnesses_are_compatible_rejects_strict_mismatches() {
    ...
    assert!(!harnesses_are_compatible("xvora-plan", "codex"));
}
#[test]
fn explicit_agent_type_wins_over_session_default() {
    assert_eq!(
        resolve_required_agent_type(Some("cursor"), "xvora-plan"),
        "cursor"
    );
}
#[test]
fn null_agent_type_falls_back_to_session_default_xvora_plan() {
    assert_eq!(
        resolve_required_agent_type(None, "xvora-plan"),
        "xvora-plan"
    );
}
#[test]
fn null_agent_type_falls_back_to_session_default_xvora() {
    assert_eq!(
        resolve_required_agent_type(None, "xvora"),
        "xvora"
    );
}
#[test]
fn null_agent_type_returns_to_session_default_after_cursor_switch() {
    let session_default = "xvora-plan";
    let required_after_null = resolve_required_agent_type(None, session_default);
    assert_eq!(required_after_null, "xvora-plan");
...
fn agent_name_unchanged_without_harness_rebuild() {
    let unchanged = agent_name_after_model_switch(false, "xvora-plan", "remote-sidebar");
    assert_eq!(
...
async fn file_toolset_override_e2e_to_finalized_toolset() {
    ...
    use xai_xvora_tools::computer::local::{LocalFs, LocalTerminalBackend};
    ...
    let builder = xai_xvora_tools::registry::types::ToolRegistryBuilder::new();
    let ctx = SessionContext {
        ...
        memory_backend: None,
        web_search_config: xai_xvora_tools::implementations::web_search::WebSearchConfig::default(),
        web_fetch_config: Default::default(),
        lsp: None,
        image_gen_config: xai_xvora_tools::implementations::xvora::image_gen::ImageGenConfig::default(),
        video_gen_config: xai_xvora_tools::implementations::xvora::video_gen::VideoGenConfig::default(),
        app_builder_deployer_config: xai_xvora_tools::implementations::xvora::deploy_app::AppBuilderDeployerConfig::default(),
...
fn make_test_handle(
    ...
    crate::session::SessionHandle {
        ...
        tool_context: crate::tools::ToolContext::new_local_context(
            xai_xvora_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap(),
            std::sync::Arc::new(xai_xvora_workspace::file_system::LocalFs::new(
                std::path::PathBuf::from("/tmp"),
            )),
        ...
        permission_handle: xai_xvora_workspace::permission::PermissionHandle::allow_all(),
        attribution_callback: None,
        agent_name: "xvora".to_string(),
        managed_mcp_proxy_base_url: String::new(),
        session_default_agent_profile: None,
        allowed_subagent_types: None,
        hook_registry: None,
        workspace_ops: xai_xvora_workspace::WorkspaceOps::for_test(),
...
async fn model_state_prefers_session_reasoning_effort_over_global() {
    ...
    use xai_xvora_sampling_types::{REASONING_EFFORT_META_KEY, ReasoningEffort};
    let agent = build_minimal_agent_for_tests();
...
async fn session_config_options_resolves_routing_slug_to_catalog_model() {
    use crate::agent::config::{EndpointsConfig, ModelEntry};
    use xai_xvora_sampling_types::ReasoningEffort;
    let agent = build_minimal_agent_for_tests();
...
async fn auth_type_session_based_no_current_returns_session_token() {
    for method_id in [
        ...
        crate::agent::auth_method::XVORA_COM_METHOD_ID,
...
async fn cached_token_fallthrough_prefers_api_key_for_deployment_key() {
    ...
    use xai_xvora_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("XVORA_DISABLE_API_KEY_AUTH");
...
async fn cached_token_fallthrough_respects_kill_switch() {
    use crate::agent::auth_method::{XVORA_COM_METHOD_ID, XAI_API_KEY_ENV_VAR};
    use xai_xvora_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("XVORA_DISABLE_API_KEY_AUTH");
    let _key = EnvGuard::set(XAI_API_KEY_ENV_VAR, "test-deployment-key");
...
async fn cached_token_fallthrough_falls_to_grok_com_without_credentials() {
    use crate::agent::auth_method::{
        XVORA_COM_METHOD_ID, LEGACY_XAI_API_KEY_ENV_VAR, XAI_API_KEY_ENV_VAR,
    };
    use xai_xvora_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("XVORA_DISABLE_API_KEY_AUTH");
...
async fn prepare_video_gen_config_disabled_when_zdr_flag_set() {
    use xai_xvora_tools::implementations::xvora::video_gen::{
...
fn chat_session_spawn_options_matches_thin_profile() {
    ...
    let cwd = xai_xvora_paths::AbsPathBuf::new(std::env::temp_dir()).expect("temp cwd");
...
fn ext_method_rewind_uses_local_dispatch_without_bridge() {
    use acp::Agent as _;
    let _env = crate::env::EnvVarGuard::remove(crate::env::XVORA_DISABLE_CUSTOM_BRIDGE_ENV);
...
fn interactive_trust_prompt_grant_reloads_project_mcp() {
    use xai_xvora_test_support::EnvGuard;
    use xai_xvora_workspace::trust::{TrustStore, workspace_key};
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("XVORA_HOME", home.path());
    let _sim = EnvGuard::set(xai_xvora_version::TEST_VERSION_ENV, "0.0-sim");
...
fn ann(id: &str) -> xai_xvora_announcements::RemoteAnnouncement {
    xai_xvora_announcements::RemoteAnnouncement {
...
/// fields as needed).
fn settings_with(
    announcements: Option<Vec<xai_xvora_announcements::RemoteAnnouncement>>,
) -> crate::util::config::RemoteSettings {
    crate::util::config::RemoteSettings {
        announcements,
        ..Default::default()
...
fn announcements_push_gate_ignores_expired_only_addition() {
    let now = test_now();
    let expired = xai_xvora_announcements::RemoteAnnouncement {
...
fn announcements_push_gate_emits_on_expiry_crossing() {
    let expiring = xai_xvora_announcements::RemoteAnnouncement {
