...
pub(crate) async fn handle_subagent_request(
    ...
    }
    {
        ...
        if effective_runtime.isolation == SubagentIsolationMode::None
            && definition.isolation
                == Some(xai_xvora_agent::config::IsolationMode::Worktree)
        {
    ...
    if let Some(ref source) = resume_source {
        ...
        if let Err(e) = xai_xvora_subagent_resolution::validate_resume_identity(
            &request.subagent_type,
    ...
    {
        use xai_xvora_tools::implementations::xvora::task::MAX_SUBAGENT_DEPTH;
    ...
    {} else if context_source != InitialContextSource::Resumed && !verbatim_mirror_fork {
        if let Some(ref pi) = effective_runtime.persona_instructions {
            let reminder = xai_xvora_sampling_types::conversation::ConversationItem::system_reminder(
    ...
    let child_cwd_abs = xai_xvora_paths::AbsPathBuf::new(child_cwd)
        .unwrap_or_else(|_| {
            xai_xvora_paths::AbsPathBuf::new(std::env::current_dir().unwrap_or_default())
    ...
    let attribution_callback: Option<xai_xvora_sampler::SharedAttributionCallback> = effective_sampling_config
    ...
    let yolo_policy_block = xai_xvora_workspace::permission::resolution::yolo_disabled_by_policy();
    ...
    if let Some(scope) = agent_memory_scope {
        ...
        use xai_xvora_tools::implementations::opencode;
        let memory_tools: Vec<xai_xvora_tools::registry::types::ToolConfig> = vec![
            (& xvora::ReadFileTool).into(), (& xvora::SearchReplaceTool)
        ...
        if memory_md.is_file() && let Ok(content) = std::fs::read_to_string(&memory_md) {
            ...
            let truncated: String = content
                ...
                .collect::<Vec<_>>()
                .join("\n");
            let truncated = xai_xvora_tools::util::truncate::truncate_str(
                    &truncated,
                    MAX_BYTES,
                )
                .to_string();
        ...
        if is_plugin_agent {
            ...
            if !specs.is_empty() {
                let specs: Vec<_> = specs
                    .into_iter()
                    .map(|mut s| {
                        if s.event == xai_xvora_hooks::event::HookEventName::Stop {
                            s.event = xai_xvora_hooks::event::HookEventName::SubagentStop;
                        }
                        s
                    })
                    .collect();
    ...
    let agent_mcp_servers: Vec<_> = if is_plugin_agent {
        ...
        definition
                ...
                .filter_map(|entry| match entry {
                    xai_xvora_agent::config::McpServerRef::Named(name) => {
                    ...
                    xai_xvora_agent::config::McpServerRef::Inline { name, config } => {
    ...
    if inherit_skills && ctx.parent_skills.is_none() {
        ...
        ctx.parent_skills = Some(
            xai_xvora_agent::prompt::skills::list_skills_with_plugins(
                    Some(&parent_cwd_str),
    ...
    xai_xvora_telemetry::session_ctx::log_event(xai_xvora_telemetry::events::SubagentLaunched {
        subagent_id: request.id.clone(),
    ...
    let spawn_result = session::spawn_session_on_thread(
            ...
            xai_xvora_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
            ...
            std::sync::Arc::new(
                parking_lot::Mutex::new(
                    xai_xvora_workspace::file_system::CodebaseIndexManager::new(),
                ),
            ),
            false,
            subagent_fs_watch,
            ...
            if inherit_skills {
                ...
                xai_xvora_agent::prompt::skills::SkillsConfig::default()
            },
            ...
            ctx.yolo_mode
                || matches!(
                    ...
                    xai_xvora_agent::config::PermissionMode::BypassPermissions
                ),
            false,
            None,
            ...
            xai_xvora_agent::prompt::context::PromptAudience::Subagent,
    ...
    let outcome = if result.success {
        xai_xvora_telemetry::events::Outcome::Completed
    } else if result.cancelled {
        xai_xvora_telemetry::events::Outcome::Cancelled
    } else {
        xai_xvora_telemetry::events::Outcome::Error
    };
    xai_xvora_telemetry::session_ctx::log_event(xai_xvora_telemetry::events::SubagentCompleted {
        subagent_id: request.id.clone(),
        parent_session_id: request.parent_session_id.clone(),
        outcome,
