...
use std::sync::Arc;
use xai_xvora_agent::prompt::skills::SkillsConfig;
use xai_xvora_sampler::{AuthScheme, SamplerConfig};
use xai_xvora_sampling_types::{
...
use xai_xvora_tools::types::compat::{
...
pub const DEFAULT_AGENT_TYPE: &str = "xvora-plan";
...
pub struct EndpointsConfig {
    ...
    /// Env: `XVORA_MODELS_BASE_URL`. Enables custom endpoint mode.
    /// List URL defaults to `{models_base_url}/models`.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub models_base_url: Option<String>,
    /// Env: `XVORA_MODELS_LIST_URL`. Overrides the default `{base}/models` list URL.
    #[serde(alias = "models_endpoint", skip_serializing_if = "Option::is_none")]
    pub models_list_url: Option<String>,
    /// Env: `XVORA_FEEDBACK_BASE_URL`. Where feedback submissions go.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub feedback_base_url: Option<String>,
    /// Env: `XVORA_TRACE_UPLOAD_URL`. Where trace uploads go.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_url: Option<String>,
    /// Env: `XVORA_TRACE_UPLOAD_BUCKET`. Direct bucket (`gs://` or `s3://`), bypasses proxy.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_bucket: Option<String>,
    /// Env: `XVORA_TRACE_UPLOAD_REGION`. AWS region (S3 only).
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_region: Option<String>,
    /// Env: `XVORA_TRACE_UPLOAD_CREDENTIALS_FILE`. Path to GCS SA key or AWS credentials file.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// Env: `XVORA_TRACE_UPLOAD_ENDPOINT_URL`. Custom S3-compatible endpoint.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_endpoint_url: Option<String>,
    /// Env: `XVORA_DEPLOYMENT_KEY`. Management API key for enterprise deployments.
    /// Sent on telemetry and service requests for deployment-level attribution.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub deployment_key: Option<String>,
    /// Env: `XVORA_MANAGED_CONFIG_URL`. Override the managed config endpoint.
    ...
    /// favor of `XVORA_INTERNAL_OTLP_TRACES_ENDPOINT`, and ignored by the internal
    /// pipeline when `XVORA_EXTERNAL_OTEL` is set (the standard `OTEL_*` vars then
    ...
    /// Env: `XVORA_INTERNAL_OTLP_TRACES_ENDPOINT`. Full INTERNAL traces endpoint,
    ...
    /// Env: `XVORA_INTERNAL_OTLP_HEADERS`. `k=v,k2=v2` extra headers for the
    ...
    /// (requirement pin > `XVORA_EXTERNAL_OTEL` env > `[telemetry].otel_enabled`
...
pub(crate) fn default_asset_server_url() -> String {
    std::env::var("XVORA_ASSET_SERVER_URL").unwrap_or_else(|_| ASSET_SERVER_URL_DEFAULT.to_owned())
}
...
/// format, shared with `XVORA_INTERNAL_OTLP_HEADERS`): split on `,`,
...
impl EndpointsConfig {
            ...
            && let Some(legacy) = self.legacy_internal_otlp_traces_endpoint()
        {
            tracing::warn!(
                "Repointing the internal trace pipeline via OTEL_EXPORTER_OTLP_ENDPOINT / \
                 ...
                 XVORA_INTERNAL_OTLP_TRACES_ENDPOINT instead — the standard OTEL_* vars will \
    ...
    /// still honored by the internal pipeline even with `XVORA_EXTERNAL_OTEL`
...
impl Default for EndpointsConfig {
    fn default() -> Self {
        Self {
            cli_chat_proxy_base_url: std::env::var("XVORA_CLI_CHAT_PROXY_BASE_URL").ok(),
            xai_api_base_url: std::env::var("XVORA_XAI_API_BASE_URL")
                .unwrap_or_else(|_| XAI_API_BASE_URL_DEFAULT.to_owned()),
            alpha_test_key: None,
            models_base_url: env_string("XVORA_MODELS_BASE_URL"),
            models_list_url: env_string("XVORA_MODELS_LIST_URL"),
            feedback_base_url: env_string("XVORA_FEEDBACK_BASE_URL"),
            trace_upload_url: env_string("XVORA_TRACE_UPLOAD_URL"),
            trace_upload_bucket: env_string("XVORA_TRACE_UPLOAD_BUCKET"),
            trace_upload_region: env_string("XVORA_TRACE_UPLOAD_REGION"),
            trace_upload_credentials_file: env_string("XVORA_TRACE_UPLOAD_CREDENTIALS_FILE"),
            trace_upload_credentials: None,
            trace_upload_endpoint_url: env_string("XVORA_TRACE_UPLOAD_ENDPOINT_URL"),
            deployment_key: env_string("XVORA_DEPLOYMENT_KEY"),
            managed_config_url: env_string("XVORA_MANAGED_CONFIG_URL"),
            ...
            grok_internal_otlp_traces_endpoint: env_string("XVORA_INTERNAL_OTLP_TRACES_ENDPOINT"),
            grok_internal_otlp_headers: env_string("XVORA_INTERNAL_OTLP_HEADERS"),
...
pub use xai_xvora_config_types::{BoolFlag, ConfigSource, LazinessDetectorPerModelConfig, Resolved};
...
pub use xai_xvora_config::env_bool;
...
fn resolve_compat_cell(
    ...
    resolve_compat_cell_with_env(xai_xvora_config::env_bool(env), cfg, remote, default)
}
...
pub(crate) fn compat_config_cell(
    ...
    cell: xai_xvora_tools::types::compat::CompatCell,
...
pub(crate) use xai_xvora_telemetry::config::env_telemetry_mode;
pub use xai_xvora_telemetry::config::{TelemetryConfig, TelemetryMode};
...
impl PluginsConfig {
    ...
    /// Native `.xvora/config.toml` entries already present take precedence:
    ...
    pub fn merge_claude_enabled_plugins(&mut self, _cwd: Option<&std::path::Path>) {
        ...
        for path in &paths {
            let (claude_enabled, claude_disabled) =
                xai_xvora_agent::plugins::marketplace::load_enabled_disabled_plugins(path);
    ...
    pub fn to_discovery_config(&self) -> xai_xvora_agent::plugins::discovery::DiscoveryConfig {
        xai_xvora_agent::plugins::discovery::DiscoveryConfig {
...
pub struct ModelsConfig {
    ...
    /// Unset = remote pin, then the client hint / built-in `xvora-0.1`
...
impl SandboxSettingsConfig {
    ...
    pub fn resolve_profile(
        ...
        resolve_string_flag(cli_arg, "XVORA_SANDBOX", self.profile.as_deref(), None)
    ...
    pub fn resolve_auto_allow_bash(&self, requirement: Option<bool>) -> Resolved<bool> {
        BoolFlag::env("XVORA_SANDBOX_AUTO_ALLOW_BASH")
...
impl SuggestionsConfig {
    pub fn resolve_enabled(
        ...
        BoolFlag::env("XVORA_SUGGESTIONS")
            .config(self.enabled)
    ...
    pub fn resolve_ai_enabled(
        ...
        BoolFlag::env("XVORA_SUGGESTIONS_AI")
    ...
    pub fn resolve_ai_model(&self) -> String {
        resolve_string_flag(
            None,
            "XVORA_SUGGESTIONS_AI_MODEL",
            self.ai_model.as_deref(),
            None,
        )
        .map(|r| r.value)
        .unwrap_or_else(|| "xvora".to_owned())
    }
}
...
pub struct Config {
    ...
    #[serde(default, skip_serializing)]
    pub announcements: Vec<xai_xvora_announcements::RemoteAnnouncement>,
    ...
    /// `[marketplace]` — also read by `xai_xvora_plugin_marketplace::load_sources()`.
    #[serde(default, skip_serializing)]
    ...
    /// Defaults to reading from XVORA_STORAGE_MODE env var.
    #[serde(skip)]
    ...
    #[serde(skip)]
    pub cli_agents: Vec<xai_xvora_agent::config::AgentDefinition>,
    #[serde(skip)]
    pub cli_agent_overrides: CliAgentOverrides,
    /// Whether subagent (task tool) support is enabled. Enabled by default;
    /// disabled only via `XVORA_SUBAGENTS=0` or `[subagents] enabled = false`.
    /// Not remotely gated.
    #[serde(skip)]
    ...
    /// and `.xvora/roles/*.toml` file discovery.
    #[serde(skip)]
    pub subagent_roles:
        std::collections::HashMap<String, xai_xvora_subagent_resolution::config::SubagentRole>,
    #[serde(skip)]
    pub subagent_personas:
        std::collections::HashMap<String, xai_xvora_subagent_resolution::config::SubagentPersona>,
    ...
    pub zdr_video_output_s3:
        Option<xai_xvora_tools::implementations::xvora::video_gen::ZdrVideoOutputS3Config>,
    ...
    /// Image describe model (`xvora` default via `ModelOverrideConfig::resolve`).
    #[serde(skip)]
...
pub struct CliAgentOverrides {
    ...
    pub permission_rules: Vec<xai_xvora_workspace::permission::types::PermissionRule>,
    pub max_turns: Option<u32>,
    pub permission_mode: Option<xai_xvora_agent::config::PermissionMode>,
}
impl CliAgentOverrides {
    ...
    pub fn apply_to_definition(&self, def: &mut xai_xvora_agent::config::AgentDefinition) {
    ...
    pub fn apply_to_subagent_definition(&self, def: &mut xai_xvora_agent::config::AgentDefinition) {
...
pub use xai_xvora_agent::config::AgentDefinition;
pub use xai_xvora_agent::config::Effort;
pub use xai_xvora_agent::config::PermissionMode;
pub use xai_xvora_shared::ui_config::{ContextualHints, UiConfig};
...
/// 4. `XVORA_AGENT` env var
/// 5. Default `xvora` agent
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
#[serde(default)]
pub struct AgentSelectionConfig {
    /// Name of a built-in or discovered agent definition.
    /// Looked up via `xai_xvora_agent::discovery::by_name_in_cwd()`.
    /// Examples: "xvora", "browser-use", or a custom agent name.
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl Default for Config {
    fn default() -> Self {
        ...
        let mut cfg = Self {
            ...
            client_version: Some(xai_xvora_version::VERSION.to_string()),
...
impl Config {
    ...
    pub fn resolve_runtime_fields(&mut self, ctx: &RuntimeResolutionContext<'_>) {
        ...
        self.auto_wake_enabled = BoolFlag::env("XVORA_AUTO_WAKE")
    ...
    fn apply_env_overrides(&mut self) {
        ...
        if let Some(mode) = env_telemetry_mode("XVORA_TELEMETRY_ENABLED") {
    ...
    /// config.toml key, or `XVORA_TWO_PASS_COMPACTION` env.
    ...
    pub(crate) fn resolve_trace_upload(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("XVORA_TELEMETRY_TRACE_UPLOAD")
    ...
    pub(crate) fn trace_upload_decision_debug(&self) -> serde_json::Value {
        ...
        serde_json::json!(
            ...
            "in_env_trace_upload" : std::env::var("XVORA_TELEMETRY_TRACE_UPLOAD").ok(),
            "in_env_telemetry_enabled" : std::env::var("XVORA_TELEMETRY_ENABLED").ok(),
    ...
    pub(crate) fn resolve_feedback(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("XVORA_FEEDBACK_ENABLED")
    ...
    pub(crate) fn resolve_two_pass_compaction(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("XVORA_TWO_PASS_COMPACTION")
    ...
    /// `XVORA_DOOM_LOOP_RECOVERY` > TOML `enabled` > remote `enabled` >
    ...
    pub(crate) fn resolve_doom_loop_recovery(
        &self,
    ) -> Option<xai_xvora_sampling_types::DoomLoopRecoveryPolicy> {
        use xai_xvora_sampling_types::DoomLoopRecoveryPolicy as Policy;
        let remote = self
            .remote_settings
            .as_ref()
            .and_then(|s| s.doom_loop_recovery.as_ref());
        let enabled = BoolFlag::env("XVORA_DOOM_LOOP_RECOVERY")
    ...
    /// Precedence: env `XVORA_OFFICIAL_MARKETPLACE_AUTO_REGISTER` > remote settings >
    ...
    pub(crate) fn resolve_official_marketplace_auto_register(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("XVORA_OFFICIAL_MARKETPLACE_AUTO_REGISTER")
            .feature_flag(ff)
            .default(false)
            .resolve()
    }
    ...
    pub(crate) fn resolve_ask_user_question(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("XVORA_ASK_USER_QUESTION")
    ...
    ///
    /// Precedence: requirements > `XVORA_VOICE_MODE` > config/managed
    ...
    /// `image_gen` tool gate. Default on; gated only by the `XVORA_IMAGE_GEN`
    /// env var and managed-config requirement pin.
    pub(crate) fn resolve_image_gen(&self) -> Resolved<bool> {
        BoolFlag::env("XVORA_IMAGE_GEN")
    ...
    pub(crate) fn resolve_image_edit(&self) -> Resolved<bool> {
        use xai_xvora_tools::implementations::xvora::IMAGE_EDIT_TOOL_NAME;
        ...
        BoolFlag::env("XVORA_IMAGE_EDIT").default(true).resolve()
    }
    ...
    /// Precedence: env `XVORA_IMAGE_GEN_MODEL_OVERRIDE` > `[features]
    ...
    pub(crate) fn resolve_image_gen_model_override(&self) -> Option<String> {
        resolve_string_flag(
            None,
            "XVORA_IMAGE_GEN_MODEL_OVERRIDE",
    ...
    pub(crate) fn resolve_goal_classifier_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("XVORA_GOAL_CLASSIFIER")
    ...
    pub(crate) fn resolve_goal_planner_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("XVORA_GOAL_PLANNER")
    ...
    pub(crate) fn resolve_goal_summary_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("XVORA_GOAL_SUMMARY")
    ...
    pub(crate) fn resolve_goal_use_current_model_only(&self) -> Resolved<bool> {
        BoolFlag::env("XVORA_GOAL_USE_CURRENT_MODEL_ONLY")
    ...
    pub(crate) fn resolve_backend_tools(&self) -> Resolved<bool> {
        BoolFlag::env("XVORA_BACKEND_SEARCH")
            .config(self.features.backend_tools)
            .default(true)
            .resolve()
    }
    /// Resolve the mode (env `XVORA_COMPACTION_MODE` > config > remote settings >
    ...
    pub(crate) fn resolve_compaction_mode(&self) -> xai_chat_state::CompactionMode {
        resolve_compaction_mode_from(
            env_string("XVORA_COMPACTION_MODE").as_deref(),
    ...
    /// Resolve verbatim-input flag: env `XVORA_COMPACTION_VERBATIM_INPUT` > config > remote settings > default `true`.
    pub(crate) fn resolve_compaction_verbatim_input(&self) -> bool {
        BoolFlag::env("XVORA_COMPACTION_VERBATIM_INPUT")
    ...
    /// Precedence: env `XVORA_COMPACTION_DETAIL`, then config
    ...
    fn resolve_compaction_detail(&self) -> xai_chat_state::CompactionDetail {
        resolve_compaction_detail_from(
            env_string("XVORA_COMPACTION_DETAIL").as_deref(),
    ...
    pub fn resolve_cancel_rewind(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("XVORA_CANCEL_REWIND")
    ...
    ///
    /// Priority: `--oauth` > XVORA_OAUTH_ENABLED env > default (true = OAuth).
    pub fn resolve_grok_oauth(&self, cli_oidc: Option<bool>) -> Resolved<bool> {
        BoolFlag::env("XVORA_OAUTH_ENABLED")
            .cli(cli_oidc)
            .default(true)
            .resolve()
    }
    ...
    /// narrow non-recursive watches for `<cwd>/` and `<cwd>/.xvora/`.
    ///
    /// Thin delegate to the canonical
...
///
/// `requirement > cli > env (XVORA_MCP_LIVENESS_WATCHERS) > config >
...
pub fn resolve_mcp_liveness_watchers(
    ...
    BoolFlag::env("XVORA_MCP_LIVENESS_WATCHERS")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
/// `BoolFlag` precedence:
///
/// `requirement > cli > env (XVORA_MCP_AUTO_RESTART) > config >
/// managed > feature_flag > default (true)`.
///
...
/// precedence is single-sourced.
///
/// Recovery is on by default; opt out via `XVORA_MCP_AUTO_RESTART=false`,
...
pub fn resolve_mcp_auto_restart(
    ...
    BoolFlag::env("XVORA_MCP_AUTO_RESTART")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
///
/// `requirement > cli > env (XVORA_MCP_PUSH_SERVER_STATUS) > config >
/// managed > feature_flag > default (true)`.
///
...
pub fn resolve_mcp_push_server_status(
    ...
    BoolFlag::env("XVORA_MCP_PUSH_SERVER_STATUS")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
/// [`resolve_mcp_liveness_watchers`]:
///
/// `requirement > cli > env (XVORA_MCP_RECURSIVE_CONFIG_WATCH) >
...
/// operators flip this flag (e.g. via `XVORA_MCP_RECURSIVE_CONFIG_
...
pub fn resolve_mcp_recursive_config_watch(
    ...
    BoolFlag::env("XVORA_MCP_RECURSIVE_CONFIG_WATCH")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
impl SyncBoolFlag {
    ...
    /// Either-direction env resolver (typically `XVORA_*`). Returns
...
pub fn is_error_reporting_disabled_sync() -> bool {
    !SyncBoolFlag::new(error_reporting_enabled_from_toml)
        ...
        .enable_env(|| env_bool("XVORA_ERROR_REPORTING"))
...
/// `XVORA_TELEMETRY_ENABLED` resolved through `TelemetryMode::parse` so the
/// extended string forms (e.g. `"session_metrics"`) are accepted.
fn grok_telemetry_env_enabled() -> Option<bool> {
    env_telemetry_mode("XVORA_TELEMETRY_ENABLED").map(|m| !m.is_disabled())
}
...
/// stream's activation does: **requirement pin > `XVORA_EXTERNAL_OTEL` env >
...
pub(crate) fn external_otel_master_switch_resolved() -> bool {
    external_otel_master_switch_from(
        xai_xvora_config::load_merged_requirements().as_ref(),
        env_bool("XVORA_EXTERNAL_OTEL"),
...
pub fn resolve_external_otel_config(
    client: xai_xvora_telemetry::external::config::ExternalClientInfo,
) -> Option<xai_xvora_telemetry::external::ExternalOtelConfig> {
...
pub(crate) fn resolve_external_otel_config_with(
    ...
    let file_cfg: Option<xai_xvora_telemetry::external::ExternalOtelFileConfig> = effective_config
        .and_then(|cfg| cfg.get("telemetry"))
        .map(|t| xai_xvora_telemetry::external::ExternalOtelFileConfig {
    ...
    let getenv_pinned = |name: &str| -> Option<String> {
        let pin = match name {
            xai_xvora_telemetry::external::config::ENV_MASTER_SWITCH => req_enabled,
    ...
    let mut resolved = xai_xvora_telemetry::external::ExternalOtelConfig::resolve_with(
        getenv_pinned,
        file_cfg.as_ref(),
    )?;
...
pub fn apply_external_otel_remote_policy(settings: Option<&crate::util::config::RemoteSettings>) {
    ...
    let policy = xai_xvora_telemetry::external::ExternalOtelRemotePolicy {
    ...
    if policy.force_disable || policy.lock_content_gates {
        xai_xvora_telemetry::external::apply_remote_policy(policy);
    }
}
...
fn managed_settings_env_flag(key: &str) -> Option<bool> {
    let path = xai_xvora_config::claude_managed_settings_path()?;
    let content = std::fs::read_to_string(&path).ok()?;
...
pub struct ModelEntryConfig {
    ...
    /// Can also be set via the `XVORA_MAX_RETRIES` environment variable.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub max_retries: Option<u32>,
...
pub struct ModelInfo {
    ...
    /// Always has a value; defaults to `"xvora-plan"` when the server
...
pub struct AutoModeConfig {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub prompt_type: Option<xai_xvora_workspace::permission::ClassifierPromptType>,
...
pub struct Features {
    ...
    /// flag in `xvora_settings` is absent. When the remote flag is present it takes
    ...
    /// env (`XVORA_COMPACTION_MODE`). Parsed via `CompactionMode::parse`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub compaction_mode: Option<String>,
    /// `none` | `minimal` | `balanced` | `verbose` (default). `None` = defer to
    /// env (`XVORA_COMPACTION_DETAIL`). The `segments` verbatim detail level.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// `XVORA_MCP_AUTO_RESTART` to opt out).
    ///
...
pub fn enforce_disable_api_key_auth(
        ...
        && crate::util::is_first_party_xai_url(&creds.base_url)
    {
        ...
        xai_xvora_telemetry::unified_log::debug(
...
pub use xai_xvora_telemetry::config::deployment_id_from_key;
...
/// on BYOK / non-proxy routes for internal slugs like `xvora`).
...
pub fn inject_url_derived_headers(
    ...
    if crate::util::is_cli_chat_proxy_url(base_url) {
        headers
            ...
            .or_insert_with(|| "xai-xvora-cli".to_string());
        headers
...
mod tests {
    ...
    use xai_xvora_test_support::EnvGuard;
    #[test]
    fn main_cli_tools_override_preserves_profile_injection_policy() {
        ...
        let mut cases = vec![(AgentDefinition::default_xvora(), true)];
    ...
    fn auto_mode_config_parses_from_toml_and_json_equivalently() {
        use xai_xvora_workspace::permission::ClassifierPromptType;
        let toml_src = r#"
enabled = true
...
"#;
    ...
    fn finalize_image_describe_sampler_some_stamps_session_fields() {
        ...
        assert_eq!(model, "xvora");
        assert_eq!(cfg.model, "xvora");
        assert_eq!(cfg.client_identifier.as_deref(), Some("cli"));
        assert_eq!(cfg.max_retries, Some(7));
    }
    #[test]
    fn resolve_aux_model_honors_xvora_override() {
        let endpoints = EndpointsConfig::default();
    ...
    fn env_keys_resolve_first_set_wins() {
        let keys = EnvKeys::new(["XVORA_TEST_ENV_KEY_PRIMARY", "XVORA_TEST_ENV_KEY_FALLBACK"]);
        assert_eq!(keys.resolve_value_with(|_| None), None, "none set");
        assert_eq!(
            keys.resolve_value_with(
                |n| (n == "XVORA_TEST_ENV_KEY_FALLBACK").then(|| "from-fallback".into())
            ),
            Some("from-fallback".into())
        );
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "XVORA_TEST_ENV_KEY_PRIMARY" => Some("from-primary".into()),
                "XVORA_TEST_ENV_KEY_FALLBACK" => Some("from-fallback".into()),
                _ => None,
            }),
        ...
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "XVORA_TEST_ENV_KEY_PRIMARY" => Some(String::new()),
                "XVORA_TEST_ENV_KEY_FALLBACK" => Some("from-fallback".into()),
                _ => None,
            }),
            Some("from-fallback".into())
    ...
    fn env_keys_resolve_skips_whitespace_only_value() {
        let keys = EnvKeys::new(["XVORA_TEST_WS_PRIMARY", "XVORA_TEST_WS_FALLBACK"]);
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "XVORA_TEST_WS_PRIMARY" => Some("   ".into()),
                "XVORA_TEST_WS_FALLBACK" => Some("real".into()),
                _ => None,
            }),
            Some("real".into())
        );
        assert_eq!(
            EnvKeys::single("XVORA_TEST_WS_ONLY").resolve_value_with(|_| Some("   ".into())),
            None
        );
        assert_eq!(
            EnvKeys::single("XVORA_TEST_WS_PAD").resolve_value_with(|_| Some("  tok  ".into())),
            Some("  tok  ".into())
        );
    }
    #[test]
    ...
    fn first_own_credential_empty_api_key_falls_through_to_env_key() {
        use xai_xvora_test_support::EnvGuard;
        let var = "XVORA_TEST_FIRST_OWN_CRED_ENV";
    ...
    fn resolve_credentials_multi_env_key_uses_lc_alias() {
        ...
        let primary = "XVORA_TEST_MULTI_ENV_PRIMARY";
        let alias = "XVORA_TEST_MULTI_ENV_LC_ALIAS";
        unsafe {
    ...
    fn resolve_credentials_empty_env_key_falls_through_to_session() {
        use xai_chat_state::AuthType;
        use xai_xvora_test_support::EnvGuard;
        let primary = "XVORA_TEST_EMPTY_ENV_PRIMARY";
        let alias = "XVORA_TEST_EMPTY_ENV_LC_ALIAS";
    ...
    fn resolve_credentials_empty_env_key_falls_through_to_global_key() {
        ...
        let primary = "XVORA_TEST_EMPTY_ENV_GLOBAL_PRIMARY";
        let alias = "XVORA_TEST_EMPTY_ENV_GLOBAL_ALIAS";
        let _primary = EnvGuard::set(primary, "");
        let _alias = EnvGuard::set(alias, "");
    ...
    fn x_api_key_auth_scheme_flows_from_config_to_sampler() {
        ...
        assert_eq!(config.api_backend, ApiBackend::Messages);
        let client = xai_xvora_sampler::SamplingClient::new(config).expect("client should build");
    ...
    fn user_override_parses_compaction_at_tokens_from_toml() {
        use xai_xvora_sampling_types::CompactionAtTokens;
        let dm = crate::models::default_model();
        let raw_config: toml::Value = toml::from_str(&format!(
            r#"
            [model."{dm}"]
    ...
    fn user_override_parses_compactions_remaining_from_toml() {
        use xai_xvora_sampling_types::CompactionsRemaining;
        let dm = crate::models::default_model();
        let raw_config: toml::Value = toml::from_str(&format!(
            r#"
            [model."{dm}"]
    ...
    /// xai-xvora-shell/src/agent/models.rs:857 for any BYOK Claude config.
    #[test]
    ...
    #[test]
    fn model_agent_type_defaults_to_xvora() {
        let raw_config: toml::Value = toml::from_str(
            r#"
            [model.my-model]
            model = "my-model"
    ...
    fn config_models_default_is_not_overwritten_by_default_models_json() {
        ...
        let resolved = resolve_string_flag(
            None,
            "XVORA_DEFAULT_MODEL_TEST_NONEXISTENT",
            config_default,
    ...
    fn unset_endpoint_env_vars() {
        for k in [
            "XVORA_CLI_CHAT_PROXY_BASE_URL",
            "XVORA_XAI_API_BASE_URL",
            "XVORA_FEEDBACK_BASE_URL",
            "XVORA_TRACE_UPLOAD_URL",
            "XVORA_MANAGED_CONFIG_URL",
            "XVORA_MODELS_BASE_URL",
            "XVORA_MODELS_LIST_URL",
            "OTEL_EXPORTER_OTLP_ENDPOINT",
            "OTEL_EXPORTER_OTLP_TRACES_ENDPOINT",
            "OTEL_EXPORTER_OTLP_HEADERS",
            "XVORA_INTERNAL_OTLP_TRACES_ENDPOINT",
            "XVORA_INTERNAL_OTLP_HEADERS",
            "XVORA_EXTERNAL_OTEL",
        ] {
    ...
    fn resolve_feedback_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("XVORA_FEEDBACK_ENABLED") };
        unsafe { std::env::remove_var("XVORA_TELEMETRY_ENABLED") };
        let cfg = Config::default();
    ...
    fn resolve_session_recap_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("XVORA_SESSION_RECAP") };
        let cfg = Config::default();
    ...
    fn resolve_session_recap_env_off_overrides_default() {
        unsafe { std::env::set_var("XVORA_SESSION_RECAP", "0") };
        let cfg = Config::default();
        let r = cfg.resolve_session_recap();
        assert!(!r.value);
    ...
    fn resolve_two_pass_compaction_precedence() {
        unsafe { std::env::remove_var("XVORA_TWO_PASS_COMPACTION") };
        ...
        assert!(r.value);
        assert_eq!(r.source, ConfigSource::Config);
        unsafe { std::env::set_var("XVORA_TWO_PASS_COMPACTION", "0") };
        let r = config_over_remote.resolve_two_pass_compaction();
    ...
    fn resolve_doom_loop_recovery_precedence() {
        ...
        unsafe { std::env::remove_var("XVORA_DOOM_LOOP_RECOVERY") };
        let default_cfg = Config::default();
        assert!(
        ...
        unsafe { std::env::set_var("XVORA_DOOM_LOOP_RECOVERY", "0") };
        assert!(
    ...
    fn resolve_feedback_env_overrides_all() {
        unsafe { std::env::set_var("XVORA_FEEDBACK_ENABLED", "true") };
        let mut cfg = Config::default();
    ...
    fn resolve_trace_upload_disabled_when_telemetry_off_despite_remote_flag() {
        unsafe { std::env::remove_var("XVORA_TELEMETRY_ENABLED") };
        unsafe { std::env::remove_var("XVORA_TELEMETRY_TRACE_UPLOAD") };
        let mut cfg = Config::default();
    ...
    fn resolve_goal_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("XVORA_GOAL") };
        let cfg = Config::default();
    ...
    fn resolve_goal_env_overrides_config() {
        unsafe { std::env::set_var("XVORA_GOAL", "1") };
        let mut cfg = Config::default();
    ...
    fn resolve_ask_user_question_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("XVORA_ASK_USER_QUESTION") };
        let cfg = Config::default();
    ...
    fn resolve_ask_user_question_env_overrides_remote_settings() {
        unsafe { std::env::set_var("XVORA_ASK_USER_QUESTION", "1") };
        let cfg = Config {
            remote_settings: Some(crate::util::config::RemoteSettings {
    ...
    fn resolve_image_gen_model_override_remote_settings_or_config() {
        unsafe { std::env::remove_var("XVORA_IMAGE_GEN_MODEL_OVERRIDE") };
    ...
    fn imagine_tools_disabled_gates_image_edit() {
        unsafe { std::env::remove_var("XVORA_IMAGE_EDIT") };
        ...
        unsafe { std::env::set_var("XVORA_IMAGE_EDIT", "1") };
    ...
    fn clear_goal_envs() {
        unsafe {
            std::env::remove_var("XVORA_GOAL");
            std::env::remove_var("XVORA_GOAL_CLASSIFIER");
            std::env::remove_var("XVORA_GOAL_PLANNER");
            std::env::remove_var("XVORA_GOAL_SUMMARY");
            std::env::remove_var("XVORA_GOAL_VERIFIER_N");
            std::env::remove_var("XVORA_GOAL_CLASSIFIER_MAX");
            std::env::remove_var("XVORA_GOAL_STRATEGIST_EVERY");
            std::env::remove_var("XVORA_GOAL_REVERIFY_AFTER");
        }
    }
    ...
    fn resolve_goal_classifier_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("XVORA_GOAL_CLASSIFIER", "0") };
        let r = cfg_with_goal_and_remote(true, remote_classifier(true))
            .resolve_goal_classifier_enabled(true);
        assert!(!r.value);
        assert_eq!(r.source, ConfigSource::Env);
        unsafe { std::env::set_var("XVORA_GOAL_CLASSIFIER", "1") };
    ...
    fn resolve_goal_planner_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("XVORA_GOAL_PLANNER", "0") };
        let r =
            cfg_with_goal_and_remote(true, remote_planner(true)).resolve_goal_planner_enabled(true);
        assert!(!r.value);
        assert_eq!(r.source, ConfigSource::Env);
        unsafe { std::env::set_var("XVORA_GOAL_PLANNER", "1") };
    ...
    fn resolve_goal_summary_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("XVORA_GOAL_SUMMARY", "0") };
        let r =
...
"#,
    ...
    const GOAL_USE_CURRENT_ENV: &str = "XVORA_GOAL_USE_CURRENT_MODEL_ONLY";
...
[goal]
enabled = true
planner_model = { model = "xvora", agent_type = "xvora-plan" }
...
"#;
        ...
        let cfg = Config::new_from_toml_cfg(&raw).unwrap();
        assert_eq!(cfg.goal.planner_model.as_ref().unwrap().model, "xvora");
        assert_eq!(
            cfg.goal.strategist_model.as_ref().unwrap().agent_type,
            "cursor"
        );
        assert_eq!(cfg.goal.skeptic_models.len(), 2);
        assert_eq!(cfg.goal.skeptic_models[0].model, "xvora");
        assert_eq!(
...
planner_model = { agent_type = "xvora-plan" }
"#;
        let raw: toml::Value = toml::from_str(toml_str).unwrap();
        let cfg = Config::new_from_toml_cfg(&raw)
...
"#,
        ...
        let xvora = crate::util::config::GoalRoleModel {
            model: "xvora".into(),
            agent_type: "xvora-plan".into(),
        };
        ...
        assert_eq!(
            ...
            vec![
                GoalRoleModelChoice::Explicit(xvora),
    ...
    fn internal_otlp_endpoint_grok_internal_wins_regardless_of_switch() {
        for switch in [false, true] {
            ...
            assert_eq!(
                ...
                "switch={switch}: XVORA_INTERNAL_OTLP_TRACES_ENDPOINT must win verbatim (trailing / trimmed)"
            );
        }
    }
    ...
    fn ext_client() -> xai_xvora_telemetry::external::config::ExternalClientInfo {
        xai_xvora_telemetry::external::config::ExternalClientInfo::default()
    }
    #[test]
    ...
    fn external_otel_requirements_pin_wins_over_env() {
        ...
        assert!(
            resolve_external_otel_config_with(
                None,
                Some(&req),
                ext_env(&[("XVORA_EXTERNAL_OTEL", "1"), ("OTEL_LOGS_EXPORTER", "otlp"),]),
                ext_client(),
                false,
            )
            .is_none()
    ...
    /// (managed config / requirements — no `XVORA_EXTERNAL_OTEL` env var) must
    ...
    fn clear_runtime_env_vars() {
        unsafe {
            std::env::remove_var("XVORA_SUBAGENTS");
            std::env::remove_var("XVORA_RESPECT_GITIGNORE");
            std::env::remove_var("XVORA_WEB_SEARCH_MODEL");
            std::env::remove_var("XVORA_SESSION_SUMMARY_MODEL");
            std::env::remove_var("XVORA_CURSOR_SKILLS_ENABLED");
            std::env::remove_var("XVORA_CURSOR_RULES_ENABLED");
            std::env::remove_var("XVORA_CURSOR_AGENTS_ENABLED");
            std::env::remove_var("XVORA_CLAUDE_SKILLS_ENABLED");
            std::env::remove_var("XVORA_CLAUDE_RULES_ENABLED");
            std::env::remove_var("XVORA_CLAUDE_AGENTS_ENABLED");
        }
    }
    fn clear_managed_mcp_env_vars() {
        unsafe {
            std::env::remove_var("XVORA_MANAGED_MCPS_ENABLED");
            std::env::remove_var("XVORA_MANAGED_MCP_GATEWAY_TOOLS_ENABLED");
        }
    }
...
"#,
    ...
    fn resolve_raw_compat_sessions_load_failure_allows_env_override() {
        let _env = isolate_compat_env();
        let _codex = EnvGuard::set("XVORA_CODEX_SESSIONS_ENABLED", "true");
        let resolved = resolve_compat_sessions_from_raw(Err(()), None);
        assert!(!resolved.cursor.sessions);
        assert!(!resolved.claude.sessions);
        assert!(resolved.codex.sessions);
    ...
    fn resolve_compat_env_sessions_disable_independently() {
        ...
        for (vendor, env_var) in [
            (CompatVendor::Cursor, "XVORA_CURSOR_SESSIONS_ENABLED"),
            (CompatVendor::Claude, "XVORA_CLAUDE_SESSIONS_ENABLED"),
            (CompatVendor::Codex, "XVORA_CODEX_SESSIONS_ENABLED"),
        ] {
    ...
    fn resolve_compat_precedence_and_reserved_codex_hook() {
        ...
        let _session = EnvGuard::set("XVORA_CURSOR_SESSIONS_ENABLED", "true");
        let _hook = EnvGuard::set("XVORA_CODEX_HOOKS_ENABLED", "true");
        let resolved = resolve_compat_config(&config, Some(&remote));
        assert!(resolved.cursor.sessions);
        assert!(resolved.codex.hooks);
    }
    ...
    fn resolve_runtime_fields_compat_asymmetric_sources() {
        let _env = isolate_compat_env();
        let _cursor = EnvGuard::set("XVORA_CURSOR_SESSIONS_ENABLED", "false");
        let raw: toml::Value =
    ...
    fn resolve_runtime_fields_gitignore_from_env() {
        clear_runtime_env_vars();
        unsafe { std::env::set_var("XVORA_RESPECT_GITIGNORE", "0") };
        let raw = empty_config();
        let mut cfg = Config::new_from_toml_cfg(&raw).unwrap();
        cfg.resolve_runtime_fields(&RuntimeResolutionContext {
            raw_config: &raw,
...
"#,
    ...
    fn is_telemetry_explicitly_disabled_sync_env_signals() {
        unsafe { std::env::set_var("XVORA_TELEMETRY_ENABLED", "0") };
        unsafe { std::env::remove_var("DISABLE_TELEMETRY") };
        assert!(is_telemetry_explicitly_disabled_sync());
        unsafe { std::env::set_var("XVORA_TELEMETRY_ENABLED", "1") };
        assert!(!is_telemetry_explicitly_disabled_sync());
        unsafe { std::env::remove_var("XVORA_TELEMETRY_ENABLED") };
...
"#,
        ...
        xai_xvora_config::apply_version_overrides(&mut value, &v).unwrap();
        let cfg = Config::new_from_toml_cfg(&value).unwrap();
        assert_eq!(cfg.models.default.as_deref(), Some("grok-4.5"));
    }
    /// Reproduce the enterprise managed config bug: [model.xvora] sets
    ...
    fn slug_propagation_enterprise_managed_config_key_mismatch() {
        ...
        assert_eq!(
            ...
            "BUG: prefetched 'grok-4.5' should inherit 500k from \
             sibling 'xvora' (same model slug), not stay at {default_cw}"
        );
    }
    ...
    fn resolve_model_list_inherits_context_window_from_default_when_prefetched_has_fallback() {
        let cfg = Config::default();
        let default_cw = DEFAULT_CONTEXT_WINDOW;
        let entry = prefetch_model_entry("xvora", default_cw, ApiBackend::default());
        let mut prefetched = IndexMap::new();
        prefetched.insert("xvora".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
        let entry = resolved.get("xvora").expect("model must exist");
        assert_ne!(
    ...
    fn resolve_model_list_does_not_override_explicitly_set_context_window() {
        ...
        let entry = prefetch_model_entry("xvora", explicit_cw, ApiBackend::default());
        let mut prefetched = IndexMap::new();
        prefetched.insert("xvora".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
        let entry = resolved.get("xvora").expect("model must exist");
    ...
    fn resolve_model_list_inherits_agent_type_and_api_backend() {
        ...
        if let Some(default) = defaults.get("xvora") {
    ...
    fn resolve_model_list_prunes_bundled_entries_not_in_prefetch() {
        ...
        if let Some(e) = defs.shift_remove("xvora") {
            p.insert("xvora".to_string(), e);
        }
        let resolved = resolve_model_list(&cfg, Some(p));
        assert!(resolved.contains_key("xvora"));
        let no_p = resolve_model_list(&cfg, None);
        assert!(no_p.contains_key("xvora"));
    }
    #[test]
    ...
    fn byok_config_overlay_visible_to_api_key_users() {
        ...
        let cfg = Config::new_from_toml_cfg(&raw).expect("config should parse");
        let resolved = resolve_model_list(&cfg, None);
        let entry = resolved.get("xvora").expect("xvora must exist");
        assert!(
    ...
    fn mcp_liveness_watchers_default_is_true() {
        unsafe { std::env::remove_var("XVORA_MCP_LIVENESS_WATCHERS") };
    ...
    fn mcp_liveness_watchers_requirement_wins_over_everything() {
        unsafe { std::env::set_var("XVORA_MCP_LIVENESS_WATCHERS", "true") };
    ...
    fn mcp_liveness_watchers_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("XVORA_MCP_LIVENESS_WATCHERS", "false") };
    ...
    fn mcp_auto_restart_default_is_true() {
        unsafe { std::env::remove_var("XVORA_MCP_AUTO_RESTART") };
    ...
    fn mcp_auto_restart_requirement_wins_over_everything() {
        unsafe { std::env::set_var("XVORA_MCP_AUTO_RESTART", "false") };
    ...
    fn mcp_auto_restart_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("XVORA_MCP_AUTO_RESTART", "true") };
    ...
    fn mcp_push_server_status_default_is_true() {
        unsafe { std::env::remove_var("XVORA_MCP_PUSH_SERVER_STATUS") };
    ...
    fn mcp_push_server_status_requirement_wins_over_everything() {
        unsafe { std::env::set_var("XVORA_MCP_PUSH_SERVER_STATUS", "true") };
    ...
    fn mcp_push_server_status_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("XVORA_MCP_PUSH_SERVER_STATUS", "false") };
    ...
    fn mcp_recursive_config_watch_default_is_true() {
        unsafe { std::env::remove_var("XVORA_MCP_RECURSIVE_CONFIG_WATCH") };
    ...
    fn mcp_recursive_config_watch_requirement_wins_over_everything() {
        unsafe { std::env::set_var("XVORA_MCP_RECURSIVE_CONFIG_WATCH", "true") };
    ...
    fn mcp_recursive_config_watch_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("XVORA_MCP_RECURSIVE_CONFIG_WATCH", "false") };
