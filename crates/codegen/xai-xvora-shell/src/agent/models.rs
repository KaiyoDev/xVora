...
use xai_xvora_sampling_types::{ReasoningEffort, ReasoningEffortOption};
...
impl ModelsManager {
    ...
    pub fn model_compactions_remaining(
        &self,
        model_id: &str,
    ) -> Option<xai_xvora_sampling_types::CompactionsRemaining> {
        self.inner
            .models
            .read()
            .get(model_id)
    ...
    pub fn model_compaction_at_tokens(
        &self,
        model_id: &str,
    ) -> Option<xai_xvora_sampling_types::CompactionAtTokens> {
        self.inner
            .models
            .read()
            .get(model_id)
    ...
    /// differ from the slug for custom/enterprise ids (e.g. key `enterprise-xvora`
    ...
    /// Hot-reload the catalog from `~/.xvora/models_cache.json` after an
    ...
    /// When another grok process sharing `~/.xvora` (a `--no-leader` run, a
    ...
    /// `ModelsCacheManager` path is fixed to `xvora_home()`, a process-wide
    /// `OnceLock`).
...
impl ModelsCacheManager {
    ...
    fn load_fresh(
        ...
        if cache.grok_version.as_deref() != Some(xai_xvora_version::VERSION) {
    ...
    fn persist(
        ...
        let cache = ModelsCache {
            fetched_at: Utc::now(),
            grok_version: Some(xai_xvora_version::VERSION.to_string()),
...
/// routing slug (e.g. "Auto" and "Xvora" both route to `xvora`)
...
fn resolve_prefetch_env(grok_com_config: Option<GrokComConfig>) -> Option<PrefetchEnv> {
    let xvora_home = crate::util::xvora_home::xvora_home();
    let auth_manager = AuthManager::new(&xvora_home, grok_com_config.unwrap_or_default());
...
/// the catalog and `/model` picker use config keys (e.g. `enterprise-xvora`).
...
mod tests {
    ...
    fn reasoning_effort_override_skips_models_that_do_not_offer_level() {
        use indexmap::IndexMap;
        use xai_xvora_sampling_types::ReasoningEffortOption;
    ...
    fn resolve_falls_back_to_session_when_nothing_set() {
        ...
        let _unset_legacy = EnvGuard::unset("XVORA_CODE_XAI_API_KEY");
        let endpoints = config::EndpointsConfig::default();
        assert_eq!(
            ModelFetchAuth::resolve(&endpoints, false),
            ModelFetchAuth::Session,
    ...
    fn build_prefetched_map_distinct_ids_same_slug() {
        let entries = vec![
            make_entry_config_with_id(Some("auto"), "xvora", Some("Auto")),
            make_entry_config_with_id(Some("xvora"), "xvora", Some("Xvora")),
        ...
        assert!(map.contains_key("xvora"));
        assert!(map.contains_key("grok-composer-2.5-fast"));
        assert_eq!(
            map["auto"].info.model, "xvora",
            "auto entry should still route to xvora"
        );
        assert_eq!(map["xvora"].info.model, "xvora");
    }
    ...
    fn build_prefetched_map_duplicate_id_overwrites() {
        let entries = vec![
            make_entry_config_with_id(Some("xvora"), "xvora", Some("First")),
            make_entry_config_with_id(Some("xvora"), "xvora", Some("Second")),
        ];
        let map = build_prefetched_map(entries, None);

        assert_eq!(map.len(), 1, "duplicate id: second overwrites first");
        assert_eq!(map["xvora"].info.name.as_deref(), Some("Second"));
    }
    ...
    fn resolve_default_model_prefers_id_over_model_slug() {
        ...
        let mut cfg = config::Config::default();
        cfg.models.default = Some("xvora".to_string());

        let (key, _, _) = resolve_default_model(&cfg, &catalog, true);
        assert_eq!(key, "xvora", "must match id, not first slug hit");
    }
    ...
    fn resolve_catalog_key_maps_routing_slug_to_config_key() {
        ...
        assert_eq!(key.0.as_ref(), "enterprise-xvora");
    }
    ...
    fn resolve_catalog_key_last_slug_match_wins() {
        ...
        let persisted = acp::ModelId::new("grok-4.5");
        let key = resolve_catalog_key(&models, &persisted).expect("slug must resolve");
        assert_eq!(key.0.as_ref(), "user-xvora");
    }
    ...
    fn selectable_prefers_available_identity_over_non_selectable_exact_key() {
        ...
        let persisted = acp::ModelId::new("xvora");
        assert_eq!(
    ...
    fn selectable_prefers_exact_key_over_later_slug_match() {
        let mut models = IndexMap::new();
        models.insert("xvora".to_string(), make_model_entry("grok-4.5"));
        models.insert("other".to_string(), make_model_entry("xvora"));

        let available = test_available_keys(&["xvora", "other"]);

        let persisted = acp::ModelId::new("xvora");
        let key = selectable_catalog_key_for_persisted(&models, &available, &persisted)
            .expect("exact selectable key must win");
        assert_eq!(key.0.as_ref(), "xvora");
    }
