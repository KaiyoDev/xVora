...
/// two-function shape in `xai-xvora-tools` (F29).
...
/// `xai-xvora-tools` — existing items keep their prior content when
...
/// binary. Wraps a `xai_xvora_sampler::SamplingClient` and pulls the
/// text content out of the response.
pub struct SamplerClassifierClient {
    inner: xai_xvora_sampler::SamplingClient,
}

impl SamplerClassifierClient {
    pub fn new(inner: xai_xvora_sampler::SamplingClient) -> Self {
        Self { inner }
    }
}
...
pub fn build_classifier_request(
    ...
    ConversationRequest {
        ...
        x_grok_agent_id: Some(xai_xvora_telemetry::id::agent_id()),
...
pub struct RunArgs {
    ...
    /// `xai_xvora_shell::util::xvora_home::xvora_home()` when `None`.
...
async fn build_sampler_client(
...
) -> Result<xai_xvora_sampler::SamplingClient> {
    let default_home;
    ...
    let config = xai_xvora_sampler::SamplerConfig {
        api_key: Some(resolved),
        base_url,
        model,
        max_completion_tokens: Some(LAZINESS_MAX_OUTPUT_TOKENS),
        ..xai_xvora_sampler::SamplerConfig::default()
    };
    xai_xvora_sampler::SamplingClient::new(config).map_err(|e| anyhow!("build SamplingClient: {e}"))
}
...
mod tests {
    use super::*;
    use xai_xvora_sampling_types::{AssistantItem, ToolCall, ToolResultItem};
    ...
    async fn classifier_request_matches_production_shape() {
        ...
        assert_eq!(
            ...
            Some(xai_xvora_telemetry::id::agent_id().as_str())
        );
    }
