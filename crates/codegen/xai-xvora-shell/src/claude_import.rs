...
use xai_xvora_workspace::permission::claude_settings::{
    find_claude_settings_paths, load_claude_settings,
};
use xai_xvora_workspace::permission::rules::parse_permission_rule;
use xai_xvora_workspace::permission::types::{PatternMode, PermissionRule, RuleAction, ToolFilter};
...
/// overwritten).
///
/// Project items are written to `<repo_root>/.xvora/config.toml` (discovered
/// via `git2::Repository::discover`), not `cwd/.xvora/config.toml`, to avoid
...
fn apply_items_to_config(config_path: &Path, items: &[ImportableItem]) -> anyhow::Result<usize> {
    ...
    for item in items {
        match item {
            ...
            // Hooks are written to .xvora/hooks/ JSON files in apply_hooks_to_dir,
...
///
/// The output JSON is the same shape that `xai-xvora-hooks` natively understands
/// (Claude-compatible). The native hooks loader scans `.xvora/hooks/*.json`
...
mod tests {
    ...
    impl Drop for MarkerGuard {
        fn drop(&mut self) {
            ...
            unsafe { std::env::remove_var("_XVORA_CLAUDE_MARKER_OVERRIDE") };
        }
    }
    ...
    fn gate_load_claude_env_returns_empty_when_marker_set() {
        ...
        refresh_marker_cache(true);
        let dir = tempfile::tempdir().unwrap();
        let env = xai_xvora_workspace::permission::claude_settings::load_claude_env_with_project(
            dir.path(),
            true,
        );
        assert!(
    ...
    fn discover_hook_source_paths_skips_cursor_when_disabled() {
        ...
        refresh_marker_cache(false);
        let dir = tempfile::tempdir().unwrap();
        let mut compat = xai_xvora_tools::types::compat::CompatConfig::default();
    ...
    fn discover_hooks_honors_claude_compat_gate() {
        ...
        let has_probe = |reg: &xai_xvora_hooks::discovery::HookRegistry| {
...
"#;
    ...
    fn scan_claude_path_dirs_dedupes_global_and_project_when_same() {
        ...
        // requires `XVORA_HOME` plumbing which is intentionally out of scope.
    ...
    async fn gate_resolve_permissions_with_provenance_skips_claude_when_marker_set() {
        ...
        unsafe { std::env::set_var("_XVORA_CLAUDE_MARKER_OVERRIDE", "1") };
        let dir = tempfile::tempdir().unwrap();
        ...
        let resolved =
            xai_xvora_workspace::permission::resolution::resolve_permissions_with_provenance(
                dir.path(),
            )
            .await;
        if let Some(r) = resolved {
            let tempdir_claude = claude_dir.join("settings.json");
            use xai_xvora_workspace::permission::types::RequirementSource;
