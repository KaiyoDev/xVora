...
const XVORA_CODE_BACKEND_URL: &str = "https://code.grok.com";
const DEFAULT_TIMEOUT: Duration = Duration::from_secs(30);
const XVORA_CODE_WEB_URL: &str = "https://grok.com";
/// Build a share URL from a permission ID
pub fn share_url(permission_id: &str) -> String {
    let web_url =
        std::env::var("XVORA_CODE_WEB_URL").unwrap_or_else(|_| XVORA_CODE_WEB_URL.to_string());
...
fn add_cli_chat_proxy_headers_blocking(
    ...
    let mut builder = builder
        ...
        .header("x-grok-client-version", xai_xvora_version::VERSION);
...
async fn fetch_bundle_inner(
    ...
    let client: reqwest_middleware::ClientWithMiddleware = if let Some(am) = auth_manager {
        let provider: std::sync::Arc<dyn xai_xvora_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
...
impl BackendClient {
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            reqwest_client,
            base_url: std::env::var("XVORA_CODE_BACKEND_URL")
                .unwrap_or_else(|_| XVORA_CODE_BACKEND_URL.to_string()),
            auth_manager: None,
        }
    }
    ...
    pub fn with_auth_manager(mut self, manager: std::sync::Arc<crate::auth::AuthManager>) -> Self {
        let credentials: std::sync::Arc<dyn xai_xvora_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
                crate::auth::credential_provider::ShellAuthCredentialProvider::new(
                    manager.clone(),
                    None,
    ...
    /// Must include X-XAI-Token-Auth so nginx auth subrequest routes to authenticate_xai_xvora_cli_token.
    /// See: crates/codegen/xai-xvora-shell/src/agent/app.rs:run_headless
    async fn auth_header_map(&self) -> Result<reqwest::header::HeaderMap, BackendError> {
        ...
        headers.insert(
            ...
            HeaderValue::from_static(xai_xvora_version::VERSION),
        );
        Ok(headers)
    }
...
/// Fetch `xvora_login_device_flow` from cli-chat-proxy `GET /v1/login-config`.
///
...
pub async fn fetch_login_device_flow(cli_chat_proxy_base_url: &str) -> Option<bool> {
    let agent_id = tokio::task::spawn_blocking(xai_xvora_telemetry::id::agent_id)
        .await
        .ok()?;
...
pub(crate) fn fetch_models_blocking(
    ...
    match source.auth {
        ...
        EndpointAuth::Session => {
            ...
            request = request
                .header("Authorization", format!("Bearer {}", &auth.key))
                .header("X-XAI-Token-Auth", "xai-xvora-cli")
                .header("x-userid", &auth.user_id)
                .header("x-grok-client-version", xai_xvora_version::VERSION)
                .header(
                    crate::http::CLIENT_MODE_HEADER,
...
pub fn parse_remote_model_value(
    ...
    Some(crate::agent::config::ModelEntryConfig {
        ...
        reasoning_efforts: obj
            ...
            .map(|arr| xai_xvora_sampling_types::parse_reasoning_effort_options(arr))
            .unwrap_or_default(),
        ...
        compactions_remaining: obj
            ...
            .or_else(|| {
                obj
                    ...
                    .and_then(|v| v.as_bool())
                    .map(xai_xvora_sampling_types::CompactionsRemaining::Dynamic)
            }),
...
mod tests {
    ...
    async fn fetch_subagent_bundle_success() {
        ...
        assert_eq!(headers.token_auth.as_deref(), Some("xai-xvora-cli"));
    ...
    fn parse_reads_show_model_fingerprint_field() {
        let value = serde_json::json!(
            { "model" : "xvora", "context_window" : 256_000,
            "show_model_fingerprint" : true }
        );
        let result = parse_remote_model_value(&value, "https://default.url").unwrap();
        assert!(result.show_model_fingerprint);
        let value = serde_json::json!(
            { "model" : "xvora", "contextWindow" : 256_000, "showModelFingerprint" :
            true }
        );
        let result = parse_remote_model_value(&value, "https://default.url").unwrap();
        assert!(result.show_model_fingerprint);
        let value = serde_json::json!(
            { "model" : "xvora", "context_window" : 256_000, "_meta" : {
    ...
    fn deployment_config_url_uses_cli_chat_proxy_when_not_overridden() {
        ...
        unsafe { std::env::set_var("XVORA_DEPLOYMENT_KEY", "xai-token-ENTERPRISE") };
        ...
        unsafe { std::env::remove_var("XVORA_DEPLOYMENT_KEY") };
    }
    #[derive(Clone)]
