...
use xai_xvora_telemetry::events::ManualAuthSurface;
...
impl AuthManager {
    pub fn new(xvora_home: &Path, grok_com_config: GrokComConfig) -> Self {
        ...
        xai_xvora_telemetry::unified_log::info(
            ...
            Some(serde_json::json!({
                ...
                "XVORA_AUTH_PATH": std::env::var("XVORA_AUTH_PATH").unwrap_or_else(|_| "(unset)".into()),
                "XVORA_AUTH": std::env::var("XVORA_AUTH").map(|_| "(set)".to_string()).unwrap_or_else(|_| "(unset)".into()),
            })),
        );
        ...
        if let Ok(inline_json) = std::env::var("XVORA_AUTH") {
            ...
            tracing::warn!("XVORA_AUTH set but failed to parse as JSON, falling back to file");
        }

        // XVORA_AUTH_PATH: custom file path (overrides default $XVORA_HOME/auth.json).
        let path = std::env::var("XVORA_AUTH_PATH")
            .map(PathBuf::from)
    ...
    /// (inline `XVORA_AUTH` vs. on-disk `auth.json`), which differ only in the
    ...
    pub(crate) async fn update(self: &Arc<Self>, auth: GrokAuth) -> std::io::Result<GrokAuth> {
        ...
        match &write_result {
            Ok(()) => xai_xvora_telemetry::unified_log::info(
            ...
            Err(e) => xai_xvora_telemetry::unified_log::error(
    ...
    fn observe_disk_state(
        ...
        match new_state {
            // Recovery (or first observation in XVORA_AUTH mode).
            DiskAuthState::Ok => {
                xai_xvora_telemetry::unified_log::info(
    ...
    pub(crate) async fn try_devbox_recovery(self: &Arc<Self>) -> Result<GrokAuth, AuthError> {
        ...
        xai_xvora_telemetry::unified_log::info("auth: devbox recovery starting", None, None);
    ...
    #[cfg(test)]
    pub(crate) fn manual_auth_last_emit(&self) -> Option<xai_xvora_telemetry::events::ManualAuth> {
...
impl xai_xvora_tools::types::ApiKeyProvider for SharedAuthKeyProvider {
...
/// Build a refreshing [`ApiKeyProvider`](xai_xvora_tools::types::ApiKeyProvider)
...
/// [`current_api_key_async`](xai_xvora_tools::types::ApiKeyProvider::current_api_key_async),
/// so it works for both OAuth/session (refreshes) and API-key auth.
pub fn shared_api_key_provider(
    auth_manager: Arc<AuthManager>,
) -> xai_xvora_tools::types::SharedApiKeyProvider {
