...
fn resolve_device_flow(
    ...
    crate::agent::config::BoolFlag::env("XVORA_LOGIN_DEVICE_FLOW")
...
///
/// Precedence: CLI (`--oauth`/`--device-auth`) > `XVORA_LOGIN_DEVICE_FLOW` env >
/// `[auth] login_device_flow` config > `xvora_login_device_flow` remote feature flag > loopback.
async fn should_use_device_flow(login_override: LoginTransportOverride) -> bool {
    ...
    let resolved = if login_override.as_cli_bool().is_some() {
        ...
        let env = crate::agent::config::env_bool("XVORA_LOGIN_DEVICE_FLOW");
...
async fn run_auth_flow_inner(
    ...
    // --device-auth / XVORA_LOGIN_DEVICE_FLOW / [auth] login_device_flow.
...
/// otherwise `XVORA_LOGIN_DEVICE_FLOW` env / `[auth] login_device_flow` config /
...
pub struct LogoutResult {
    ...
    /// `true` if `XAI_API_KEY` / `XVORA_CODE_XAI_API_KEY` env var is set.
...
pub fn perform_logout(
    ...
    if was_logged_in {
        ...
        xai_xvora_telemetry::external::set_identity(
            xai_xvora_telemetry::external::IdentityAttrs::default(),
        );
        xai_xvora_telemetry::external::flush();
...
mod tests {
    ...
    /// Run `f` with `XVORA_LOGIN_DEVICE_FLOW` set to `value` (unset for `None`).
    ...
    fn with_device_flow_env<T>(value: Option<bool>, f: impl FnOnce() -> T) -> T {
        let _guard = match value {
            Some(true) => EnvVarGuard::set("XVORA_LOGIN_DEVICE_FLOW", "true"),
            Some(false) => EnvVarGuard::set("XVORA_LOGIN_DEVICE_FLOW", "false"),
            None => EnvVarGuard::remove("XVORA_LOGIN_DEVICE_FLOW"),
        };
        f()
    }
    ...
    async fn persist_or_use_minted_returns_token_when_save_fails() {
        ...
        // Read-only xvora_home: reading a missing auth.json succeeds (empty), but
    ...
    async fn preresolved_bypasses_resolver_and_is_never_cli() {
        ...
        {
            let _guard = EnvVarGuard::set("XVORA_LOGIN_DEVICE_FLOW", "false");
            assert!(
        ...
        {
            let _guard = EnvVarGuard::set("XVORA_LOGIN_DEVICE_FLOW", "true");
            assert!(
    ...
    async fn run_auth_flow_falls_through_when_no_refresh_token() {
        ...
        // XVORA_LOGIN_DEVICE_FLOW / the real config file (the CLI override
