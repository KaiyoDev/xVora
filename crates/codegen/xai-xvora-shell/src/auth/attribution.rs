...
//!
//! 1. [`xai_xvora_telemetry::unified_log::warn`] for the local
...
//!
//! [`xai_xvora_sampler`] is intentionally decoupled from this crate. It
//! invokes the trait [`xai_xvora_sampler::Auth401AttributionCallback`] at
//! its six 401 arms; this module provides [`ShellAttribution`], the
//! concrete impl that the shell wires into
//! [`xai_xvora_sampler::SamplerConfig::attribution_callback`] at every
...
use xai_xvora_sampler::{Auth401AttributionCallback, SamplingConsumer};
use xai_xvora_tools::{Auth401AttributionCallback as ToolAuth401AttributionCallback, ToolConsumer};
...
impl ShellAttribution {
    ...
    /// [`xai_xvora_sampler::SamplerConfig::attribution_callback`].
    ///
    /// (Returns `Arc<dyn Trait>` rather than `Self` because the
    /// `xai_xvora_sampler::SamplerConfig` field expects exactly that;
    ...
    /// `Arc<dyn xai_xvora_tools::Auth401AttributionCallback>` for the
...
impl Auth401AttributionCallback for ShellAttribution {
    fn record_401(&self, consumer: SamplingConsumer, sent_bearer_prefix: Option<&str>) {
        ...
        // to `xai_xvora_sampler::SENT_BEARER_PREFIX_LEN` characters
...
/// in `xai-xvora-tools` emits a 401 attribution event through this
...
/// All 401 emit sites in `xai-xvora-shell` go through this helper -- the
...
mod tests {
    ...
    /// `ShellAttribution` implements `xai_xvora_tools::Auth401AttributionCallback`
