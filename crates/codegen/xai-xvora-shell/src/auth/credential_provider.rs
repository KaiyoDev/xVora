...
fn api_key_id_for(auth: Option<&crate::auth::GrokAuth>) -> Option<String> {
    auth.filter(|a| matches!(a.auth_mode, crate::auth::AuthMode::ApiKey))
        .map(|a| xai_xvora_telemetry::config::deployment_id_from_key(&a.key))
}
...
///
/// - `"xvora-shell"`   — classic Xvora CLI / TUI (xai-xvora-shell)
/// - `"xvora-pager"`   — new Grok Pager / TUI (xai-xvora-pager)
...
pub fn build_storage_client_for_proxy(
    ...
    if let Some(am) = auth_manager {
        ...
        .with_client_identity(xai_xvora_version::VERSION, client_identifier)
...
pub fn sync_external_otel_identity() {
    if let Some(provider) = OTEL_PROVIDER.get() {
        ...
        xai_xvora_telemetry::external::set_identity(
            xai_xvora_telemetry::external::IdentityAttrs::from_snapshot(&snapshot),
        );
    }
}
...
/// `xai-xvora-pager` and `xai-xvora-tui` need at tracing init time.
///
...
pub fn build_default_otel_layer_config() -> xai_xvora_telemetry::otel_layer::OtelLayerConfig {
    let endpoints = crate::agent::config::EndpointsConfig::default();
    let grok_com_config = crate::auth::GrokComConfig::default();
    let exporter = xai_xvora_telemetry::otel_layer::OtelExporterConfig {
    ...
    xai_xvora_telemetry::otel_layer::OtelLayerConfig {
...
mod tests {
    ...
    fn snapshot_populates_tenant_id_per_auth_mode() {
        use xai_xvora_telemetry::config::deployment_id_from_key;
        let _guard = EarlyInvalidationGuard::pin_to_default();
        let dir = tempfile::tempdir().unwrap();
