...
- The legacy `agent` command is now kept in sync with `grok` after running `xvora update`.
- Headless (`xvora -p`) runs now wait for background tasks and subagents to finish before exiting.
