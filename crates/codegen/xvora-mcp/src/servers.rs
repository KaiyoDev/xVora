...
/// Canonical definition lives in `xvora_workspace_types`; re-exported here
...
/// MCP initialization strategy. Defined in `xvora-telemetry`; re-exported
...
/// External-config overrides for an MCP server, surfaced to xvora-mcp
...
fn drain_mcp_stderr_to_log(server_name: &str, mut stderr: tokio::process::ChildStderr) {
    let log_dir = xvora_config::xvora_home().join("logs").join("mcp");
