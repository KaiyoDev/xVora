...
enum AttemptOutcome {
    ...
    Failed {
        error: SamplingError,
        doom_loop_signals: Vec<String>,
        recovery_items: Vec<xvora_sampling_types::ConversationItem>,
    },
...
async fn run_one_attempt(
    ...
    event_tx: &mpsc::UnboundedSender<SamplingEvent>,
    cancel_token: &CancellationToken,
    doom_check: Option<xvora_sampling_types::DoomLoopRecoveryPolicy>,
...
async fn drive_l2(
    ...
    output_observed: Arc<AtomicBool>,
    length_policy: xvora_sampling_types::LengthPolicy,
) -> AttemptOutcome {
    let mut l2 = pin!(l2);
            ...
            next = l2.next() => match next {
                Some(SamplingEvent::Completed { response, metrics, .. }) => {
                    ...
                    let content_filtered = response.stop_reason
                        == Some(xvora_sampling_types::StopReason::ContentFilter);
...
fn build_empty_context(
    reason: xvora_sampling_types::EmptyReason,
...
mod tests {
    ...
    fn completed_response(
        stop_reason: Option<xvora_sampling_types::StopReason>,
        content: &str,
        ...
        ConversationResponse {
            ...
            doom_loop_signals: vec![xvora_sampling_types::doom_loop::DoomLoopSignal::parse(
    ...
    async fn terminal_outcome(response: ConversationResponse) -> AttemptOutcome {
        ...
        drive_l2(
            ...
            FailedResponseCapture::default(),
            Arc::new(AtomicBool::new(false)),
            xvora_sampling_types::LengthPolicy::Fail,
        )
        .await
    }
    ...
    async fn terminal_detector_signals_are_bounded_before_forwarding() {
        ...
        response.doom_loop_signals =
            ...
            .chain((0..MAX_COLLECTED_DOOM_LOOP_SIGNALS + 20).map(|index| {
                xvora_sampling_types::doom_loop::DoomLoopSignal::parse(&format!(
    ...
    async fn length_policy_salvage_completes_with_partial_content() {
        let outcome = drive_length_event(
            length_completed_event("partial"),
            xvora_sampling_types::LengthPolicy::CompletePartial,
        )
        .await;
    ...
    fn with_tool_call(mut event: SamplingEvent, arguments: &str) -> SamplingEvent {
        ...
        a.tool_calls = vec![xvora_sampling_types::ToolCall {
            id: "call_1".into(),
            name: "do_thing".into(),
    ...
    async fn length_policy_truncated_tool_call_arguments_still_fail() {
        for policy in [
            xvora_sampling_types::LengthPolicy::CompleteToolCalls,
            xvora_sampling_types::LengthPolicy::CompletePartial,
        ] {
    ...
    async fn length_policy_default_completes_with_completed_tool_calls() {
        ...
        let outcome =
            drive_length_event(event, xvora_sampling_types::LengthPolicy::default()).await;
        match outcome {
