...
use xvora_sampling_types::error::{
...
fn apply_terminal_event_overrides(event: &mut rs::ResponseStreamEvent, data: &str) {
    ...
    if let Some(ticks) = xvora_sampling_types::reported_cost_ticks(
        value
...
pub(crate) const COST_USD_TICKS_METADATA_KEY: &str = "xvora.cost_usd_ticks";
...
struct ClientDefaults {
    ...
    doom_loop_recovery: Option<xvora_sampling_types::DoomLoopRecoveryPolicy>,
}
...
impl SamplingClient {
    ...
    pub async fn create_response(
        ...
        xvora_sampling_types::patch_reasoning_text_types(&mut request_body);
        let SentRequest {
            builder,
            sent_bearer,
    ...
    pub async fn conversation_stream_responses(
        ...
        let extra_tools = xvora_sampling_types::extra_tool_entries(&request.hosted_tools);
    ...
    ///
    /// Honors the request's [`LengthPolicy`](xvora_sampling_types::LengthPolicy) like the actor path.
...
/// Applies the request's [`xvora_sampling_types::LengthPolicy`] to a collected response.
...
pub(crate) fn apply_length_policy(
    policy: xvora_sampling_types::LengthPolicy,
    response: xvora_sampling_types::ConversationResponse,
) -> Result<xvora_sampling_types::ConversationResponse> {
    use xvora_sampling_types::LengthVerdict;
...
mod tests {
    ...
    use xvora_sampling_types::ApiErrorCode;
    use xvora_sampling_types::types::ChatRequestMessage;
