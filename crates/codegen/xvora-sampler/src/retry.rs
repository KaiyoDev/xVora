...
use xvora_sampling_types::{SamplingError, is_retryable_api_status};
...
mod tests {
    ...
    fn zero_retry_budget_never_reuses_a_model_output_cap() {
        for err in [
            ...
            SamplingError::EmptyResponse {
                context: xvora_sampling_types::EmptyResponseContext {
                    reason: xvora_sampling_types::EmptyReason::NoVisibleContent,
                    had_reasoning: false,
                    content_len: 0,
                    tool_call_count: 0,
    ...
    fn classify_cloudflare_522_is_retryable() {
        let err = api_err(
            ...
            "Connection to xvora timed out or was interrupted. (HTTP 522).",
        );
    ...
    fn classify_cloudflare_525_is_fatal_even_with_should_retry_true() {
        for should_retry in [None, Some(true)] {
            let err = SamplingError::Api {
                ...
                message: "Secure connection to xvora failed. (HTTP 525).".into(),
                model_metadata: None,
                retry_after_secs: None,
                should_retry,
                error_code: None,
    ...
    fn drifted_size_overflow_wordings_are_fatal_on_turn_path() {
        ...
        let coded = SamplingError::StreamError {
            ...
            code: Some(xvora_sampling_types::ApiErrorCode::parse(
    ...
    fn byte_size_coded_errors_get_image_strip_before_the_veto() {
        ...
        for code in ["413", "payload_too_large", "request_too_large"] {
            let coded = SamplingError::StreamError {
                error_type: "BAD_REQUEST".into(),
                message: "request rejected".into(),
                code: Some(xvora_sampling_types::ApiErrorCode::parse(code)),
            };
            assert!(
                matches!(
                    classify_error(&coded, 0, 15, RATE_LIMIT_RETRY_THRESHOLD),
