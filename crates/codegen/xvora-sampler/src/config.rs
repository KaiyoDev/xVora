...
//! It deliberately does **not** alias `xvora_sampling_types::SamplingConfig`.
//! Aliasing would pull transitive dependencies on shell-specific types (`xvora-tools`, etc.) into the sampler crate.
...
///
/// # Construction in `xvora-shell`
///
/// `SamplerConfig` is the single source of truth for sampler configuration.
/// The shell builds it directly by composing chat-state's `xvora_sampling_types::SamplingConfig` with `Credentials` (api key, client version).
