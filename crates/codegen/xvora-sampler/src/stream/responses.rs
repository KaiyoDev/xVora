...
pub(crate) fn stream_responses_tracked<'a>(
    ...
    async_stream::stream! {
                ...
                ResponseStreamEvent::ResponseFailed(failed_event) => {
                    ...
                    let err = SamplingError::Api {
                        ...
                        error_code: response
                            .error
                            .as_ref()
                            .map(|e| xvora_sampling_types::ApiErrorCode::parse(&e.code)),
                    };
                    yield SamplingEvent::Failed {
                        request_id: request_id.clone(),
                        error: SamplingErrorInfo::from(&err),
                ...
                ResponseStreamEvent::ResponseError(error_event) => {
                    ...
                    let err = SamplingError::Api {
                        ...
                        error_code: error_event
                            .code
                            .as_deref()
                            .map(xvora_sampling_types::ApiErrorCode::parse),
                    };
                    yield SamplingEvent::Failed {
                        request_id: request_id.clone(),
                        error: SamplingErrorInfo::from(&err),
        ...
        let mut items = xvora_sampling_types::response_to_conversation_items(response);
        xvora_sampling_types::inject_streaming_reasoning_fallback(&mut items, reasoning_acc);
...
mod tests {
    ...
    async fn an_abort_on_a_tool_input_frame_vetoes_the_replay() {
        for tool_frame in [
            ...
            let collector = crate::doom_loop::DoomLoopSignalCollector::new(
                xvora_sampling_types::DoomLoopRecoveryPolicy::default(),
            );
            collector.absorb(
                xvora_sampling_types::doom_loop::DOOM_LOOP_CHECK_EVENT_TYPE,
    ...
    async fn response_failed_yields_failed_500() {
        ...
        match events.last().unwrap() {
            SamplingEvent::Failed { error, .. } => {
                ...
                assert_eq!(
                    error.error_code,
                    Some(xvora_sampling_types::ApiErrorCode::Other(
                        "server_error".into()
                    ))
                );
            }
    ...
    async fn response_error_event_carries_code_into_failed() {
        let error_event = rs::ResponseStreamEvent::ResponseError(rs_types::ResponseErrorEvent {
            sequence_number: 0,
            code: Some(xvora_sampling_types::INVALID_IMAGE_ERROR_CODE.into()),
    ...
    async fn doom_loop_collector_signals_land_on_completed_response() {
        use xvora_sampling_types::doom_loop::{
