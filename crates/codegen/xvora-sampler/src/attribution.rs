...
//! it" buckets.
//!
//! `xvora-sampler` is intentionally decoupled from `xvora-shell`
...
/// tools live in `xvora-tools`
/// (`crates/codegen/xvora-tools/src/implementations/`), have their
/// own HTTP clients that do not flow through `SamplingClient`, and
/// hook into the `xvora_tools::ApiKeyProvider` trait rather than
/// this enum.
...
/// `xvora_shell::auth::token_suffix` (which truncates to 12 chars
...
/// `xvora-shell/src/auth/manager.rs` to match, otherwise the
