    ...
    assert!(
        events.iter().any(|e| match e {
            SamplingEvent::ImagesStripped {
                stripped_urls,
                reason: xvora_sampler::StripReason::ServerRejected,
                ..
...
async fn fatal_decision_does_not_strip_or_emit_images_stripped() {
    ...
    tokio::spawn(async move {
        // RST every connection: peek then drop (see xvora-http).
        loop {
            tokio::select! {
...
async fn connection_reset_emits_payload_heuristic_and_strips_request() {
    ...
    let (shutdown_tx, shutdown_rx) = oneshot::channel::<()>();
    tokio::spawn(async move {
        // Peek then drop so the peer sees RST (see xvora-http).
...
async fn responses_doom_loop_does_not_resample_after_output_when_retry_only_before_output() {
    ...
    assert!(
        matches!(
            result,
            Err(xvora_sampling_types::SamplingError::DoomLoopDetected { .. })
        ),
