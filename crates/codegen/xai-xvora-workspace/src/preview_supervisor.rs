...
//! spawns the unchanged `/usr/local/bin/xai-xvora-preview-proxy` binary as a
...
pub const PREVIEW_PROXY_BIN_PATH: &str = "/usr/local/bin/xai-xvora-preview-proxy";
...
impl PreviewArgs {
    ...
    /// `xai-xvora-preview-proxy/src/cli.rs`). Absent options and a false
...
/// Build the unspawned proxy command. Secrets (`XVORA_SERVER_KEY` /
/// `XVORA_SESSION_ID`) reach the proxy by env inheritance — never argv.
...
/// `xai-xvora-preview-proxy`'s `/__control/activity` route).
...
/// the default in `xai-xvora-preview-proxy/src/cli.rs`).
...
mod tests {
    ...
    fn to_argv_maps_every_flag_to_the_proxy_cli_names() {
        // Flag names must match xai-xvora-preview-proxy/src/cli.rs exactly.
        assert_eq!(
            sample_cfg().to_argv(),
            vec![
                "--preview-port",
    ...
    #[cfg(target_os = "linux")]
    const PDEATHSIG_HELPER_ENV: &str = "XVORA_PDEATHSIG_HELPER";
    #[cfg(target_os = "linux")]
