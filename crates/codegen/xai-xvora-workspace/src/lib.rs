...
pub use xai_xvora_workspace_client::WorkspaceClient;
pub use xai_xvora_workspace_types::WorkspaceEvent;
...
/// environment (`XVORA_HOME`, `HOME`, …). nextest isolates each test in its own
...
/// can clobber `XVORA_HOME` mid-test. A single shared lock (used by every
