...
//! `xai-xvora-agent` and `xai-xvora-tools` rather than duplicating them.
...
//!
//! Re-exports [`AgentsMdTracker`] from `xai-xvora-tools` for external
...
// workspace crate without a direct xai-xvora-tools dependency.
pub use xai_xvora_tools::types::agents_md_tracker::AgentsMdTracker;

// Re-export the config types that callers pass into WorkspaceConfig.
pub use xai_xvora_agent::plugins::discovery::DiscoveryConfig as PluginDiscoveryConfig;
pub use xai_xvora_agent::plugins::trust::TrustStore as PluginTrustStore;
pub use xai_xvora_agent::prompt::skills::SkillsConfig;
...
///
/// Delegates to [`xai_xvora_agent::prompt::skills::list_skills`] with
...
pub async fn discover_skills(root_cwd: &Path, config: &SkillsConfig) -> Vec<Value> {
    ...
    let skills = xai_xvora_agent::prompt::skills::list_skills(
        Some(&cwd_str),
        config,
...
pub async fn discover_agents_md(root_cwd: &Path) -> Vec<Value> {
    let cwd_str = root_cwd.to_string_lossy();
    let files = xai_xvora_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd_str,
    ...
    files
        .into_iter()
        .map(|mut file| {
            // Strip rules-file YAML frontmatter so it does not leak as raw YAML (matches xvora render).
            if file.file_path.contains("/.xvora/rules/")
                || file.file_path.contains("/.claude/rules/")
            {
                file.content = xai_xvora_tools::implementations::skills::skill::extract_skill_body(
                    &file.content,
                );
            }
            file
...
///
/// Delegates to [`xai_xvora_agent::plugins::discover_plugins`] with
...
pub fn discover_plugins(
    ...
    let discovered = xai_xvora_agent::plugins::discover_plugins(
        Some(root_cwd),
        config,
        trust_store,
        project_trusted,
...
pub fn load_project_config(root_cwd: &Path) -> Value {
    ...
    match xai_xvora_config::load_config_file(&config_path) {
...
mod tests {
    ...
    fn agent_config_file_wire_matches_workspace_types_mirror() {
        // The RPC serializes xvora's AgentConfigFile and the remote
        ...
        // break discovery.
        let src = xai_xvora_agent::prompt::agents_md::AgentConfigFile {
            file_name: "AGENTS.md".to_string(),
            file_path: "/repo/AGENTS.md".to_string(),
            content: "# Instructions\n".to_string(),
        };
        let json = serde_json::to_value(&src).unwrap();
        let mirror: xai_xvora_workspace_types::rpc::agents_md::AgentConfigFile =
    ...
    async fn discover_agents_md_strips_rules_frontmatter() {
        let tmp = tempfile::tempdir().unwrap();
        let rules_dir = tmp.path().join(".xvora").join("rules");
        ...
        let rule = files
            ...
            .find(|f| {
                f["file_path"]
                    .as_str()
                    .is_some_and(|p| p.ends_with("/.xvora/rules/xyzzy-discover-agents-md-test.md"))
            })
