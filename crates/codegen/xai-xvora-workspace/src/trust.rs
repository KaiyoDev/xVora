...
//!
//! The store is rooted at [`xai_xvora_config::user_xvora_home`] — the **Option**
//! home that resolves to `None` (rather than a cwd-relative `./.xvora`) when
//! neither `$XVORA_HOME` nor a home directory is set (e.g. a minimal container /
...
//! a `./.xvora/trusted_folders.toml` that self-trusts its own checkout (fail
//! closed).
...
impl TrustStore {
    /// Load the trust store from `<user_xvora_home>/trusted_folders.toml`.
    ///
    ...
    /// Default on-disk path: `<user_xvora_home>/trusted_folders.toml`, or `None`
    /// when no user home resolves.
    ///
    /// Resolves via [`xai_xvora_config::user_xvora_home`], never
    /// [`xai_xvora_config::xvora_home`], so it never falls back to a cwd-relative
    /// `./.xvora` — that fallback would let an untrusted cloned repo's `.xvora`
    /// masquerade as the user-global store and self-trust the checkout.
    pub fn default_path() -> Option<PathBuf> {
        Self::default_path_in(xai_xvora_config::user_xvora_home())
    }

    /// Map a resolved user-xvora-home to the store path, preserving "no home" as
    ...
    fn default_path_in(user_xvora_home: Option<PathBuf>) -> Option<PathBuf> {
        Some(user_xvora_home?.join(TRUST_FILE_NAME))
    }
...
/// ROOT (via the `~/.xvora/worktrees.db` registry), so every `grok -w` worktree
...
pub fn migrate_legacy_hook_trust() {
    ...
    MIGRATED.call_once(|| {
        let Some(legacy_file) = xai_xvora_hooks::trust::legacy_trust_file_path() else {
            return;
        };
...
/// is testable without the process-global xvora-home cache. Returns the number
...
fn migrate_legacy_hook_trust_in(legacy_file: &Path, store: &mut TrustStore) -> usize {
    ...
    let projects = match xai_xvora_hooks::trust::list_trusted_projects_with_file(legacy_file) {
        Ok(p) => p,
        Err(e) => {
            tracing::warn!(
...
mod tests {
    ...
    fn default_path_in_maps_home_and_preserves_no_home() {
        ...
        // cwd-relative `./.xvora` that xvora_home() would invent, which is exactly
        // how a cloned repo's own `<repo>/.xvora/trusted_folders.toml` could
        // masquerade as the user-global store and self-trust the checkout.
        assert_eq!(TrustStore::default_path_in(None), None);
    }

    #[test]
    fn default_path_sources_from_user_xvora_home() {
        // Thin source-pin: the production accessor reads user_xvora_home()
        // (Option, no cwd fallback), not xvora_home(). The real regression guard
        // is the seam test above (default_path_in(None) == None).
        assert_eq!(
            TrustStore::default_path(),
            xai_xvora_config::user_xvora_home().map(|h| h.join(TRUST_FILE_NAME))
        );
    }
    ...
    /// Point `XVORA_HOME` at an isolated tempdir and register one grok-managed
    ...
    /// dir)`; the [`LockedTestEnv`] holds the lock and restores `XVORA_HOME` on
    ...
    fn workspace_key_ignores_registry_for_cwd_outside_worktrees_dir() {
        ...
        // `<xvora_home>/worktrees`: `worktree_record_for_cwd` skips the registry
