...
///
/// ```bash
/// XAI_API_KEY='xai-some-key' cargo run --bin xai-xvora-pager
/// ```
///
...
///   highlighted_words: ["cargo", "run", "--bin", "xai-xvora-pager"]
///   suffix: []
...
mod tests {
    ...
    fn test_parse_plain_commands_from_script() {
        ...
        let environment_key_command = "XAI_API_KEY='xai-some-key' cargo run --bin xai-xvora-pager";
        assert_eq!(
        ...
        let redirection_command = "cargo build --bin xai-xvora-pager 2>&1";
        assert_eq!(
        ...
        let another_long_command = "cargo test --package xai-xvora-shell --lib -- permission::bash_command_splitting::tests::test_parse_plain_commands_from_script --exact --nocapture 2>&1";
        assert_eq!(
