...
use xai_xvora_tools::implementations::xvora::web_fetch::{
...
fn permission_mode_artifact_str(mode: xai_xvora_telemetry::enums::PermissionMode) -> &'static str {
    use xai_xvora_telemetry::enums::PermissionMode;
    match mode {
...
fn spawn_permission_manager_with_pin(
    ...
    let _task = tokio::task::spawn_local(async move {
        ...
        // persist it to ~/.xvora/sessions/<cwd>/permission.toml. This caused
            ...
            match cmd {
                ...
                PermissionCommand::Request {
                    ...
                    if matches!(&access, AccessKind::Bash(_))
                        && xai_xvora_sandbox::should_auto_allow_bash()
