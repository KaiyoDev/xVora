...
use xai_xvora_tools::implementations::xvora::web_fetch::domain_from_url;
...
/// Canonical definition lives in `xai_xvora_workspace_types` (so both the
/// permission-validation layer and the MCP transport in `xai-xvora-mcp` can
...
/// reached `xai_xvora_workspace::permission::MCP_TOOL_NAME_DELIMITER`.
