//! Permission resolution engine: merges native `.xvora/config.toml`,
...
fn load_requirements_permissions() -> Vec<Sourced<PermissionRule>> {
    xai_xvora_config::requirements_layers()
        .into_iter()
        .flat_map(|layer| {
...
/// Find every `<dir>/.xvora/config.toml` from `cwd` upward to the git repo
/// root (or just `<cwd>/.xvora/config.toml` when there is no git repo).
///
/// Returned paths are ordered from repo root (lowest priority) to `cwd`
/// (highest priority), matching `xai-xvora-shell::config::find_project_configs`.
fn find_project_grok_configs(cwd: &Path) -> Vec<PathBuf> {
    ...
    if let Some(ref root) = git_root {
        ...
        while let Some(dir) = current {
            let p = dir.join(".xvora").join("config.toml");
            if p.is_file() {
                configs.push(p);
            }
            if dir == *root {
        ...
        let p = cwd.join(".xvora").join("config.toml");
        if p.is_file() {
            configs.push(p);
        }
    }
...
///   * Each `.xvora/config.toml` from the git repo root down to `cwd`
...
fn load_config_toml_permissions(cwd: &Path) -> Vec<Sourced<PermissionRule>> {
    ...
    // Gated on user_xvora_home() so a project's .xvora/config.toml is never read as
    // global permissions when neither XVORA_HOME nor a home dir resolves.
    if let Some(global_path) = xai_xvora_config::user_xvora_home().map(|g| g.join("config.toml"))
        && global_path.is_file()
    {
        match xai_xvora_config::load_config_file(&global_path) {
    ...
    for path in find_project_grok_configs(cwd) {
        match xai_xvora_config::load_config_file(&path) {
            Ok(value) => rules.extend(extract_toml_permissions(&value, || {
...
fn managed_config_permissions(
    layers: &[xai_xvora_config::ManagedConfigLayer],
) -> Vec<Sourced<PermissionRule>> {
    layers
        .iter()
        .flat_map(|layer| {
    ...
    fn live() -> Self {
        Self {
            ...
            managed_config_rules: managed_config_permissions(
                &xai_xvora_config::managed_config_layers(),
            ),
        }
    }
}
...
/// [`xai_xvora_config::claude_managed_settings_path`] (platform path such as
...
fn load_managed_settings() -> ManagedSettings {
    let Some(path) = xai_xvora_config::claude_managed_settings_path() else {
...
pub fn yolo_disabled_by_policy() -> Option<&'static str> {
    let layers = xai_xvora_config::requirements_layers();
...
/// without `~/.xvora`); `path` only names the layer in a non-bool warning.
...
mod tests {
    ...
    // Shared so `XVORA_HOME`/`HOME` mutations here also serialize against the
    ...
    fn load_claude_env_merges_with_precedence() {
        // XVORA_HOME-isolate so the claude-import marker reads clean (an imported
        ...
        let _marker_guard = EnvVarGuard::unset("_XVORA_CLAUDE_MARKER_OVERRIDE");
        let tmp = tempfile::tempdir().unwrap();
        let claude_dir = tmp.path().join(".claude");
        std::fs::create_dir_all(&claude_dir).unwrap();
    ...
    fn load_claude_env_with_project_drops_repo_env_when_untrusted() {
        ...
        // drop it. Isolate XVORA_HOME so the claude-import marker reads clean (an
    ...
    async fn managed_config_toml_rules_resolve_as_non_admin_defaults() {
        ...
        let layers =
            xai_xvora_config::managed_config_layers_at(Some(system.path()), Some(user.path()));
        ...
        .unwrap();
        assert_eq!(
            xai_xvora_config::managed_config_layers_at(Some(system.path()), Some(user.path())).len(),
            1
        );
