...
impl HubHandle {
    ...
    pub(crate) async fn connect(
        ...
        let mut server_builder = ToolServerBuilder::default()
            ...
            .binary_version(xai_xvora_version::VERSION)
            ...
            match transport {
                ...
                None => {
                    tracing::warn!(
                        ...
                        "XVORA_HITL_PERMISSION_LIVE set but no hub ToolServer; rejecting guarded tool"
                    );
                    return terminal_only(Err(ToolError::new(
                        ToolErrorKind::PermissionDenied,
...
mod tests {
    ...
    use xai_xvora_tools::types::tool_metadata::ToolMetadata as XaiToolMetadata;
    #[derive(Debug)]
    ...
    fn bg_config() -> ToolServerConfig {
        ToolServerConfig {
            tools: vec![
                ...
                tc("Xvora:monitor", Some(ToolKind::Monitor)),
            ],
    ...
    fn bg_started_notif(task_id: &str) -> ToolNotification {
        use xai_xvora_tools::notification::types::{
    ...
    fn task_completed_notif(task_id: &str) -> ToolNotification {
        use xai_xvora_tools::computer::types::{TaskKind, TaskSnapshot};
