...
use std::sync::Arc;
use xai_xvora_tools::registry::types::{SessionContext, ToolRegistryBuilder, ToolServerConfig};
...
impl SessionTerminalBackend {
    ...
    pub fn local(backend: xai_xvora_tools::computer::local::LocalTerminalBackend) -> Self {
    ...
    pub fn backend(&self) -> &Arc<dyn xai_xvora_tools::computer::types::TerminalBackend> {
        &self.backend
    }
...
pub struct WorkspaceBindConfig {
    ...
    pub tools: Option<Vec<xai_xvora_tools_api::ToolConfigEntry>>,
...
pub enum ResolvedToolset {
    ...
    InvalidToolConfig(xai_xvora_tools::registry::proto_convert::ToolConfigEntryError),
}
    ...
    pub fn resolve(
        ...
        if let Some(cfg) = &self.tool_config {
            for (idx, tool) in cfg.tools.iter().enumerate() {
                if let Err(err) = xai_xvora_tools_api::config_validation::validate_name_override(
                    idx,
                    &tool.id,
        ...
        if let Some(tools) = &self.tools {
            ...
            for (idx, entry) in tools.iter().enumerate() {
                ...
                match xai_xvora_tools::registry::proto_convert::tool_config_from_entry(
                    idx,
                    entry.clone(),
                ) {
            ...
            if !unserved_tool_ids.is_empty() {
                tracing::warn!(
                    ...
                    .manifest_version, running_version = xai_xvora_version::VERSION,
...
mod bind_config_tests {
    ...
    fn tools_entries_resolve_to_tool_server_config() {
        let v = serde_json::json!(
            { "preset" : "explore", "tools" : [{ "id" : "Xvora:grep", "params_json" :
        ...
        assert_eq!(toolset.tools[1].id, "Xvora:read_file");
    }
    #[test]
...
pub struct WorkspaceConfig {
    ...
    /// `<cwd>/.xvora/lsp.json`: `false` drops them at load, `true` keeps them. The
...
pub struct WorkspaceServerMetadata {
    ...
    /// Logical sandbox-service session UUID, from the `XVORA_SESSION_ID` env
...
impl WorkspaceServerMetadata {
    ...
    ///
    /// `env_session_id` is the raw `XVORA_SESSION_ID`; empty is normalized to
...
/// `*.json` hook files. Maps 1:1 to [`xai_xvora_hooks::discovery::HookSource`]
