...
//!
//! Moved from `xai-xvora-shell/src/session/worktree.rs` into the workspace
...
// Canonical in xai-xvora-workspace-types; re-exported for existing paths.
...
/// `worktrees.db` (`xai_fast_worktree::resolve_xvora_home`), so checkout dirs and
/// the metadata DB always live under the same `.xvora` tree. That resolver
/// canonicalizes its `$HOME` fallback to match `xai_xvora_config::xvora_home()`,
/// so worktree paths also agree with trust/hooks and other xvora-home paths.
fn xvora_home() -> std::path::PathBuf {
    xai_fast_worktree::resolve_xvora_home().unwrap_or_else(|_| {
        dirs::home_dir()
...
use xai_fast_worktree::{
    ...
    gc_worktrees as fw_gc_worktrees, rebuild_worktree_db, resolve_xvora_home,
};
...
mod tests {
    ...
    /// Point `XVORA_HOME` at an isolated tempdir (`resolve_xvora_home` re-reads
    ...
    /// and restores `XVORA_HOME` on drop (before releasing the lock), so the
    ...
    async fn cleanup_cancelled_worktree_removes_dir_and_deregisters() {
        ...
        // can't clobber this row (XvoraHomeFixture is not visible across crates).
