...
//! `project_scope_allowed`, the loader filters) lives in `xai-xvora-shell`.
//!
...
//! provisional and re-checked rather than cached — is a `xai-xvora-shell`
...
use xai_xvora_config_types::{BoolFlag, RemoteSettings};
...
/// this binary — true on a local/dev build (no `XVORA_VERSION` release stamp).
///
...
/// (`XVORA_VERSION` unset at compile time) — i.e. a local/dev build.
///
/// Kept local (not in `xai-xvora-version`) on purpose: adding a symbol to that
...
fn is_local_build() -> bool {
    // Runtime escape hatch: a pinned XVORA_TEST_VERSION simulates a release build,
    // so tests/CI (which run unstamped, i.e. local-looking) can exercise the gate.
    if std::env::var(xai_xvora_version::TEST_VERSION_ENV).is_ok() {
        return false;
    }
    option_env!("XVORA_VERSION").is_none()
}

/// Resolve whether the folder-trust gate is enabled.
///
/// On a local/dev build (no `XVORA_VERSION` release stamp) the feature is OFF
...
/// env `XVORA_FOLDER_TRUST` > `[folder_trust] enabled` (user) > managed >
...
fn feature_enabled_for_build(remote: Option<&RemoteSettings>, is_local_build: bool) -> bool {
    ...
    // shipped, release-stamped binaries. Even an explicit XVORA_FOLDER_TRUST/config
    ...
    let user = xai_xvora_config::load_from_disk().ok();
    let managed = xai_xvora_config::load_managed_config().ok();
    BoolFlag::env("XVORA_FOLDER_TRUST")
...
/// same `~/.xvora/trusted_folders.toml`. Best-effort; a write failure is logged,
/// not fatal.
...
/// `.claude` / `.xvora/agents` / subdir-launch gaps. NOT itself the trust gate.
...
fn collect_repo_config_kinds(cwd: &Path, first_only: bool) -> Vec<&'static str> {
    ...
    let chain = xai_xvora_agent::repo::RepoDirChain::resolve(cwd);
    ...
    // Project `.xvora/config.toml` declaring repo-controlled code-exec: a
    ...
    for path in crate::project_config::find_project_configs_in(&chain.dirs) {
        let Ok(root) = xai_xvora_config::load_config_file(&path) else {
            continue;
        };
    ...
    // must be gated — else a hooks-only clone (e.g. `.xvora/hooks/evil.json`) would
    ...
    if hook_root.join(".xvora").join("hooks").is_dir()
    ...
    // that must be gated — else a plugin clone (e.g. `.xvora/plugins/evil/`, even
    ...
    if !xai_xvora_agent::plugins::project_plugin_dirs_in(&chain.dirs).is_empty() {
        hit!("plugins");
    }
    // Project AGENT dirs (`.xvora/agents` / `.claude/agents`): a project agent
    ...
    if !xai_xvora_agent::discovery::project_agent_dirs_in(&chain.dirs).is_empty() {
        hit!("agents");
    }
...
mod tests {
    ...
    fn repo_configs_present_detects_project_agents() {
        // A `.xvora/agents`-only clone must be gated: a project agent definition
    ...
    fn repo_config_kinds_matches_gate_and_reports_all_kinds() {
        ...
        // `.xvora/agents` — even when launched from a SUBDIR (the cwd→git-root walk
    ...
    // XVORA_HOME-isolation idiom mirrored from this crate's `permission::claude_compat`
    // tests (the workspace crate has no `serial_test`/`xai-xvora-test-support`
    ...
    fn simulate_release_build() -> EnvVarGuard {
        EnvVarGuard::set(xai_xvora_version::TEST_VERSION_ENV, Path::new("0.0.0-sim"))
    }
    ...
    fn local_build_ignores_remote_rollout() {
        ...
        let _flag = EnvVarGuard::unset("XVORA_FOLDER_TRUST");
    ...
    fn release_build_keeps_gate_when_enabled() {
        ...
        // config nor an ambient env flag can override it: empty XVORA_HOME (no
        // config.toml/managed_config.toml) + XVORA_FOLDER_TRUST unset. nextest's
        // process-per-test makes xvora_home()'s OnceLock pick up the temp dir.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("XVORA_HOME", home.path());
        let _flag = EnvVarGuard::unset("XVORA_FOLDER_TRUST");
    ...
    fn local_build_ignores_explicit_env_optin() {
        ...
        // XVORA_FOLDER_TRUST=1 does NOT enable the feature (so a self-built grok
        // never prompts). XVORA_HOME isolated so on-disk config can't influence it.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("XVORA_HOME", home.path());
        let _flag = EnvVarGuard::set("XVORA_FOLDER_TRUST", Path::new("1"));
    ...
    fn release_build_defaults_on() {
        ...
        // the feature ON. Empty XVORA_HOME (no config.toml/managed config) +
        // XVORA_FOLDER_TRUST unset so only the default applies.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("XVORA_HOME", home.path());
        let _flag = EnvVarGuard::unset("XVORA_FOLDER_TRUST");
    ...
    fn is_local_build_honors_test_version_override() {
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        // A pinned XVORA_TEST_VERSION simulates a release build => not a local build.
        {
            let _sim = EnvVarGuard::set(xai_xvora_version::TEST_VERSION_ENV, Path::new("0.0.0-sim"));
            assert!(!is_local_build());
        }
        // With it unset, an unstamped build (no XVORA_VERSION) is a local build.
        // Guard to the unstamped case so a release-stamped test binary (CI release)
        // doesn't spuriously fail this arm.
        let _unset = EnvVarGuard::unset(xai_xvora_version::TEST_VERSION_ENV);
        if option_env!("XVORA_VERSION").is_none() {
            assert!(is_local_build());
        }
    }
    ...
    fn store_io_is_noop_on_local_build() {
        ...
        // (mirrors `is_local_build_honors_test_version_override`); XVORA_HOME-isolated
        // and ENV_LOCK-serialized so toggling XVORA_TEST_VERSION is race-safe.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("XVORA_HOME", home.path());
        let _unset = EnvVarGuard::unset(xai_xvora_version::TEST_VERSION_ENV);
    ...
    fn revoke_folder_trust_store_persists_untrust_for_trusted_folder() {
        ...
        // XVORA_HOME-isolated so the seed/deny hit a temp store, not the real file.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
    ...
    fn revoke_folder_trust_store_writes_no_deny_for_never_trusted_folder() {
        ...
        // XVORA_HOME-isolated so the grant writes to a temp store.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _env = EnvVarGuard::set("XVORA_HOME", home.path());
        let _sim = simulate_release_build();
