...
use std::sync::Arc;
use xai_xvora_tools::registry::types::{
...
pub(crate) fn resolve_session_toolset(
    ...
    notification_handle: Option<xai_xvora_tools::notification::types::ToolNotificationHandle>,
) -> WorkspaceResult<(
    ToolServerConfig,
...
pub(crate) fn resolve_session_toolset_rebuild(
    ...
    viewer_ctx: Option<xai_tool_runtime::WorkspaceViewerContext>,
    notification_handle: Option<xai_xvora_tools::notification::types::ToolNotificationHandle>,
    terminal_backend: Arc<dyn xai_xvora_tools::computer::types::TerminalBackend>,
    ...
    let toolset = builder
        .finalize_with_trunc_config(
            finalize_config,
            ctx,
            xai_xvora_tools::types::context::TruncationConfig::default(),
            viewer_ctx,
        )
        .map_err(|errs| {
...
/// enabled (`XVORA_WORKSPACE_TOOL_STATE_ENABLED=true`; any other value keeps
/// legacy behavior).
pub fn tool_state_enabled() -> bool {
    std::env::var("XVORA_WORKSPACE_TOOL_STATE_ENABLED").as_deref() == Ok("true")
}
...
/// `XVORA_WORKSPACE_TOOL_STATE_ENABLED`. Aliased to the crate-wide
...
/// [`LocalTerminalBackend`]: xai_xvora_tools::computer::local::LocalTerminalBackend
pub struct WorkspaceSessionContextFactory {
    auth: Option<xai_computer_hub_sdk::SharedAuthProvider>,
    api_base_url: Option<String>,
    /// Resolved `$XVORA_WORKSPACE_HOME` when tool-state persistence is enabled;
...
impl WorkspaceSessionContextFactory {
    ...
    /// (`$XVORA_WORKSPACE_HOME`). Callers should only invoke this when
...
impl SessionContextFactory for WorkspaceSessionContextFactory {
    fn build_session_context(
        ...
        let fs = Arc::new(xai_xvora_tools::computer::local::LocalFs)
            as Arc<dyn xai_xvora_tools::computer::types::AsyncFileSystem>;
        let notification_handle = xai_xvora_tools::notification::ToolNotificationHandle::noop();
...
fn build_proxy_headers(base_url: &str) -> indexmap::IndexMap<String, String> {
    ...
    let version = xai_xvora_version::VERSION;
    headers.insert(
        "user-agent".to_string(),
        format!("xai-xvora-workspace/{version}"),
    );
    ...
    if base_url.contains("cli-chat-proxy") || base_url.contains("chat-proxy") {
        headers.insert("X-XAI-Token-Auth".to_string(), "xai-xvora-cli".to_string());
        headers.insert(
...
fn build_web_fetch_config() -> xai_xvora_tools::implementations::xvora::web_fetch::WebFetchConfig
{
    use xai_xvora_tools::implementations::xvora::web_fetch::{WebFetchConfig, WebFetchParams};
    ...
    if let Ok(proxy) = std::env::var("XVORA_WEB_FETCH_PROXY") {
    ...
    pub fn baseline_config() -> ToolServerConfig {
        ToolServerConfig {
            tools: vec![
                ...
                tc("Xvora:search_replace", Some(ToolKind::Edit)),
                tc("Xvora:grep", Some(ToolKind::Search)),
...
mod tests {
    ...
    fn backfill_tool_kinds_fills_known_kindless_ids_only() {
        let kinds = HashMap::from([
            ("Xvora:search_replace".to_owned(), ToolKind::Edit),
            ("Xvora:read_file".to_owned(), ToolKind::Read),
        ]);
        let config = ToolServerConfig {
            tools: vec![
                test_support::tc("Xvora:search_replace", None),
                test_support::tc("adhoc.opaque", None),
                test_support::tc("Xvora:read_file", Some(ToolKind::Search)),
            ],
        ...
        assert_eq!(kind_of("Xvora:search_replace"), Some(ToolKind::Edit));
        assert_eq!(
    ...
    async fn resolve_session_toolset_readonly_filters_kindless_pinned_tools() {
        ...
        let baseline = ToolServerConfig {
            tools: vec![
                test_support::tc("Xvora:read_file", None),
                test_support::tc("Xvora:grep", None),
                test_support::tc("Xvora:list_dir", None),
                test_support::tc("Xvora:search_replace", None),
                test_support::tc("Xvora:run_terminal_cmd", None),
            ],
            behavior_preset: None,
        };
        let (eff, ts, _backend) = resolve_session_toolset(
    ...
    async fn resolve_session_toolset_mcp_kind_none_dropped_under_readonly() {
        ...
        let baseline = ToolServerConfig {
            tools: vec![
                test_support::tc("Xvora:read_file", Some(ToolKind::Read)),
        ...
        assert!(
            kept_ids.contains(&"Xvora:read_file"),
    ...
    fn tool_state_enabled_only_true_enables() {
        ...
        let var = "XVORA_WORKSPACE_TOOL_STATE_ENABLED";
    ...
    async fn tool_state_rehydrates_same_session_and_cold_starts_other() {
        use xai_xvora_tools::types::resources::{State, WebCitationCounter};
