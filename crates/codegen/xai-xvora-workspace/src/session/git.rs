...
///
/// ```
/// use xai_xvora_workspace::session::git::normalize_repo_url;
///
/// assert_eq!(
...
pub fn restore_code_checkout_allowed(supplied_cwd: &Path, persisted_cwd: Option<&str>) -> bool {
    let worktrees_dir = xai_xvora_tools::util::xvora_home::xvora_home().join("worktrees");
...
pub fn git_rewind_enabled() -> bool {
    xai_xvora_config::env_bool(REWIND_GIT_ENV).unwrap_or(false)
}
