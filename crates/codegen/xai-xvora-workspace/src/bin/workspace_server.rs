...
use url::Url;
use xai_xvora_workspace::config::WorkspaceServerMetadata;
use xai_xvora_workspace::daemonize;
use xai_xvora_workspace::diag_server;
use xai_xvora_workspace::preview_supervisor::{self, PreviewArgs, PreviewVisibility};
...
struct Args {
    ...
    /// `XVORA_WORKSPACE_UPLOAD_QUEUE_ENABLED` env var to `false`) to fall back to
    /// the legacy inline path. Accepts `true`/`false`.
    #[arg(
        long,
        env = "XVORA_WORKSPACE_UPLOAD_QUEUE_ENABLED",
    ...
    /// Override with `XVORA_WORKSPACE_CONFINE_FS_TO_ROOT=false` (e.g. local dev).
    #[arg(
        long,
        env = "XVORA_WORKSPACE_CONFINE_FS_TO_ROOT",
        default_value_t = true,
        action = clap::ArgAction::Set,
    )]
...
/// `/usr/local/bin/xai-xvora-preview-proxy` child (see `cli.rs` for the proxy's
...
async fn run(args: Args, cwd: PathBuf) -> anyhow::Result<()> {
    ...
    tracing_subscriber::registry()
        ...
        .with(donating.clone())
        .init();
    let direct_otlp = match std::env::var("XVORA_WORKSPACE_OTLP_ENDPOINT") {
    ...
    {
        use xai_xvora_sandbox::{ProfileName, SandboxManager};
        let profile = match std::env::var("XVORA_SANDBOX_PROFILE").ok() {
            Some(val) => {
                let parsed = val
                    .parse::<ProfileName>()
                ...
                if matches!(parsed, ProfileName::Custom(_)) {
                    tracing::warn!(
                        value = % val,
                        "Unrecognized XVORA_SANDBOX_PROFILE, defaulting to workspace"
                    );
            ...
            None if xai_xvora_sandbox::trust_bwrap_marker_for_devbox() => ProfileName::Devbox,
        ...
        if profile == ProfileName::Off {
            tracing::info!(
                ...
                "Sandbox explicitly disabled via XVORA_SANDBOX_PROFILE=off"
            );
        } else {
            ...
            sandbox.install();
            let active = xai_xvora_sandbox::is_active();
            ...
            tracing::info!(
                ...
                xai_xvora_sandbox::should_restrict_child_network(), "{status_msg}"
            );
        }
    }
    let auth_provider = xai_xvora_workspace::hub_auth::provider(&url, args.auth_config.as_deref())?;
    tracing::info!(hub_url = % url, cwd = % cwd.display(), "Starting workspace server");
    let cwd_display = cwd.display().to_string();
    let session_id = std::env::var("XVORA_SESSION_ID").ok();
    ...
    let status_config = xai_xvora_workspace::StatusConfig::from_env();
    ...
    xai_xvora_workspace::init_metrics();
    let ws_handle = xai_xvora_workspace::handle::connect_local_workspace(
        cwd,
        url,
        auth_provider,
        metadata,
    ...
    let grace_budget = xai_xvora_workspace::handle::termination_grace_from_env();
    ws_handle
        .two_phase_drain(
            grace_budget,
            xai_xvora_workspace::handle::DrainReason::Sigterm,
        )
        .await;
