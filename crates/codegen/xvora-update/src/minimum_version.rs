...
//!
//! Set `XVORA_TEST_VERSION` to manually exercise either path without producing
...
pub(crate) enum MinimumVersionError {
    ...
    #[error(
        "This version of xVora ({current}) is no longer supported. \
         Run `xvora update` to install version {minimum} or later."
    )]
    AutoUpdateDisabled { current: String, minimum: String },
    /// `npm` / `gh` / `internal` — none detected.
    #[error(
        "This version of xVora ({current}) is no longer supported. \
         Run `xvora update` to install version {minimum} or later."
    )]
    ...
    /// `xvora update --version X` requested a version below the floor.
    #[error(
        "Cannot install xVora {target}: the configured minimum is {minimum}. \
         Run `xvora update` to install the latest allowed version."
    )]
...
///
/// `XVORA_TEST_VERSION` lets devs override the running version to skip
...
mod tests {
    ...
    fn version_env_var_flows_through_to_decision() {
        let saved = std::env::var("XVORA_TEST_VERSION").ok();

        // SAFETY: #[serial] excludes other env-touching tests.
        unsafe { std::env::set_var("XVORA_TEST_VERSION", "0.1.50") };
        let decision =
        ...
        match saved {
            Some(v) => unsafe { std::env::set_var("XVORA_TEST_VERSION", v) },
            None => unsafe { std::env::remove_var("XVORA_TEST_VERSION") },
        }
    }
}
