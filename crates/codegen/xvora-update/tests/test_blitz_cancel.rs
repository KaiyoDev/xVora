...
//!
//! > `~/.xvora/bin/xvora` resolves to a binary that passes the smoke-test, OR it
...
/// Seed a previous-good versioned binary and both managed symlinks (`xvora` and `agent`; see `swap_managed_bin_links`).
...
/// What the active `xvora` should resolve to after an install attempt.
...
/// Applied to both `xvora` and `agent`; `swap_managed_bin_links` moves them together.
...
/// Gated behind `#[ignore]`; invoke via `just blitz-stress` or `cargo nextest run -p xvora-update --run-ignored all`.
#[tokio::test(flavor = "multi_thread")]
#[serial]
