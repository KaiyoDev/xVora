...
//!
//! Each `tests/*.rs` integration test is its own binary, so each binary has its own `OnceLock<xvora_home>`.
...
/// Returns a process-wide test `xvora_home`, initialized exactly once per test binary.
/// Once initialized, `config::xvora_home()` will resolve to this directory for the lifetime of the process.
///
...
/// Host `{os}-{arch}` string matching the versioned binary naming scheme (`xvora-{version}-{platform}`).
