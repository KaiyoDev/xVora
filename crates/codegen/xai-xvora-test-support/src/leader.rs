...
/// (`XVORA_BINARY_LEADER`, else the shared [`grok_binary`] resolution).
pub fn leader_binary() -> PathBuf {
    role_binary(LEADER_BINARY_ENV)
}

/// Binary for the client side of a version-skew test (`XVORA_BINARY_CLIENT`,
...
impl LeaderStdioClient {
    ...
    pub async fn spawn_with_binary(
        ...
        cmd.args(["agent", "--leader", "stdio"])
            .current_dir(cwd)
            // Hermetic env: the developer's shell may export XVORA_* vars
            // (e.g. XVORA_LEADER_SOCKET pointing at a REAL leader on this
            ...
            // ~/.xvora/leader.log — keep it chatty for diagnosis.
            .env("RUST_LOG", "xai_xvora_shell=debug");
